...
public class GoodCangWarehouseHandler extends AbstractThirdWarehouseHandler<GoodCangWarehouseResp, PlatformWarehouseDTO> {
    ...
    private void checkResponse(GoodCangResponse<?> response) {
        if (!isSuccess(response.getAsk())) {
            log.error("谷仓查询仓库数据失败," + response.getMessage());
            throw new ServiceException("谷仓查询仓库数据失败," + response.getMessage());
        }
    }
