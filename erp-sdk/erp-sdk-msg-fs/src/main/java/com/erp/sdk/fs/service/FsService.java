 ...
 * TODO 有问题待处理
 */
public void uploadApprovalFile(File file, String fileName) {
    try {
        // 创建 Client
        Client client = Client.newBuilder("cli_a8858e6f51b95013", "dMU3PHMMoC172dOxFdn8agJeQvYpKYd3").build();
        ...
        // 解析响应
        String responseBody = new String(rawResponse.getBody(), StandardCharsets.UTF_8);
        JsonObject jsonResponse = JsonParser.parseString(responseBody).getAsJsonObject();
        // 检查响应状态
        if (jsonResponse.get("code").getAsInt() == 0) {
            JsonObject data = jsonResponse.getAsJsonObject("data");
            String fileCode = data.get("code").getAsString(); // 文件标识码
