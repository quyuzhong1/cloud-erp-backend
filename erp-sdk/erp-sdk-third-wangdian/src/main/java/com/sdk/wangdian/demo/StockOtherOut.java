...
import com.sdk.wangdian.sdk.api.wms.stockother.out.StockOtherOutAPI;
...
public class StockOtherOut
{
	public static void main(String[] args) throws WdtErpException
	{
		...
		StockOtherOutAPI stockOtherOutAPI = ApiFactory.get(client, StockOtherOutAPI.class);

		queryWithDetail(stockOtherOutAPI);
	}

	private static void queryWithDetail(StockOtherOutAPI api) throws WdtErpException
	{
		int pageSize = 5;
		Pager pager = new Pager(pageSize, 0, true);
		StockOtherOutQueryRequest request = new StockOtherOutQueryRequest();
		request.setStartTime("2022-09-01");
		request.setEndTime("2022-09-30");
		StockOtherOutQueryResponse response = api.queryWithDetail(request, pager);
