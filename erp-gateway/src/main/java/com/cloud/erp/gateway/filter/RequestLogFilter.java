...
import com.cloud.erp.gateway.context.GatewayApiLog;
...
import com.cloud.erp.gateway.utils.IpUtils;
    ...
    private Mono<Void> logApiRequest(ServerWebExchange exchange){
        ...
        if (gatewayPluginProperties.getLogRequest().getRequestLog())
        {
            ...
            if(length>0 && null != contentType  && contentType.includes(MediaType.APPLICATION_FORM_URLENCODED)){
                ...
                gatewayApiLog.setRequestBody(JSONUtil.toJsonStr(gatewayContext.getFormData()));
            }
        }
        ...
        // API日志记录级别
        try {
            log.info("{\"serviceId\":{}, \"data\":{}}", routeServiceId, JSONUtil.toJsonStr(gatewayApiLog));
        } catch (Exception e) {
