pipeline {
    ...
    stages {
        ...
        stage('maven部署项目') {
            steps {
                script {
                    configFileProvider([configFile(fileId: 'ac50ca9a-4b3f-4cc7-b694-ac4017915e6a', variable: 'MY_SETTINGS_XML')]) {
        ...
        stage('远程执行k8s-master的kubectl命令') {
            steps {
                script {
                    ...
                    for (line in lines) {
                        sh """
                        scp /var/jenkins_home/workspace/${JOB_NAME}/erp-server/${line}/${line}.yaml root@172.16.100.60:/k8s-yaml/erpprod-server
                        ssh -tt root@172.16.100.60 "sed -i -e 's|\\\${tag}|${TAG}|g' -e 's|\\\${namespace}|${NAMESPACE}|g' /k8s-yaml/erpprod-server/${line}.yaml"
                        """
                    }
                }
                sh """
                scp /var/jenkins_home/workspace/${JOB_NAME}/erp-gateway/erp-gateway.yaml root@172.16.100.60:/k8s-yaml/erpprod-server
                ssh -tt root@172.16.100.60 "sed -i -e 's|\\\${tag}|${TAG}|g' -e 's|\\\${namespace}|${NAMESPACE}|g' /k8s-yaml/erpprod-server/erp-gateway.yaml"
                ssh -tt root@172.16.100.60 "/usr/bin/kubectl delete -f /k8s-yaml/erpprod-server/ || true"
                ssh -tt root@172.16.100.60 "/usr/bin/kubectl apply -f /k8s-yaml/erpprod-server/"
                """
            }
        }
    }
