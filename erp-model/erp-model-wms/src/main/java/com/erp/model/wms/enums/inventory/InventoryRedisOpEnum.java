...
public enum InventoryRedisOpEnum implements EnumMessage {
	...
	private static final String OVERRIDE_LUA = "local delimiter = '&&'; local i = 0; local key = ARGV[1]; local qty = ARGV[2]; local value = redis.call('get', key); if value == 0 or value == false then     redis.call('set', key, qty); else     for sku in string.gmatch(value, '([^' .. delimiter .. ']+)') do         i = i + 1;     end     if i > 1 then         return '1&&存在未提交流水'     else         redis.call('set', key, qty);     end end return '0'; ";
	private static final String COMMIT_OR_ROLLBACK_LUA = "local split = '&&'; local delimiter = '@@'; local type = ARGV[1]; local key = ARGV[2]; local value = redis.call('SMEMBERS', key); for _, v in ipairs(value) do     local currvalue = redis.call('get', v);     if currvalue == 0 or currvalue == false then         return '99&&即时库存key不存在' .. v;     else         local uqty = 0;         local uvalue = split;         local i = 0;         for sku in string.gmatch(currvalue, '([^' .. split .. ']+)') do             if i == 0 then                 uqty = sku;             else                 local flag = 0;                 for s in string.gmatch(sku, '([^' .. delimiter .. ']+)') do                     if flag == 0 and s == key then                         flag = 1;                     else                         if type == 'commit' and flag == 1 then                             uqty = uqty + s;                         end                     end                 end                 if flag == 0 then                     uvalue = uvalue .. sku .. split;                 end             end             i = i + 1;         end         local newvalue = uqty .. uvalue;         redis.call('set', v, newvalue);         redis.call('SREM', key, v);     end end return '0'; ";
