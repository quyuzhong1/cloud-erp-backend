...
| 2026-07-20 | 启动「尾程-出库单对接」（文档 6.3.3 节），先做 AIYA 文档 vs 现有 WEGO 出库单实现的差异分析（未写代码）。核心发现：① AIYA 建单接口**不回传出库单号**（文档明确"爱亚仓不会返回出库单号"），查询/取消必须靠自定义的 `extOrderNumber`（=三方仓发货单号）做唯一键，与 WEGO"成功即拿到 no，仅异常兜底才反查"的模式本质不同，更接近 AIYA 入库单（`asnNumber`）的模式；② AIYA 出库单取消是**异步**流程（先"拦截中"生成《发货拦截单》，定时器轮询才能确认"拦截成功/失败"），与 WEGO 截单同步返回结果不同，但 ERP 统一契约 `ThirdWarehouseCancelResultEnum` 已预留 `INTERCEPTING` 三态、`ThirdWarehouseCancelOutboundReq.confirmInterceptResult` 也已有类似"通邮"异步确认场景的设计（但全仓库未见实际调用方，需要找参照或重新设计轮询）；③ 当前 `erp-sdk-wms-aiya` 里出库单相关骨架（`AiyaConstants.TWO_C_ORDER_SAVE/SEARCH/QUERY_PAGE/INTERCEPT` 四个 serviceType 常量、`AiyaOutboundResp` 的嵌套 `PageResultDTO` 结构、`AiyaEnums.OrderStatusEnum` 的数字状态码）系逐字照抄 `WegoConstants`/`WegoOutboundResp`/`WegoEnums`，均标注"骨架/TODO"，与文档描述的字段名（`shippingTimeFrom/shippingTimeTo`按发运时间查、响应 `orderNumber/shippingTime/actualLogistic/trackingNumber`、单据状态字母码 A/B/C/D）出入很大，且四个 serviceType 常量疑似应遵循 AIYA 其它已验证接口的 `GLINK_XXX_NOTIFY` 命名规律而非 WEGO 风格命名，需要重写并联调核实；④ 文档独有、WEGO 无对应实现的新流程：寄件人 `shipFrom` 四要素（默认取仓库维度寄件地址，需确认 ERP 是否已有该数据源）、`shippingLabelSource` 三态面单来源（WEGO 只有二态 `wayBillType`）、"超量发货"异常处理、"库存不足/锁住"状态分支、"汉化管理"错误码翻译能力。详细对比清单见下方「尾程-出库单相关」待确认项 |
...
对照文档 6.3.3 节与现有 `WegoHandlerServiceImpl`/`WegoOpenApiService`/`WegoOutboundResp`/`AiyaEnums.OrderStatusEnum` 分析后，待确认项：

- [ ] **【高优】出库单四个接口的真实 serviceType 标识**：`AiyaConstants.TWO_C_ORDER_SAVE="2c.order.save"` 等 4 个常量是骨架期照抄 WEGO 命名，文档只给了通用的「接口参数信息」表（bizData/serviceType/partnerId/sign），没写具体 serviceType 取值；对照已验证的 AIYA 其它接口均为 `GLINK_XXX_NOTIFY` 风格，这 4 个常量大概率需要改名，需要联调测试环境或找爱亚/产品确认真实值。
- [ ] **建单响应无出库单号，排重键改用 `extOrderNumber`**：需确认 AIYA 网关对重复提交同一 `extOrderNumber` 的真实行为（报错拒绝 / 静默覆盖 / 产生重复单），决定 `AiyaHandlerServiceImpl.createOutboundBill` 的幂等写法；`ThirdWarehouseQueryOutboundResponse.shippingOrderNo` 应直接回填我方传入的 `extOrderNumber`，而非从响应提取。
- [ ] **取消/拦截为异步流程**：需确认「4、爱亚出库单查询」定时轮询是否就是唯一的拦截结果确认渠道（文档未给出单独的"查询拦截结果"接口），以及 `ThirdWarehouseCancelResultEnum.INTERCEPTING` 三态在 ERP 侧（三方仓发货单/发货拦截单）如何落地展示；`ThirdWarehouseCancelOutboundReq.confirmInterceptResult` 全仓库未见实际调用方，需要确认是否为可复用的既有设计还是废弃字段。
- [ ] **`shipFrom`（寄件人四要素）数据源**：文档"默认取海外仓库维度寄件地址"，需确认仓库 Entity/维度表是否已有国家/省/市/邮编/地址字段可取，还是需要新增。
- [ ] **`shippingLabelSource` 三态 vs 现有配置二态**：AIYA 需要 ATTACHMENT/API/WMS_GEN 三态，ERP 现有"物流渠道-是否推海外仓面单"配置是二态（是/否），需确认二态能否推出三态，还是要新增配置维度。
- [ ] **出库单查询响应结构是扁平还是嵌套**：文档给出的字段（`Code/message/success/total`+list）看起来是扁平结构（与已验证的库存/SKU接口一致），但当前 `AiyaOutboundResp` 骨架抄的是 WEGO 风格的嵌套 `result{pageNum,pageSize,total,pages,emptyFlag,list}`，需要真实响应样例核实，不能直接照搬骨架。
- [ ] **单据状态字母码 A/B/C/D 的完整语义**：文档只写了 A-已出库/B-已取消/C-库存不足/D-锁住的处理方向，其中"D-锁住"的业务含义及 ERP 侧应做什么响应未展开说明，需要产品补充；`AiyaEnums.OrderStatusEnum` 当前是照抄 WEGO 的数字状态码占位，需要整套重新设计。
- [ ] **"汉化管理"错误码翻译能力是否已有可复用实现**：文档要求把爱亚返回的技术化失败原因转成业务可读文案+处理建议，现有 WEGO/AIYA 入库单代码只有简单的 `buildErrorMessage` 拼接，未见"汉化管理配置表"类的通用能力，需要确认是否已有其它模块实现可复用，还是本次需要新建。
- [ ] **"超量发货"处理流程**：文档描述的比对SKU数量、生成带备注的已审核销售出库单等逻辑，WEGO 现有代码无对应实现，是 AIYA 独有的新分支。
