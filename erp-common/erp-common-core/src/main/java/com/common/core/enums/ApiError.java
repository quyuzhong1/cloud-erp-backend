...
public enum ApiError implements Serializable {
    ...
    SO_RETURN_PRESTOCK_DETAIL_MODIFIED(10782,"预入库单详情行数据已被修改，请刷新后重试"),
    SO_RETURN_PRESTOCK_WAREHOUSE_REQUIRED_FOR_INSTOCK(10783,"预入库单未设置收货仓库，无法生成退货入库单"),
    SO_RETURN_PRESTOCK_LINK_AFTER_SALE_GROUP_EXCEEDS(10784,"本次关联涉及售后单数量过多（{0}个），单次事务内生成退货入库单数量超限，请分批关联"),
    SO_RETURN_PRESTOCK_NOT_FOUND_OR_DELETED(10785,"存在不存在或已删除的预入库单，请刷新后重试"),
    SO_RETURN_PRESTOCK_BILL_TYPE_MIXED_FORBIDDEN(10786,"所选预入库单不能同时包含B2B和B2C类型，请分别关联"),
    SO_RETURN_PRESTOCK_WAREHOUSING_DEPT_NOT_FOUND(10787,"获取不到仓储部门信息"),
    SO_RETURN_PRESTOCK_INSTOCK_GENERATE_FAILED(10788,"退货入库单生成失败，无法审核"),
    SO_RETURN_PRESTOCK_INSTOCK_SUBMIT_FAILED(10789,"退货入库单提交失败：{0}"),
    SO_RETURN_PRESTOCK_LINK_SHOP_GROUP_EXCEEDS(10790,"本次关联涉及店铺数量过多（{0}个），单次事务内生成退货入库单数量超限，请分批关联"),
    SO_RETURN_PRESTOCK_LINK_SHOP_REQUIRED(10791,"关联店铺不能为空"),
    SO_RETURN_PRESTOCK_LINK_CUSTOMER_REQUIRED(10803,"关联客户不能为空"),
    SO_RETURN_PRESTOCK_LINK_SHOP_NOT_FOUND(10792,"关联店铺不存在：{0}"),
    SO_RETURN_PRESTOCK_LINK_SHOP_CUSTOMER_REQUIRED(10793,"关联店铺未绑定客户，无法生成退货入库单：{0}"),
    SO_RETURN_PRESTOCK_WAREHOUSE_REQUIRED(10794,"仓库不能为空"),
    SO_RETURN_PRESTOCK_INSTOCK_WAREHOUSE_INCONSISTENT(10795,"退货入库明细仓库不一致，无法生成预入库单"),
    SO_RETURN_PRESTOCK_INSTOCK_RETURN_TYPE_INCONSISTENT(10796,"退货入库明细退货类型不一致，无法生成预入库单"),
    SO_RETURN_PRESTOCK_DETAIL_NOT_BELONG(10797,"详情行不属于当前预入库单"),
    SO_RETURN_PRESTOCK_RETURN_QTY_INVALID(10798,"退货数量必须大于0：{0}"),
    SO_RETURN_PRESTOCK_CUSTOMER_NOT_EMPTY_FORBIDDEN(10799,"退货客户不为空时无法创建预入库单，请直接保存退货入库单"),
    SO_RETURN_PRESTOCK_RETURN_LOGISTIC_CODE_REQUIRED(10800,"退货物流单号不能为空"),
    SO_RETURN_PRESTOCK_FORCE_CLOSE(10801, "该预入库单已强制关闭，不可再关联"),
    SO_RETURN_PRESTOCK_ALL_LINKED(10802, "该预入库单无可关联的明细行"),
