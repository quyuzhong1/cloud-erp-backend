...
public enum ApiError implements Serializable {
    ...
    AUTH_API_TOKEN_EXPIRED_RECREATE(2027, "个人访问令牌已过期，请重新生成"),
    AUTH_API_TOKEN_ID_REQUIRED(2028, "id不能为空"),
    AUTH_API_TOKEN_NOT_FOUND(2029, "个人访问令牌不存在"),
    AUTH_API_TOKEN_USER_NOT_LOGIN(2030, "用户未登录"),
    AUTH_API_TOKEN_NAME_REQUIRED(2031, "令牌名称不能为空"),
    AUTH_API_TOKEN_NAME_TOO_LONG(2032, "令牌名称最大长度不能超过50位"),
    AUTH_API_TOKEN_VALIDITY_REQUIRED(2033, "有效期不能为空"),
    AUTH_API_TOKEN_VALIDITY_INVALID(2034, "有效期只能选择30天、90天、180天、365天或永不过期"),
    AUTH_API_TOKEN_ENCRYPT_FAILED(2035, "加密令牌失败"),
    AUTH_API_TOKEN_CIPHERTEXT_INVALID(2036, "令牌密文格式不合法"),
    AUTH_API_TOKEN_DECRYPT_FAILED(2037, "解密令牌失败，请确认API Token加密密钥配置未变更"),
    AUTH_API_TOKEN_AES_KEY_REQUIRED(2038, "API Token加密密钥未配置，请配置erp.api-token.aes-key或环境变量ERP_API_TOKEN_AES_KEY"),
    AUTH_API_TOKEN_AES_KEY_INIT_FAILED(2039, "初始化API Token加密密钥失败"),
    AUTH_API_TOKEN_HASH_FAILED(2040, "生成令牌哈希失败"),
    AUTH_API_TOKEN_PATH_REQUIRED(2041, "接口路径不能为空"),
    AUTH_API_TOKEN_PATH_BACKSLASH_FORBIDDEN(2042, "接口路径不允许包含反斜杠"),
    AUTH_API_TOKEN_PATH_INVALID(2043, "接口路径格式不合法"),
    AUTH_API_TOKEN_PATH_TRAVERSAL_FORBIDDEN(2044, "接口路径不允许包含路径穿越"),
    ...
    WF_APPROVE_TASK_INFO_ERROR(4023,"查询流程拉取报错,{0}"),
    WF_DEFINITION_NODE_NOT_EXIST(4024,"节点已审核或不存在"),
    WF_TASK_NOT_FOUND(4025,"任务已审核或不存在"),
    WF_APPROVE_TASK_NOT_FOUND(4026,"流程拉取不存在"),
    WF_TASK_DETAIL_NOT_FOUND(4027,"流程拉取明细不存在"),
    ...
    WH_WEGO_SDK_SIGN_PARAMS_EMPTY(11186,"WEGO 签名参数不能为空"),
    WH_WEGO_SDK_SIGN_SECRET_EMPTY(11187,"WEGO 签名 secret 不能为空"),
    WH_WEGO_SDK_SIGN_JSON_EMPTY(11188,"WEGO 签名 JSON 不能为空"),
    WH_WEGO_RETURN_AUTH_INFO_NOT_FOUND(11189,"WEGO 退货入库：授权信息不存在"),
    WH_WEGO_RETURN_AUTH_ID_NOT_FOUND(11190,"WEGO 退货入库：对应授权ID不存在, nextLevelId={0}"),
    WH_WEGO_RETURN_AUTH_JSON_EMPTY(11191,"WEGO 退货入库：服务商[{0}] auth_json 为空"),
    WH_WEGO_RETURN_TOKEN_SECRET_MISSING(11192,"WEGO 退货入库：服务商[{0}] appToken/appSecret 缺失"),
    WH_WEGO_RETURN_PAGE_QUERY_ERROR(11193,"WEGO 退货入库分页查询调用异常, pageNum={0}: {1}"),
    WH_WEGO_RETURN_RESPONSE_EMPTY(11194,"WEGO 退货入库分页查询接口响应为空, pageNum={0}"),
    WH_WEGO_RETURN_RESPONSE_FAILED(11195,"WEGO 退货入库分页查询接口失败: errorCode={0}, errorMsg={1}"),
    WH_WEGO_RETURN_RESULT_EMPTY(11196,"WEGO 退货入库分页查询接口 result 为空, pageNum={0}"),
    ...
    WH_BORROW_WAREHOUSE_USABLE_INSUFFICIENT(11218,"SKU【{0}】，借调仓【{1}】，可用数【{2}】，可用库存不足，无法借调"),
    ...
    FIRST_MILE_SHIPMENT_SAVE_FAILED(11534,"头程发货单保存失败"),
    FIRST_MILE_SHIPMENT_DECLARE_SOURCE_NOT_FOUND(11535,"头程发货单【{0}】未查询到可生成报关明细的来源数据"),
    FIRST_MILE_SHIPMENT_PACKING_TASK_NOT_GENERATED(11536,"未生成装箱任务，不允许审核"),
    FIRST_MILE_SHIPMENT_PACKING_WEIGHT_REQUIRED_FOR_APPROVE(11537,"已装箱且全部称重后才能审核通过"),
    FIRST_MILE_SHIPMENT_AVAILABLE_INVENTORY_INSUFFICIENT(11538,"仓库【{0}】SKU【{1}】发货数量【{2}】无足够可用库存【{3}】"),
    FIRST_MILE_SHIPMENT_MACHINE_EXISTS_DELETE_FORBIDDEN(11539,"存在关联的加工单【{0}】，头程发货单禁止删除"),
    FIRST_MILE_SHIPMENT_INBOUND_EXISTS_DELETE_FORBIDDEN(11540,"存在下游海外入库单【{0}】，禁止删除"),
    FIRST_MILE_SHIPMENT_THIRD_WAREHOUSE_APPROVE_PUSH_FAILED(11541,"推送第三方仓库【发货单审核】失败：{0}"),
    ...
    VM_WDT_ENTITY_INVENTORY_INSUFFICIENT(12526,"提交失败，旺店通【{0}】【{1}】可用库存不足无法分货，可用库存【{2}】，分配数量【{3}】"),
    VM_CHECK_OUT_VIRTUAL_WAREHOUSE_INVENTORY(12527,"SKU【{0}】实体仓【{1}】虚拟仓库【{2}】库存已分配【{3}】，出库数量【{4}】超过分配数量【{5}】"),
    /**
    ...
    LOGISTICS_PRODUCT_LOGISTIC_DECLARE_PRICE_REQUIRED(13633,"商品物流申报单价缺失，商品编码：{0}"),
    LOGISTICS_COMBO_DECLARE_CHILD_EMPTY(13634,"组合品拆分子件为空，商品编码：{0}"),
    LOGISTICS_COMBO_DECLARE_CHILD_QTY_EMPTY(13635,"组合品拆分子件数量为空，商品编码：{0}"),
    LOGISTICS_DECLARE_DETAIL_MID_AUTO_GENERATE_FAILED(13636,"{0}【{1}】自动生成报关明细中间表失败：{2}"),
    LOGISTICS_DECLARE_BILL_AUTO_GENERATE_FAILED(13637,"{0}【{1}】自动生成报关单失败：{2}"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_REQUIRED(13638,"请选择需要预览的报关明细"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_NOT_FOUND(13639,"部分报关明细中间表数据不存在"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_STATUS_LIMIT(13640,"仅支持未生成和待确认的报关明细预览"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_SOURCE_TYPE_CONFLICT(13641,"不同来源类型的报关明细不能合并预览"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_COUNTRY_CONFLICT(13642,"不同业务单号目的国不一致，不能合并预览"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_RULE_NOT_FOUND(13643,"未找到匹配的报关规则"),
    LOGISTICS_DECLARE_DETAIL_MID_PREVIEW_SENDER_TYPE_CONFLICT(13644,"报关规则发货人类型不一致，不能合并预览"),
    LOGISTICS_DECLARE_DETAIL_MID_MERGE_MIN_COUNT_REQUIRED(13645,"至少选择两条明细进行合并"),
    LOGISTICS_DECLARE_STATUS_UPDATE_FORBIDDEN(13646,"报关状态不允许从【{0}】更新为【{1}】"),
    LOGISTICS_DECLARE_STATUS_CONFIRM_DATE_REQUIRED(13647,"报关确认日期不能为空"),
    LOGISTICS_DECLARE_STATUS_CONFIRM_USER_REQUIRED(13648,"报关确认人不能为空"),
    LOGISTICS_DECLARE_STATUS_DETAIL_REQUIRED(13649,"报关明细【{0}】不能为空"),
    LOGISTICS_DECLARE_STATUS_DETAIL_INCONSISTENT(13650,"报关明细【{0}】必须保持一致"),
    ...
    LOGISTICS_DECLARE_RULE_SENDER_RECEIVER_DUPLICATE(13656,"报关规则发货人和收货人组合不能重复"),
    LOGISTICS_DECLARE_RULE_CONDITION_DUPLICATE(13657,"报关规则条件字段【{0}】的值【{1}】不能重复"),
    LOGISTICS_DECLARE_RULE_SAVE_FAILED(13658,"报关规则配置保存失败"),
    LOGISTICS_DECLARE_GENERATABLE_DELIVERY_NOT_FOUND(13659,"没有可生成报关单的发货单"),
    LOGISTICS_DECLARE_DETAIL_SAVE_REQUIRED(13660,"请选择需要保存的报关明细"),
    LOGISTICS_DECLARE_GENERATABLE_DETAIL_NOT_FOUND(13661,"没有可生成报关单的明细"),
    LOGISTICS_DECLARE_BILL_SAVE_FAILED(13662,"报关单保存失败"),
    LOGISTICS_DECLARE_WAIT_STATUS_REQUIRED_FOR_EDIT(13663,"报关单状态不是待确认，不能编辑"),
    LOGISTICS_DECLARE_DETAIL_SAVE_FAILED(13664,"报关单明细保存失败"),
    LOGISTICS_DECLARE_DELIVERY_NOT_FOUND(13665,"未找到发货单信息"),
    LOGISTICS_DECLARE_SO_OUT_NOT_FOUND(13666,"未找到销售出库单信息"),
    LOGISTICS_DECLARE_STATUS_INVALID(13667,"报关状态无效"),
    LOGISTICS_DECLARE_SELECTED_SKU_REQUIRED(13668,"选中的SKU信息不能为空"),
    LOGISTICS_DECLARE_SOURCE_HEADER_NOT_FOUND(13669,"未找到来源单据，无法查询报关表头"),
    LOGISTICS_DECLARE_GENERATABLE_SO_DELIVERY_NOT_FOUND(13670,"没有可生成报关单的销售发货通知单"),
    LOGISTICS_DECLARE_BATCH_UPDATE_FAILED(13671,"报关单批量更新失败"),
    LOGISTICS_DECLARE_FIELD_DATE_FORMAT_INVALID(13672,"{0}格式错误，请使用yyyy-MM-dd"),
    LOGISTICS_DECLARE_FIELD_NUMBER_FORMAT_INVALID(13673,"{0}格式错误，请输入数字"),
    LOGISTICS_DECLARE_FM_SPLIT_VIEW_FORBIDDEN(13674,"非头程报关单，无法查看拆分明细"),
    LOGISTICS_DECLARE_SPLIT_WAIT_STATUS_REQUIRED(13675,"仅待确认报关单可以拆分明细"),
    LOGISTICS_DECLARE_SPLIT_SINGLE_BILL_REQUIRED(13676,"批量添加拆分明细只能关联同一张报关单"),
    LOGISTICS_DECLARE_FM_SPLIT_ADD_FORBIDDEN(13677,"非头程报关单，无法添加拆分明细"),
    LOGISTICS_DECLARE_B2B_SPLIT_ADD_FORBIDDEN(13678,"非B2B报关单，无法添加拆分明细"),
    LOGISTICS_DECLARE_MERGE_WAIT_STATUS_REQUIRED(13679,"报关单【{0}】仅待确认状态可以操作合并"),
    LOGISTICS_DECLARE_RULE_NOT_FOUND_FOR_SOURCE(13680,"来源单【{0}】未找到匹配的报关规则，请检查来源单信息是否正确"),
    LOGISTICS_DECLARE_MERGE_SKU_LIMIT_EXCEEDED(13681,"合并后SKU条数不可超过{0}条"),
    LOGISTICS_DECLARE_MERGE_COUNTRY_MISMATCH(13682,"不同业务单号必须归属同一个国家"),
    LOGISTICS_DECLARE_DETAIL_SOURCE_REQUIRED(13683,"第{0}行来源明细不能为空"),
    LOGISTICS_DECLARE_DETAIL_SOURCE_DUPLICATE(13684,"第{0}行来源明细重复，请勿重复保存"),
    LOGISTICS_DECLARE_DETAIL_QTY_MISMATCH(13685,"第{0}行数量必须等于来源明细数量合计"),
    LOGISTICS_DECLARE_SOURCE_DETAIL_NOT_FOUND_FOR_SAVE(13686,"未找到来源明细，无法保存报关单"),
    LOGISTICS_DECLARE_SELECTED_DETAIL_GENERATED(13687,"所选明细已生成报关单，请勿重复保存"),
    LOGISTICS_DECLARE_SOURCE_GENERATED(13688,"来源单【{0}】已生成报关单，请勿重复保存"),
    LOGISTICS_DECLARE_SOURCE_BOX_IMMUTABLE(13689,"业务单号、SKU、箱号不可编辑"),
    LOGISTICS_DECLARE_SOURCE_QTY_IMMUTABLE(13690,"业务单号、SKU、数量不可编辑"),
    LOGISTICS_DECLARE_DUPLICATE_SKU_ROW(13691,"相同SKU【{0}】存在不同行明细：第{1}行、第{2}行"),
    LOGISTICS_DECLARE_DETAIL_SOURCE_VALUE_MISMATCH(13692,"第{0}行{1}与来源明细不一致"),
    LOGISTICS_DECLARE_DETAIL_FIELD_REQUIRED(13693,"第{0}行{1}不能为空"),
    LOGISTICS_DECLARE_DETAIL_FIELD_POSITIVE_REQUIRED(13694,"第{0}行{1}必须大于0"),
    LOGISTICS_DECLARE_REPLACE_TYPE_MISMATCH(13695,"报关单【{0}】类型与当前保存不一致，无法合并替换"),
    ...
    LOGISTICS_DECLARE_BOX_NOT_FULL_SELECTED(13704,"业务单号【{0}】箱号【{1}】未勾选完整，缺失SKU：{2}"),
    ...
    LOGISTICS_DECLARE_B2B_CUSTOMER_RECEIVER_NOT_FOUND(13713,"来源单【{0}】未找到客户信息，无法按客户分发规则设置报关收货人"),
    ...
    COMMON_CFG_SETTING_SAVE_FAILED(13718,"系统配置管理保存失败"),
    COMMON_CFG_SETTING_TYPE_INVALID(13719,"系统配置类型不正确"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_CONFIG_REQUIRED(13720,"合同协议号配置不能为空"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_COMPANY_REQUIRED(13721,"核算公司不能为空"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_REQUIRED(13722,"合同协议号不能为空"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_LETTERS_ONLY(13723,"合同协议号只能输入英文字母"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_COMPANY_INVALID(13724,"核算公司不存在或已禁用"),
    LOGISTICS_CONTRACT_AGREEMENT_NO_DUPLICATE(13725,"核算公司和合同协议号不可重复"),
    ...
    LOGISTICS_RECON_BILL_CONFIRM_ALREADY_TO_BE_CONFIRM(13673,"当前关联数据已是待确认，无需更新"),
    ...
    LOGISTICS_DECLARE_SENDER_REQUIRED(13649,"境内发货人未选择"),
    /**
