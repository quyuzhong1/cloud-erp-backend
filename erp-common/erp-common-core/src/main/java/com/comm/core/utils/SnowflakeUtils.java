...
public class SnowflakeUtils {
     ...
     * 时间部分所占长度
     */
    private static final int TIME_LEN = 40;
     ...
     */
    private static final long DATA_ID = 9;

    /**
     * 自动机器id（可以手动定义 0-31之间的数）
     */
    private static final long WORK_ID = 3;
    ...
    public synchronized static long genId() {
        ...
        if (now < LAST_TIME_STAMP) {
            throw new RuntimeException(String.format("系统时间错误！ %d 毫秒内拒绝生 成雪花ID！", START_TIME - now));
        }
        //等于当前时间，毫秒内生成id序列加1
        if (now == LAST_TIME_STAMP) {
        ...
        //上次生成ID的时间截
        LAST_TIME_STAMP = now;
        return ((now - START_TIME) << TIME_LEFT_BIT) | (DATA_ID << DATA_LEFT_BIT)
                | (WORK_ID << WORK_LEFT_BIT) | LAST_SEQ;
    }
