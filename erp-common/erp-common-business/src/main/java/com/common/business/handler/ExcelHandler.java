...
import com.common.business.annotation.ImportExcel;
import com.common.business.utils.ImportExcelUtil;
...
public class ExcelHandler implements InitializingBean {
     ...
     * @return
     */
    public ApiResult dealExcel(MultipartFile excelFile, String handlerName) {
        ApiResult baseResult = new ApiResult();
        ...
        if (Objects.isNull(fileName) || StringUtils.isBlank(fileName) || fileName.lastIndexOf("//.") < 0) {
            throw new ServiceException(StrUtil.format("导入文件名称错误：{}", String.valueOf(fileName)));
        }
        // 2、获取filetype
        ...
        try {
            ...
            log.error("文件解析失败", e);
            throw new ServiceException(StrUtil.format("文件解析异常：{}", e.getMessage()));
        } finally {
            try {
                workbook.close();
            } catch (IOException e) {
