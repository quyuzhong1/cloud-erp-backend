    ...
    */
    ...
    private void handleSubcontractOrder(List<String> ids,List<SubcontractChangeEntity> list) {
        ...
        if (CollectionUtils.isEmpty(detailEntityList)) {
            throw new ServiceException(ApiError.ERROR_SCM_OUTSOURCING_CHANGE_DETAIL_NOT_FOUND);
        }
        //新增
        ...
        if (CollectionUtils.isNotEmpty(deleteList)) {
            ...
            if (CollectionUtils.isNotEmpty(sourceDetailIds)) {
                ...
                if (CollectionUtils.isNotEmpty(podList)) {
                    ...
                    throw new ServiceException(new ApiResult(ApiError.ERROR_SCM_SKU_HAS_PUSHED_DOC_CHANGE_DELETE_FORBIDDEN.getCode(), StrUtil.format(ApiError.ERROR_SCM_SKU_HAS_PUSHED_DOC_CHANGE_DELETE_FORBIDDEN.getMsg(),codes)));
                }
                //删除委外订单明细
    ...
    private void checkGenerateUpdate (List<SubcontractChangeDetailEntity> updateList) {
        ...
        for (SubcontractChangeDetailEntity updateEntity : updateList) {
            ...
            if (purchaseQty > updateEntity.getQty()) {
                throw new ServiceException(new ApiResult(ApiError.ERROR_TRANSFER_MB_UPDATE.getCode(),StrUtil.format(ApiError.ERROR_TRANSFER_MB_UPDATE.getMsg(),updateEntity.getSkuNo(),updateEntity.getQty(),purchaseQty)));
            }
            //仓库和供应商未改变则直接跳过
            ...
            if (receiveCount > 0) {
                throw new ServiceException(new ApiResult(ApiError.ERROR_SCM_DOC_ALREADY_GENERATED_DELIVERY_CANNOT_CHANGE_WH_OR_SUPPLIER.getCode(),StrUtil.format(ApiError.ERROR_SCM_DOC_ALREADY_GENERATED_DELIVERY_CANNOT_CHANGE_WH_OR_SUPPLIER.getMsg(),updateEntity.getSkuNo())));
            }

            //入库数量
            long instockCount = poInstockDetailList.stream().filter(obj -> podIdList.contains(obj.getPurchaseOrderDetailId())).count();
            if (instockCount > 0) {
                throw new ServiceException(new ApiResult(ApiError.ERROR_SCM_DOC_ALREADY_GENERATED_INBOUND_CANNOT_CHANGE_WH_OR_SUPPLIER.getCode(),StrUtil.format(ApiError.ERROR_SCM_DOC_ALREADY_GENERATED_INBOUND_CANNOT_CHANGE_WH_OR_SUPPLIER.getMsg(),updateEntity.getSkuNo())));
            }
        }
    }
    /**
    ...
    private void generateAdd (List<SubcontractChangeDetailEntity> addList,List<SubcontractChangeEntity> list) {
        ...
        if (CollectionUtils.isEmpty(parentList)) {
            throw new ServiceException(ApiError.ERROR_SCM_OUTSOURCING_CHANGE_DETAIL_PARENT_SKU_NOT_FOUND);
        }
        //新增(根据变更单分组)
        ...
        for (Map.Entry<String, List<SubcontractChangeDetailEntity>> entry : map.entrySet()) {
            ...
            for (SubcontractChangeDetailEntity addEntity : value) {
                ...
                if (CollectionUtils.isEmpty(childList)) {
                    throw new ServiceException(ApiError.ERROR_SCM_OUTSOURCING_CHANGE_DETAIL_CHILD_SKU_NOT_FOUND);
                }
