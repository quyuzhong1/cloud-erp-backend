...
//                        .reduce(MathUtil.ZERO, Integer::sum);
    ...
    public void updatePurchaseOrderByUnique(PurchaseOrderDTO.UpdateContractStampStatusParamsDTO dto) {
        //使用唯一键找到采购订单
        PurchaseOrderEntity purchaseOrderEntity = this.getOne(new QueryWrapper<PurchaseOrderEntity>().eq(dto.getField(), dto.getValue()));
        if (ObjectUtil.isEmpty(purchaseOrderEntity)) {
            throw new ServiceException("未找到采购订单数据");
        }
        String name = ContractStampStatusEnum.getName(dto.getContractStampStatus());
        purchaseOrderEntity.setContractStampStatus(dto.getContractStampStatus());
        this.updateById(purchaseOrderEntity);
        moduleOperateLogService.addModuleOperateLog(String.format("合同盖章状态由[%s]变更为[%s]", ContractStampStatusEnum.getName(purchaseOrderEntity.getContractStampStatus()), name), ModuleTypeEnum.PURCHASE_ORDER.getCode(), purchaseOrderEntity.getId(), "合同盖章状态更新");
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    @GlobalTransactional(rollbackFor = Exception.class)
    public void insert(PurchaseOrderDTO.AddDTO dto) {
        PurchaseOrderEntity add = this.add(dto);
        moduleOperateLogService.addModuleOperateLog(String.format("新增采购订单[%s]", add.getCode()), ModuleTypeEnum.PURCHASE_ORDER.getCode(), add.getId(), "新增采购订单");
    }

    @Override
    public List<PurchaseOrderEntity> getPurchaseOrderByCode(HashSet<String> strings) {
        return this.list(new LambdaQueryWrapper<PurchaseOrderEntity>().in(PurchaseOrderEntity::getCode, strings));
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    @GlobalTransactional(rollbackFor = Exception.class)
    public void updateApproveStatus(PurchaseOrderDTO.UpdateApprovalStatusDTO updateApprovalStatusDTO) {
        PurchaseOrderEntity purchaseOrderEntity = updateApprovalStatusDTO.getPurchaseOrderEntity();
        String approveStatus = updateApprovalStatusDTO.getApproveStatus();
        //更新审核状态
        lambdaUpdate().in(PurchaseOrderEntity::getId, purchaseOrderEntity.getId())
                .set(PurchaseOrderEntity::getApproveStatus, approveStatus)
                .set(PurchaseOrderEntity::getApproveUserId, purchaseOrderEntity.getApproveUserId())
                .set(PurchaseOrderEntity::getApproveUserName, purchaseOrderEntity.getApproveUserName())
                .set(PurchaseOrderEntity::getApproveTime, null)
                .update();
    }
}