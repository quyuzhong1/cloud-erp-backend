...
import com.erp.server.scm.mapper.AssetPurchaseOrderSupplierMapper;
...
@Slf4j
@Service
public class AssetPurchaseOrderSupplierServiceImpl extends SuperServiceImpl<AssetPurchaseOrderSupplierMapper, AssetPurchaseOrderSupplierEntity> implements AssetPurchaseOrderSupplierService {
    @Autowired
    ...
    @Override
    public BaseResultDTO.AddDTO add(AssetPurchaseOrderSupplierDTO.AddDTO addDTO) {
        AssetPurchaseOrderSupplierEntity assetPurchaseOrderSupplierEntity = new AssetPurchaseOrderSupplierEntity();
        BeanMapperUtils.copy(addDTO, assetPurchaseOrderSupplierEntity);

        // 数据处理
        handleData(assetPurchaseOrderSupplierEntity);

        log.info("开始新增");
        boolean save = super.save(assetPurchaseOrderSupplierEntity);
        if(!save) {
        ...
        moduleOperateLogService.addModuleOperateLog(msg, null, assetPurchaseOrderSupplierEntity.getId(), "新增操作");
        // TODO 新增明细（如果有明细的话）
    ...
    */
    ...
    public Boolean update(AssetPurchaseOrderSupplierDTO.UpdateDTO addOrUpdateDTO) {
        ...
        boolean save = super.updateById(assetPurchaseOrderSupplierEntity);
        if(!save) {
            throw new ServiceException("保存失败");
        }
        ...
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        moduleOperateLogService.addModuleOperateLogByObj(old, assetPurchaseOrderSupplierEntity, null, assetPurchaseOrderSupplierEntity.getId(),"", msg);
        return Boolean.TRUE;
    }
    ...
    * 新增修改处理数据
    */
    private void handleData(AssetPurchaseOrderSupplierEntity assetPurchaseOrderSupplierEntity) {
    // TODO 验证数据 & 数据赋值
    }
}
