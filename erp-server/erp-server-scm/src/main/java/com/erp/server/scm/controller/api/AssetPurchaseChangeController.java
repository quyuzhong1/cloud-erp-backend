...
public class AssetPurchaseChangeController extends BaseController {
    ...
    */
    ...
    @LogAction(value = LogActionEnum.ADD_AND_SUBMIT, desc = "资产采购变更单新增并提交")
    ...
    */
    ...
    @LogAction(value = LogActionEnum.UPDATE_AND_SUBMIT, desc = "资产采购变更单修改并提交")
    @DataPermission(operationType = DataAttributeEnum.CHECK_BY_ID,
            tableField = "create_user_id",
    ...
    */
    ...
    public ApiResult<List<BatchResultDTO>> batchCancelProcess(@RequestBody @Validated BaseIdsDTO.IdsDTO dto) {
        ...
        for (String id : dto.getIds()) {
            ...
            try {
                cancelResult = assetPurchaseChangeService.cancelProcess(new ApproveDTO.CancelProcessDTO(id));
            }catch (Exception e){
                log.error("撤回流程失败",e);
                AssetPurchaseChangeEntity entity = idEntityMap.get(id);
                if (ObjectUtil.isEmpty(entity)) {
