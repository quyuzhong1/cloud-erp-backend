...
public class TmsReconciliationCostServiceImpl extends SuperServiceImpl<TmsReconciliationCostMapper, TmsReconciliationCostEntity> implements TmsReconciliationCostService {
    ...
    */
    ...
    public Boolean update(TmsB2cDeclareReconciliationCostDTO.UpdateDTO updateDTO) {
        TmsReconciliationCostEntity old = super.getById(updateDTO.getId());
        Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.NOT_EXIST_BILL, "对账费用单"));
        TmsReconciliationCostEntity tmsReconciliationCostEntity =  BeanMapperUtils.map(TmsReconciliationCostEntity.class, updateDTO);

        // 数据处理
        handleData(tmsReconciliationCostEntity);
        log.info("编辑 开始修改对账费用单数据，id：【{}】", old.getId());
        boolean save = super.updateById(tmsReconciliationCostEntity);
        if(!save) {
            throw new ServiceException("对账费用单保存失败");
        }
        return Boolean.TRUE;
