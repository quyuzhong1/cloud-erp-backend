...
public class LogisticsMappingServiceImpl extends SuperServiceImpl<LogisticsMappingMapper, LogisticsMappingEntity> implements LogisticsMappingService {
    ...
    @Override
    public BaseResultDTO.AddDTO add(LogisticsMappingDTO.AddDTO addDTO) {
        LogisticsMappingEntity logisticsMappingEntity = new LogisticsMappingEntity();
        BeanMapperUtils.copy(addDTO, logisticsMappingEntity);

        // 数据处理
        handleData(logisticsMappingEntity);

        log.info("开始新增物流渠道映射单");
        boolean save = super.save(logisticsMappingEntity);
        if(!save) {
            throw new ServiceException("物流渠道映射单保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", commonService.getUserInfo().getUserName(), "物流渠道映射单" , logisticsMappingEntity.getId());
