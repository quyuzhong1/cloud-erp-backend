...
public class InventorySkuCostDetailServiceImpl extends SuperServiceImpl<InventorySkuCostDetailMapper, InventorySkuCostDetailEntity> implements InventorySkuCostDetailService {
    ...
    public BaseResultDTO.AddDTO add(InventorySkuCostDetailDTO.AddDTO addDTO) {
        ...
        log.info("开始新增SKU存货成本明细");
        boolean save = super.save(inventorySkuCostDetailEntity);
        if(!save) {
            throw new ServiceException("SKU存货成本明细保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", UserContext.getDefaultLoginUser().getUserName(), "SKU存货成本明细" , inventorySkuCostDetailEntity.getId());
    ...
    */
    ...
    public Boolean update(InventorySkuCostDetailDTO.UpdateDTO updateDTO) {
        ...
        Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.NOT_EXIST_BILL, "SKU存货成本明细"));
        ...
        // 数据处理
        handleData(inventorySkuCostDetailEntity);
        log.info("编辑 开始修改SKU存货成本明细数据，id：【{}】", old.getId());
        ...
        // 记录主单操作日志
            log.info("编辑 开始记录SKU存货成本明细日志数据，id：【{}】", inventorySkuCostDetailEntity.getId());
            String msg = StrUtil.format("用户【{}】编辑id为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), inventorySkuCostDetailEntity.getId(), "SKU存货成本明细");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
