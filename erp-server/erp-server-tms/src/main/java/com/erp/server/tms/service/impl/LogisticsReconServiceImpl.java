...
public class LogisticsReconServiceImpl
    ...
    /** 异步匹配 preload 缓存有效期，覆盖同一任务的多个批次，避免重复 Feign/配置查询。 */
    private static final long RECON_PRELOAD_CACHE_TTL_MILLIS = TimeUnit.MINUTES.toMillis(10);

    /** 单实例最多保留的异步匹配 preload 数量，避免异常任务量造成无界增长。 */
    private static final int RECON_PRELOAD_CACHE_MAX_SIZE = 256;

    /** 异步匹配任务级 preload 缓存。配置数据只读，超时后自动重新加载。 */
    private final Map<String, ReconPreloadCacheEntry> reconPreloadCache = new ConcurrentHashMap<>();

    /** 异步匹配任务的 PENDING 费用项识别组缓存，避免每个批次重复加载整批任务明细。 */
    private final Map<String, ReconPendingGroupCacheEntry> reconPendingGroupCache = new ConcurrentHashMap<>();
    ...
    public BaseResultDTO.AddDTO importExcel(LogisticsReconDTO.ImportDTO dto) {
        ...
        String taskId;
        try {
            taskId = downloadTaskFeign.saveImportTask(DOC_NAME + "导入", IMPORT_TMS_LOGISTICS_RECON.getCode(), dto);
        } catch (Exception e) {
            self.markImportFail(dto.getMainIdMap() == null ? Collections.emptyList()
                    : new ArrayList<>(dto.getMainIdMap().values()), e.getMessage());
            throw e;
        }
    ...
    public void executeImportTask(LogisticsReconDTO.ImportDTO dto) {
        ...
        try {
            ...
            self.rollbackImportTask(createdMainIds, e.getMessage());
            throw e;
        }
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void markImportFail(List<String> mainIds, String failReason) {
        if (CollUtil.isEmpty(mainIds)) {
            return;
        }
        if (StrUtil.isBlank(failReason)) {
            failReason = "导入任务处理失败，请重试";
        }
    ...
    private ImportSubResolveResult resolveOrBuildImportSub(LogisticsReconDTO.ImportDTO dto,
        ...
        if (Boolean.TRUE.equals(dto.getReimportUpdate())) {
            ...
            if (StrUtil.isNotBlank(existingSubId)) {
                LogisticsReconDetailSubEntity snapshot = dto.getImportDetailSubSnapshotMap() == null
                        ? null : dto.getImportDetailSubSnapshotMap().get(subKey);
                if (snapshot != null
                        && (LogisticsReconDetailMatchStatusEnum.MATCHED.getCode().equals(snapshot.getMatchStatus())
                        || LogisticsReconReconciliationStatusEnum.CONFIRMED.getCode().equals(snapshot.getReconciliationStatus())
                        || LogisticsReconReconciliationStatusEnum.PARTIAL_CONFIRM.getCode().equals(snapshot.getReconciliationStatus()))) {
                    return new ImportSubResolveResult(null, false, seqNo,
                            "费用项已匹配或部分/全部确认，禁止重导覆盖");
                }
    ...
    private LogisticsReconEntity createImportingMain(LogisticsReconDTO.ImportDTO dto,
        ...
        LogisticsReconEntity entity = new LogisticsReconEntity()
                ...
                .setCostCount(0)
                .setMatchCount(0)
                .setMatchSuccessAmount(BigDecimal.ZERO)
                .setMatchFailAmount(BigDecimal.ZERO)
                .setReconciliationStatus(LogisticsReconReconciliationStatusEnum.TO_BE_CONFIRM.getCode());
    ...
    /**
     * 批量提交对账单自动匹配：逐主单校验 → 统计可匹配费用项 → 一主单一异步任务并游标分片预落费用项明细。
     */
    @Override
    public List<BatchResultDTO> batchMatch(LogisticsReconDTO.BatchMatchDTO dto) {
        ...
        for (String mainId : dto.getIds()) {
            try {
                ...
                // 仅在主单空闲时清理宕机残留，禁止提交线程越过主单锁误伤正在执行的匹配任务。
                if (!cleanupStaleMatchingSubsIfMainIdle(mainId, "batchMatch")) {
                    throw new ServiceException("已有匹配/确认任务正在执行，请稍后重试");
                }
                int eligibleCount = countMatchableDetailSubs(mainId);
                if (eligibleCount <= 0) {
                ...
                BatchResultDTO dispatchResult = createLogisticsReconTaskWithDetails(
                        businessType, methodType, payload, eligibleCount,
                        (taskId, bt) -> saveMatchableTaskDetails(taskId, bt, mainId),
                        "物流商对账单合并匹配异步任务派发成功，taskId: {}, 预计处理数据量: {}");
                String successMsg = StrUtil.format("已提交匹配，任务编号【{}】，请在异步任务列表查看进度",
                        dispatchResult.getCode());
                results.add(BatchResultDTO.success(dispatchResult.getId(), entity.getCode(), successMsg));
            } catch (Exception e) {
    ...
    /**
     * 分批执行匹配：主单锁内扩组 → 按识别组打包 → 整组认领校验 → {@link #doMatchSubsChunk} → 回写任务明细。
     * <p>扩组/认领均在主单锁内完成；识别组未完整认领时整组跳过，避免半组合并覆盖费用。</p>
     */
    @Override
    public TmsAsyncTaskRecordDTO.BatchProcessResult processMatchBatch(String taskId, String mainId,
                                                                      List<String> batchSubIds,
                                                                      boolean isConfirm, LoginUser operatorUser) {
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(batchSubIds)) {
            return new TmsAsyncTaskRecordDTO.BatchProcessResult(0, 0);
        }
        List<String> seedSubIds = batchSubIds.stream().filter(StrUtil::isNotBlank).distinct()
                .collect(Collectors.toList());
        ...
        List<String> expandedSubIds = Collections.emptyList();
        // 配置、币别和汇率查询可能走 Feign，先在主单锁外完成，避免远程调用占用锁。
        LogisticsReconEntity entity = super.getByIdOpt(mainId)
                .orElseThrow(() -> new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, DOC_NAME));
        LogisticsReconMatchDTO.ReconMatchPreloadDTO preload = getOrBuildReconMatchPreload(taskId, entity);
        ...
        try {
            ...
            if (!locked) {
                ...
                for (String subId : seedSubIds) {
                    ...
                    finishTaskDetail(detail, false, "获取主单锁超时，已有匹配/确认任务在执行，请稍后错误重试");
                    failed++;
                }
                return new TmsAsyncTaskRecordDTO.BatchProcessResult(success, failed);
            }
            UserContext.setLoginUser(operatorUser);
            LogisticsReconEntity currentEntity = super.getByIdOpt(mainId)
                    .orElseThrow(() -> new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, DOC_NAME));
            ...
            expandedSubIds = expandTaskPendingSubIdsByIdentifyGroup(
            ...
            // 本批已处理费用项移出 PENDING 组键缓存；任务无剩余 PENDING 时清理 preload/组键缓存
            if (!lockTimeout) {
                prunePendingGroupCache(taskId, expandedSubIds);
                if (!hasPendingTaskDetails(taskId)) {
                    invalidateReconMatchTaskCaches(taskId);
                }
            }
    ...
    private boolean hasPendingTaskDetails(String taskId) {
        if (StrUtil.isBlank(taskId)) {
            return false;
        }
        return tmsAsyncTaskDetailService.lambdaQuery()
    ...
    /**
     * 处理单个识别组打包分片：任务明细认领 → 费用项 MATCHING 认领 → 整组完整性校验 → 匹配 → 回写。
     *
     * @return int[0]=success, int[1]=failed
     */
    private int[] processMatchBatchChunk(String taskId,
                                         String mainId,
                                         List<String> chunkSubIds,
                                         boolean isConfirm,
    ...
    private void asyncClaimAndMatchByMain(String mainId, LoginUser user, boolean isConfirm) {
        ...
        try {
            ...
            // 锁内先清理超时 MATCHING，再认领（与 processMatchBatch 入口策略一致）
            self.failStaleMatchingSubsAndApplyMainStats(mainId);
    ...
    /**
     * 流式匹配整单 matching 费用项：整单级预加载一次，按 id 游标取种子 → 识别组扩展 → 再按组打包匹配。
     * <p>以种子 id 游标向前推进；同识别组费用项完整进入同一次 {@link #doMatchSubsChunk}，保证组内相同费用类型合并加总。
     * 匹配成功/失败的费用项离开 matching 后天然收敛；游标按种子推进，扩展带出的更大 id 随后会被跳过。</p>
     */
    private void streamMatchMatchingSubs(String mainId, boolean isConfirm) {
        ...
        List<CfgLogisticsCostImportDetailEntity> uniqueKeyList = resolveUniqueKeyList(preload);
        if (CollUtil.isEmpty(uniqueKeyList)) {
            log.warn("[streamMatchMatchingSubs] 对账单未配置识别唯一键，无法按识别组扩组合并 mainId={}", mainId);
        }
        String lastId = null;
        while (true) {
            ...
            List<String> seedChunk = query.list().stream()
                    .map(LogisticsReconDetailSubEntity::getId)
                    .collect(Collectors.toList());
            if (CollUtil.isEmpty(seedChunk)) {
                break;
            }
            ...
            for (List<String> chunk : chunks) {
                try {
                    ...
                    log.error("[streamMatchMatchingSubs] 分片匹配失败 mainId={} chunkSize={} nonRetryable={}",
                            mainId, chunk.size(),
                            LogisticsReconMatchFailReasonSupport.isNonRetryable(reason), e);
    ...
    /**
     * 对账单整批匹配（同步）：按识别组打包分片调用 {@link #doMatchSubsChunk}，单组不拆分，单片失败仅回写该分片。
     */
    @Override
    ...
    private LogisticsReconMatchDTO.ReconMatchPreloadDTO buildReconMatchPreload(LogisticsReconEntity entity) {
        ...
        preload.setCfgImportDetailList(Collections.unmodifiableList(new ArrayList<>(cfgDetails)));
        importHistoryRecordService.fillReconMatchCurrencyContext(preload);
        if (preload.getCurrencyLookupMap() != null) {
            preload.setCurrencyLookupMap(Collections.unmodifiableMap(new HashMap<>(preload.getCurrencyLookupMap())));
        }
        if (preload.getCurrencyRateMap() != null) {
            preload.setCurrencyRateMap(Collections.unmodifiableMap(new HashMap<>(preload.getCurrencyRateMap())));
        }
        if (preload.getCfgCostList() != null) {
            preload.setCfgCostList(Collections.unmodifiableList(new ArrayList<>(preload.getCfgCostList())));
        }
        return preload;
    }

    /**
     * 获取异步匹配任务的 preload；同一任务的多个批次复用配置、币别和费用配置，过期后重新加载。
     *
     * @param taskId 异步任务 ID
     * @param entity 对账主单
     * @return 匹配预加载上下文
     */
    private LogisticsReconMatchDTO.ReconMatchPreloadDTO getOrBuildReconMatchPreload(
            String taskId, LogisticsReconEntity entity) {
        if (StrUtil.isBlank(taskId)) {
            return buildReconMatchPreload(entity);
        }
        long now = System.currentTimeMillis();
        ReconPreloadCacheEntry cached = reconPreloadCache.get(taskId);
        if (cached != null && cached.expireAt > now) {
            return cached.preload;
        }
        if (cached != null) {
            reconPreloadCache.remove(taskId, cached);
        }
        LogisticsReconMatchDTO.ReconMatchPreloadDTO preload = buildReconMatchPreload(entity);
        reconPreloadCache.put(taskId,
                new ReconPreloadCacheEntry(preload, now + RECON_PRELOAD_CACHE_TTL_MILLIS));
        evictExpiredTaskLocalCaches(now);
        return preload;
    }

    /**
     * 任务结束或无剩余 PENDING 时清理本地 preload / PENDING 组键缓存。
     */
    private void invalidateReconMatchTaskCaches(String taskId) {
        if (StrUtil.isBlank(taskId)) {
            return;
        }
        reconPreloadCache.remove(taskId);
        reconPendingGroupCache.remove(taskId);
    }

    /**
     * 本批已扩组费用项移出 PENDING 组键缓存，避免后续批次重复携带。
     */
    private void prunePendingGroupCache(String taskId, Collection<String> processedSubIds) {
        if (StrUtil.isBlank(taskId) || CollUtil.isEmpty(processedSubIds)) {
            return;
        }
        ReconPendingGroupCacheEntry cached = reconPendingGroupCache.get(taskId);
        if (cached == null || CollUtil.isEmpty(cached.subIdGroupKeyMap)) {
            return;
        }
        for (String subId : processedSubIds) {
            if (StrUtil.isNotBlank(subId)) {
                cached.subIdGroupKeyMap.remove(subId);
            }
        }
        if (cached.subIdGroupKeyMap.isEmpty()) {
            reconPendingGroupCache.remove(taskId, cached);
        }
    }

    /**
     * 清理过期或超出容量的任务级本地缓存（preload + PENDING 组键）。
     */
    private void evictExpiredTaskLocalCaches(long now) {
        reconPreloadCache.entrySet().removeIf(entry -> entry.getValue() == null
                || entry.getValue().expireAt <= now);
        reconPendingGroupCache.entrySet().removeIf(entry -> entry.getValue() == null
                || entry.getValue().expireAt <= now);
        evictOldestTaskCacheEntries(reconPreloadCache, RECON_PRELOAD_CACHE_MAX_SIZE);
        evictOldestTaskCacheEntries(reconPendingGroupCache, RECON_PRELOAD_CACHE_MAX_SIZE);
    }

    private <T extends ReconTaskLocalCacheEntry> void evictOldestTaskCacheEntries(
            Map<String, T> cache, int maxSize) {
        while (cache.size() > maxSize) {
            ...
            for (Map.Entry<String, T> entry : cache.entrySet()) {
                ...
                if (entry.getValue().expireAt < oldestExpireAt) {
                    oldestExpireAt = entry.getValue().expireAt;
                    oldestKey = entry.getKey();
                }
            }
            if (oldestKey == null) {
    ...
    private abstract static class ReconTaskLocalCacheEntry {
        protected final long expireAt;

        private ReconTaskLocalCacheEntry(long expireAt) {
            this.expireAt = expireAt;
        }
    }

    /** 任务级 preload 缓存条目。 */
    private static final class ReconPreloadCacheEntry extends ReconTaskLocalCacheEntry {
        private final LogisticsReconMatchDTO.ReconMatchPreloadDTO preload;

        private ReconPreloadCacheEntry(LogisticsReconMatchDTO.ReconMatchPreloadDTO preload, long expireAt) {
            super(expireAt);
            this.preload = preload;
        }
    }

    /** 任务级 PENDING 费用项识别组缓存。 */
    private static final class ReconPendingGroupCacheEntry extends ReconTaskLocalCacheEntry {
        private final Map<String, String> subIdGroupKeyMap;

        private ReconPendingGroupCacheEntry(Map<String, String> subIdGroupKeyMap, long expireAt) {
            super(expireAt);
            this.subIdGroupKeyMap = subIdGroupKeyMap;
        }
    }

    /**
     * 首次加载任务全部 PENDING 的识别组键并缓存；后续批次复用，避免 O(批次×PENDING) 重复建键。
     */
    private ReconPendingGroupCacheEntry getOrBuildPendingGroupCache(
            String taskId, String mainId, List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        long now = System.currentTimeMillis();
        ReconPendingGroupCacheEntry cached = reconPendingGroupCache.get(taskId);
        if (cached != null && cached.expireAt > now) {
            return cached;
        ...
        List<String> pendingSubIds = tmsAsyncTaskDetailService.lambdaQuery()
                ...
                .distinct()
                .collect(Collectors.toList());
        Map<String, String> groupKeyMap = CollUtil.isEmpty(pendingSubIds)
                ? new ConcurrentHashMap<>()
                : new ConcurrentHashMap<>(buildSubIdGroupKeyMap(mainId, pendingSubIds, uniqueKeyList));
        ReconPendingGroupCacheEntry created = new ReconPendingGroupCacheEntry(
                groupKeyMap, now + RECON_PRELOAD_CACHE_TTL_MILLIS);
        reconPendingGroupCache.put(taskId, created);
        evictExpiredTaskLocalCaches(now);
        return created;
    }
     ...
     * 从预加载上下文解析模板唯一识别字段配置。
     */
    private List<CfgLogisticsCostImportDetailEntity> resolveUniqueKeyList(
            LogisticsReconMatchDTO.ReconMatchPreloadDTO preload) {
        if (preload == null || CollUtil.isEmpty(preload.getCfgImportDetailList())) {
            return Collections.emptyList();
        }
        List<CfgLogisticsCostImportDetailEntity> uniqueKeyList = preload.getCfgImportDetailList().stream()
                .filter(detail -> Boolean.TRUE.equals(detail.getIsUniqueKey()))
                .collect(Collectors.toList());
        return validateUniqueKeyList(uniqueKeyList);
    }

    /**
     * 剔除掉不支持的识别字段，保留可用字段继续扩组；全部非法时返回空列表。
     */
    private List<CfgLogisticsCostImportDetailEntity> validateUniqueKeyList(
            List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        if (CollUtil.isEmpty(uniqueKeyList)) {
            return Collections.emptyList();
        }
        List<String> unsupportedFields = uniqueKeyList.stream()
                .map(CfgLogisticsCostImportDetailEntity::getTargetField)
                .filter(field -> !isSupportedIdentifyTargetField(field))
                .distinct()
                .collect(Collectors.toList());
        if (CollUtil.isNotEmpty(unsupportedFields)) {
            log.warn("[validateUniqueKeyList] 忽略不支持的识别字段，继续使用其余合法字段 fields={}",
                    unsupportedFields);
        }
        return uniqueKeyList.stream()
                .filter(detail -> isSupportedIdentifyTargetField(detail.getTargetField()))
                .collect(Collectors.toList());
    }
     ...
     * 判断识别字段是否能映射到物流对账明细列。
     */
    private boolean isSupportedIdentifyTargetField(String targetField) {
        return LogisticsCostImportTargetFieldConstant.SOURCE_CODE.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.SO_CODE.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.PLATFORM_CODE.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.PLATFORM_ORDER_NO.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.TRACK_NO.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.TRANSPORT_NO.equals(targetField)
                || LogisticsCostImportTargetFieldConstant.SO_DELIVERY_CODE.equals(targetField);
    }

    /**
     * 识别组扩展只使用完整识别值，避免空字段把候选范围扩大为 NULL/空串的大集合。
     */
    private boolean hasCompleteIdentifyValues(LogisticsReconDetailEntity detail,
                                              List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        if (detail == null || CollUtil.isEmpty(uniqueKeyList)) {
            return false;
        }
        return uniqueKeyList.stream()
                .allMatch(uniqueKey -> StrUtil.isNotBlank(
                        LogisticsReconMatchGroupHelper.detailIdentifyValue(detail, uniqueKey.getTargetField())));
    }

    /**
     * 按对账单加载模板唯一识别字段（轻量，不含币别 Feign），供异步分批扩组使用。
     */
    private List<CfgLogisticsCostImportDetailEntity> loadUniqueKeyListByMainId(String mainId) {
        LogisticsReconEntity entity = super.getByIdOpt(mainId).orElse(null);
        if (entity == null || StrUtil.isBlank(entity.getCfgImportId())) {
            return Collections.emptyList();
        }
        List<CfgLogisticsCostImportDetailEntity> cfgDetails =
                cfgLogisticsCostImportDetailService.listByMainIdList(Collections.singletonList(entity.getCfgImportId()));
        if (CollUtil.isEmpty(cfgDetails)) {
            return Collections.emptyList();
        }
        return validateUniqueKeyList(cfgDetails.stream()
                .filter(detail -> Boolean.TRUE.equals(detail.getIsUniqueKey()))
                .collect(Collectors.toList()));
    }

    /**
     * 异步任务：将本批种子费用项扩展为同识别组内仍为 PENDING 的完整费用项集合。
     */
    private List<String> expandTaskPendingSubIdsByIdentifyGroup(String taskId,
                                                                String mainId,
                                                                List<String> seedSubIds,
                                                                List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        LinkedHashSet<String> result = seedSubIds == null ? new LinkedHashSet<>()
                : seedSubIds.stream().filter(StrUtil::isNotBlank).collect(Collectors.toCollection(LinkedHashSet::new));
        if (result.isEmpty() || StrUtil.isBlank(taskId) || CollUtil.isEmpty(uniqueKeyList)) {
            return new ArrayList<>(result);
        }
        // 复用任务级 PENDING 组键缓存；种子单独补键，避免每批全量扫 PENDING。
        ReconPendingGroupCacheEntry pendingCache = getOrBuildPendingGroupCache(taskId, mainId, uniqueKeyList);
        Map<String, String> candidateGroupKeyMap = new HashMap<>(pendingCache.subIdGroupKeyMap);
        Map<String, String> seedGroupKeyMap = buildSubIdGroupKeyMap(mainId, new ArrayList<>(result), uniqueKeyList);
        candidateGroupKeyMap.putAll(seedGroupKeyMap);
        LinkedHashSet<String> candidateSubIds = new LinkedHashSet<>(pendingCache.subIdGroupKeyMap.keySet());
        candidateSubIds.addAll(result);
        Set<String> targetGroupKeys = result.stream()
                .map(candidateGroupKeyMap::get)
                .filter(StrUtil::isNotBlank)
                .collect(Collectors.toSet());
        if (targetGroupKeys.isEmpty()) {
            return result.stream().sorted().collect(Collectors.toList());
        }
        return candidateSubIds.stream()
                .filter(subId -> targetGroupKeys.contains(candidateGroupKeyMap.get(subId)))
                .sorted()
                .collect(Collectors.toList());
    }
     ...
     * 将种子费用项按识别组扩展为完整组。
     * <ul>
     *   <li>{@code candidateSubIds == null}：扩展同组全部仍为 MATCHING 的费用项（流式匹配）</li>
     *   <li>非 null：仅保留候选集合内的同组成员（同步 scope）</li>
     * </ul>
     */
    private List<String> expandSubIdsByIdentifyGroup(String mainId,
                                                     List<String> seedSubIds,
                                                     Collection<String> candidateSubIds,
                                                     List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        LinkedHashSet<String> result = seedSubIds == null ? new LinkedHashSet<>()
                : seedSubIds.stream().filter(StrUtil::isNotBlank).collect(Collectors.toCollection(LinkedHashSet::new));
        if (result.isEmpty() || CollUtil.isEmpty(uniqueKeyList)) {
            return new ArrayList<>(result);
        }
        String matchStatusFilter = candidateSubIds == null
                ? LogisticsReconDetailMatchStatusEnum.MATCHING.getCode() : null;
        List<String> siblingSubIds = listSiblingSubIdsByIdentifyGroup(
                mainId, new ArrayList<>(result), uniqueKeyList, matchStatusFilter);
        if (CollUtil.isEmpty(siblingSubIds)) {
            return new ArrayList<>(result);
        }
        if (candidateSubIds != null) {
            Set<String> candidateSet = candidateSubIds.stream()
                    .filter(StrUtil::isNotBlank)
                    .collect(Collectors.toSet());
            siblingSubIds = siblingSubIds.stream().filter(candidateSet::contains).collect(Collectors.toList());
        }
        result.addAll(siblingSubIds);
        return result.stream().sorted().collect(Collectors.toList());
    }

    /**
     * 查询与种子费用项同识别组的费用项 id；{@code matchStatus} 非空时仅返回该匹配状态。
     */
    private List<String> listSiblingSubIdsByIdentifyGroup(String mainId,
                                                          List<String> seedSubIds,
                                                          List<CfgLogisticsCostImportDetailEntity> uniqueKeyList,
                                                          String matchStatus) {
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(seedSubIds) || CollUtil.isEmpty(uniqueKeyList)) {
            return Collections.emptyList();
        }
        Map<String, LogisticsReconDetailEntity> seedDetailMap = new HashMap<>();
        Map<String, String> seedGroupKeyMap = buildSubIdGroupKeyMap(
                mainId, seedSubIds, uniqueKeyList, seedDetailMap);
        Set<String> targetGroupKeys = seedGroupKeyMap.values().stream()
                .filter(StrUtil::isNotBlank)
                .collect(Collectors.toCollection(LinkedHashSet::new));
        if (targetGroupKeys.isEmpty()) {
            return Collections.emptyList();
        }
        Set<String> siblingDetailIds = findDetailIdsByGroupKeys(
                mainId, targetGroupKeys, uniqueKeyList, seedDetailMap.values());
        if (CollUtil.isEmpty(siblingDetailIds)) {
            return Collections.emptyList();
        }
        List<String> siblingSubIds = new ArrayList<>();
        List<String> detailIdList = new ArrayList<>(siblingDetailIds);
        for (int i = 0; i < detailIdList.size(); i += MATCH_ID_BATCH_SIZE) {
            List<String> batch = detailIdList.subList(i, Math.min(detailIdList.size(), i + MATCH_ID_BATCH_SIZE));
            LambdaQueryChainWrapper<LogisticsReconDetailSubEntity> subQuery = logisticsReconDetailSubService.lambdaQuery()
                    .select(LogisticsReconDetailSubEntity::getId)
                    .eq(LogisticsReconDetailSubEntity::getMainId, mainId)
                    .in(LogisticsReconDetailSubEntity::getDetailId, batch);
            if (StrUtil.isNotBlank(matchStatus)) {
                subQuery.eq(LogisticsReconDetailSubEntity::getMatchStatus, matchStatus);
            }
            subQuery.list().forEach(sub -> {
                if (StrUtil.isNotBlank(sub.getId())) {
                    siblingSubIds.add(sub.getId());
                }
            });
        }
        return siblingSubIds;
    }

    /**
     * 识别组完整性校验：分包内同组「仍需处理」的成员必须全部成功进入 MATCHING 才允许匹配。
     * <p>任务明细已 FINISH 的成员视为既往已完成，不参与完整性判断；其余任一未认领成功则整组跳过。</p>
     *
     * @param expectedSubIds      分包内费用项全集
     * @param claimedSubIds       实际认领成功进入 MATCHING 的费用项
     * @param detailMap           任务明细（用于排除已 FINISH）
     * @param incompleteSubIdsOut 不完整组中仍需处理的成员
     * @return 可安全匹配的完整组费用项 id
     */
    private List<String> filterCompleteIdentifyGroups(String mainId,
        ...
        Set<String> claimedSet = claimedSubIds == null ? Collections.emptySet() : claimedSubIds;
        if (CollUtil.isEmpty(uniqueKeyList)) {
        ...
        for (String subId : expectedSubIds) {
            ...
            byGroupKey.computeIfAbsent(groupKey, key -> new ArrayList<>()).add(subId);
        }
        List<String> executable = new ArrayList<>();
        for (List<String> members : byGroupKey.values()) {
            List<String> activeMembers = new ArrayList<>();
            for (String subId : members) {
                TmsAsyncTaskDetailEntity detail = detailMap == null ? null : detailMap.get(subId);
                if (detail != null && TmsAsyncTaskRecordStatusEnum.FINISH.getCode().equals(detail.getStatus())) {
                    continue;
                }
                activeMembers.add(subId);
            }
            if (CollUtil.isEmpty(activeMembers)) {
                continue;
            }
            boolean complete = claimedSet.containsAll(activeMembers);
            if (complete) {
                executable.addAll(activeMembers);
            } else if (incompleteSubIdsOut != null) {
                incompleteSubIdsOut.addAll(activeMembers);
            }
        }
        return executable;
    }
    ...
    /**
     * 按识别组打包费用项分片：完整组不拆分；单组超过阈值时整组单独一批（与导入匹配一致）。
     */
    ...
    private List<List<String>> packSubIdsByIdentifyGroup(String mainId,
        ...
        int limit = maxChunkSize > 0 ? maxChunkSize : MATCH_CHUNK_SIZE;
        if (CollUtil.isEmpty(uniqueKeyList)) {
            List<List<String>> plainChunks = new ArrayList<>();
            for (int i = 0; i < subIds.size(); i += limit) {
                plainChunks.add(new ArrayList<>(subIds.subList(i, Math.min(subIds.size(), i + limit))));
            }
            return plainChunks;
        }
        ...
        for (String subId : subIds) {
            ...
            inputsByGroupKey.computeIfAbsent(groupKey, key -> new ArrayList<>()).add(subId);
        }
        List<List<String>> chunks = new ArrayList<>();
        List<String> chunk = new ArrayList<>();
        for (Map.Entry<String, List<String>> entry : inputsByGroupKey.entrySet()) {
            List<String> groupSubIds = entry.getValue();
            if (groupSubIds.size() > limit) {
                log.warn("[packSubIdsByIdentifyGroup] 单识别组费用项超过分片阈值，整组提交不拆分 mainId={} groupKey={} subCount={} threshold={}",
                        mainId, entry.getKey(), groupSubIds.size(), limit);
                if (CollUtil.isNotEmpty(chunk)) {
                    chunks.add(new ArrayList<>(chunk));
                    chunk.clear();
                }
                chunks.add(new ArrayList<>(groupSubIds));
                continue;
            }
            if (chunk.size() + groupSubIds.size() > limit) {
                chunks.add(new ArrayList<>(chunk));
                chunk.clear();
            }
            chunk.addAll(groupSubIds);
        }
        if (CollUtil.isNotEmpty(chunk)) {
            chunks.add(chunk);
        }
     ...
     * 构造费用项 id → 识别组键。
     */
    private Map<String, String> buildSubIdGroupKeyMap(String mainId,
                                                      List<String> subIds,
                                                      List<CfgLogisticsCostImportDetailEntity> uniqueKeyList) {
        return buildSubIdGroupKeyMap(mainId, subIds, uniqueKeyList, null);
    }

    /**
     * 构造费用项识别组键，并按需返回本次已加载的所属明细，供扩组查询复用以避免重复读取。
     *
     * @param mainId 主单 ID
     * @param subIds 费用项 ID
     * @param uniqueKeyList 唯一识别字段配置
     * @param detailMapOut 已加载明细输出；为 null 时不输出
     * @return 费用项 ID 到识别组键的映射
     */
    private Map<String, String> buildSubIdGroupKeyMap(String mainId,
                                                      List<String> subIds,
                                                      List<CfgLogisticsCostImportDetailEntity> uniqueKeyList,
                                                      Map<String, LogisticsReconDetailEntity> detailMapOut) {
        Map<String, String> result = new HashMap<>();
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(subIds) || CollUtil.isEmpty(uniqueKeyList)) {
            return result;
        }
        ...
        for (int i = 0; i < subIds.size(); i += MATCH_ID_BATCH_SIZE) {
            ...
            logisticsReconDetailSubService.lambdaQuery()
                    .select(LogisticsReconDetailSubEntity::getId, LogisticsReconDetailSubEntity::getDetailId)
                    .eq(LogisticsReconDetailSubEntity::getMainId, mainId)
                    .in(LogisticsReconDetailSubEntity::getId, batch)
                    .list()
                    .forEach(sub -> subMap.put(sub.getId(), sub));
        }
        Set<String> detailIds = subMap.values().stream()
                .map(LogisticsReconDetailSubEntity::getDetailId)
                .filter(StrUtil::isNotBlank)
                .collect(Collectors.toSet());
        Map<String, LogisticsReconDetailEntity> detailMap = loadDetailsByIds(mainId, detailIds);
        if (detailMapOut != null) {
            detailMapOut.putAll(detailMap);
        }
        for (String subId : subIds) {
            ...
            LogisticsReconDetailEntity detail = detailMap.get(sub.getDetailId());
            if (!hasCompleteIdentifyValues(detail, uniqueKeyList)) {
                continue;
            }
            result.put(subId, LogisticsReconMatchGroupHelper.buildDetailGroupKey(detail, uniqueKeyList));
        }
        return result;
    }

    /**
     * 按识别组键反查本主单下可能命中的明细 id（先按识别字段 IN 粗查，再精确过滤组键）。
     */
    private Set<String> findDetailIdsByGroupKeys(String mainId,
                                                 Set<String> targetGroupKeys,
                                                 List<CfgLogisticsCostImportDetailEntity> uniqueKeyList,
                                                 Collection<LogisticsReconDetailEntity> seedDetails) {
        Set<String> result = new LinkedHashSet<>();
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(targetGroupKeys) || CollUtil.isEmpty(uniqueKeyList)) {
            return result;
        }
        Set<String> soCodes = new LinkedHashSet<>();
        Set<String> platformOrderNos = new LinkedHashSet<>();
        Set<String> trackNos = new LinkedHashSet<>();
        Set<String> transportNos = new LinkedHashSet<>();
        Set<String> soDeliveryCodes = new LinkedHashSet<>();
        for (LogisticsReconDetailEntity detail : seedDetails) {
            collectIdentifyFilterValues(detail, uniqueKeyList, soCodes, platformOrderNos, trackNos, transportNos, soDeliveryCodes);
        }
        if (CollUtil.isEmpty(soCodes) && CollUtil.isEmpty(platformOrderNos)
                && CollUtil.isEmpty(trackNos) && CollUtil.isEmpty(transportNos) && CollUtil.isEmpty(soDeliveryCodes)) {
            return seedDetails.stream()
                    .map(LogisticsReconDetailEntity::getId)
                    .filter(StrUtil::isNotBlank)
                    .collect(Collectors.toCollection(LinkedHashSet::new));
        }

        List<LogisticsReconDetailEntity> validSeedDetails = seedDetails.stream()
                .filter(Objects::nonNull)
                .filter(detail -> StrUtil.isNotBlank(detail.getId()))
                .collect(Collectors.toList());
        if (CollUtil.isEmpty(validSeedDetails)) {
            return result;
        }
        Map<String, LogisticsReconDetailEntity> distinctSeedDetails = validSeedDetails.stream()
                .filter(detail -> hasCompleteIdentifyValues(detail, uniqueKeyList))
                .collect(Collectors.toMap(
                        detail -> LogisticsReconMatchGroupHelper.buildDetailGroupKey(detail, uniqueKeyList),
                        detail -> detail,
                        (first, ignored) -> first,
                        LinkedHashMap::new));
        distinctSeedDetails.keySet().removeIf(key -> !targetGroupKeys.contains(key));
        if (CollUtil.isEmpty(distinctSeedDetails)) {
            return validSeedDetails.stream()
                    .map(LogisticsReconDetailEntity::getId)
                    .filter(StrUtil::isNotBlank)
                    .collect(Collectors.toCollection(LinkedHashSet::new));
        }
        LambdaQueryChainWrapper<LogisticsReconDetailEntity> detailQuery = logisticsReconDetailService.lambdaQuery()
                .select(LogisticsReconDetailEntity::getId,
                        LogisticsReconDetailEntity::getSoCode,
                        LogisticsReconDetailEntity::getPlatformOrderNo,
                        LogisticsReconDetailEntity::getTrackNo,
                        LogisticsReconDetailEntity::getTransportNo,
                        LogisticsReconDetailEntity::getSoDeliveryCode)
                .eq(LogisticsReconDetailEntity::getMainId, mainId)
                .and(wrapper -> {
                    boolean firstGroup = true;
                    for (LogisticsReconDetailEntity seedDetail : distinctSeedDetails.values()) {
                        if (!firstGroup) {
                            wrapper.or();
                        }
                        wrapper.and(group -> {
                            ...
                            for (CfgLogisticsCostImportDetailEntity uniqueKey : uniqueKeyList) {
                                String targetField = uniqueKey.getTargetField();
                                String value = LogisticsReconMatchGroupHelper.detailIdentifyValue(seedDetail, targetField);
                                if (!appendIdentifyFieldCondition(group, targetField, value, firstField)) {
                                    continue;
                                }
                                firstField = false;
                            }
        ...
        for (LogisticsReconDetailEntity detail : detailQuery.list()) {
            String groupKey = LogisticsReconMatchGroupHelper.buildDetailGroupKey(detail, uniqueKeyList);
            if (targetGroupKeys.contains(groupKey)) {
                result.add(detail.getId());
            }
        }
        validSeedDetails.stream()
                .map(LogisticsReconDetailEntity::getId)
                .forEach(result::add);
        return result;
    }
    ...
    /**
     * 追加单个识别字段的精确等值条件；空值/未知字段不拼宽条件，避免候选集爆炸。
     *
     * @param wrapper 明细查询条件
     * @param targetField 模板识别字段
     * @param value 种子明细识别值
     * @param firstField 是否组内首个已拼字段（首个直接 eq，后续包一层 and）
     * @return true 表示已追加条件
     */
    private boolean appendIdentifyFieldCondition(LambdaQueryWrapper<LogisticsReconDetailEntity> wrapper,
                                                 String targetField,
                                                 String value,
                                                 boolean firstField) {
        if (StrUtil.isBlank(value) || !isSupportedIdentifyTargetField(targetField)) {
            return false;
        }
        if (firstField) {
            return appendIdentifyFieldEq(wrapper, targetField, value);
        }
        wrapper.and(condition -> appendIdentifyFieldEq(condition, targetField, value));
        return true;
    }

    private boolean appendIdentifyFieldEq(LambdaQueryWrapper<LogisticsReconDetailEntity> wrapper,
                                          String targetField,
                                          String value) {
        switch (targetField) {
            case LogisticsCostImportTargetFieldConstant.SOURCE_CODE:
            case LogisticsCostImportTargetFieldConstant.SO_CODE:
                wrapper.eq(LogisticsReconDetailEntity::getSoCode, value);
                return true;
            case LogisticsCostImportTargetFieldConstant.PLATFORM_CODE:
            case LogisticsCostImportTargetFieldConstant.PLATFORM_ORDER_NO:
                wrapper.eq(LogisticsReconDetailEntity::getPlatformOrderNo, value);
                return true;
            case LogisticsCostImportTargetFieldConstant.TRACK_NO:
                wrapper.eq(LogisticsReconDetailEntity::getTrackNo, value);
                return true;
            case LogisticsCostImportTargetFieldConstant.TRANSPORT_NO:
                wrapper.eq(LogisticsReconDetailEntity::getTransportNo, value);
                return true;
            case LogisticsCostImportTargetFieldConstant.SO_DELIVERY_CODE:
                wrapper.eq(LogisticsReconDetailEntity::getSoDeliveryCode, value);
                return true;
            default:
                return false;
        }
    ...
    private void collectIdentifyFilterValues(LogisticsReconDetailEntity detail,
                                             List<CfgLogisticsCostImportDetailEntity> uniqueKeyList,
                                              Set<String> soCodes,
                                              Set<String> platformOrderNos,
                                              Set<String> trackNos,
                                              Set<String> transportNos,
                                              Set<String> soDeliveryCodes) {
        if (detail == null || CollUtil.isEmpty(uniqueKeyList)) {
            return;
        }
        for (CfgLogisticsCostImportDetailEntity uniqueKey : uniqueKeyList) {
            String value = LogisticsReconMatchGroupHelper.detailIdentifyValue(detail, uniqueKey.getTargetField());
            if (StrUtil.isBlank(value)) {
                continue;
            }
            String targetField = uniqueKey.getTargetField();
            if (LogisticsCostImportTargetFieldConstant.SOURCE_CODE.equals(targetField)
                    || LogisticsCostImportTargetFieldConstant.SO_CODE.equals(targetField)) {
                soCodes.add(value);
            } else if (LogisticsCostImportTargetFieldConstant.PLATFORM_CODE.equals(targetField)
                    || LogisticsCostImportTargetFieldConstant.PLATFORM_ORDER_NO.equals(targetField)) {
                platformOrderNos.add(value);
            } else if (LogisticsCostImportTargetFieldConstant.TRACK_NO.equals(targetField)) {
                trackNos.add(value);
            } else if (LogisticsCostImportTargetFieldConstant.TRANSPORT_NO.equals(targetField)) {
                transportNos.add(value);
            } else if (LogisticsCostImportTargetFieldConstant.SO_DELIVERY_CODE.equals(targetField)) {
                soDeliveryCodes.add(value);
            }
        }
    }

    private Map<String, LogisticsReconDetailEntity> loadDetailsByIds(String mainId, Collection<String> detailIds) {
        Map<String, LogisticsReconDetailEntity> detailMap = new HashMap<>();
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(detailIds)) {
            return detailMap;
        }
        List<String> idList = detailIds.stream().filter(StrUtil::isNotBlank).distinct().collect(Collectors.toList());
        for (int i = 0; i < idList.size(); i += MATCH_ID_BATCH_SIZE) {
            List<String> batch = idList.subList(i, Math.min(idList.size(), i + MATCH_ID_BATCH_SIZE));
            logisticsReconDetailService.lambdaQuery()
                    .select(LogisticsReconDetailEntity::getId,
                            ...
                            LogisticsReconDetailEntity::getSoDeliveryCode)
                    .eq(LogisticsReconDetailEntity::getMainId, mainId)
                    .in(LogisticsReconDetailEntity::getId, batch)
                    .list()
                    .forEach(detail -> detailMap.put(detail.getId(), detail));
        }
        return detailMap;
    }
    ...
    public void doMatchSubsChunk(String mainId, List<String> detailSubIds, boolean isConfirm,
        ...
        // 仅回写「已通过 MATCHING + 未确认过滤、但 detail 缺失」的费用项；
        // 不能用入参 detailSubIds 差集，否则已确认/部分确认被过滤的 id 会被误标 FAILED。
        Set<String> processedSubIds = units.stream().map(unit -> unit.sub.getId()).collect(Collectors.toSet());
        List<String> orphanSubIds = subList.stream()
                .map(LogisticsReconDetailSubEntity::getId)
                .filter(id -> !processedSubIds.contains(id))
                .collect(Collectors.toList());
        if (CollUtil.isNotEmpty(orphanSubIds)) {
            // 走 markReconMatchFailed：明细 FAILED + 主表统计同事务更新
            self.markReconMatchFailed(mainId, orphanSubIds, "对账明细不存在");
        }
    ...
    private static boolean isNonRetryableMatchFailure(LogisticsReconDetailSubEntity sub) {
        return sub != null
                && LogisticsReconDetailMatchStatusEnum.FAILED.getCode().equals(sub.getMatchStatus())
                && LogisticsReconMatchFailReasonSupport.isNonRetryable(sub.getMatchFailReason());
    }
    ...
    private LogisticsReconMatchExecutionResultDTO executeReconMatch(LogisticsReconEntity entity,
        ...
        ctx.setRows(rows);
        // 仅在费用真正落库前打不可重放标记；Feign/预查询失败仍可自动重试。
        ctx.setBeforePersistHook(() -> markReconMatchExecutionStarted(executionSubIds));
    ...
    public void markReconMatchFailed(String mainId, List<String> detailSubIds, String reason) {
        List<LogisticsReconDetailSubEntity> matchingRows = logisticsReconDetailSubService.lambdaQuery()
                .select(LogisticsReconDetailSubEntity::getId,
                        LogisticsReconDetailSubEntity::getMatchFailReason,
                        LogisticsReconDetailSubEntity::getLocalAmount)
                .eq(LogisticsReconDetailSubEntity::getMainId, mainId)
                .in(CollUtil.isNotEmpty(detailSubIds), LogisticsReconDetailSubEntity::getId, detailSubIds)
                .eq(LogisticsReconDetailSubEntity::getMatchStatus,
                        LogisticsReconDetailMatchStatusEnum.MATCHING.getCode())
                .last("FOR UPDATE")
                .list();
        if (CollUtil.isEmpty(matchingRows)) {
            return;
        }
        List<String> protectedIds = matchingRows.stream()
                .filter(row -> LogisticsReconMatchFailReasonSupport.isNonRetryable(row.getMatchFailReason()))
                .map(LogisticsReconDetailSubEntity::getId)
                .collect(Collectors.toList());
        List<String> ordinaryIds = matchingRows.stream()
                .filter(row -> !LogisticsReconMatchFailReasonSupport.isNonRetryable(row.getMatchFailReason()))
                .map(LogisticsReconDetailSubEntity::getId)
                .collect(Collectors.toList());
        if (CollUtil.isNotEmpty(protectedIds)) {
            String protectedReason = LogisticsReconMatchFailReasonSupport.isNonRetryable(reason)
                    ? reason : LogisticsReconMatchFailReasonSupport.resolveNonRetryablePartialCommit(null);
            logisticsReconDetailSubService.batchUpdateMatchStatus(protectedIds,
                    LogisticsReconDetailMatchStatusEnum.FAILED.getCode(), protectedReason, MATCHING_FROM_STATUS);
        }
        if (CollUtil.isNotEmpty(ordinaryIds)) {
            logisticsReconDetailSubService.batchUpdateMatchStatus(ordinaryIds,
        ...
        for (LogisticsReconDetailSubEntity row : matchingRows) {
            stats.addFailed(row.getLocalAmount());
        }
        applyMainMatchStatsDelta(mainId, stats);
    }
    ...
    public List<BatchResultDTO> submitManualMatch(List<LogisticsReconMatchDTO.SubErpInputDTO> inputs, String matchType) {
        ...
        // 提交前仅在主单空闲时清理超时 MATCHING，避免越过主单锁误伤运行中的任务。
        Set<String> mainIdsForStaleReset = subMap.values().stream()
                .map(LogisticsReconDetailSubEntity::getMainId)
                .filter(StrUtil::isNotBlank)
                .collect(Collectors.toCollection(LinkedHashSet::new));
        for (String mainId : mainIdsForStaleReset) {
            cleanupStaleMatchingSubsIfMainIdle(mainId, "submitManualMatch");
        }
        if (CollUtil.isNotEmpty(mainIdsForStaleReset)) {
            subMap = logisticsReconDetailSubService.lambdaQuery()
                    .in(LogisticsReconDetailSubEntity::getId, detailSubIds)
                    .list().stream()
                    .collect(Collectors.toMap(LogisticsReconDetailSubEntity::getId, s -> s, (a, b) -> a));
        }
        ...
        for (LogisticsReconMatchDTO.SubErpInputDTO input : inputs) {
            ...
            if (isNonRetryableMatchFailure(sub)) {
                results.add(BatchResultDTO.fail(detailSubId, detailSubId, "费用或关联状态可能已部分落库，请人工核对"));
                continue;
            }
        ...
        for (Map.Entry<String, List<LogisticsReconMatchDTO.SubErpInputDTO>> entry : inputsByMain.entrySet()) {
            ...
            List<String> claimedIds = claimManualMatchIds(mainId, claimIds);
    ...
    private void asyncMatchDetailSubsByErp(String mainId, List<LogisticsReconMatchDTO.SubErpInputDTO> inputs,
        ...
        try {
            UserContext.setLoginUser(user);
            locked = tryLockReconMain(lock, mainId, "asyncMatchDetailSubsByErp");
            if (!locked) {
                self.markReconMatchFailed(mainId, claimIds, "获取对账单锁超时，匹配未执行");
                return;
            }
            ...
            try {
                self.markReconMatchFailed(mainId, claimIds, resolveMatchChunkFailureReason(claimIds, e));
            } catch (Exception ex) {
            ...
            if (locked && lock.isHeldByCurrentThread()) {
                try {
                    ...
                    log.error("[asyncMatchDetailSubsByErp] 释放主单锁失败 mainId={}", mainId, ex);
                }
            }
        }
    }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void rollbackImportTask(List<String> mainIds, String failReason) {
        if (CollUtil.isEmpty(mainIds)) {
            return;
        }
        List<String> ids = mainIds.stream().filter(StrUtil::isNotBlank).distinct().collect(Collectors.toList());
        if (CollUtil.isEmpty(ids)) {
            return;
        }
        logisticsReconRefLogisticsBillService.removeByMainIds(ids);
        logisticsReconDetailSubService.removeByMainIds(ids);
        logisticsReconDetailService.removeByMainIds(ids);
        markImportFail(ids, failReason);
    }

    /** 手动匹配认领与主单确认共用同一把锁，避免校验通过后被并发确认/重导覆盖。 */
    private List<String> claimManualMatchIds(String mainId, List<String> claimIds) {
        if (CollUtil.isEmpty(claimIds)) {
            return Collections.emptyList();
        }
        RLock lock = redissonClient.getLock(buildReconMainLockKey(mainId));
        boolean locked = false;
        try {
            locked = tryLockReconMain(lock, mainId, "submitManualMatch");
            if (!locked) {
                return Collections.emptyList();
            }
            LogisticsReconEntity mainEntity = super.getById(mainId);
            if (mainEntity == null
                    || !LogisticsReconCheckStatusEnum.CONFIRMED.getCode().equals(mainEntity.getCheckStatus())) {
                return Collections.emptyList();
            }
            self.failStaleMatchingSubsAndApplyMainStats(mainId);
            return logisticsReconDetailSubService.batchClaimMatchStatus(claimIds,
            ...
            if (locked && lock.isHeldByCurrentThread()) {
                try {
                    ...
                    log.warn("[claimManualMatchIds] 释放主单锁失败 mainId={}", mainId, ex);
                }
            }
        }
    }
    ...
    public List<BatchResultDTO> matchDetailSubsByErp(String mainId, List<LogisticsReconMatchDTO.SubErpInputDTO> inputs, String matchType) {
        ...
        LogisticsReconMatchExecutionResultDTO executionResult = executeReconMatch(
                entity, units, true, false, null, matchingSubIds);
    ...
    private void writeReconMatchResult(String mainId, String matchType,
        ...
        List<String> updatedFailedSubIds = new ArrayList<>();
        for (Map.Entry<String, List<String>> failedEntry : failedSubIdsByReason.entrySet()) {
            updatedFailedSubIds.addAll(logisticsReconDetailSubService.batchUpdateMatchStatus(
        ...
        if (CollUtil.isNotEmpty(successWithoutRefSubIds)) {
            updatedFailedSubIds.addAll(logisticsReconDetailSubService.batchUpdateMatchStatus(successWithoutRefSubIds,
        ...
        List<String> updatedMatchedSubIds = logisticsReconDetailSubService.batchUpdateMatchStatus(matchedSubIds,
                LogisticsReconDetailMatchStatusEnum.MATCHED.getCode(), null, MATCHING_FROM_STATUS);
        // 与明细状态同事务维护主表冗余；失败金额由 SQL 按当前 FAILED 状态重算
        Map<String, LogisticsReconDetailSubEntity> subById = new HashMap<>();
        for (List<LogisticsReconDetailSubEntity> subs : rowKeyToSubs.values()) {
            if (CollUtil.isEmpty(subs)) {
                continue;
            }
            for (LogisticsReconDetailSubEntity sub : subs) {
                if (sub != null && StrUtil.isNotBlank(sub.getId())) {
                    subById.putIfAbsent(sub.getId(), sub);
                }
            }
        }
        LogisticsReconDTO.MatchTransitionStats stats = new LogisticsReconDTO.MatchTransitionStats();
        for (String subId : updatedMatchedSubIds) {
            LogisticsReconDetailSubEntity sub = subById.get(subId);
            stats.addMatched(sub == null ? null : sub.getLocalAmount());
        }
        for (String subId : updatedFailedSubIds) {
            LogisticsReconDetailSubEntity sub = subById.get(subId);
            stats.addFailed(sub == null ? null : sub.getLocalAmount());
        }
        applyMainMatchStatsDelta(mainId, stats);
    }
    ...
    /**
     * 批量提交账单确认异步任务：逐主单校验 → 过滤可确认费用项 → 一主单一任务并预落费用项明细。
     */
    @Override
    public List<BatchResultDTO> submitConfirmBillAsync(List<String> ids, String reconciliationStatus,
        ...
        for (String mainId : mainIds) {
            try {
                ...
                if (CollUtil.isEmpty(scanResult.affectedSubIds)) {
                    // 与同步 confirmBill 对齐：无可用费用项时抛出明确业务异常，禁止软成功掩盖不可回退等场景
                ...
                TmsAsyncTaskRecordDTO.LogisticsReconConfirmBillPayloadDTO payload =
                        new TmsAsyncTaskRecordDTO.LogisticsReconConfirmBillPayloadDTO(
                                mainId, reconciliationStatus, confirmTime);
    ...
    /**
     * 分批执行账单确认：主单锁内按本批费用项筛选可确认 cost → {@link #confirmBillRefCostBatch} → 回写任务明细。
     */
    @Override
    public TmsAsyncTaskRecordDTO.BatchProcessResult processConfirmBillBatch(String taskId, String mainId,
                                                                            List<String> batchSubIds,
        ...
        List<String> subIds = batchSubIds.stream().filter(StrUtil::isNotBlank).distinct()
                .collect(Collectors.toList());
        if (CollUtil.isEmpty(subIds)) {
            return new TmsAsyncTaskRecordDTO.BatchProcessResult(0, 0);
        }
        ...
        LoginUser prev = UserContext.getLoginUser();
        try {
            locked = tryLockReconMain(lock, mainId, "processConfirmBillBatch");
            if (!locked) {
                for (String subId : subIds) {
                    TmsAsyncTaskDetailEntity detail = detailMap.get(subId);
                    if (detail == null || isTaskDetailTerminal(detail.getStatus())) {
            ...
            if (matchingCount > 0) {
                for (String subId : subIds) {
                    TmsAsyncTaskDetailEntity detail = detailMap.get(subId);
                    if (detail != null && !isTaskDetailTerminal(detail.getStatus())) {
                        finishTaskDetail(detail, false, ApiError.LOGISTICS_RECON_MATCHING_CONFIRM_FORBIDDEN.getMsg());
                        failed++;
                    }
                }
                return new TmsAsyncTaskRecordDTO.BatchProcessResult(success, failed);
            ...
            Set<String> batchSubIdSet = claimedDetails.stream()
                    .map(TmsAsyncTaskDetailEntity::getBusinessId).collect(Collectors.toSet());
            ConfirmBillScanResult scanResult = scanConfirmBillEligibility(mainId, reconciliationStatus);
            Set<String> eligibleSubIds = scanResult.affectedSubIds.stream()
                    .filter(batchSubIdSet::contains).collect(Collectors.toCollection(LinkedHashSet::new));
            Map<String, String> subToCostId = listRefCostIdBySubIds(mainId, eligibleSubIds);
            List<String> eligibleCostIds = subToCostId.values().stream()
            ...
            if (CollUtil.isNotEmpty(eligibleCostIds)) {
                self.confirmBillRefCostBatch(mainId, eligibleCostIds, reconciliationStatus, effectiveConfirmTime);
                // 刷新范围：本批 + 本次更新 cost 在本主单下关联的全部费用项（避免跨批共享费用单状态滞后）
            ...
            Map<String, LogisticsReconDetailSubEntity> subStatusMap = logisticsReconDetailSubService.lambdaQuery()
                    ...
                    .in(LogisticsReconDetailSubEntity::getId, batchSubIdSet)
                    .list().stream()
                    .collect(Collectors.toMap(LogisticsReconDetailSubEntity::getId, s -> s, (a, b) -> a));
            for (TmsAsyncTaskDetailEntity detail : claimedDetails) {
                // 仅按落库后对账状态判定成功，禁止仅因“曾入选 eligible”就记成功
                if (isConfirmTargetReached(subStatusMap.get(detail.getBusinessId()), reconciliationStatus)) {
                    finishTaskDetail(detail, true, null);
                    success++;
                } else if (eligibleSubIds.contains(detail.getBusinessId())) {
                    finishTaskDetail(detail, false, "确认后未达目标对账状态");
                    failed++;
                } else {
                    finishTaskDetail(detail, false, "费用项无可确认关联或已确认");
                    failed++;
                }
            }
            maybeWriteConfirmBillOperateLog(taskId, mainId, reconciliationStatus);
        } catch (Exception e) {
            log.error("[processConfirmBillBatch] 批次异常 taskId={} mainId={}", taskId, mainId, e);
            String reason = BatchResultDTO.resolveFailMsg(e);
            for (TmsAsyncTaskDetailEntity detail : detailMap.values()) {
                if (detail != null && !isTaskDetailTerminal(detail.getStatus())) {
            ...
            if (locked && lock.isHeldByCurrentThread()) {
                try {
                    ...
                    log.error("[processConfirmBillBatch] 释放锁失败 mainId={}", mainId, ex);
                }
            }
        }
        return new TmsAsyncTaskRecordDTO.BatchProcessResult(success, failed);
    ...
    /**
     * 创建一主单异步任务：先落主任务，再由 writer 游标/分片写入费用项明细，最后派发 MQ；
     * 任一步失败则关闭任务，避免空任务占坑拦截重提。
     */
    private BatchResultDTO createLogisticsReconTaskWithDetails(String businessType, String methodType,
                                                               Object payload,
                                                               int expectedDetailCount,
                                                               LogisticsReconTaskDetailWriter detailWriter,
                                                               String dispatchSuccessLogTemplate) {
        if (expectedDetailCount <= 0 || detailWriter == null) {
            throw new ServiceException("无可处理的费用项明细");
        }
        TmsAsyncTaskRecordEntity taskRecord = null;
        try {
            ...
            taskRecord = tmsAsyncTaskRecordService.addManualTask(
                    new TmsAsyncTaskRecordDTO.ManualCreateDTO(businessType, methodType, expectedDetailCount, jsonStr));
            int written = detailWriter.writeDetails(taskRecord.getId(), businessType);
            if (written <= 0) {
                throw new ServiceException("任务明细写入为空，请重试");
            }
            if (written != expectedDetailCount) {
                tmsAsyncTaskRecordService.lambdaUpdate()
                        .set(TmsAsyncTaskRecordEntity::getDetailCount, written)
                        .eq(TmsAsyncTaskRecordEntity::getId, taskRecord.getId())
                        .update();
            }
            tmsAsyncTaskRecordService.claimAndDispatch(taskRecord, true);
            if (StrUtil.isNotBlank(dispatchSuccessLogTemplate)) {
                log.info(dispatchSuccessLogTemplate, taskRecord.getId(), written);
            }
            return BatchResultDTO.success(taskRecord.getId(), taskRecord.getCode());
        } catch (Exception e) {
            if (taskRecord != null && StrUtil.isNotBlank(taskRecord.getId())) {
                try {
                    String reason = StrUtil.sub(StrUtil.blankToDefault(
                            BatchResultDTO.resolveFailMsg(e), "任务创建后置处理失败"), 0, 490);
                    tmsAsyncTaskRecordService.finishTaskWithError(taskRecord.getId(), reason);
                } catch (Exception compensateEx) {
                    log.error("[createLogisticsReconTaskWithDetails] 补偿关闭任务失败 taskId={}",
                            taskRecord.getId(), compensateEx);
                }
            }
            if (e instanceof ServiceException) {
                throw (ServiceException) e;
            }
            throw new ServiceException(StrUtil.blankToDefault(e.getMessage(), "创建异步任务失败"));
        }
    }

    @FunctionalInterface
    private interface LogisticsReconTaskDetailWriter {
        /**
         * 分片写入任务明细，避免十万+ 费用项全量入内存。
         *
         * @return 实际写入条数
         */
        int writeDetails(String taskId, String businessType);
    }

    /**
     * 统计可匹配费用项数量（未匹配/失败，且对账状态非已确认）。
     */
    private int countMatchableDetailSubs(String mainId) {
        Integer count = logisticsReconDetailSubService.lambdaQuery()
                .eq(LogisticsReconDetailSubEntity::getMainId, mainId)
                .in(LogisticsReconDetailSubEntity::getMatchStatus, MATCH_CLAIM_FROM_STATUSES)
                .and(wrapper -> wrapper
    ...
    /**
     * 游标分页写入可匹配费用项任务明细，单批最多 {@link #MATCH_ID_BATCH_SIZE}，不攒全量 List。
     */
    private int saveMatchableTaskDetails(String taskId, String businessType, String mainId) {
        int written = 0;
        String lastId = null;
        LocalDateTime now = LocalDateTime.now();
        ...
        while (true) {
            ...
            List<LogisticsReconDetailSubEntity> batch = query.list();
    ...
    private int saveConfirmableTaskDetails(String taskId, String businessType, String mainId,
        ...
        List<String> sortedIds = subIds.stream().filter(StrUtil::isNotBlank).distinct().sorted()
                .collect(Collectors.toList());
        int written = 0;
        LocalDateTime now = LocalDateTime.now();
        List<String> identifyFields = loadUniqueKeyListByMainId(mainId).stream()
    ...
    /**
     * 本主单下关联指定物流费用单的全部费用项 id（用于确认后跨批共享 cost 状态刷新）。
     */
    private Set<String> listDetailSubIdsByCostIds(String mainId, Collection<String> costIds) {
        Set<String> result = new LinkedHashSet<>();
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(costIds)) {
            return result;
        }
        List<String> idList = costIds.stream().filter(StrUtil::isNotBlank).distinct().collect(Collectors.toList());
        for (int i = 0; i < idList.size(); i += MATCH_ID_BATCH_SIZE) {
            List<String> batch = idList.subList(i, Math.min(idList.size(), i + MATCH_ID_BATCH_SIZE));
            logisticsReconRefLogisticsBillService.lambdaQuery()
                    .select(LogisticsReconRefLogisticsBillEntity::getDetailSubId)
                    .eq(LogisticsReconRefLogisticsBillEntity::getMainId, mainId)
                    .eq(LogisticsReconRefLogisticsBillEntity::getIsDeleted, false)
                    .in(LogisticsReconRefLogisticsBillEntity::getLogisticsBillCostId, batch)
                    .list()
                    .forEach(ref -> {
                        if (StrUtil.isNotBlank(ref.getDetailSubId())) {
                            result.add(ref.getDetailSubId());
                        }
                    });
        }
        return result;
    ...
    private Map<String, TmsAsyncTaskDetailEntity> loadOrCreateTaskDetails(String taskId, String businessType,
        ...
        List<TmsAsyncTaskDetailEntity> existing = tmsAsyncTaskDetailService.lambdaQuery()
                .eq(TmsAsyncTaskDetailEntity::getMainId, taskId)
    ...
    private void finishTaskDetail(TmsAsyncTaskDetailEntity detail, boolean success, String errorMsg) {
        if (detail == null) {
            return;
        }
        ...
        if (success) {
            ...
            detail.setErrorData(StrUtil.sub(StrUtil.blankToDefault(errorMsg, "处理失败"), 0, 490));
        }
        try {
            if (StrUtil.isBlank(detail.getId())) {
                tmsAsyncTaskDetailService.save(detail);
            } else {
                // 仅允许从非终态回写，避免并发覆盖已成功/已失败结果
                tmsAsyncTaskDetailService.lambdaUpdate()
                        .set(TmsAsyncTaskDetailEntity::getStatus, detail.getStatus())
                        .set(TmsAsyncTaskDetailEntity::getEndTime, detail.getEndTime())
                        .set(TmsAsyncTaskDetailEntity::getErrorData, detail.getErrorData())
                        .eq(TmsAsyncTaskDetailEntity::getId, detail.getId())
                        .notIn(TmsAsyncTaskDetailEntity::getStatus,
                                Arrays.asList(TmsAsyncTaskRecordStatusEnum.FINISH.getCode(),
                                        TmsAsyncTaskRecordStatusEnum.FAILED.getCode()))
                        .update();
            }
        } catch (Exception e) {
            log.error("[finishTaskDetail] 回写任务明细失败 taskId={} businessId={}",
                    detail.getMainId(), detail.getBusinessId(), e);
        }
    }
     ...
     * 认领本批任务明细进入执行态。
     *
     * @return null 表示认领成功需继续业务；非 null 为 [success增量, failed增量]（已记账，调用方 continue）
     */
    private int[] tryClaimTaskDetailForBatch(TmsAsyncTaskDetailEntity detail) {
        if (detail == null) {
            return new int[]{0, 0};
        }
        if (TmsAsyncTaskRecordStatusEnum.FINISH.getCode().equals(detail.getStatus())) {
            return new int[]{1, 0};
        }
        if (TmsAsyncTaskRecordStatusEnum.PENDING.getCode().equals(detail.getStatus())
                && StrUtil.isNotBlank(detail.getId())) {
            if (!tmsAsyncTaskDetailService.tryClaimDetailForExecution(detail.getId())) {
        ...
        if (TmsAsyncTaskRecordStatusEnum.FAILED.getCode().equals(detail.getStatus())
            ...
            if (!tmsAsyncTaskDetailService.tryClaimDetailForRetry(detail.getId())) {
                return countClaimSkippedDetail(detail.getId());
            }
            detail.setStatus(TmsAsyncTaskRecordStatusEnum.ING.getCode());
            detail.setStartTime(LocalDateTime.now());
        ...
        finishTaskDetail(detail, false, "任务明细状态异常: " + detail.getStatus());
        return new int[]{0, 1};
    }

    /**
     * 按费用项落库 matchStatus 回写任务明细；未到终态的先 markReconMatchFailed 再记失败，避免卡在 MATCHING。
     *
     * @return int[0]=success, int[1]=failed
     */
    private int[] settleMatchTaskDetailsByFeeStatus(String mainId,
        ...
        List<String> executableSubIds = executableDetails.stream()
                .map(TmsAsyncTaskDetailEntity::getBusinessId)
                .filter(StrUtil::isNotBlank)
                .distinct()
                .collect(Collectors.toList());
    ...
    /**
     * 批次异常兜底：先尝试把仍 MATCHING 的费用项打回失败，再按真实 matchStatus 回写任务明细，
     * 禁止把已 MATCHED 的费用项对应任务明细误标 FAILED。
     */
    private int[] settleMatchTaskDetailsAfterException(String mainId,
        ...
        int success = 0;
        int failed = 0;
        if (CollUtil.isNotEmpty(feeItemsClaimedMatching)) {
            try {
                self.markReconMatchFailed(mainId, feeItemsClaimedMatching, reason);
            } catch (Exception ex) {
                log.error("[settleMatchTaskDetailsAfterException] 回写匹配失败状态异常 mainId={}", mainId, ex);
            }
        }
        ...
        List<String> businessIds = unsettled.stream()
                .map(TmsAsyncTaskDetailEntity::getBusinessId)
                .filter(StrUtil::isNotBlank)
                .distinct()
                .collect(Collectors.toList());
        ...
        for (TmsAsyncTaskDetailEntity detail : unsettled) {
            ...
            if (sub != null && LogisticsReconDetailMatchStatusEnum.MATCHED.getCode().equals(sub.getMatchStatus())) {
                ...
                failed++;
                    ...
                    || LogisticsReconDetailMatchStatusEnum.MATCHING.getCode().equals(sub.getMatchStatus())) {
                stillMatchingIds.add(detail.getBusinessId());
            } else {
                finishTaskDetail(detail, false, reason);
                failed++;
            }
    ...
    private Map<String, LogisticsReconDetailSubEntity> loadFeeItemMatchStatusMap(Collection<String> subIds) {
        if (CollUtil.isEmpty(subIds)) {
            return Collections.emptyMap();
        }
        Map<String, LogisticsReconDetailSubEntity> result = new HashMap<>();
        List<String> idList = subIds.stream().filter(StrUtil::isNotBlank).distinct().collect(Collectors.toList());
        for (int i = 0; i < idList.size(); i += MATCH_ID_BATCH_SIZE) {
            List<String> batch = idList.subList(i, Math.min(idList.size(), i + MATCH_ID_BATCH_SIZE));
            logisticsReconDetailSubService.lambdaQuery()
                    .select(LogisticsReconDetailSubEntity::getId,
                    ...
                    .in(LogisticsReconDetailSubEntity::getId, batch)
                    .list()
                    .forEach(sub -> result.put(sub.getId(), sub));
        }
        return result;
    }

    /**
     * 本批费用项 → 关联物流费用单 id（取每条费用项下任一条未删 ref）。
     */
    private Map<String, String> listRefCostIdBySubIds(String mainId, Collection<String> subIds) {
        Map<String, String> result = new LinkedHashMap<>();
        if (StrUtil.isBlank(mainId) || CollUtil.isEmpty(subIds)) {
            return result;
        }
        ...
        for (int i = 0; i < idList.size(); i += MATCH_ID_BATCH_SIZE) {
            ...
            for (LogisticsReconRefLogisticsBillEntity ref : refs) {
                if (StrUtil.isNotBlank(ref.getDetailSubId()) && StrUtil.isNotBlank(ref.getLogisticsBillCostId())) {
                    result.putIfAbsent(ref.getDetailSubId(), ref.getLogisticsBillCostId());
                }
            }
        }
        return result;
    ...
    /**
     * 费用项确认状态是否已达本次账单确认目标（含被共享费用单连带更新的场景）。
     * <p>
     * 目标为已确认时：{@code confirmed} 与 {@code partialConfirm} 均视为确认动作生效
     * （部分关联不可确认时聚合为部分确认属预期结果）；不得仅因“曾入选 eligible”就记成功。
     * </p>
     */
    private boolean isConfirmTargetReached(LogisticsReconDetailSubEntity sub, String targetReconciliationStatus) {
        if (sub == null || StrUtil.isBlank(sub.getReconciliationStatus())) {
            return false;
        }
        if (ReconciliationStatusEnum.CONFIRMED.getCode().equals(targetReconciliationStatus)) {
            return LogisticsReconReconciliationStatusEnum.CONFIRMED.getCode().equals(sub.getReconciliationStatus())
                    || LogisticsReconReconciliationStatusEnum.PARTIAL_CONFIRM.getCode()
                    .equals(sub.getReconciliationStatus());
        }
        if (ReconciliationStatusEnum.TO_BE_CONFIRM.getCode().equals(targetReconciliationStatus)) {
            return LogisticsReconReconciliationStatusEnum.TO_BE_CONFIRM.getCode()
                    .equals(sub.getReconciliationStatus());
        }
        return false;
    }
    ...
    /**
     * 等待获取对账单主单锁（异步分批默认 {@link #RECON_MAIN_LOCK_WAIT_SECONDS}）。
     */
    private boolean tryLockReconMain(RLock lock, String mainId, String action) {
        try {
            boolean locked = lock.tryLock(RECON_MAIN_LOCK_WAIT_SECONDS, TimeUnit.SECONDS);
            if (!locked) {
                log.warn("[{}] 获取主单锁超时 mainId={} waitSeconds={}", action, mainId, RECON_MAIN_LOCK_WAIT_SECONDS);
            }
            return locked;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            log.warn("[{}] 获取主单锁被中断 mainId={}", action, mainId);
            return false;
        }
    }

    /**
     * 提交侧仅在主单锁空闲时清理超时 MATCHING；活动任务持锁时立即返回，禁止并发改写其状态。
     */
    private boolean cleanupStaleMatchingSubsIfMainIdle(String mainId, String action) {
        RLock lock = redissonClient.getLock(buildReconMainLockKey(mainId));
        boolean locked = false;
        try {
            locked = lock.tryLock();
            if (!locked) {
                log.info("[{}] 主单正在执行匹配/确认，跳过超时 MATCHING 清理 mainId={}", action, mainId);
                return false;
            }
            self.failStaleMatchingSubsAndApplyMainStats(mainId);
            return true;
            ...
            if (locked && lock.isHeldByCurrentThread()) {
                try {
                    ...
                    log.warn("[{}] 释放超时 MATCHING 清理锁失败 mainId={}", action, mainId, e);
                }
            }
        }
    }
    ...
    /**
     * 分片/批次失败原因：仅当已写入不可重放标记（进入费用写路径）时才标 PARTIAL_COMMIT，
     * 写前的查询/远程/配置异常保持可自动重试。
     */
    private String resolveMatchChunkFailureReason(Collection<String> detailSubIds, Throwable e) {
        boolean executionStarted = CollUtil.isNotEmpty(detailSubIds)
                && logisticsReconDetailSubService.lambdaQuery()
                .select(LogisticsReconDetailSubEntity::getMatchFailReason)
                .in(LogisticsReconDetailSubEntity::getId, detailSubIds)
                .list().stream()
                .anyMatch(sub -> LogisticsReconMatchFailReasonSupport.isNonRetryable(sub.getMatchFailReason()));
        return executionStarted
                ? LogisticsReconMatchFailReasonSupport.resolveNonRetryablePartialCommit(e)
                : LogisticsReconMatchFailReasonSupport.resolve(e);
    }

    /**
     * 在进入费用写路径前持久化不可重放标记；正常结果回写会同步清空或替换该原因。
     */
    private void markReconMatchExecutionStarted(List<String> detailSubIds) {
        LocalDateTime now = LocalDateTime.now();
        for (int i = 0; i < detailSubIds.size(); i += MATCH_ID_BATCH_SIZE) {
            List<String> batch = detailSubIds.subList(i, Math.min(detailSubIds.size(), i + MATCH_ID_BATCH_SIZE));
            logisticsReconDetailSubService.lambdaUpdate()
    ...
    /**
     * 认领失败时不覆盖终态：按库中最新状态计入 success/failed。
     *
     * @return int[0]=success增量, int[1]=failed增量
     */
    private int[] countClaimSkippedDetail(String detailId) {
        if (StrUtil.isBlank(detailId)) {
            return new int[]{0, 0};
        }
        TmsAsyncTaskDetailEntity latest = tmsAsyncTaskDetailService.getById(detailId);
        if (latest == null) {
            return new int[]{0, 0};
        }
        if (TmsAsyncTaskRecordStatusEnum.FINISH.getCode().equals(latest.getStatus())) {
            return new int[]{1, 0};
        }
        if (TmsAsyncTaskRecordStatusEnum.FAILED.getCode().equals(latest.getStatus())) {
            return new int[]{0, 1};
        }
        // ING：其他线程处理中，本批既不计成功也不强制 FAILED
        log.debug("[countClaimSkippedDetail] 明细已被认领处理中 detailId={} status={}",
                detailId, latest.getStatus());
        return new int[]{0, 0};
    }

    /**
     * 任务全部明细终态后写一次账单确认操作日志，避免分批重复落日志。
     */
    private void maybeWriteConfirmBillOperateLog(String taskId, String mainId, String reconciliationStatus) {
        int unfinished = tmsAsyncTaskDetailService.lambdaQuery()
                .eq(TmsAsyncTaskDetailEntity::getMainId, taskId)
                .notIn(TmsAsyncTaskDetailEntity::getStatus,
                        Arrays.asList(TmsAsyncTaskRecordStatusEnum.FINISH.getCode(),
                                TmsAsyncTaskRecordStatusEnum.FAILED.getCode()))
    ...
    private void syncReconStatusByCostIdsInternal(java.util.Collection<String> logisticsBillCostIds,
        ...
        // ref 快照变更后，按受影响的费用项范围重算 detail_sub 聚合确认状态，并持锁回刷主表列表冗余
        for (Map.Entry<String, Set<String>> entry : affectedSubIdsByMain.entrySet()) {
            ...
            refreshMainPagingStatsWithLock(entry.getKey());
        }
    }
    ...
    private void fillList(List<LogisticsReconDTO.ListDTO> list) {
        ...
        for (LogisticsReconDTO.ListDTO data : list) {
            ...
            if (StrUtil.isBlank(data.getReconciliationStatus())) {
            ...
            // 列表统计读主表冗余；matchStatus 由 matchCount/costCount 派生
            int matchCount = data.getMatchCount() == null ? 0 : data.getMatchCount();
            int costCount = data.getCostCount() == null ? 0 : data.getCostCount();
            data.setValidCostCount(costCount);
            if (data.getMatchSuccessAmount() == null) {
                data.setMatchSuccessAmount(BigDecimal.ZERO);
            }
            if (data.getMatchFailAmount() == null) {
                data.setMatchFailAmount(BigDecimal.ZERO);
            }
    ...
    /**
     * 清理超时 MATCHING 并同事务增量回写主表匹配冗余。
     */
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void failStaleMatchingSubsAndApplyMainStats(String mainId) {
        LogisticsReconDTO.MatchTransitionStats stats =
                logisticsReconDetailSubService.failStaleMatchingSubsByMainId(mainId);
        applyMainMatchStatsDelta(mainId, stats);
    }

    /**
     * 收敛已确认仍 MATCHING 的费用项，并同事务增量回写主表匹配冗余。
     */
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void settleConfirmedMatchingSubsAndApplyMainStats(String mainId) {
        LogisticsReconDTO.MatchTransitionStats stats =
                logisticsReconDetailSubService.settleConfirmedMatchingSubsByMainId(mainId);
        applyMainMatchStatsDelta(mainId, stats);
    }

    /**
     * 原子维护主表匹配冗余（与明细状态变更同事务调用，失败则整单回滚）。
     */
    private void applyMainMatchStatsDelta(String mainId, LogisticsReconDTO.MatchTransitionStats stats) {
        if (StrUtil.isBlank(mainId) || stats == null || !stats.hasChange()) {
            return;
        }
    ...
    /**
     * 持主单锁回刷列表冗余，避免与匹配/确认并发交叉写。
     */
    private void refreshMainPagingStatsWithLock(String mainId) {
        if (StrUtil.isBlank(mainId)) {
            return;
        }
        RLock lock = redissonClient.getLock(buildReconMainLockKey(mainId));
        boolean locked = false;
        try {
            locked = tryLockReconMain(lock, mainId, "refreshMainPagingStats");
            if (!locked) {
                throw new ServiceException(ApiError.BILL_DATA_LOCKED);
            }
            ...
            if (locked && lock.isHeldByCurrentThread()) {
                try {
                    ...
                    log.error("[refreshMainPagingStatsWithLock] 释放锁失败 mainId={}", mainId, ex);
                }
            }
        }
    }
    ...
    /**
     * 按有效费用项聚合回刷主表列表冗余统计（单主单）。
     * 用于导入完成、确认批次、确认匹配结束等低频路径；匹配分片请走 {@link #applyMainMatchStatsDelta}。
     * 失败直接抛错，禁止静默吞掉导致列表不准。
     */
    private void refreshMainPagingStats(String mainId) {
        if (StrUtil.isBlank(mainId)) {
            return;
        }
        try {
            ...
            log.error("[refreshMainPagingStats] 回刷主表列表冗余失败 mainId={}", mainId, e);
            throw new ServiceException(ApiError.LOGISTICS_RECON_SAVE_FAILED);
        }
    }
