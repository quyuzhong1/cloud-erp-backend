...
@Component
@Slf4j
public class ExportOmsInvoicePdfHandler implements FileEventHandler {
    ...
    public void handle(FileTask fileTask) {
        log.info("开始处理导出发票PDF任务，taskId={}", fileTask.getId());
        ...
        try {
            ...
            // 2. 调用OMS服务构建ZIP文件
            String zipUrl = exportOmsFeign.buildInvoiceAttachZip(dto, AttachmentTypeEnum.INVOICE_INFO_PDF.getCode());
            ...
            // 3. 查询附件数量用于设置count
            int count = CollUtil.size(exportOmsFeign.listExportUrl(dto, AttachmentTypeEnum.INVOICE_INFO_PDF.getCode()));
            
            // 4. 更新任务结果
            fileTask.setFileUrl(zipUrl);
            fileTask.setCount(count);
            
            log.info("导出发票PDF任务处理完成，taskId={}, zipUrl={}, count={}", 
                    fileTask.getId(), zipUrl, fileTask.getCount());
        } catch (Exception e) {
            log.error("导出发票PDF任务处理失败，taskId={}", fileTask.getId(), e);
            throw new ServiceException("导出发票PDF失败: " + e.getMessage());
        }
    }
