...
public class ExportOmsCustomerSkuHandler extends AbstractPageFileEventHandler<SkuMappingDTO.CustomerPagingViewDTO, SkuMappingDTO.CustomerPagingParamDTO> {
    ...
    @Override
    protected List<SkuMappingDTO.CustomerPagingViewDTO> getData(FileTask fileTask) {
        SkuMappingDTO.CustomerPagingParamDTO dto = readValue(fileTask.getMetaInfo(), new TypeReference<SkuMappingDTO.CustomerPagingParamDTO>() {
        });
