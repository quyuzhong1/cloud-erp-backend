...
public class SkuStdRetailPriceServiceImpl extends SuperServiceImpl<SkuStdRetailPriceMapper, SkuStdRetailPriceEntity> implements SkuStdRetailPriceService {
    ...
    public BaseResultDTO.AddDTO add(SkuStdRetailPriceDTO.AddDTO addDTO) {
        ...
        operateLogService.addModuleOperateLog(msg, null, skuStdRetailPriceEntity.getId(), "新增操作");
        // TODO 新增明细（如果有明细的话）
    ...
    */
    ...
    public Boolean update(SkuStdRetailPriceDTO.UpdateDTO addOrUpdateDTO) {
        ...
        boolean save = super.updateById(skuStdRetailPriceEntity);
        if(!save) {
            throw new ServiceException("sku标准零售价单保存失败");
        }
        ...
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLogByObj(old, skuStdRetailPriceEntity, null, skuStdRetailPriceEntity.getId(), msg);
        return Boolean.TRUE;
    }
    ...
    public List<SkuStdRetailPriceDTO.TabListDTO> tabList(PermissionsDTO param) {
        SkuStdRetailPriceDTO.PagingParamDTO searchParam = new SkuStdRetailPriceDTO.PagingParamDTO();
        searchParam.setPermissionSql(param.getPermissionSql());
        List<SkuStdRetailPriceDTO.TabListDTO> list = baseMapper.tabList(searchParam);
        // 获取状态列表
        // TODO 替换当前表Tab状态字段
        List<String> statusList = null;
        // 不存在的状态赋值为0
        List<String> existStatusList = list.stream().map(SkuStdRetailPriceDTO.TabListDTO::getTabFlag).collect(Collectors.toList());
        statusList.parallelStream().forEach(status -> {
            if(!existStatusList.contains(status)) {
            list.add(new SkuStdRetailPriceDTO.TabListDTO(status, 0));
        }
        });
        list.add(new SkuStdRetailPriceDTO.TabListDTO("all", list.stream().mapToInt(SkuStdRetailPriceDTO.TabListDTO::getCount).sum()));
        // 计算合计数量
        return list;
    }
