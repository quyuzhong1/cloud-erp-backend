...
@RocketMQMessageListener(topic = "${spring.cloud.nacos.discovery.namespace}-plm_workflow_sync_fs_comment_topic",
        selectorExpression = "plm-workflow_sync_fs_instance_tag",
...
public class MQSyncProcessCommentService implements RocketMQListener<CfgApproveSyncDTO.SyncFsCommentToMqDTO> {
    ...
    public void onMessage(CfgApproveSyncDTO.SyncFsCommentToMqDTO commentToMqDTO) {
        log.info("plm MQSyncProcessCommentService 开始");
        ...
        entity.setClassPath("")
                ...
                .setContent(msg)
                .setOperation("审核操作")
                .setCreateUserId(dto.getOperator())
                .setCreateUserName(dto.getOperatorName());
        Boolean b = operateLogService.save(entity);
        if(b){
        ...
        log.info("plm MQSyncProcessCommentService 结束");
    }
