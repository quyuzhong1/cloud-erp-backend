...
public class MoldRefSkuServiceImpl extends SuperServiceImpl<MoldRefSkuMapper, MoldRefSkuEntity> implements MoldRefSkuService {
    ...
    */
    ...
    public BatchResultDTO cancelProcess(String id) {
        ...
        return BatchResultDTO.success(entity.getId(), entity.getMoldCode(), OperationTypeEnum.CANCEL_PROCESS);
    }
    ...
    */
    private void fillList(List<MoldRefSkuDTO.ListDTO> list) {
        ...
        list.forEach(obj -> {
            dtoList.add(new ProcessManagementDTO.HistoryActivityDTO(SourceTypeEnum.MOLD_REF_SKU.getCode(), obj.getId()));
        });
    ...
    */
    ...
    public Boolean updateOutputQtyById(MoldInfoDTO.UpdateQty dto) {
        ...
        // 记录主单操作日志
        log.info("编辑 开始记录模具关联SKU日志数据，单号：【{}】", old.getMoldCode());
        String msg = StrUtil.format("用户【{}】编辑单号为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), old.getMoldCode(), "模具关联SKU");
    ...
    public void importMoldRefSku(BaseDTO.ImportDTO dto) {
        ...
        Map<String, SkuVO> skuMap = skuList.stream().filter(e -> !Objects.equals(e.getPropertyName(), ProductConstant.PRODUCT_PROPERTY_ASSET))
