...
public class ProjectTaskServiceImpl extends ServiceImpl<ProjectTaskMapper, ProjectTaskEntity> implements ProjectTaskService {
     ...
     */
    @Override
    public PagingVO<TaskPagingShowDTO> paging(PagingDTO<TaskPagingDTO> dto) {
        ...
        Page<TaskPagingDTO> query = new Page<>(dto.getCurrPage(), dto.getPageSize());
        IPage<TaskPagingShowDTO> pageData = new Page<>();
        ...
        if (pageData != null) {
            ...
            for (TaskPagingShowDTO item : list) {
                ...
                Boolean ifEditTask = getIfEditTask(item.getStatus());
                item.setIfEditTask(ifEditTask);
                List<Map<String, Object>> operateList = getOperateList(state);
    ...
    public Boolean save(ProjectTaskDTO dto) {
        ...
        if (flag) {
            ...
            //新增操作日志
            operateLogService.addSysLogBySave("新增了一个：[" + taskEntity.getName() + "]", SysLogClassPathEnum.PROJECTTASKENTITY.getDesc(), taskEntity.getId(), null);
            if (StringUtils.isNotBlank(taskEntity.getPid())) {
                //创建子任务时父级任务新增操作日志
                OperateLogEntity operateLogEntity = new OperateLogEntity().setContent(String.format("创建子任务[%s]", taskEntity.getName()))
    ...
    public ProjectTaskEntity automationTask(ProjectTaskEntity taskEntity, Integer taskType, List<String> chargeIdList, String userId) {
        ...
        if (reviewTask.equals(taskType)) {
            ...
            if (processEntity != null && CollectionUtils.isNotEmpty(chargeIdList)) {
                //评审任务 由任务负责人审核
    ...
    public List<BatchResultDTO> removeTask(String taskId) {
        ...
        if (flag) {
            ...
            //新增操作日志
            OperateLogEntity operateLogEntity = new OperateLogEntity().setContent(String.format("删除任务[%s]", entity.getName()))
    ...
    public List<TaskConductDTO> getTaskConductList(String productId) {
        ...
        for (ProjectMembersEntity item : membersList) {
            ...
            postponeTaskCount = taskList.stream().filter(t -> t.getPlanEndTime() != null && t.getRealityEndTime() != null && t.getRealityEndTime().compareTo(LocalDateTimeUtil.of(t.getPlanEndTime())) > MathUtil.ZERO).collect(Collectors.toList()).size();
    ...
    public TaskConductDTO getTaskConduct(String productId) {
        ...
        //延期的任务数
        int postponeTaskCount = list.stream().filter(t -> t.getPlanEndTime() != null && t.getRealityEndTime() != null && t.getRealityEndTime().compareTo(LocalDateTimeUtil.of(t.getPlanEndTime())) > MathUtil.ZERO).collect(Collectors.toList()).size();
        dto.setTotalTaskCount(totalTaskCount);
        dto.setFinishTaskCount(finishTaskCount);
        dto.setIngTaskCount(ingTaskCount);
        dto.setPostponeTaskCount(postponeTaskCount);
    ...
    public ProjectTaskDetailsDTO getTaskDetails(String taskId) {
        ...
        StringBuilder realityTime = new StringBuilder();
        ...
        if (RelatedSkuTypeEnum.ALL_RELATED.getCode().equals(taskEntity.getRelatedSkuType()) && CollectionUtils.isNotEmpty(details)) {
    ...
    public Boolean updateTask(ProjectTaskDTO dto) {
        ...
        if (MathUtil.ONE.equals(taskEntity.getIsFixed())) {
            ...
            if (!taskEntity.getName().equals(dto.getName())) {
                throw new ServiceException(ApiError.PROJECT_TASK_FIXED_RENAME_FORBIDDEN);
            }
            //目标交付文档
            ...
            if (!equalList) {
                throw new ServiceException(ApiError.PROJECT_TASK_FIXED_DOC_FORBIDDEN);
            }
            //审核流程
            if (!StringUtils.equals(dto.getBusinessProcessId(), taskEntity.getBusinessProcessId())) {
                throw new ServiceException(ApiError.PROJECT_TASK_FIXED_APPROVAL_FORBIDDEN);
            }
        }
    ...
    public ProductTaskCountDTO getProductTaskCount(ProductTaskCountShowDTO showDTO, Date date) {
        ...
        int totalTaskCount = taskList.size();
        //延期的任务数
        int postponeTaskCount = taskList.stream().filter(t -> t.getPlanEndTime() != null && t.getRealityEndTime() != null && t.getRealityEndTime().compareTo(LocalDateTimeUtil.of(t.getPlanEndTime())) > MathUtil.ZERO).collect(Collectors.toList()).size();
    ...
    public Boolean updateBaseTask(UpdateTaskDTO dto) {
        String updateJson = toJSONString(dto);
    ...
    public void checkSonTaskFinish(List<String> taskIds, String productId) {
        ...
        for (String taskId : taskIds) {
            ...
            if (count > 0) {
                throw new ServiceException(ApiError.PROJECT_TASK_CHILD_UNFINISHED);
            }
        }
    }
    ...
    public void checkTaskFinish(List<ProjectTaskEntity> list) {
        ...
        if (parentCount > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_UNFINISHED_FORBIDDEN);
        }
    }
    ...
    @Override
    public PagingVO<TaskPagingShowDTO> expertPaging(PagingDTO<TaskSearchParamDTO> searchParamDTO) {
        LoginUser loginUser = UserContext.getDefaultLoginUser();
        String userId = loginUser.getUid();
        TaskSearchParamDTO params = searchParamDTO.getParams();
        params.setPermissionSql(searchParamDTO.getPermissionSql());
        Page<TaskSearchParamDTO> query = new Page<>(searchParamDTO.getCurrPage(), searchParamDTO.getPageSize());
        //"assignToMe", "myCreate", "all"
        String taskProperty = TaskConstant.ALL;
        //任务条件 1 待完成  2 全部
        Integer taskCondition = params.getTaskCondition();
        IPage<TaskPagingShowDTO> pageData ;
        //分组的标示
     ...
     */
    @Override
    public PagingVO<TaskPagingShowDTO> assignToMePaging(PagingDTO<TaskSearchParamDTO> searchParamDTO) {
     ...
     */
    @Override
    public PagingVO<TaskPagingShowDTO> assignToMeWaitAuditPaging(PagingDTO<TaskSearchParamDTO> searchParamDTO) {
    ...
    public void flyingBookReminder(FlyingBookReminderDTO dto) {
        ...
        taskIds.forEach(obj -> {
            ...
            operateLogEntity.setOperation("飞书提醒");
            operateLogEntity.setBusinessId(obj);
            operateLogEntity.setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc());
            operateLogEntity.setContent("发送飞书提醒【".concat(dto.getContent()).concat("】"));
            if (ObjectUtils.isNotEmpty(loginUser)) {
                operateLogEntity.setCreateUserId(loginUser.getUid());
                operateLogEntity.setCreateUserName(loginUser.getUserName());
            }
            logList.add(operateLogEntity);
        });
    ...
    public Boolean batchUpdate(BatchScheduleTaskDTO dto) {
        ...
        if (dto.getPlanStartTime() != null && dto.getPlanEndTime() != null) {
            ...
            if (count > 0) {
                throw new ServiceException(ApiError.PROJECT_PLAN_EDIT_REQUIRED);
            }
        }
     ...
     */
    @Override
    public PagingVO<TaskPagingShowDTO> myCreatePaging(PagingDTO<TaskSearchParamDTO> searchParamDTO) {
        searchParamDTO.getParams().setPermissionSql(searchParamDTO.getPermissionSql());
        LoginUser loginUser = UserContext.getDefaultLoginUser();
        String userId = loginUser.getUid();
        TaskSearchParamDTO params = searchParamDTO.getParams();
    ...
    public List<Map<String, Object>> operateMoreList(String taskId) {
        ...
        Boolean ifEditTask = getIfEditTask( taskEntity.getStatus());
     ...
     */
    public Boolean getIfEditTask(Integer taskState) {
        //待发布
        Integer releasedState = TaskStateEnum.TO_BE_RELEASED.getCode();
        //未开始
        Integer notStartState = TaskStateEnum.NOT_START.getCode();
     ...
     */
    private List<Map<String, Object>> getOperateList(Integer state) {
    ...
    public Boolean startTask(OperateBaseTaskDTO dto) {
        ...
        if (!TaskStateEnum.NOT_START.getCode().equals(state)
            ...
            throw new ServiceException(ApiError.PROJECT_TASK_START_REQUIRED);
        }
        //检查是否在变更中
        ...
        if (CollectionUtils.isNotEmpty(generalTasks)) {
            ...
            taskIdList.forEach(taskId -> {
                operateLogEntityList.add(
                        new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.NOT_START.getName(), TaskStateEnum.ING.getName()))
                                .setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc())
    ...
    public Boolean publishTask(OperateBaseTaskDTO dto) {
        ...
        if (releasedCount > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_PUBLISH_REQUIRED);
        }
        ...
        if (flag) {
            ...
            generalTaskIds.forEach(taskId -> {
                operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.TO_BE_RELEASED.getName(), TaskStateEnum.NOT_START.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(taskId));
            });
            operateLogService.addSysLogByBatchSave(operateLogEntityList);
            //发布任务消息
    ...
    private void checkTaskChargeId(String chargeId) {
        if (StringUtils.isBlank(chargeId)) {
            throw new ServiceException(ApiError.WF_APPROVE_START_FAILED);
        }
        ...
        if (CollectionUtils.isNotEmpty(list)) {
            if (list.size() > 1) {
                ...
                if (list.size() == stateList.size()) {
                    throw new ServiceException(ApiError.PROJECT_TASK_BATCH_STATUS_FORBIDDEN);
                }
            }
    ...
    private void checkTaskAuditor(List<String> taskIdList) {
        if (CollectionUtils.isNotEmpty(taskIdList)) {
            ...
            if (count > 0) {
                throw new ServiceException(ApiError.PROJECT_TASK_APPROVER_MISSING);
            }
        }
    }
    ...
    public Boolean cancelPublishTask(OperateBaseTaskDTO dto) {
        ...
        if (releasedCount > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_UNPUBLISH_STATUS_INVALID);
        }
        ...
        if (flag) {
            ...
            List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
            list.forEach(task -> {
                operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.getName(task.getStatus()), TaskStateEnum.TO_BE_RELEASED.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(task.getId()));
            });
            operateLogService.addSysLogByBatchSave(operateLogEntityList);
        }
        return flag;
    ...
    public void checkScheduleChangeStatus(List<ProjectTaskEntity> list) {
        ...
        if (CollectionUtils.isNotEmpty(scheduleStatusList)) {
            throw new ServiceException(ApiError.PROJECT_SCHEDULE_UNDER_REVIEW_FORBIDDEN);
        }
    }
    ...
    public Boolean closeTask(OperateBaseTaskDTO dto) {
        ...
        if (releasedCount > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_CLOSE_REQUIRED);
        }
        ...
        if (flag) {
            ...
            List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
            list.forEach(task -> {
                operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.getName(task.getStatus()), TaskStateEnum.CLOSE.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(task.getId()));
            });
            operateLogService.addSysLogByBatchSave(operateLogEntityList);
            //发送关闭任务通知
    ...
    public Boolean finishTask(OperateBaseTaskDTO dto) {
        ...
        if (CollectionUtils.isNotEmpty(reviewList) && reviewList.size() > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_REVIEW_COMPLETE_FORBIDDEN);
        }
        ...
        if (CollectionUtils.isNotEmpty(noSkuChangeTaskIds) && configTaskIds.containsAll(noSkuChangeTaskIds)) {
            throw new ServiceException(ApiError.PROJECT_TASK_COMPLETE_FORBIDDEN_PRODUCT_INCOMPLETE);
        }
        //一般任务code
        Integer generalTaskCode = TaskTypeEnum.GENERAL_TASK.getCode();
        //是否确认完成
        ...
        if (!isConfirmFinish) {
            ...
            if (CollectionUtils.isNotEmpty(notFinishSkuList)) {
                ...
                throw new ServiceException(ApiError.WARNING_SUBMIT_CONFIRM, skuNo);
            }
        ...
        if (noProcess > 0) {
            throw new ServiceException(ApiError.PROJECT_TASK_COMPLETE_REQUIRED);
        }
        ...
        List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
        list.forEach(task -> {
            operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.getName(task.getStatus()), TaskStateEnum.FINISH.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(task.getId()));
        });
        operateLogService.addSysLogByBatchSave(operateLogEntityList);
        //发送完成任务通知
    ...
    public Boolean approvalPass(TaskOperateDTO dto) {
        ...
        if (!processInstanceIds.containsAll(processIds)) {
            throw new ServiceException(ApiError.WF_NOT_YOUR_APPROVAL);
        }
        ...
        if (!waitConfirmCode.equals(state) &&
            ...
            throw new ServiceException(ApiError.BILL_APPROVAL_STATUS_INVALID);
        }
        ...
        List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
        taskIdList.forEach(taskId -> {
            operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s]", TaskStateEnum.WAIT_CONFIRM.getName(), TaskStateEnum.APPROVAL_ING.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(taskId));
        });
        operateLogService.addSysLogByBatchSave(operateLogEntityList);
    ...
    public Boolean approvalReject(TaskOperateDTO dto) {
        ...
        if (!approvalIngCode.equals(state) && !waitConfirmCode.equals(state)
            ...
            throw new ServiceException(ApiError.BILL_REJECT_STATUS_INVALID);
        }
        List<TaskCommentDTO.AddDTO> taskCommentList = new ArrayList<>(taskIds.size());
        ...
        if (flag) {
            ...
            List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
            taskIdList.forEach(taskId -> {
                operateLogEntityList.add(new OperateLogEntity().setContent(String.format("编辑了一个[任务状态]由[%s]为[%s],原因[%s]", TaskStateEnum.APPROVAL_ING.getName(), TaskStateEnum.APPROVAL_NO_PASS.getName(), dto.getComment())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(taskId));
            });
            operateLogService.addSysLogByBatchSave(operateLogEntityList);
    ...
    public Boolean cancelProcess(ApproveDTO.BatchCancelProcessDTO dto) {
        ...
        if (count > 0) {
            throw new ServiceException(ApiError.WF_REVOCATION_REQUIRED);
        }
        //操作日志
        List<OperateLogEntity> operateLogEntityList = new LinkedList<>();
        //流程id 集合
            ...
            if (result) {
                ...
                if (CollectionUtils.isNotEmpty(changeDocTaskList)) {
                    ...
                    for (ProjectTaskEntity changeDoc : changeDocTaskList) {
                        ...
                        operateLogEntityList.add(new OperateLogEntity().setContent(String.format("[%s]撤销一个任务[%s]", userName, changeDoc.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(changeDoc.getId()));
                ...
                if (CollectionUtils.isNotEmpty(otherTaskList)) {
                    for (ProjectTaskEntity task : otherTaskList) {
                        ...
                        task.setProcessId("");
                        operateLogEntityList.add(new OperateLogEntity().setContent(String.format("[%s]撤销一个任务[%s]", userName, task.getName())).setClassPath(SysLogClassPathEnum.PROJECTTASKENTITY.getDesc()).setBusinessId(task.getId()));
                    }
    ...
    public Boolean restartTask(OperateBaseTaskDTO dto) {
        ...
        if (!approvalNoPass.equals(state)) {
            throw new ServiceException(ApiError.PROJECT_TASK_RESTART_REQUIRED);
        }
        //一般任务有审核
        Integer generalApproval = TaskProcessTypeEnum.GENERAL_APPROVAL_TASK.getCode();
        Integer reviewTask = TaskProcessTypeEnum.REVIEW_TASK.getCode();
    ...
    public List<TaskProcessNodeDTO> getByProcessTye(Integer processType, ProjectTaskEntity taskEntity) {
        ...
        if (general.equals(processType)) {
            //添加待开始
            resultList.add(getProcessNode(notStartEntity, notStart, userList, approveRecordShowList));
            //添加进行中
            resultList.add(getProcessNode(ingStateEntity, ingState, userList, approveRecordShowList));
            //添加已完成
            resultList.add(getProcessNode(finishStateEntity, finishState, userList, approveRecordShowList));
        }
        ...
        if (generalApproval.equals(processType)) {
            ...
            if (waitConfirmState.equals(taskState)) {
                //添加待审核
                resultList.add(getProcessNode(waitConfirmEntity, waitConfirmState, userList, approveRecordShowList));
            }
            if (approvalIngState.equals(taskState)) {
                //添加审核中
                resultList.add(getProcessNode(approvalIngEntity, approvalIngState, userList, approveRecordShowList));
            }

            if (approvalNoPassState.equals(taskState)) {//添加审核不通过
                resultList.add(getProcessNode(approvalNoPassEntity, approvalNoPassState, userList, approveRecordShowList));
            }
    ...
    public TaskProcessNodeDTO getProcessNode(TaskOperatorRecordEntity entity, Integer
            state, List<FindUserDTO> userList, List<AuditorHandleDTO> approveRecordShowList) {
    ...
    private void checkTaskName(String taskId, String productId, String name) {
        ...
        if (nameLength > 50) {
            throw new ServiceException(ApiError.PROJECT_TASK_NAME_TOO_LONG, String.valueOf(nameLength));
        }
        //根据产品很任务id 获取任务名
        List<ProjectTaskEntity> taskList = getByProductId(productId);
        if (StringUtils.isNotBlank(taskId)) {
    ...
    private void addProjectTaskDTOLog(ProjectTaskDTO projectTaskDTO, ProjectTaskEntity oldEntity, String businessId) {
        ...
        operateLogService.addSysLogByUpdate(oldDto, projectTaskDTO, SysLogClassPathEnum.PROJECTTASKENTITY.getDesc(), businessId, null, String.format("任务[%s]", oldEntity.getName()));
    }
    ...
    private void addUpdateTaskDTOLog(ProjectTaskEntity newEntity, ProjectTaskEntity oldEntity, String businessId) {
        operateLogService.addSysLogByUpdate(oldEntity, newEntity, SysLogClassPathEnum.PROJECTTASKENTITY.getDesc(), businessId, null, String.format("任务[%s]", oldEntity.getName()));
    }
    ...
    public List<BatchResultDTO> removeBatch(List<String> ids) {
        ...
        for (ProjectTaskEntity req : entity) {
            ...
            if (flag) {
                ...
                //新增操作日志
                OperateLogEntity operateLogEntity = new OperateLogEntity().setContent(String.format("删除任务[%s]", req.getName()))
    ...
    public PagingVO<ProjectTaskDTO.SimpleViewDTO> pagingByAdvanceQuery(PagingDTO<ProjectTaskDTO.PagingParamDTO> pagingParamDTO) {
        ...
        Page<ProjectTaskDTO.PagingParamDTO> query = new Page<>(pagingParamDTO.getCurrPage(), pagingParamDTO.getPageSize());
        ...
        for (ProjectTaskDTO.SimpleViewDTO viewDTO : pageData.getRecords()) {
            viewDTO.setStatusName(TaskStateEnum.getName(viewDTO.getStatus()));
        }
        // 数据处理
        return new PagingVO<>(pageData);
    }
