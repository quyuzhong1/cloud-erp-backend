...
import com.erp.server.mrp.mapper.CfgNoticeMapper;
...
public class CfgNoticeServiceImpl extends SuperServiceImpl<CfgNoticeMapper, CfgNoticeEntity> implements CfgNoticeService {
    ...
    public BaseResultDTO.AddDTO add(CfgNoticeDTO.AddDTO addDTO) {
        ...
        cfgNoticeEntity.setNoticeNode(addDTO.getNoticeNode());
        cfgNoticeEntity.setNoticeRule(JSONUtil.parseObj(addDTO.getNoticeRule()));
    ...
    */
    ...
    public Boolean update(CfgNoticeDTO.UpdateDTO addOrUpdateDTO) {
        ...
        CfgNoticeEntity cfgNoticeEntity = new CfgNoticeEntity();
        cfgNoticeEntity.setNoticeNode(addOrUpdateDTO.getNoticeNode());
        cfgNoticeEntity.setNoticeRule(JSONUtil.parseObj(addOrUpdateDTO.getNoticeRule()));
        cfgNoticeEntity.setId(addOrUpdateDTO.getId());
    ...
    public CfgNoticeDTO.ViewDTO view(String id) {
        ...
        viewDTO.setNoticeRule(JSONUtil.toBean(cfgNoticeEntity.getNoticeRule(), CfgNoticeDTO.NoticeRuleDTO.class));
        ...
        for (CfgNoticeDetailEntity detailEntity : cfgdetailList) {
            ...
            switch (Objects.requireNonNull(CfgNoticeObjectTypeEnum.getEnum(detailEntity.getNoticeType()))) {
                case NOTICE_USER:
                case NOTICE_GROUP:
    ...
    public void updateDisabled(CfgNoticeDTO.UpdateDisabledDTO dto) {
        ...
        if (ObjectUtil.isEmpty(cfgNoticeEntity)) {
            throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "通知配置单");
        }
    ...
    private void doOpHandlePaging(List<CfgNoticeDTO.ListDTO> records) {
        ...
        //人员
        List<FindUserDTO> allUserList = sysUserFeign.getUserList();
        ...
        for (CfgNoticeDTO.ListDTO item : records) {
            ...
            //通知规则
            item.setNoticeRuleName(noticeRuleName.toString());
            //通知平台
            String platformName = dictBasicList.stream().filter(obj -> CharSequenceUtil.equals(obj.getValue(), item.getNoticePlatform())).map(DictBasicEntity::getName).findFirst().orElse("");
            item.setNoticePlatformName(platformName);

            //明细信息
            List<CfgNoticeDetailEntity> detailList = cfgNoticeDetailList.stream().filter(obj -> CharSequenceUtil.equals(obj.getMainId(), item.getId())).collect(Collectors.toList());
