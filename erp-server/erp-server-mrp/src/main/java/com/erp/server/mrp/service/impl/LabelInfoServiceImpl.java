...
public class LabelInfoServiceImpl extends SuperServiceImpl<LabelInfoMapper, LabelInfoEntity> implements LabelInfoService {
    ...
    */
    ...
    public BatchResultDTO delete(String id) {
        ...
        //删除标签
        ApplicationContextUtils.getBean(LabelInfoServiceImpl.class).removeById(old.getId());
    ...
    private List<String> getDeleteIds(List<LabelInfoEntity> newList, List<LabelInfoEntity> oldList) {
        List<String> newIds = newList.stream().map(LabelInfoEntity::getId).
    ...
    */
    private void checkData(List<LabelInfoEntity> list) {
       if (CollectionUtils.isEmpty(list)) {
        ...
        String names = list.stream().collect(Collectors.groupingBy(LabelInfoEntity::getName)).entrySet().stream().filter(obj -> obj.getValue().size() > MathUtil.ONE).map(Map.Entry::getKey).collect(Collectors.joining(","));
        if (CharSequenceUtil.isNotBlank(names)) {
            throw new ServiceException(CharSequenceUtil.format("标签名称【{}】重复，不支持新增",names));
        }
    }
    @Override
