...
public class PurchaseSuggestionMergeQueryHandler extends AbstractQueryHandler {
    ...
    public String getTabSql (Object value) {
        ...
        if (DeliverySuggestTabEnum.WAIT_PUSH.getCode().equals(value)) {
            return "not EXISTS(select 1 from foreign_purchase_application_detail, jsonb_array_elements(source_json) AS elem where is_deleted = false and elem ->> 'id' = psm.id) and psm.status = 'finish' and psm.invalid_status = false";
        }
