...
public class CfgRuleWarehouseServiceImpl extends SuperServiceImpl<CfgRuleWarehouseMapper, CfgRuleWarehouseEntity> implements CfgRuleWarehouseService {
    ...
    @Override
    public BaseResultDTO.AddDTO add(CfgRuleWarehouseDTO.AddDTO addDTO) {
        CfgRuleWarehouseEntity cfgRuleWarehouseEntity = new CfgRuleWarehouseEntity();
        BeanMapperUtils.copy(addDTO, cfgRuleWarehouseEntity);

        // 数据处理
        handleData(cfgRuleWarehouseEntity);

        log.info("开始新增仓库（规则设置）");
        boolean save = super.save(cfgRuleWarehouseEntity);
        if(!save) {
            throw new ServiceException("仓库（规则设置）保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", UserContext.getDefaultLoginUser().getUserName(), "仓库（规则设置）" , cfgRuleWarehouseEntity.getId());
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLog(msg, null, cfgRuleWarehouseEntity.getId(), "新增操作");
        // TODO 新增明细（如果有明细的话）

        return new BaseResultDTO.AddDTO(cfgRuleWarehouseEntity.getId(), cfgRuleWarehouseEntity.getId());
    }
    ...
    */
    ...
    public Boolean update(CfgRuleWarehouseDTO.UpdateDTO updateDTO) {
        ...
        Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.NOT_EXIST_BILL, "仓库（规则设置）"));
        ...
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLogByObj(old, cfgRuleWarehouseEntity, null, cfgRuleWarehouseEntity.getId(), msg);
        return Boolean.TRUE;
    }
