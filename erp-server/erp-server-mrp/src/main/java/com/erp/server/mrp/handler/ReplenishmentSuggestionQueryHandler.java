    ...
    protected String handleSqlLogic(String field, Object value, String compareCodeSplicingValueSql) {
        ...
        if("restockDate".equals(field)) {
            if (QueryConditionEnum.BETWEEN.equals(queryConditionEnum)) {
                ...
                return "EXISTS ((SELECT 1 FROM delivery_suggest WHERE source_id = rsd.id AND is_deleted = FALSE AND suggest_delivery_date " + compareCodeSplicingValueSql1 + ")  OR (SELECT 1 FROM purchase_suggest WHERE source_id = rsd.id AND is_deleted = FALSE AND suggest_purchase_date " + compareCodeSplicingValueSql2 + "))";
