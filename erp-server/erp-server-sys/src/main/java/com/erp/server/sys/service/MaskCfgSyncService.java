 ...
 *
 * <p>定位：{@code CfgMaskFieldService} / {@code CfgMaskWordService} 现有"service 写完即广播"机制
 ...
 *   <li>发现变化时调对应 service 的 {@code publishFullCache()} 触发"重算 + 写 Bucket + 广播"</li>
 *   <li>更新快照</li>
 * </ol>
 *
 * <h3>责任边界</h3>
...
public interface MaskCfgSyncService {
    ...
    /**
     * 执行一次同步：扫两张 cfg 表 → diff → 调 publishFullCache。XXL-JOB 调用入口。
     *
     ...
     * 重置内存快照
     *
     * <p>用于诊断：把 firstRun 置回，下一次扫描重新填充快照不发广播；之后开始正常 diff。
