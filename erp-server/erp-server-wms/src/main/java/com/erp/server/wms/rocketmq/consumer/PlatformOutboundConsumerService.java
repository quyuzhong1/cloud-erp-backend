...
public class PlatformOutboundConsumerService<T extends DmpSyncTaskIdDTO> extends AbstractPlatformConsumerHandler<T> {
        ...
        if(referenceNo.contains(BusinessNoConstant.WFHD)){
            ...
            if(Objects.isNull(thirdWarehouseDeliveryEntity)){
                if(SoB2cBillStatusEnum.ENUM_SHIPPED.getCode().equals(dto.getOrderStatus())
                    ...
                    thirdWarehouseDeliveryEntity = checkAndGenThirdDelivery(dto, referenceNo);
                    if (Objects.isNull(thirdWarehouseDeliveryEntity)) return ApiResult.success();
                }else {
    ...
    private ThirdWarehouseDeliveryEntity checkAndGenThirdDelivery(PlatformOutboundDTO dto, String referenceNo) {
        SoB2cEntity mainEntity;
        //平台订单号
        ...
        if(CollUtil.isNotEmpty(listBySwOrderNumber)){
            mainEntity = listBySwOrderNumber.get(0);
        }else {
            mainEntity = listByReferenceNo.get(0);
        }
        ...
        //4.“三方仓发货单”
        return generateThirdWarehouseDelivery(detailList, dto, mainEntity,platformCode);
    }
