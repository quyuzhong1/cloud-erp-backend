...
public class QcInfoServiceImpl extends SuperServiceImpl<QcInfoMapper, QcInfoEntity> implements QcInfoService {
    ...
    public QcInfoEntity add(QcInfoDTO.SaveOrUpdateDTO dto) {
        ...
        if (Objects.nonNull(dto.getQcInfo())
            ...
            if (Objects.nonNull(qcResult)) {
                ...
                operateLogService.addModuleOperateLog(msg, ModuleTypeEnum.QC_ORDER.getCode(), billId, "新增操作");
            }
        }
        bill.setId(billId);
        //处理相关数据
            ...
            if (CharSequenceUtil.isNotBlank(purchaseOrderId)) {
                ...
                if (purchaseOrder != null) {
                    ...
                    if (CharSequenceUtil.isNotBlank(skuNos)) {
                        throw new ServiceException(ApiError.PO_SKU_PUSH_DOWN_NOT_ALLOWED, purchaseOrder.getCode(), skuNos);
                    }
    ...
    public QcInfoDTO.ViewDTO view(String id) {
        ...
        view.setSourceTypeName(SourceTypeEnum.getName(bill.getSourceType()));
        //产品信息
...
//            code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.QC, BusinessNoTypeEnum.CODE_QC.getCode()));
    ...
    public void updateProductPack(List<String> billIdList) {
        ...
        //采购信息
        List<String> poIdList = qcInfoEntityList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getPurchaseOrderId())).map(QcInfoEntity::getPurchaseOrderId).distinct().collect(Collectors.toList());
    ...
    private void HandleData(String qcUserId, String qcDeptId, QcInfoEntity entity, String sourceType, String sourceId) {
        ...
        if (CharSequenceUtil.isNotBlank(qcUserId)) {
            ...
            if (ObjectUtils.isEmpty(userDTO)) {
                throw new ServiceException(ApiError.AUTH_USER_NOT_FOUND, qcUserId);
            }
...
//            PoInstockEntity entity = poInstockService.getById(createResultId);
    ...
    private void autoBatchStockInBill(List<String> idList) {
        ...
        //只要有采购订单的以及是b2b质检类型
        stockInList = stockInList.stream().filter(s -> CharSequenceUtil.isNotBlank(s.getPurchaseOrderId()) &&
...
//                operateLogService.addModuleOperateLog(String.format("新增了质检单【%s】", code), ModuleTypeEnum.QC_ORDER.getCode(),billId , "新增操作");
    ...
    public BatchResultDTO finish(QcInfoEntity entity) {
        ...
        for (QcInfoEntity item : qcList) {
            ...
            if (CharSequenceUtil.isBlank(item.getCode())) {
//                String code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.QC, BusinessNoTypeEnum.CODE_QC.getCode()));
     ...
     * <p>
     * 该方法主要完成两个功能：
     * 1. 根据来源类型和来源明细ID追溯并补全采购订单明细ID
     * 2. 当质检结果为合格时，将合格数量累计到对应的采购订单
     *
     * @param entity                     质检单实体对象，用于获取来源类型和来源明细ID
     * @param finalPurchaseOrderDetailId 采购订单明细ID，如果为空则尝试从来源数据中追溯
     * @param qcResult                   质检结果编码，用于判断是否为合格
    ...
    private void handlePurchaseOrderQcAccumulation(QcInfoEntity entity, String finalPurchaseOrderDetailId, String qcResult, Integer qcGoodQty) {
        ...
        if (CharSequenceUtil.isBlank(finalPurchaseOrderDetailId)) {
            ...
            String sourceDtlId = entity.getSourceDetailId();
            if (CharSequenceUtil.isNotBlank(sourceType) && CharSequenceUtil.isNotBlank(sourceDtlId)) {
                if (SourceTypeEnum.PO_RECEIVE.getCode().equals(sourceType)) {
                    WarehouseReceiveDetailEntity wrd = warehouseReceiveDetailService.getById(sourceDtlId);
                    if (wrd != null) {
                        finalPurchaseOrderDetailId = wrd.getPurchaseOrderDetailId();
                    }
                } else if (SourceTypeEnum.QC_NOTICE.getCode().equals(sourceType)) {
                    QcNoticeDetailEntity qnd = qcNoticeDetailService.getById(sourceDtlId);
                    if (qnd != null && CharSequenceUtil.isNotBlank(qnd.getMainId())) {
                        QcNoticeEntity notice = qcNoticeService.getById(qnd.getMainId());
                        if (notice != null) {
                            if (SourceTypeEnum.PO_RECEIVE.getCode().equals(notice.getSourceType())) {
                                WarehouseReceiveDetailEntity wrd = warehouseReceiveDetailService.getById(qnd.getSourceDetailId());
                                if (wrd != null) {
                                    finalPurchaseOrderDetailId = wrd.getPurchaseOrderDetailId();
                                }
                            } else if (SourceTypeEnum.QC_APPLICATION.getCode().equals(notice.getSourceType())) {
                                QcApplicationDetailEntity qad = qcApplicationDetailService.getById(qnd.getSourceDetailId());
                                if (qad != null) {
                                    finalPurchaseOrderDetailId = qad.getSourceDetailId();
                                }
                            }
                        }
                    }
                ...
                if (CharSequenceUtil.isNotBlank(finalPurchaseOrderDetailId)) {
                    qcResultService.lambdaUpdate()
                            .set(QcResultEntity::getPurchaseOrderDetailId, finalPurchaseOrderDetailId)
                            .eq(QcResultEntity::getMainId, entity.getId())
                            .update();
                }
            }
        }
...
//                String code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.QC, BusinessNoTypeEnum.CODE_QC.getCode()));
    ...
    public BatchResultDTO batchCancel(QcInfoEntity entity) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.PO_QC_VOIDED_OPERATION_NOT_ALLOWED, "取消质检");
        }
        String qcStatus = QcBillStatusEnum.WAIT_QC.getCode();
        long count = qcList.stream().filter(s -> !s.getQcStatus().getCode().equals(qcStatus)).count();
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_CANCEL_ALLOWED_STATUS_ONLY);
        }
        //质检状态
    ...
    public BatchResultDTO delete(QcInfoEntity entity) {
        ...
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_DELETE_ALLOWED_STATUS_ONLY);
        }
        //删除操作日志
    ...
    public BatchResultDTO cancelProcess(QcInfoEntity entity) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.PO_QC_VOIDED_OPERATION_NOT_ALLOWED, "撤销质检");
        }
        ...
        long count = qcList.stream().filter(s -> !statusList.contains(s.getQcStatus().getCode())).count();
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_REVOKE_ALLOWED_STATUS_ONLY);
        }
        //入库的
        ...
        if (stockInCount > 0) {
            throw new ServiceException(ApiError.PO_INBOUND_EXISTS_REVOKE_FORBIDDEN);
        }
        ...
        if (returnCount > 0) {
            throw new ServiceException(ApiError.PO_RETURN_EXISTS_REVOKE_FORBIDDEN);
        }
    ...
    public Boolean updateHandleMode(QcResultDTO.UpdateHandleModeDTO dto) {
        ...
        long count = qcList.stream().filter(s -> !statusList.contains(s.getQcStatus().getCode())).count();
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_UPDATE_MEASURE_ALLOWED_STATUS_ONLY);
        }
    ...
    public List<PurchaseReturnOrderDTO.ViewGeneratePurchaseReturnOrderDTO> viewGeneratePurchaseReturnOrder(List<String> ids) {
        ...
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_PUSH_RETURN_ALLOWED_ONLY_IF_REJECTED);
        }
...
//            throw new ServiceException(ApiError.ERROR_99029);
            qcStatus.forEach(qcInfoEntity -> {
                resultDTOS.add(BatchResultDTO.fail(qcInfoEntity.getId(), qcInfoEntity.getCode(), ApiError.PO_QC_PUSH_RETURN_ALLOWED_ONLY_IF_REJECTED.getMsg()));
            });
            return resultDTOS;
        }
        List<QcResultEntity> qcResultList = qcResultService.getByMainIdList(ids);
//        long handleCount = qcResultList.stream().filter(r -> !r.getHandleModeDict().equals(WmsConstant.QC_RESULT_HANDLE_MODE)).count();
        ...
        if (CollectionUtils.isNotEmpty(qchHandles)) {
            qchHandles.forEach(qcResultEntity -> {
                ...
                resultDTOS.add(BatchResultDTO.fail(qcResultEntity.getMainId(), code, ApiError.PO_QC_PUSH_RETURN_ALLOWED_ONLY_IF_REJECTED.getMsg()));
            });
            return resultDTOS;
//            throw new ServiceException(ApiError.ERROR_99029);
        }
        ...
        for (Map.Entry<String, List<PoInstockDTO.GeneratePurchaseReturnOrderDTO>> entry : map.entrySet()) {
            ...
            if (Objects.isNull(qcInfoEntity)) {
                resultDTOS.add(BatchResultDTO.fail(sourceId, "", ApiError.PO_QC_ORDER_NOT_FOUND.getMsg()));
//                throw new ServiceException(ApiError.ERROR_99015);
                continue;
            }
...
//                addDetailDTO.setWarehouseLocation(detail.getWarehouseLocation());
    ...
    public Boolean returnReceiveGenerateQCSave(List<String> receiveIds) {
        ...
        //质检类型的集合
        List<QcReportDTO.ListDTO> list = qcReportService.listByQcType(Collections.singletonList(QcTypeEnum.RETURN_QC.getCode()));
        Map<String, List<QcReportDTO.ListDTO>> qcTypeMap = list.stream().collect(Collectors.groupingBy(QcReportDTO.ListDTO::getQcType));
    ...
    public List<SoReturnInstockDTO.GenerateSoReturnInstockView> generateSoReturnInstockView(List<String> ids) {
        ...
        if (receiveCount > 0) {
            throw new ServiceException(ApiError.SO_RETURN_PUSH_ALLOWED_SOURCE_SALES_RETURN_RECEIPT_ONLY);
        }
        ...
        for (SoReturnInstockDTO.GenerateSoReturnInstockView view : list) {
            ...
            if (CharSequenceUtil.isNotBlank(returnTypeDict)) {
            ...
            if (CharSequenceUtil.isNotBlank(returnReason)) {
            ...
            if (CharSequenceUtil.isBlank(soReturnReceiveEntity.getSourceCode())) {
            ...
            List<WarehouseDTO.UpdateDTO> warehouseList = warehouseService.listWarehouseByIds(Collections.singletonList(view.getWarehouseId()));
            ...
            for (QcInfoEntity item : qcList) {
                ...
                if (qcInfo != null) {
                    ...
                    if (!isExemption && (goodQty + badQty > qcQty)) {
                        Integer errorCode = ApiError.PO_QC_TOTAL_GOOD_BAD_QTY_INVALID.getCode();
    ...
    private void checkQcQty(QcResultDTO.AddDTO qcInfo, String mainId, String purchaseOrderId, String skuId) {
        ...
        if (qcInfo != null) {
            ...
            if (qcQty > totalQty) {
                throw new ServiceException(ApiError.PO_QC_TOTAL_QTY_EXCEEDS);
            }

            if (goodQty + badQty > qcQty) {
                throw new ServiceException(ApiError.PO_QC_TOTAL_GOOD_BAD_QTY_INVALID);
            }
            ...
            if (CollectionUtils.isNotEmpty(purOrderDetailList)) {
                 ...
                 * 获取到已质检数量
                 */
                List<QcResultDTO.QcQtyDTO> qcQtyList = qcResultService.getPurOrderIds(Collections.singletonList(purchaseOrderId));
                //已完成的质检
                Integer finishQcQty = qcQtyList.stream().filter(q ->
    ...
    private void generateDailyRptExcel(List<QcInfoDTO.QcDailyReportDTO> resultList, HttpServletResponse response) {
        ...
        String fileName = CharSequenceUtil.format("质检日报数据{}.xlsx", LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMddHHmmss")));
        try {
    ...
    public void reQcSample(QcInfoDTO.ReQcDTO dto) {
        ...
        dto.getIds().stream().forEach(id -> {
            ...
            if (Objects.isNull(qcInfoEntity)) {
                throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "质检单信息");
            }
            // 检查是否已作废
            if (Objects.equals(qcInfoEntity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
                throw new ServiceException(ApiError.PO_QC_VOIDED_OPERATION_NOT_ALLOWED, "复检抽检");
            }
    ...
    private void buildQcPagingView(List<QcInfoDTO.OpenPagingViewDTO> list) {
        ...
        list.forEach(item -> {
            ...
            if (entity != null) {
                ...
                if (qcResult != null) {
                    item.setTotalQty(qcResult.getTotalQty());
    ...
    public QcInfoDTO.ViewDTO qcView(String id) {
        ...
        view.setQcUserId(loginUser.getUid());
        view.setQcUserName(loginUser.getUserName());
        view.setQcDate(LocalDate.now());
        SysDepartmentUserNumberDTO dept = sysUserFeign.getDeptByUserId(loginUser.getUid());
        if (Objects.nonNull(dept)) {
            view.setQcDeptId(dept.getDepartmentId());
            view.setQcDeptName(dept.getDepartmentName());
        }
        String qcType = view.getQcInfo().getQcType();
        view.setQcType(qcType);
        view.setQcTypeName(view.getQcInfo().getQcTypeName());
        view.setSuggestSamplingQty(view.getQcStandardView().getSuggestSamplingQty());
