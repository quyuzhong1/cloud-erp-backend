...
//        String code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.FHTZ, BusinessNoTypeEnum.CODE_FHTZ.getCode()));
        ...
        SoDeliveryNoticeServiceImpl bean = ApplicationContextUtils.getBean(SoDeliveryNoticeServiceImpl.class);
        bean.autoGenerateDeclareDetailMid(soDeliveryNoticeEntity, BillGenerateTimingEnum.AFTER_ADD, Boolean.TRUE);
        ...
        if (ApproveTypeEnum.PASS.getStatus().equals(dto.getType())) {
            ...
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                ...
                public void afterCommit() {
                    SoDeliveryNoticeServiceImpl bean = ApplicationContextUtils.getBean(SoDeliveryNoticeServiceImpl.class);
                    bean.autoGenerateDeclareDetailMid(entity, BillGenerateTimingEnum.AFTER_APPROVE, Boolean.TRUE);
                }
            });
        } else {
            //审核不通过
...
//        List<WarehouseEntity> warehouseEntityList = warehouseService.listByIds(transferWarehouseIdList);
     ...
     */
    @Override
    public void autoGenerateDeclareDetailMid(SoDeliveryNoticeEntity entity, BillGenerateTimingEnum billGenerateTimingEnum, Boolean checkCfg) {
        ...
        try {
            ...
            submitB2bDeclareAutoGenerateAfterCommit(autoGenerateBillDTO);
            log.info("B2B发货通知单{}已提交自动生成报关明细异步任务", entity.getCode());
        } catch (Exception e) {
    ...
    /**
     * 事务提交后异步执行B2B报关自动生成任务。
     *
     * @param dto 自动生成参数
     */
    private void submitB2bDeclareAutoGenerateAfterCommit(AutoGenerateBillDTO dto) {
        if (TransactionSynchronizationManager.isSynchronizationActive()) {
            TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
                @Override
                public void afterCommit() {
                    soDeliveryNoticeService.asyncConsumeDeclareAutoGenerateTask(dto);
                }
            });
            return;
        }
    ...
    public void asyncConsumeDeclareAutoGenerateTask(AutoGenerateBillDTO dto) {
        log.info("B2B发货通知单自动生成报关明细异步任务开始，dto={}", JSONUtil.toJsonStr(dto));
        try {
            Boolean result = soDeliveryNoticeService.consumeDeclareAutoGenerateTask(dto);
            if (!Boolean.TRUE.equals(result)) {
                log.error("B2B发货通知单自动生成报关明细异步任务处理失败，dto={}", JSONUtil.toJsonStr(dto));
                return;
            }
            log.info("B2B发货通知单自动生成报关明细异步任务完成，id={}", dto.getId());
        } catch (Exception e) {
            log.error("B2B发货通知单自动生成报关明细异步任务异常，dto={}", JSONUtil.toJsonStr(dto), e);
        }
    }
