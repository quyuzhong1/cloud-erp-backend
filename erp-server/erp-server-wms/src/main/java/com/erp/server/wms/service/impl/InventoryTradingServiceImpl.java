...
public class InventoryTradingServiceImpl implements InventoryTradingService {
    ...
    private void checkVirtualInventoryList (List<InventoryTransactionDTO> transactionList) {
        ...
        for (Map.Entry<String, List<InventoryTransactionDTO>> entry : map.entrySet()) {
            ...
            log.warn("未分配预检(PG) 仓库【{}】SKU【{}】已分配【{}】实体【{}】", value.get(0).getWarehouseName(), value.get(0).getSkuNo(), virtualQty, realInventoryTotal);
            if (Math.abs(qty) > allowedUnalloc) {
                ServiceException.runError(ApiError.VM_CHECK_OUT_VIRTUAL_INVENTORY,value.get(0).getSkuNo(),value.get(0).getWarehouseName(),virtualQty, Math.max(allowedUnalloc, 0));
            }
        }
    }
