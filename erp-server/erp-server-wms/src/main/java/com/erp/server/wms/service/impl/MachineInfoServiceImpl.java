...
//        }
        ...
        for (MachineDetailDTO.ViewDTO viewDetailDTO : viewDetailList) {
            ...
            for (MachineSubComponentsDTO.ViewDTO subComponentsDTO : subComponentsDTOList) {
                ...
                if (ObjectUtils.isEmpty(quantity)) {
                    throw new ServiceException(ApiError.BOM_CHILD_NOT_FOUND_PARENT,viewDetailDTO.getSkuNo());
                }
    ...
    public BatchResultDTO disApprove(MachineInfoEntity entity) {
        ...
        if (CollectionUtils.isNotEmpty(transferInfoList)) {
            ...
            throw new ServiceException(ApiError.WH_TRANSFER_DIRECT_ALREADY_PUSHED_REVERSE_FORBIDDEN,codes);
        }
        //已存在出库单
        ...
        if (CollectionUtils.isNotEmpty(purchaseReturnOrderList)) {
            ...
            throw new ServiceException(ApiError.WH_MACHINE_EXIST_PURCHASE_RETURN,codes);
        }
    ...
    public BatchResultDTO invalid(String id, String reason) {
        ...
        //根据id查询
        MachineInfoEntity entity = this.getByIdOpt(id).orElseThrow(()-> new ServiceException(ApiError.WH_SUBCONTRACT_PROCESS_ORDER_NOT_FOUND));
        //非待提交和审核不通过不能作废
