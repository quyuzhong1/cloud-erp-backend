...
public class SampleReturnInfoServiceImpl extends SuperServiceImpl<SampleReturnInfoMapper, SampleReturnInfoEntity> implements SampleReturnInfoService {
    ...
    public BaseResultDTO.AddDTO add(SampleReturnInfoDTO.AddDTO addDTO) {
        ...
        if(!save) {
            throw new ServiceException("样品归还单主单保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据单号为【{}】", UserContext.getDefaultLoginUser().getUserName(), "样品归还单主单" , sampleReturnInfoEntity.getCode());
    ...
    */
    ...
    public Boolean update(SampleReturnInfoDTO.UpdateDTO addOrUpdateDTO) {
        ...
        old = Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.NOT_EXIST_BILL, "样品归还单主单"));
        // 待提交和审核不通过允许修改
        ...
        // 数据处理
        handleData(sampleReturnInfoEntity);
        log.info("编辑 开始修改样品归还单主单数据，单号：【{}】", old.getCode());
        ...
        // 记录主单操作日志
            log.info("编辑 开始记录样品归还单主单日志数据，单号：【{}】", sampleReturnInfoEntity.getCode());
            String msg = StrUtil.format("用户【{}】编辑单号为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), sampleReturnInfoEntity.getCode(), "样品归还单主单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
    ...
    public List<SampleReturnInfoDTO.TabListDTO> tabList(PermissionsDTO param) {
        ...
        statusList.parallelStream().forEach(status -> {
            ...
            list.add(new SampleReturnInfoDTO.TabListDTO(status, 0));
        }
        });
        list.add(new SampleReturnInfoDTO.TabListDTO("all", list.stream().mapToInt(SampleReturnInfoDTO.TabListDTO::getCount).sum()));
        // 计算合计数量
        return list;
    }
    ...
    public BatchResultDTO submit(String id) {
        ...
        if (ObjectUtil.isEmpty(entity)) {
            throw new ServiceException("未找到样品归还单主单数据");
        }
        validateSubmit(entity);
        // 更新单据审核状态
        log.info("提交 开始修改样品归还单主单状态数据，id：【{}】", id);
        this.updateApproveStatus(id, ApproveStatusEnum.APPROVE_ING.getStatus());

        // TODO 启动流程（如果需要的话）
        log.info("提交 开始启动样品归还单主单流程，id=：【{}】", entity.getId());
        startProcess(entity);
        // 记录操作日志
        log.info("提交 开始记录样品归还单主单日志数据，id：【{}】", id);
        String msg = StrUtil.format("用户【{}】单号为【{}】的【{}】单据提交审核 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品归还单主单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
    ...
    public BatchResultDTO approve(ApproveOneDTO dto) {
        ...
        // 操作日志
        String msg = StrUtil.format("用户【{}】单号为【{}】的【{}】单据审核操作  审核结果：【{}】 审核意见 ：【{}】", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品归还单主单", approveType.getName(), dto.getComment());
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
    ...
    */
    ...
    public BatchResultDTO disApprove(String id) {
        SampleReturnInfoEntity entity = super.getByIdOpt(id).orElseThrow(() -> new ServiceException("未找到样品归还单主单单数据"));
        // 反审核条件判断
        ...
        // 操作日志
        String msg = StrUtil.format("用户【{}】单号为【{}】的【{}】单据反审核操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品归还单主单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
    ...
    public BatchResultDTO delete(String id) {
        SampleReturnInfoEntity entity = super.getByIdOpt(id).orElseThrow(() -> new ServiceException("未找到样品归还单主单数据"));
        // 只有待提交数据允许删除
        ...
        // 删除主单数据
        log.info("删除 开始删除样品归还单主单主单数据，id：【{}】", id);
        super.removeById(id);
        // 删除日志数据
        log.info("删除 开始删除样品归还单主单日志数据，id：【{}】", id);
        String msg = StrUtil.format("用户【{}】单号为【{}】的【{}】单据删除操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品归还单主单");
        operateLogService.addModuleOperateLog(msg, null, entity.getCode(), "删除样品归还单主单数据");
    ...
    */
    ...
    public BatchResultDTO cancelProcess(String id) {
        ...
        log.info("撤销 开始修改样品归还单主单状态，id：【{}】", id);
        updateApproveStatus(id, ApproveStatusEnum.WAIT_SUBMIT.getStatus());

        //操作日志
        log.info("撤销 开始记录操作日志，id：【{}】", id);
        String msg = StrUtil.format("用户【{}】单号为【{}】的【{}】单据撤销流程操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品归还单主单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
    ...
    public SampleReturnInfoDTO.ViewDTO view(String id) {
        SampleReturnInfoEntity sampleReturnInfoEntity = super.getByIdOpt(id).orElseThrow(()->new ServiceException("未找到样品归还单主单数据"));
