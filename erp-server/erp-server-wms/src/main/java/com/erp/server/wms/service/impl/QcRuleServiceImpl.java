    ...
    public String add(QcRuleDTO.AddDTO dto) {
        ...
        if (existReport) {
            if (CollectionUtils.isEmpty(reportList)) {
                throw new ServiceException(ApiError.PO_QC_REPORT_NOT_FOUND);
            }
        }
...
//        String code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.QCGZ, BusinessNoTypeEnum.CODE_ZJGZ.getCode()));
    ...
    private void checkQcType(String id, QcTypeEnum qcType) {
        ...
        if (count > 0) {
            throw new ServiceException(ApiError.PO_QC_TYPE_EXISTS);
        }
    }
    ...
    public QcRuleDTO.ViewDTO view(String id) {
        ...
        if (Objects.isNull(rule)) {
            throw new ServiceException(ApiError.PO_QC_RULE_NOT_FOUND);
        }
...
//        }
        ...
        List<String> removeIdList = removeList.stream().map(QcRuleEntity::getId).collect(Collectors.toList());
        ...
        if (result) {
            //删除质检报告
            qcReportService.removeByRuleIds(removeIdList);
        }
        return resultDTOList;
    }
