...
public class SyncKingdeePoReceiveServiceImpl implements SyncKingdeePoReceiveService {
    ...
    public void syncDataToKingdee(WarehouseReceiveEntity entity, String operate) {
        ...
        if (StringUtils.isNotBlank(entity.getPurchaseOrderId())) {
            ...
            DmpPushTaskEntity purchaseOrderTask = dmpMqFeign.getByParam(new DmpSyncTaskDTO.OneDTO(SourceTypeEnum.PURCHASE_ORDER.getCode(), purchaseOrderEntity.getId(), PlatformEnum.KINGDEE.getName(), PlatformEnum.ERP.getName()));
...
/*        //收料员
        ...
        CompletableFuture.supplyAsync(() -> {
            SendResult result = mQProducerService.syncClassMsg(RocketMqTopic.SYNC_KINGDEE_ERP_TOPIC, RocketMqTagEnum.KINGDEE_PO_RECEIVE_TAG.getName(), resultMap, String.valueOf(resultMap.get("id")));
            if (result.getSendStatus().equals(SendStatus.SEND_OK)) {
                //mq发送成更新业务表状态及时间
                return warehouseReceiveService.updateSyncKingdeeStatus(entity.getId(), SyncStatusEnum.IN_SYNC.getCode(), "", operate);
            }
            return Boolean.TRUE;
        });
