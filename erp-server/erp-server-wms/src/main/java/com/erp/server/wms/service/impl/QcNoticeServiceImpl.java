...
public class QcNoticeServiceImpl extends SuperServiceImpl<QcNoticeMapper, QcNoticeEntity> implements QcNoticeService {
    ...
    public Boolean update(QcNoticeDTO.UpdateDTO updateDTO) {
        ...
        if (Objects.equals(old.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "编辑");
        }
        // 待提交和审核不通过允许修改
    ...
    public BatchResultDTO submit(String id) {
        ...
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "提审");
        }
    ...
    public BatchResultDTO approve(ApproveOneDTO dto) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "审核");
        }
        // 审核中的数据允许审核
        ...
        if (Objects.equals(approveType, ApproveTypeEnum.PASS)){
            ...
            for (QcNoticeDetailEntity detail : qcNoticeDetailEntities) {
                ...
                if (inventoryQty <= 0 || inventoryQty.intValue() < noticeQty.intValue()) {
                    sb.append(StrUtil.format(ApiError.ERROR_QC_STOCK_INSUFFICIENT.getMsg(), skuNo, noticeQty, inventoryQty));
                    sb.append(";");
                }
            }
    ...
    public BatchResultDTO disApprove(String id) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "反审核");
        }
        // 反审核条件判断
        ...
        if(CollUtil.isNotEmpty(qcInfoList)){
            ...
            return BatchResultDTO.fail(entity.getId(), entity.getCode(), CharSequenceUtil.format(ApiError.ERROR_QC_ALREADY_COMPLETED_REVERSE_FORBIDDEN.getMsg(),sb.toString()));
        }
    ...
    public BatchResultDTO delete(String id) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "删除");
        }
        // 只有待提交数据允许删除
    ...
    public BatchResultDTO cancelProcess(String id) {
        ...
        // 检查是否已作废
        if (Objects.equals(entity.getInvalidStatus(), InvalidStatusEnum.VOIDED.getStatus())) {
            throw new ServiceException(ApiError.ERROR_99200, "撤销");
        }
        // 只有审核中的单据允许撤销
        if (!Objects.equals(entity.getApproveStatus(), ApproveStatusEnum.APPROVE_ING)) {
...
//                    transferOutDetail.setOutWarehouseLocation("");
        ...
        for (String qcNoticeId : mainIdList) {
            if(noticeMap.containsKey(qcNoticeId)){
                ...
                if(CollUtil.isNotEmpty(waitList)){
                    ...
                    BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.ERROR_QC_NOT_COMPLETED_REVERSE_FORBIDDEN.getMsg(), qcNoticeEntity.getCode(),skuNos));
                    results.add(fail);
                    continue;
                }
                //调入调出仓库是同一个，则不存在调出单
                if(Objects.equals(qcNoticeEntity.getQcWarehouseId(),qcNoticeEntity.getPutawayWarehouseId())){
                    ...
                    if(!transferOutMap.containsKey(qcNoticeId)){
                        if(CollUtil.isNotEmpty(qcNoticeDetailList)){
                            ...
                            BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.ERROR_QC_NO_TRANSFER_OUT.getMsg(), qcNoticeEntity.getCode()));
                            results.add(fail);
                        }
                    }else {
                        for (TransferOutEntity transferOutEntity : transferOutMap.get(qcNoticeId)) {
                            if(transferOutEntity.getApproveStatus().equals(ApproveStatusEnum.APPROVE_ING.getCode())
                                ...
                                BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.ERROR_QC_TRANSFER_OUT_ALREADY_GENERATED.getMsg(), qcNoticeEntity.getCode(),transferOutEntity.getCode()));
                                results.add(fail);
                            }else {
    ...
    public List<BatchResultDTO> checkInventory(QcNoticeDTO.AddDTO dto) {
        ...
        for (QcNoticeDetailDTO.AddDTO detail : detailList) {
            ...
            Integer inventoryQty = skuInventoryMap.getOrDefault(skuId, 0);
            if (inventoryQty <= 0 || inventoryQty.intValue() < noticeQty.intValue()) {
                results.add(BatchResultDTO.fail(skuId, skuNo, CharSequenceUtil.format(ApiError.ERROR_QC_STOCK_INSUFFICIENT_CONTINUE_CONFIRM.getMsg(), noticeQty, inventoryQty)));
            }
        }
        return results;
    }
