...
public class QcNoticeServiceImpl extends SuperServiceImpl<QcNoticeMapper, QcNoticeEntity> implements QcNoticeService {
     ...
     */
    private void autoAssignQcUser(QcNoticeDTO.AddDTO addDTO) {
        ...
        for (QcNoticeDetailDTO.AddDTO detail : addDTO.getDetailList()) {
            ...
            } else if (QcTypeEnum.STOCK_OUT.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getStockoutQcUserId();
                qcUserName = cfgQcUser.getStockoutQcUserName();
            } else if (QcTypeEnum.OUTSIDE_QC.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getOutsideQcUserId();
                qcUserName = cfgQcUser.getOutsideQcUserName();
            } else if (QcTypeEnum.INSIDE_QC.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getInsideQcUserId();
                qcUserName = cfgQcUser.getInsideQcUserName();
            } else if (QcTypeEnum.NEW_PRODUCT_STOCK_IN.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getNewProductStockinQcUserId();
                qcUserName = cfgQcUser.getNewProductStockinQcUserName();
            } else if (QcTypeEnum.B2B_OUTSIDE_QC.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getB2bOutsideQcUserId();
                qcUserName = cfgQcUser.getB2bOutsideQcUserName();
            } else if (QcTypeEnum.RETURN_QC.getCode().equals(qcType)) {
                qcUserId = cfgQcUser.getReturnQcUserId();
                qcUserName = cfgQcUser.getReturnQcUserName();
            }
            
            // 如果配置了质检员，设置到明细中
            if (StrUtil.isNotBlank(qcUserId)) {
                detail.setQcUserId(qcUserId);
                detail.setQcUserName(qcUserName);
            }
        }
    }

    @Override
    public List<QcNoticeDTO.UpdateQcUserViewDTO> updateQcUserView(QcNoticeDTO.UpdateQcUserViewParamDTO dto) {
        ...
        List<QcInfoEntity> qcInfoList = qcInfoService.lambdaQuery()
                ...
                .list();
        Map<String, String> qcInfoCodeMap = CollUtil.isEmpty(qcInfoList)
                ? Collections.emptyMap()
                : qcInfoList.stream()
                        .filter(q -> StrUtil.isNotBlank(q.getSourceDetailId()))
                        .collect(Collectors.toMap(QcInfoEntity::getSourceDetailId,
                                q -> StrUtil.nullToEmpty(q.getCode()), (o1, o2) -> o1));

        Set<String> supplierIds = detailList.stream()
        ...
        for (QcNoticeDetailEntity detail : detailList) {
            ...
            QcNoticeDTO.UpdateQcUserViewDTO view = new QcNoticeDTO.UpdateQcUserViewDTO();
            view.setDetailId(detail.getId());
            view.setNoticeId(detail.getMainId());
            view.setSkuNo(detail.getSkuNo());
            view.setSupplierId(detail.getSupplierId());
            view.setQcUserId(detail.getQcUserId());
            view.setQcUserName(detail.getQcUserName());
            if (notice != null) {
                view.setNoticeCode(notice.getCode());
                view.setQcType(notice.getQcType());
                view.setQcTypeName(QcTypeEnum.getByCode(notice.getQcType()));
            }
            view.setQcInfoCode(qcInfoCodeMap.getOrDefault(detail.getId(), ""));
            SupplierDTO.SupplierSimpleDTO supplier = supplierMap.get(detail.getSupplierId());
            if (supplier != null) {
                view.setSupplierName(supplier.getName());
            }
            result.add(view);
        }
        return result;
    ...
    public List<QcNoticeDTO.QcInfoFullView> generateQcInfoFullView(QcNoticeDTO.QcNoticeParamDTO qcNoticeParamDTO) {
        List<QcNoticeDTO.QcInfoFullView> qcInfoViews;

        // 如果有勾选明细，则按明细ID查询
        if (CollUtil.isNotEmpty(qcNoticeParamDTO.getDetailIds())) {
            // 校验所选明细必须存在且同属一个审核通过的质检通知单
            List<QcNoticeDetailEntity> detailEntities = qcNoticeDetailService.listByIds(qcNoticeParamDTO.getDetailIds());
            if (CollUtil.isEmpty(detailEntities) || detailEntities.size() != qcNoticeParamDTO.getDetailIds().size()) {
                throw new ServiceException(ApiError.PO_QC_NOTICE_DETAIL_NOT_FOUND);
            }
            Set<String> mainIds = detailEntities.stream()
                    .map(QcNoticeDetailEntity::getMainId)
                    .filter(StrUtil::isNotBlank)
                    .collect(Collectors.toSet());
            if (mainIds.size() != 1) {
                throw new ServiceException(ApiError.QC_NOTICE_DETAILS_MUST_SAME_NOTICE);
            }
            QcNoticeEntity qcNotice = this.getById(mainIds.iterator().next());
            ...
            qcInfoViews = baseMapper.listQcInfoViewByDetailIds(qcNoticeParamDTO.getDetailIds());
            if (CollUtil.isEmpty(qcInfoViews)) {
                throw new ServiceException(ApiError.PO_QC_NOTICE_DETAIL_NOT_FOUND);
            }
        } else if (StrUtil.isNotBlank(qcNoticeParamDTO.getQcNoticeCode())) {
            // 如果有通知单号，则按通知单号查询
            ...
            qcInfoViews = baseMapper.listQcInfoViewByCode(qcNoticeParamDTO.getQcNoticeCode());

            if (qcInfoViews.isEmpty()) {
                throw new ServiceException(ApiError.PO_QC_NOTICE_FINISH, qcNoticeParamDTO.getQcNoticeCode());
            }
        } else {
            throw new ServiceException(ApiError.QC_NOTICE_PARAM_REQUIRED);
        }
        ...
        for (QcNoticeDTO.QcInfoFullView qcInfoView : qcInfoViews) {
            ...
            QcInfoEntity qcInfoEntity = qcInfos.stream()
                    .filter(item -> Objects.equals(qcInfoView.getDetailId(), item.getSourceDetailId()))
                    .findFirst()
                    .orElse(null);
            if (Objects.nonNull(qcInfoEntity)) {
                qcInfoView.setQcBillId(qcInfoEntity.getId());
    ...
    public void generateFullQcInfo(List<QcNoticeDTO.QcInfoFullView> dto) {
        ...
        for (QcNoticeDetailEntity detail : noticeDetailList) {
            QcNoticeDetailEntity snapshot = new QcNoticeDetailEntity();
            BeanUtils.copyProperties(detail, snapshot);
            oldDetailSnapshotMap.put(detail.getId(), snapshot);
        }

        //不参与本次质检的明细
        List<QcNoticeDetailEntity> leftDetailList = qcNoticeDetailService.lambdaQuery()
                .in(QcNoticeDetailEntity::getMainId,qcNoticeIdList)
        ...
        // 批量更新集合
        List<String> standardRefBatchRemoveMainIds = new ArrayList<>();
        ...
        Map<String, List<AttachDTO>> attachmentBatchSaveMap = new HashMap<>(); // key: defectId, value: attachments
        List<QcInfoEntity> qcInfoBatchUpdateList = new ArrayList<>();
        List<QcResultEntity> qcResultBatchUpdateList = new ArrayList<>();
        List<QcInfoDTO.BatchQcAccumulationParam> accumulateParams = new ArrayList<>();
        ...
        for (QcNoticeDTO.QcInfoFullView qcInfoView : dto) {
            ...
            if(qcResultView.getQcGoodQty().equals(0)
                    && qcResultView.getQcBadQty().equals(0)){
            ...
            if (CollUtil.isNotEmpty(qcInfoView.getDefectViewList())) {
                ...
                for (QcNoticeDTO.DefectView defectView : defectViewList) {
                    ...
                    QcDefectEntity existing = existDefectMap.get(qcInfoView.getQcBillId() + "_" + defectView.getDefectLevl());

                    if (existing != null) {
                        String id = existing.getId();
                        BeanUtils.copyProperties(defectView, existing);
                        existing.setDefectLevel(defectView.getDefectLevl());
                        ...
                        if (Objects.nonNull(defectView.getBadImageViewList()) && !defectView.getBadImageViewList().isEmpty()) {
                            ...
                            attachmentIdBatchRemoveList.add(id);
                        }
                    } else {
                        // 新增操作
            ...
            //质检单
            QcInfoEntity qcInfo = qcInfoEntityMap.get(qcInfoView.getQcBillId());
            if (qcInfo != null) {
                qcInfo.setQcStatus(QcBillStatusEnum.FINISH_QC);
                qcInfo.setQcFinishTime(nowTime);
                qcInfoBatchUpdateList.add(qcInfo);
            }
            ...
            QcNoticeDetailEntity snapshot = oldDetailSnapshotMap.get(qcInfoView.getDetailId());
            operateLogService.addModuleOperateLogByObj(snapshot, qcNoticeDetailEntity, ModuleTypeEnum.QC_NOTICE.getCode(), qcNoticeDetailEntity.getMainId(), "", StrUtil.format("【{}】", qcNoticeDetailEntity.getSkuNo()));
        ...
        // ====== 统一执行批量持久化 ======
        if (CollUtil.isNotEmpty(standardRefBatchRemoveMainIds)) {
            qcStandardRefService.remove(new QueryWrapper<QcStandardRefEntity>().in("main_id", standardRefBatchRemoveMainIds));
        }
...
//                    transferOutDetail.setOutWarehouseLocation("");
    ...
    public void generateQcInfo(List<QcNoticeDTO.QcInfoView> dto) {
        ...
        //质检通知单审核通过更新质检单
        for (QcNoticeDTO.QcInfoView qcInfoView : dto) {
            if(qcInfoView.getQcGoodQty() == null || qcInfoView.getQcBadQty() == null) {
                continue;
            }
            ...
            QcResultEntity qcResult = qcResultMap.get(qcInfoView.getQcBillId());
            if (qcResult != null) {
                qcResult.setQcQty(qcInfoView.getQcQty());
                qcResult.setQcResult(qcInfoView.getQcResult());
                qcResult.setLotQualifiedQty(qcInfoView.getLotQualifiedQty());
...
//                    transferOutDetail.setOutWarehouseLocation("");
        ...
        for (String qcNoticeId : mainIdList) {
            if(noticeMap.containsKey(qcNoticeId)){
                ...
                if(CollUtil.isNotEmpty(waitList)){
                    ...
                    BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.PO_QC_NOT_COMPLETED_REVERSE_FORBIDDEN.getMsg(), qcNoticeEntity.getCode(),skuNos));
                    results.add(fail);
                    continue;
                }
                //调入调出仓库是同一个，则不存在调出单
                if(Objects.equals(qcNoticeEntity.getQcWarehouseId(),qcNoticeEntity.getPutawayWarehouseId())){
                    ...
                    if(!transferOutMap.containsKey(qcNoticeId)){
                        if(CollUtil.isNotEmpty(qcNoticeDetailList)){
                            ...
                            BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.PO_QC_NO_TRANSFER_OUT.getMsg(), qcNoticeEntity.getCode()));
                            results.add(fail);
                        }
                    }else {
                        for (TransferOutEntity transferOutEntity : transferOutMap.get(qcNoticeId)) {
                            if(transferOutEntity.getApproveStatus().equals(ApproveStatusEnum.APPROVE_ING.getCode())
                                ...
                                BatchResultDTO fail = BatchResultDTO.fail(qcNoticeEntity.getId(), qcNoticeEntity.getCode(), CharSequenceUtil.format(ApiError.PO_QC_TRANSFER_OUT_ALREADY_GENERATED.getMsg(), qcNoticeEntity.getCode(),transferOutEntity.getCode()));
                                results.add(fail);
                            }else {
    ...
    public List<QcNoticeDTO.PrintQcStandardDTO> printQcStandard(QcNoticeDTO.PrintQcStandardParamDTO paramDTO) {
        ...
        List<QcNoticeEntity> qcNotices = this.listByIds(ids);
        List<String> qcInfoIds = qcNotices.stream()
                .map(item -> item.getId())
                .collect(Collectors.toList());
        List<QcInfoEntity> qcInfos = qcInfoService.listQCBySourceIds(qcInfoIds);
        for (QcNoticeDetailEntity qcNoticeDetail : qcNoticeDetails) {
            QcNoticeDTO.PrintQcStandardDTO printQcStandardDTO = new QcNoticeDTO.PrintQcStandardDTO();
            QcNoticeEntity qcNoticeEntity = qcNotices.stream()
                    .filter(item -> Objects.equals(item.getId(), qcNoticeDetail.getMainId()))
                    .findFirst()
                    .orElse(null);
            if (Objects.nonNull(qcNoticeEntity)) {
                //质检通知单号
                printQcStandardDTO.setCode(qcNoticeEntity.getCode());

                QcInfoEntity qcInfoEntity = qcInfos.stream()
                        .filter(item -> Objects.equals(item.getSourceId(), qcNoticeEntity.getId()))
                        .findFirst()
                        .orElse(null);
                //质检单号
                if (Objects.nonNull(qcInfoEntity)) {
                    printQcStandardDTO.setQcInfoCode(qcInfoEntity.getCode());
                }

                //抽样信息
                SamplingPlanDTO.PlanParamDTO planParamDTO = new SamplingPlanDTO.PlanParamDTO();
                planParamDTO.setQcType(qcNoticeEntity.getQcType());
                planParamDTO.setQty(qcNoticeDetail.getQcNoticeQty());
                planParamDTO.setSkuId(qcNoticeDetail.getSkuId());
