...
@Slf4j
@Component
@PlatformShipOrderAnno(method = PlatformDictEnum.EBAY)
public class EBayShipOrder extends AbstractShipOrder {
    ...
    public List<String> shipOrder(PlatformShipOrderDTO dto) {
        ...
        for (SoB2cEntity mainEntity : sourceOrderList) {
            ...
            if(StringUtils.isBlank(mainEntity.getThirdCode())){
                throw new ServiceException("【EBay标记发货】操作失败，订单号【{}】领星单号为空",mainEntity.getThirdCode());
            }
            // 校验捆绑商品拆分
            ...
            try {
                ...
                throw new ServiceException("EBay标记发货失败:" + e.getMessage());
            }
        }
