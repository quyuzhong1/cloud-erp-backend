...
//        });
    ...
    private void handleImportSoReturnstockUpdateFile(List<SoReturnStockUpdateImportExcelDTO> allList,
        ...
        List<String> codeList = successList.stream().map(SoReturnStockUpdateImportExcelDTO::getCode).distinct().collect(Collectors.toList());
        Map<String, SoReturnInstockEntity> entityMap = lambdaQuery().in(SoReturnInstockEntity::getCode, codeList).list().stream()
                .collect(Collectors.toMap(SoReturnInstockEntity::getCode, Function.identity(), (a, b) -> a));

        List<String> customerNameList = successList.stream().map(SoReturnStockUpdateImportExcelDTO::getCustomerName).distinct().collect(Collectors.toList());
    ...
    private void handleImportSoReturnstockFile ( List<SoReturnStockImportExcelDTO> successList,List<SoReturnStockImportExcelDTO> errorList) {
        ...
        for (Map.Entry<String, List<SoReturnStockImportExcelDTO>> entry : map.entrySet()) {
            ...
            SoReturnInstockDTO.ImportAddBundle bundle = new SoReturnInstockDTO.ImportAddBundle(add, new ArrayList<>(value),
                    ...
                    warehouseEntity);
            try {
                enrichImportAddBundles(Collections.singletonList(bundle));
                enrichImportAddDetailContext(Collections.singletonList(bundle), warehouseNameById);
                ApplicationContextUtils.getBean(SoReturnInstockServiceImpl.class)
                        .persistAllImportAdd(Collections.singletonList(bundle));
                updateImportAddSkuOccupyStatus(Collections.singletonList(bundle));
            } catch (Exception e) {
