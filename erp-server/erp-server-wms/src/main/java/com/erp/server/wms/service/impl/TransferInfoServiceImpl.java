...
public class TransferInfoServiceImpl extends SuperServiceImpl<TransferInfoMapper, TransferInfoEntity> implements TransferInfoService {
    ...
    public Boolean update(TransferInfoDTO.UpdateDTO dto) {
        ...
        if (ThirdPartySystemEnum.ENUM_MB.getCode().equals(dto.getCode())) {
            throw new ServiceException(ApiError.WH_TRANSFER_MB_UPDATE_NOT_ALLOWED);
        }
    ...
    private void validateTransferDetail(TransferInfoEntity entity, List<TransferInfoDetailEntity> detailList) {
        ...
        for (TransferInfoDetailEntity detailEntity : detailList) {
            ...
            if (detailEntity.getInWarehouseId().equals(detailEntity.getOutWarehouseId())
                ...
                throw new ServiceException(new ApiResult(ApiError.WH_TRANSFER_IN_OUT_WAREHOUSE_MUST_DIFFER.getCode(),
                        String.format(ApiError.WH_TRANSFER_IN_OUT_WAREHOUSE_MUST_DIFFER.getMsg(), entity.getCode())));
            }
        }
    }
    ...
    public Boolean delete(List<String> ids) {
        ...
        if (CharSequenceUtil.isNotBlank(codes)) {
            throw new ServiceException(ApiError.WH_TRANSFER_MB_UPDATE_NOT_ALLOWED,codes);
        }
...
//        }
        ...
        for (TransferInfoEntity entity : list) {
            ...
            if (ThirdPartySystemEnum.ENUM_MB.getCode().equals(entity.getCode())){
                resultDTOList.add(BatchResultDTO.fail(entity.getId(), entity.getCode(), ApiError.WH_TRANSFER_MB_UPDATE_NOT_ALLOWED.getMsg()));
                continue;
            }
    ...
    private void updateFirstMileDeliveryInventory (List<TransferInfoEntity> pushFirstMileDeliveryList,List<TransferInfoDetailEntity> detailList) {
        ...
        //来源是awd出库货件不操作虚拟仓库存
        if (awdOutstockService.listByIds(requisitionIds).isEmpty()) {
    ...
    private void updateInventoryTransCore (List<TransferInfoEntity> list, List<TransferInfoDetailEntity> detailList) {
        ...
        for (TransferInfoDetailEntity detailEntity : detailList) {
            ...
            transferDTO.setCurVirtualWarehouseId(detailEntity.getVirtualWarehouseId());
        ...
        if(CollectionUtils.isNotEmpty(firstMileTransferList)){
            ...
            TransferInfoEntity transferInfoEntity = this.getById(firstMileTransferList.get(0).getSourceId());
            FirstMileDeliveryEntity firstMileDeliveryEntity = firstMileDeliveryService.getById(transferInfoEntity.getSourceId());
            AwdOutstockEntity awdOutstockEntity = awdOutstockService.getById(firstMileDeliveryEntity.getSourceId());
            if (Objects.nonNull(awdOutstockEntity)) {
                inventoryTransferDTO.setBusinessType(InventoryBusinessTypeEnum.AWD_FIRST_MILE_PUSH_TRANSFER.getCode());
            }else {
    ...
    public String generateFromOverseasInbound(OverseasWarehouseInboundEntity mainEntity, List<OverseasWarehouseInboundDetailEntity> detailList, List<OverseasWarehouseInboundReceivedEntity> receivedEntityList, String remark,Boolean isToOnwayWarehouse) {
        ...
        for(OverseasWarehouseInboundReceivedEntity receivedEntity : receivedEntityList){
            ...
            // 调入端仅 wego / 极风 / 极兔 / 爱亚 等支持不良品标记的平台正向签收(在途仓→目的仓)且不良品标记时落不良品，其余默认可用。
            ...
            boolean supportDefectiveFlag = OmsPlatformEnum.WE_GO.getCode().equalsIgnoreCase(dictPlatform)
                    ...
                    || OmsPlatformEnum.AI_YA.getCode().equalsIgnoreCase(dictPlatform);
