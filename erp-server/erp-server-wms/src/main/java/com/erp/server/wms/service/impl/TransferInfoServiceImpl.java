...
public class TransferInfoServiceImpl extends SuperServiceImpl<TransferInfoMapper, TransferInfoEntity> implements TransferInfoService {
    ...
    public BatchResultDTO approve(TransferInfoEntity entity, String type, String comment, Boolean isNeedProcess, Boolean isSyncKingDee){
        ...
        //操作日志
        operateLogService.addModuleOperateLog(String.format("审核【%s】了一个直接调拨单", ApproveTypeEnum.getName(type)).concat("【%s】").concat(StringUtils.isNotBlank(comment) ? String.format(",意见：%s", comment) : ""), ModuleTypeEnum.TRANSFER_INFO.getCode(), entity.getId(), "审核操作");
    ...
    @Transactional(rollbackFor = Exception.class)
    @GlobalTransactional(rollbackFor = Exception.class, timeoutMills = 180000)
    public BatchResultDTO disApprove(TransferInfoEntity entity, Boolean isPushKingDee) {
        //已审核允许反审核
        ...
        //操作日志
        operateLogService.addModuleOperateLog("反审核了一个直接调拨单【%s】", ModuleTypeEnum.TRANSFER_INFO.getCode(), entity.getId(), "反审核操作");
        return BatchResultDTO.success(entity.getId(), entity.getCode(), "操作成功");
    }
    ...
    public void requisitionApplicationCancelProcess(String code, String sourceType) {
        ...
        for (TransferInfoEntity entity : list) {
            ...
            if (ApproveStatusEnum.APPROVE.getStatus().equals(entity.getApproveStatus())) {
                try {
                    this.disApprove(entity, Boolean.TRUE);
                } catch (Exception e) {
    ...
    public Boolean checkHistoryAndDel(String sourceCode, String sourceType, LocalDate billDate) {
        ...
        if (CollectionUtils.isNotEmpty(approveList)){
            approveList.forEach(transferInfoEntity -> {
                // 反审核
                disApprove(transferInfoEntity, true);
            });
        }
