                ...
                if (ObjectUtil.isNotEmpty(purchaseOrderDetailEntity)) {
                    ...
                    if (dto.getSourceType().equals(SourceTypeEnum.PO_RECEIVE.getCode())) {
                        ...
                        if (addDTO.getReturnQty() > inventoryTotal) {
                            throw new ServiceException(ApiError.ERROR_WMS_RETURN_QTY_EXCEEDS_PENDING_QC, JSONUtil.toJsonStr(purchaseOrderDetailEntity.getSkuNo()));
                        }
                    } else {
                        ...
                        if (addDTO.getReturnQty() > stockInQty) {
                            throw new ServiceException(ApiError.ERROR_WMS_RETURN_QTY_EXCEEDS_INBOUND, purchaseOrderDetailEntity.getSkuNo());
                        }

                        if (addDTO.getReturnQty() + returnQty > stockInQty) {
                            throw new ServiceException(ApiError.ERROR_WMS_RETURN_TOTAL_QTY_EXCEEDS_INBOUND, purchaseOrderDetailEntity.getSkuNo());
                        }
                    }
