...
public class WarehouseLocationReplenishServiceImpl extends SuperServiceImpl<WarehouseLocationReplenishMapper, WarehouseLocationReplenishEntity> implements WarehouseLocationReplenishService {
    ...
    public List<BatchResultDTO> addList(List<WarehouseLocationReplenishDTO.AddDTO> addList) {
    ...
    public BatchResultDTO add(WarehouseLocationReplenishDTO.AddDTO dto) {
        ...
        if(dto.getSourceType().equals(ReplenishTypeEnum.DELIVER_STOCK_OUT)){
            ...
            if(pickAreaIds.isEmpty()){
                throw new ServiceException(ApiError.WH_REPLENISH_AREA_NOT_FOUND, dto.getSkuNo());
            }
            ...
            if(pickLocationCodeList.isEmpty()){
                throw new ServiceException(ApiError.WH_REPLENISH_LOCATION_NOT_FOUND, dto.getSkuNo());
            }
        ...
        if(dto.getSourceType().equals(ReplenishTypeEnum.SAFETY_INVENTORY)){
            ...
            this.save(entity);
            //记录日志
            String msg = StrUtil.format("用户【{}】新增【{}】单据SKU为【{}】", UserContext.getDefaultLoginUser().getUserName(), "仓位库存预警" , entity.getSkuNo());
            operateLogService.addModuleOperateLog(msg, ModuleTypeEnum.WAREHOUSE_LOCATION_REPLENISH.getCode(), entity.getId(), "新增操作");
        }
    ...
    private InventoryEntity findLastInventory(WarehouseLocationReplenishDTO.AddDTO dto , List<String> pickLocationCodeList , List<InventoryEntity> pickInventoryList){
        ...
        if(null != transactionFlow){
            String warehouseLocation = transactionFlow.getWarehouseLocation();
