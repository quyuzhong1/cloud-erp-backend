...
public class SampleInitialLedgerDetailController extends BaseController {
    ...
    */
     ...
     * @date: 2025-09-01
     */
    @LogAction(value = LogActionEnum.EXPORT, desc = "下载样品期初台账明细导入模板")
    @GetMapping("/downloadTemplate")
    public ApiResult<Object> downloadTemplate(HttpServletRequest request, HttpServletResponse response) {
        String standardPath = "classpath:excel/sampleInitialLedgerDetailTemplate.xlsx";
        String standardExcelName = "sampleInitialLedgerDetailTemplate.xlsx";
    ...
    @LogAction(value = LogActionEnum.IMPORT, desc = "样品期初台账明细导入")
    public ApiResult<SampleInitialLedgerDetailDTO.ImportDTO> importFile(@RequestParam(value = "excelFile") MultipartFile excelFile, HttpServletResponse response) {
        return success(sampleInitialLedgerDetailService.importFile(excelFile, response));
    }
