...
public class SoOutstockServiceImpl extends SuperServiceImpl<SoOutstockMapper, SoOutstockEntity> implements SoOutstockService {
    ...
    public String add(SoOutstockDTO.AddDTO dto) {
        ...
        //添加成功
        if (addResult) {
            soOutstockDetailService.add(id, addDetailList, b2b);
            //添加日志
    ...
    public Boolean disApprove(BaseIdsDTO.IdsDTO dto, Boolean isPushKingDee) {
        ...
        String b2bType=OrderTypeEnum.B2B.getCode();
        //反审核
        if (result) {
            //反审核
            ...
            if (isPushKingDee) {
                //B2B 反审核发送金蝶
                list.stream().filter(l->b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_DISAPPROVE.getCode()));
                //B2C 反审核发送金蝶
                list.stream().filter(l->!b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncB2cDataToKingdee(obj, SyncOperateEnum.OPERATE_DISAPPROVE.getCode()));
    ...
    public Boolean delete(List<String> ids) {
        ...
        if (result) {
            ...
            //B2B 删除发送金蝶
            list.stream().filter(l->b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_DELETE.getCode()));
            //B2C 删除发送金蝶
            list.stream().filter(l->!b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncB2cDataToKingdee(obj, SyncOperateEnum.OPERATE_DELETE.getCode()));
    ...
    public Boolean invalid(List<String> ids, String remark) {
        ...
        String b2bType=OrderTypeEnum.B2B.getCode();
        //B2B 作废发送金蝶
        list.stream().filter(l->b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_INVALID.getCode()));
        //B2C作废发送金蝶
        list.stream().filter(l->!b2bType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncB2cDataToKingdee(obj, SyncOperateEnum.OPERATE_INVALID.getCode()));
     ...
     * @param list
     */
    private void fillPaging(List<SoOutstockDTO.PagingViewDTO> list) {
        ...
        for (SoOutstockDTO.PagingViewDTO item : list) {
            ...
            if (isB2b) {
                ...
                SoB2cDTO.CustomerDTO b2cCustomer = b2cCustomerList.stream().filter(s -> s.getSoId().equals(soId)).findFirst().orElse(new SoB2cDTO.CustomerDTO());
