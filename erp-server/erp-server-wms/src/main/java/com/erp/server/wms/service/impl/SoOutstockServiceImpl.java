...
public class SoOutstockServiceImpl extends SuperServiceImpl<SoOutstockMapper, SoOutstockEntity> implements SoOutstockService {
    ...
    public String add(SoOutstockDTO.AddDTO dto) {
        ...
        //添加成功
        if (addResult) {
            soOutstockDetailService.add(id, addDetailList, soInfo.getOrderType());
            //添加日志
    ...
    public Boolean disApprove(BaseIdsDTO.IdsDTO dto, Boolean isPushKingDee) {
        ...
        if (result) {
            ...
            if (isPushKingDee) {
                ...
                //B2C 反审核发送金蝶
                //  haveSoIdList.stream().filter(l->b2cType.equals(l.getOrderType())).forEach(obj -> syncKingdeeSoOutstockService.syncB2cDataToKingdee(obj, SyncOperateEnum.OPERATE_DISAPPROVE.getCode()));
