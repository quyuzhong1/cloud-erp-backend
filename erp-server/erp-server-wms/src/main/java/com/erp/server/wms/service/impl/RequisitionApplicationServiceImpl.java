...
public class RequisitionApplicationServiceImpl extends SuperServiceImpl<RequisitionApplicationMapper, RequisitionApplicationEntity> implements RequisitionApplicationService {
    ...
    */
    ...
    public BatchResultDTO submit(String id) {
        ...
        String msg = CharSequenceUtil.format("用户【{}】单号为【{}】的【{}】单据提交审核 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "要货申请");
    ...
    public List<RequisitionApplicationDTO.HandleListDTO> handleList(List<String> ids) {
        ...
        for (RequisitionApplicationDTO.HandleListDTO handleListDTO : list) {
            ...
            //如果没有仓位用空仓位
            handleListDTO.setFromWarehouseLocation(CharSequenceUtil.isBlank(handleListDTO.getFromWarehouseLocation()) ? "" : handleListDTO.getFromWarehouseLocation());
            handleListDTO.setToWarehouseLocation(CharSequenceUtil.isBlank(handleListDTO.getToWarehouseLocation()) ? "" : handleListDTO.getToWarehouseLocation());
        }
        return list;
    }
    ...
    public Boolean handleSave(List<RequisitionApplicationDTO.HandleListDTO> list) {
        ...
        for (RequisitionApplicationDTO.HandleListDTO handleListDTO : list) {
            ...
            //虚拟仓数量
            if (CharSequenceUtil.isNotBlank(handleListDTO.getFromVirtualWarehouseId())) {
        ...
        for (RequisitionApplicationEntity entity : requisitionApplicationEntities) {
            String msg = CharSequenceUtil.format("用户【{}】处理了一个单号为【{}】的【{}】单", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "要货申请");
...
//                fromThirdMappingList.forEach(thirdMapping -> {
    ...
    public List<RequisitionApplicationDTO.FinishListDTO> finishList(List<String> ids) {
        ...
        for (RequisitionApplicationDTO.FinishListDTO finishListDTO : list) {
            if (!RequisitionApplicationStatusEnum.HANDLE_ING.getStatus().equals(finishListDTO.getStatus())) {
                throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_HANDLE_ING_FINISH_ONLY, finishListDTO.getSourceCode());
            }
    ...
    public Boolean finishSave(List<RequisitionApplicationDTO.FinishListDTO> list) {
        ...
        if (countQty <= 0) {
            throw new ServiceException(ApiError.SO_PICKLIST_TOTAL_QTY_ZERO_FORBIDDEN);
        }
        List<String> raIds = list.stream().map(req -> req.getSourceId()).distinct().collect(Collectors.toList());
        //根据调出调入仓id查询仓库信息
        List<String> toWarehouseIds = list.stream().map(req -> req.getToWarehouseId()).collect(Collectors.toList());
        ...
        List<RequisitionApplicationEntity> requisitionApplicationEntities = this.listByIds(raIds);
        for (RequisitionApplicationEntity entity : requisitionApplicationEntities) {
            String msg = CharSequenceUtil.format("用户【{}】完成了一个单号为【{}】的【{}】单", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "要货申请");
    ...
    public List<RequisitionApplicationDTO.printPickingViewDTO> printPickingView(List<String> ids) {
        ...
        long count = list.stream()
                ...
                .count();
        if (count > 0) {
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_HANDLE_PRINT_PICKING_ALLOWED);
        }
        ...
        for (RequisitionApplicationDetailEntity requisitionApplicationDetailEntity : requisitionApplicationDetailEntities) {
            ...
            if(mainEntity.getType().equals(RequisitionApplicationTypeEnum.THIRD_WAREHOUSE.getCode())){
                List<OverseasProviderWarehouseDTO.ViewDTO> viewDTOList = overseasProviderWarehouseService.listByWarehouseIdList(Collections.singletonList(mainEntity.getChannelId()));
                if(CollectionUtils.isNotEmpty(viewDTOList)){
                    String provideCode = viewDTOList.get(0).getProviderCode();
                    if(CharSequenceUtil.isNotBlank(provideCode)){
            ...
            if (CollectionUtils.isNotEmpty(sonSkuList)) {
                for (BomChildrenSkuDTO bomChildrenSkuDTO : sonSkuList) {
                    ...
                    //修改仓位设置为 推荐仓位(大货区)
                    viewDTO.setWarehouseLocation(CharSequenceUtil.isBlank(skuVO.getWarehouseLocationLarge()) ? "" : skuVO.getWarehouseLocationLarge());
...
/*        // 只有待处理的单据允许撤销
        ...
        if (Objects.equals(entity.getStatus(), RequisitionApplicationStatusEnum.WAIT_HANDLE.getStatus())) {
            ...
            throw new ServiceException(ApiError.BILL_WAIT_HANDLE_CANCEL_PROCESS_ALLOWED);
        }
        log.info("撤销 开始修改要货申请状态，id：【{}】", id);

        //操作日志
        log.info("撤销 开始记录操作日志，id：【{}】", id);
        String msg = CharSequenceUtil.format("用户【{}】单号为【{}】的【{}】单据撤销流程操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "头程发货单");
...
revokeDTO.setSourcePlatform(dto.getSourcePlatform());
    ...
    private List<VirtualInventoryStockDTO.OutInStockDTO> getOutInStockDTOS(RequisitionApplicationEntity
        ...
        if (CollectionUtils.isNotEmpty(detailEntityList)) {
            detailEntityList.forEach(detailEntity -> {
                if (CharSequenceUtil.isNotBlank(detailEntity.getFromVirtualWarehouseId()) && MathUtil.compareTo(detailEntity.getVirtualFrozenQty(),MathUtil.ZERO) > MathUtil.ZERO) {                    RequisitionApplicationDTO.HandleListDTO handleListDTO = new RequisitionApplicationDTO.HandleListDTO();
    ...
    public BatchResultDTO delete(String id) {
        ...
        pickingListsService.exist(id);
        // 删除明细数据
        requisitionApplicationDetailService.removeByMainIds(Collections.singletonList(id));
        // 删除主单数据
        log.info("删除 开始删除要货申请单主单数据，id：【{}】", id);
        super.removeById(id);
        // 删除日志数据
        log.info("删除 开始删除要货申请单日志数据，id：【{}】", id);
        String msg = CharSequenceUtil.format("用户【{}】单号为【{}】的【{}】单据删除操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "要货申请单");
    ...
    public List<RequisitionApplicationDTO.ChildViewDTO> listChildBySku(RequisitionApplicationDTO.ChildParamDTO dto) {
        ...
        //查询历史子件信息
        List<BomChildrenSkuDTO> bomChildrenSkuDTOS = plmTaskFeign.listHistoryBomChildBySkuIds(Collections.singletonList(dto.getSkuId()));
        //查询最新版本的sku子件信息
    ...
    public List<BatchResultDTO> bindShipment(List<RequisitionApplicationDTO.BindShipment> dto) {
        ...
        for (RequisitionApplicationDTO.BindShipment bindShipment : dto) {
            ...
            if(CollectionUtils.isEmpty(firstMileDeliveryDetailList) || (CharSequenceUtil.isNotBlank(firstMileDeliveryDetailList.get(0).getFbaShipmentCode()) && CharSequenceUtil.isBlank(requisitionApplicationEntity.getFbaShipmentCode()))){
            ...
            String msg = CharSequenceUtil.format("用户【{}】绑定单号为【{}】货件号为【{}】 ", UserContext.getDefaultLoginUser().getUserName(), requisitionApplicationEntity.getCode(), fbaShipmentEntity.getCode());
    ...
    public void writeBackData(List<String> sourceDetailIds,Boolean isCheck) {
        ...
        if (CharSequenceUtil.isNotBlank(codes) && isCheck) {
    ...
    public BatchResultDTO handleData(String id,Boolean isFlag) {
        ...
        List<RequisitionApplicationDetailEntity> detailList = requisitionApplicationDetailService.listByMainIds(Collections.singletonList(id));
    ...
    public List<RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO> fbaBindShipmentView(String id) {
        ...
        List<WmsCartonEntity> cartonEntityList = Optional.ofNullable(cartonService.listByTaskIds(Collections.singletonList(packingTaskEntity.getId()))).filter(list -> !list.isEmpty())
    ...
    public RequisitionApplicationDTO.FbaBindShipmentViewDTO fbaBindShipmentMatching(RequisitionApplicationDTO.FbaBindShipmentMatchingDTO dto) {
        ...
        waitMatchList = waitMatchList.stream().filter(v->CharSequenceUtil.isBlank(v.getFbaShipmentId())).collect(Collectors.toList());
        ...
        if(fbaShipmentPackingEntityList.stream().anyMatch(v->CharSequenceUtil.isNotBlank(v.getCartonId()))){
            throw new ServiceException("{}货件装箱已绑定，无法重复绑定",fbaShipmentEntity.getCode());
        }
        //过滤掉混装和单装分开的
        List<FbaShipmentPackingEntity> mixedSkuList = fbaShipmentPackingEntityList.stream().filter(v->CharSequenceUtil.isBlank(v.getMsku())).collect(Collectors.toList());
        fbaShipmentPackingEntityList = fbaShipmentPackingEntityList.stream().filter(v->CharSequenceUtil.isNotBlank(v.getMsku())).collect(Collectors.toList());
        //箱号分组，组成sku*qty 匹配
    ...
    private void generateDeliveryWithFbt(RequisitionApplicationEntity entity, List<RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO> detailList) {
        if (!ThirdDeliveryTypeEnum.SELF_TO_THIRD.getCode().equals(entity.getDeliveryType())) {
            throw new ServiceException("当前FBT要货单发货类型不支持按装箱绑定货件下推");
        }
        List<RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO> releaseList = detailList.stream()
                .filter(e -> Objects.nonNull(e.getIsReleaseInventory()) && e.getIsReleaseInventory())
                .collect(Collectors.toList());
        if (CollUtil.isNotEmpty(releaseList)) {
            throw new ServiceException("存在已释放库存的装箱，请重新选择");
        }
        detailList = detailList.stream()
                .filter(v -> CharSequenceUtil.isBlank(v.getDeliveryCode()) && CharSequenceUtil.isNotBlank(v.getFbaShipmentId()))
                .collect(Collectors.toList());
        if (CollectionUtils.isEmpty(detailList)) {
            throw new ServiceException("请先绑定货件再下推");
        }
        ...
        if (detailList.stream().anyMatch(v -> !v.getFbaBoxNo().matches("^[A-Za-z0-9]+$"))) {
            throw new ServiceException("货件箱号只允许输入英文字母、数字");
        }
        List<String> cartonIds = detailList.stream().map(RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO::getCartonId).distinct().collect(Collectors.toList());
        List<FbaShipmentPackingEntity> existsPackingList = fbaShipmentPackingService.listByCartonIds(cartonIds);
        if (CollUtil.isNotEmpty(existsPackingList)) {
            throw new ServiceException("装箱已绑定货件，不允许重复下推");
        }
        ...
        List<String> shipmentIdList = detailList.stream().map(RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO::getFbaShipmentId).distinct().collect(Collectors.toList());
        ...
        for (RequisitionApplicationDTO.FbaBindShipmentViewDetailDTO bindDTO : detailList) {
            FbaShipmentEntity shipmentEntity = shipmentMap.get(bindDTO.getFbaShipmentId());
        ...
        List<RequisitionApplicationDTO.GenerateDeliverViewDTO> deliveryViewList = generateDeliverView(Collections.singletonList(entity.getId()));
        if (CollUtil.isEmpty(deliveryViewList)) {
            throw new ServiceException("未找到可下推的发货明细");
        }
        List<RequisitionApplicationDetailEntity> requisitionDetailList = requisitionApplicationDetailService.listByMainIds(Collections.singletonList(entity.getId()));
        Map<String, RequisitionApplicationDetailEntity> requisitionDetailMap = requisitionDetailList.stream().collect(Collectors.toMap(RequisitionApplicationDetailEntity::getId, v -> v));
        List<WmsCartonDetailEntity> cartonDetailEntityList = wmsCartonDetailService.listByMainIds(cartonIds);
        List<String> skuIdList = deliveryViewList.stream().map(RequisitionApplicationDTO.GenerateDeliverViewDTO::getSkuId).distinct().collect(Collectors.toList());
        List<SkuVO> skuVOList = plmTaskFeign.listSkuPackByIds(skuIdList);
        ...
        for (FbaShipmentEntity shipmentEntity : shipmentEntityList) {
            List<String> shipmentCartonIds = detailList.stream()
                    ...
                    .distinct()
                    .collect(Collectors.toList());
            List<WmsCartonDetailEntity> currentCartonDetails = cartonDetailEntityList.stream()
                    .filter(v -> shipmentCartonIds.contains(v.getMainId()))
                    .collect(Collectors.toList());
            if (CollUtil.isEmpty(currentCartonDetails)) {
                throw new ServiceException("货件{}未关联装箱明细，无法下推发货单", shipmentEntity.getCode());
            }

            RequisitionApplicationDTO.GenerateDeliverViewDTO headView = deliveryViewList.get(0);
            FirstMileDeliveryDTO.AddDTO addDTO = RequisitionApplicationConverter.INSTANCE.generateDeliverFDD(headView);
            ...
            List<FirstMileDeliveryDetailDTO.AddDTO> detailAddList = new ArrayList<>();
            for (RequisitionApplicationDTO.GenerateDeliverViewDTO viewDTO : deliveryViewList) {
                Integer deliveryQty = currentCartonDetails.stream()
                        .filter(cartonDetail -> CharSequenceUtil.equals(cartonDetail.getSkuId(), viewDTO.getSkuId()))
                        .filter(cartonDetail -> CharSequenceUtil.isBlank(viewDTO.getFnSku())
                                || CharSequenceUtil.equals(cartonDetail.getFnSku(), viewDTO.getFnSku())
                                || CharSequenceUtil.equals(cartonDetail.getFnSku(), viewDTO.getPlatformSku()))
                        .map(WmsCartonDetailEntity::getPackQty)
                ...
                List<BomChildrenSkuDTO> sonSkuList = bomChildrenSkuDTOS.stream().filter(req -> req.getParentSkuId().equals(viewDTO.getSkuId())).collect(Collectors.toList());
                detailAddDto.setIsCombination(CollectionUtils.isNotEmpty(sonSkuList));
                SkuVO skuVO = skuVOList.stream().filter(req -> req.getSkuId().equals(viewDTO.getSkuId())).findFirst().orElse(new SkuVO());
                if (ObjectUtil.isNotEmpty(skuVO)) {
                ...
                RequisitionApplicationDetailEntity requisitionDetailEntity = requisitionDetailMap.get(viewDTO.getSourceDetailId());
                if (Objects.nonNull(requisitionDetailEntity) && CharSequenceUtil.equals(viewDTO.getDeliveryWarehouseId(), requisitionDetailEntity.getToWarehouseId())) {
                    CfgRulePickingStagingEntity pickingStaging = warehouseStagingList.stream()
                            .filter(staging -> PickingBillTypeEnum.firstLegs().contains(staging.getBillType()))
                            .filter(staging -> staging.getWarehouseId().equals(requisitionDetailEntity.getToWarehouseId()))
                            .findFirst().orElseThrow(() -> new ServiceException(ApiError.WH_LOCATION_DEFAULT_STAGING_NOT_FOUND));
                    detailAddDto.setWarehouseLocation(pickingStaging.getWarehouseLocation());
                } else {
                ...
                detailAddList.add(detailAddDto);
                deliveryQtyMap.merge(viewDTO.getSourceDetailId(), deliveryQty, Integer::sum);
            }
            if (CollUtil.isEmpty(detailAddList)) {
                throw new ServiceException("货件{}关联不到要货申请装箱明细", shipmentEntity.getCode());
            }
            addDTO.setDetailList(detailAddList);
            firstMileDeliveryService.add(addDTO);
        }

        List<RequisitionApplicationDetailEntity> updateDetailList = requisitionDetailList.stream()
                .filter(v -> deliveryQtyMap.containsKey(v.getId()))
                .peek(v -> v.setDeliveryQty(deliveryQtyMap.get(v.getId())))
                .collect(Collectors.toList());
    ...
    private void handleFirstMileTransferInfo (RequisitionApplicationEntity entity,List<RequisitionApplicationDetailEntity> detailList) {
        //头程发货单
        List<FirstMileDeliveryEntity> firstMileDeliveryList = firstMileDeliveryService.listBySourceIds(Collections.singletonList(entity.getId()));
    ...
    public PagingVO<RequisitionApplicationDTO.PickingViewDTO> generatePickingView(PagingDTO<RequisitionApplicationDTO.GetPickingViewDTO> page) {
        ...
        if (ObjectUtil.isNotEmpty(firstMileDelivery)) {
            throw new ServiceException(ApiError.SO_PICKLIST_ALREADY_EXISTS, "头程发货单");
        }
...
//            String warehouseName = application.getRequisitionWarehouseName();
            ...
            for (Map.Entry<String, Integer> entry : errorList.entrySet()) {
                ...
                if(null != skuVO){
                    ...
                    if(CharSequenceUtil.isNotBlank(inWarehouseLocation) && inWarehouseLocation.contains(",")){
                        inWarehouseLocation = inWarehouseLocation.split(",")[0];
                    }
                }
                if(CharSequenceUtil.isBlank(inWarehouseLocation) && CollectionUtils.isNotEmpty(cfgRulePickingInventoryDTOList)){
                    //推荐仓位（大货区） 不存在
    ...
    */
    private void handleData(RequisitionApplicationEntity requisitionApplicationEntity) {
        ...
        if(CharSequenceUtil.isBlank(requisitionApplicationEntity.getChannelName())){
            if(CharSequenceUtil.isBlank(requisitionApplicationEntity.getChannelId())){
                return;
            }
            String channelName = "";
...
//                List<OverseasProviderWarehouseDTO.ViewDTO> viewDTOList = overseasProviderWarehouseService.listByWarehouseIdList(Collections.singletonList(data.getChannelId()));
//                if(CollectionUtils.isNotEmpty(viewDTOList)){
//                    String provideCode = viewDTOList.get(0).getProviderCode();
//                    listingInfoWithSkuMappingDTO = listingWithSkuMappingDTOList.stream().filter(
...
//                if(CharSequenceUtil.isNotBlank(data.getChannelId())){
...
//            }
    ...
    private void fillList(List<RequisitionApplicationDTO.ListDTO> list) {
        ...
        for (RequisitionApplicationDTO.ListDTO listDTO : list) {
            if (CharSequenceUtil.isNotBlank(listDTO.getPackingStatus())) {
            ...
            //发货类型中文
            listDTO.setDeliveryTypeName(ThirdDeliveryTypeEnum.getName(listDTO.getDeliveryType()));
    ...
    private void validateSubmit(RequisitionApplicationEntity entity) {
        ...
        if (!entity.getStatus().equals(RequisitionApplicationStatusEnum.WAIT_SUBMIT.getStatus())) {
            throw new ServiceException(ApiError.BILL_SUBMIT_ALLOWED_PENDING_ONLY);
        }
        return;
    }
    ...
    public List<RequisitionApplicationDTO.PrintFnskuDetailDTO> printFnskuPreview(BaseIdsDTO.IdsDTO dto) {
        ...
        List<String> platformSpuNoList = detailList.stream()
                .filter(v -> CharSequenceUtil.isNotBlank(v.getPlatformSpu()))
        ...
        List<String> skuIds = detailList.stream()
                .filter(v -> CharSequenceUtil.isNotBlank(v.getSkuId()))
        ...
        Map<String, String> collect = productDTOS.stream().filter(v -> CharSequenceUtil.isNotBlank(v.getDeclareEnglishName())).collect(Collectors.toMap(LogisticsProductDTO.ProductDTO::getSkuId, LogisticsProductDTO.ProductDTO::getDeclareEnglishName));
    ...
    public void printFnskuBillConfirm(RequisitionApplicationDTO.PrintFnskuBillConfirmDTO dto, HttpServletResponse response) {
        ...
        if(CollUtil.isEmpty(details)){
            throw new ServiceException(ApiError.BILL_DETAIL_REQUIRED,"FNSKU标签");
        }
    ...
    private List<InOutStockDTO> listInventory(RequisitionApplicationEntity entity, List<RequisitionApplicationDetailEntity> detailEntityList, List<RequisitionApplicationDTO.InventoryDTO> dtoList) {
        ...
        dtoList.forEach(dto -> {
            ...
            if (Objects.isNull(stagingEntity)){
                throw new ServiceException(ApiError.WH_LOCATION_STAGING_NOT_FOUND, detailEntity.getFromWarehouseName());
            }
    ...
    @Override
    public void generateThirdDelivery(RequisitionApplicationDTO.GenerateThirdDelivery dto) {
        RequisitionApplicationEntity entity = this.getById(dto.getId());
        if (Objects.isNull(entity)){
            throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE,"要货申请单");
        }
        if (!ThirdDeliveryTypeEnum.THIRD_TO_THIRD.getCode().equals(entity.getDeliveryType())){
            throw new ServiceException("发货类型（三方仓发三方仓）类型的要货申请单才能下推FBA发货单");
        }
        List<RequisitionApplicationDetailEntity> detailEntityList = requisitionApplicationDetailService.listByMainIds(Collections.singletonList(dto.getId()));
        WmsDeliveryPlanEntity planEntity = wmsDeliveryPlanService.getById(entity.getSourceId());
        if (Objects.isNull(planEntity)){
            throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE,"发货计划单");
        }
        List<FbaShipmentEntity> shipmentEntityList = fbaShipmentService.listByCodes(Collections.singletonList(dto.getFbaShipmentCode()));
        if (CollUtil.isEmpty(shipmentEntityList)){
        ...
        if (!planEntity.getShopId().equals(shipmentEntity.getShopId())){
            throw new ServiceException("FBA货件【"+dto.getFbaShipmentCode()+"】和发货计划单【"+planEntity.getCode()+"】的店铺信息不一致");
        }
        ShopInfoEntity shopInfo = Optional.ofNullable(FeignQuery.getById(ShopInfoEntity.class,planEntity.getShopId())).orElseThrow(()->new ServiceException("查询不到店铺"));
        //发货明细
...
//            }
            ...
            detailAddDto.setFbaShipmentCode(shipmentEntity.getCode());
            detailAddDto.setDeliveryQty(planDetailEntity.getQty());
            detailAddDto.setWarehouseLocation(pickingStaging.getWarehouseLocation());
            detailAddList.add(detailAddDto);
