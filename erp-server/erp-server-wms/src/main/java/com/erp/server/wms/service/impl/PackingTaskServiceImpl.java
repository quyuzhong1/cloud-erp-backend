...
//            List<String> ids = packDateDTOS.stream().map(WmsCartonSpecDTO.PackDateDTO::getId).distinct().collect(Collectors.toList());
    ...
    private void checkPackQtyByPickQty(PackingTaskEntity packingTask, List<WmsCartonDetailDTO.AddDTO> detailList) {
        ...
        detailPickDTOS.forEach(detailPickDTO -> {
            ...
            if ((packQty + packedQty)> pickQty){
                throw new ServiceException(StrUtil.format(ApiError.ERROR_92252.msg,detailPickDTO.getSkuNo(), packQty + packedQty, pickQty));
            }
        });
    }
...
//        List<String> skuIds = Stream.concat(skuIds1.stream(), skuIds2.stream()).distinct().collect(Collectors.toList());
    ...
    private void checkAdjustData(WmsCartonDTO.AdjustSaveDTO dto) {
        ...
        if (AdjustTypeEnum.LOAD.getCode().equals(dto.getAdjustType()) || AdjustTypeEnum.REPACKING.getCode().equals(dto.getAdjustType())){
            dto.getCartonDetailList().forEach(adjustDetailDTO -> {
                ...
                if ((packQty + adjustQty)> pickQty){
                    throw new ServiceException(StrUtil.format(ApiError.ERROR_92252.msg,adjustDetailDTO.getSkuNo(), packQty + adjustQty, pickQty));
                }
            });
    ...
    private List<WmsCartonDTO.CartonDetailDTO> buildCartonDetail(PackingTaskEntity packingTaskEntity, List<WmsCartonDetailEntity> detailEntityList,WmsCartonDTO.AdjustDTO adjustDTO) {
        ...
        for (WmsCartonDetailEntity wmsCartonDetailEntity : cartonDetailEntityList) {
            ...
            //拣货数量
            Integer pickQty = detailPickDTOS.stream().filter(e -> e.getSkuId().equals(wmsCartonDetailEntity.getSkuId())).map(PickingListsDTO.DetailPickDTO::getQty).reduce(MathUtil.ZERO, Integer::sum);
