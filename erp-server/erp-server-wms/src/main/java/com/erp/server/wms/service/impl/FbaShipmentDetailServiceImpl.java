...
public class FbaShipmentDetailServiceImpl extends SuperServiceImpl<FbaShipmentDetailMapper, FbaShipmentDetailEntity> implements FbaShipmentDetailService {
    ...
    public String add(FbaShipmentDetailDTO.AddDTO addDTO) {
        ...
        log.info("开始新增FBI拣货明细单");
        boolean save = super.save(fbaShipmentDetailEntity);
        if(!save) {
            throw new ServiceException("FBI拣货明细单保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", commonService.getUserInfo().getUserName(), "FBI拣货明细单" , fbaShipmentDetailEntity.getId());
    ...
    */
    ...
    public Boolean update(FbaShipmentDetailDTO.UpdateDTO updateDTO) {
        ...
        Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.NOT_EXIST_BILL, "FBI拣货明细单"));
        FbaShipmentDetailEntity fbaShipmentDetailEntity =  BeanMapperUtils.map(FbaShipmentDetailEntity.class, updateDTO);

        // 数据处理
        handleData(fbaShipmentDetailEntity);
        log.info("编辑 开始修改FBI拣货明细单数据，id：【{}】", old.getId());
        ...
        // 记录主单操作日志
            log.info("编辑 开始记录FBI拣货明细单日志数据，id：【{}】", fbaShipmentDetailEntity.getId());
            String msg = StrUtil.format("用户【{}】编辑id为【{}】的【{}】单据 ", commonService.getUserInfo().getUserName(), fbaShipmentDetailEntity.getId(), "FBI拣货明细单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
