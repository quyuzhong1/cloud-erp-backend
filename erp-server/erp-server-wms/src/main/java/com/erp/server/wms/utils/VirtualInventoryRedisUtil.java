...
@Component
@Slf4j
public class VirtualInventoryRedisUtil extends AbstractRedisUtil{
	...
	@Qualifier("virtualInventoryRedisson")
	@Autowired
    private RedissonClient virtualInventoryRedisson;
	
    @Resource
    private RedisTemplate virtualInventoryRedisTemplate;

	@Override
	public RedisTemplate getRedisTemplate() {
		return virtualInventoryRedisTemplate;
	}
	...
	public RedissonMultiLock tryLock(List<String> keys , long waitTime) {
		List<RLock> rLocks = keys.stream()
                .map(virtualInventoryRedisson::getLock)
	...
	public void execute(InventoryRedisOpEnum inventoryRedisOpEnum , Object... args) {
		...
		String logMsg = StringUtil.appendLogMsg("VirtualInventoryRedisUtil的execute操作：" + opName , args);
    	log.info("{}开始" , logMsg);
		int i = 0;
		...
		String execute = "";
		while(i < 3) {
			execute = (String) virtualInventoryRedisTemplate.execute(InventoryRedisOpEnum.getDefaultRedisScript(inventoryRedisOpEnum), stringRedisSerializer, stringRedisSerializer, Arrays.asList(), args);
			log.warn("虚拟库存redis操作{}，入参{}，lua结果：{}" , opName , args ,execute);
			long sleep = 0L;
		...
		if(!success) {
			log.error("虚拟库存redis操作{}，结果为：{}，lua原始结果： {}" , opName , errormsg , execute);
