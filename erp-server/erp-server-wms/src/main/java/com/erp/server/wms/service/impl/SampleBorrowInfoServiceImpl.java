...
public class SampleBorrowInfoServiceImpl extends SuperServiceImpl<SampleBorrowInfoMapper, SampleBorrowInfoEntity> implements SampleBorrowInfoService,SampleLedgerFlowBuilder {
    ...
    public BaseResultDTO.AddDTO add(SampleBorrowInfoDTO.AddDTO addDTO) {
        ...
        // 操作日志
        String msg = StrUtil.format(addDTO.getClientType().getName()+"用户【{}】新增【{}】单据单号为【{}】", UserContext.getDefaultLoginUser().getUserName(), "样品借用单" , sampleBorrowInfoEntity.getCode());
    ...
    */
    ...
    public Boolean update(SampleBorrowInfoDTO.UpdateDTO addOrUpdateDTO) {
        ...
        old = Optional.ofNullable(old).orElseThrow(()->new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "样品借用单"));
        // 待提交和审核不通过允许修改
        ...
        String msg = StrUtil.format(addOrUpdateDTO.getClientType().getName()+"用户【{}】编辑单号为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), old.getCode(), "样品借用单");
    ...
    private void updateDetail(SampleBorrowInfoDTO.UpdateDTO addOrUpdateDTO,  SampleBorrowInfoEntity sampleBorrowInfoEntity) {
        ...
        if(CollUtil.isNotEmpty(addList)){
            ...
            operateLogService.batchAddModuleOperateLog(addOrUpdateDTO.getClientType().getName()+"添加SKU【%s】", ModuleTypeEnum.SAMPLE_BORROW_INFO.getCode(), addPairList, "编辑操作");
        }
        //处理需要更新的数据
        ...
        if(CollUtil.isNotEmpty(updateList)){
            ...
            for (SampleBorrowDetailEntity sampleBorrowDetailEntity : updateList) {
                ...
                if(Objects.nonNull(oldDetail)){
                    operateLogService.addModuleOperateLogByObj(oldDetail, sampleBorrowDetailEntity, ModuleTypeEnum.SAMPLE_BORROW_INFO.getCode(), sampleBorrowInfoEntity.getId(), String.format(addOrUpdateDTO.getClientType().getName()+"编辑SKU【%s】",oldDetail.getSkuNo()));
                }
            }
        }
    }
    ...
    public BatchResultDTO submit(String id, ClientTypeEnum clientType) {
        ...
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据提交审核 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单");
    ...
    public BatchResultDTO approve(ApproveOneDTO dto, ClientTypeEnum clientType) {
        ...
        // 操作日志
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据审核操作  审核结果：【{}】 审核意见 ：【{}】", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单", approveType.getName(), dto.getComment());
    ...
    */
    ...
    public BatchResultDTO disApprove(String id, ClientTypeEnum clientType) {
        ...
        // 操作日志
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据反审核操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单");
    ...
    private Boolean validateDisApprove(SampleBorrowInfoEntity entity) {
        ...
        Integer count = sampleReturnInfoService.lambdaQuery()
                ...
                .count();
        if (count > 0) {
            throw new ServiceException(ApiError.SAMPLE_RETURN_EXIST_REVERSE_FORBIDDEN);
        }
        return true;
    }
    ...
    public BatchResultDTO delete(String id, ClientTypeEnum clientType) {
        ...
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据删除操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单");
    ...
    public BatchResultDTO invalid(String id,String remark, ClientTypeEnum clientType) {
        ...
        lambdaUpdate()
                ...
                .update();
        // 日志
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据作废操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单");
    ...
    */
    ...
    public BatchResultDTO cancelProcess(ApproveDTO.CancelProcessDTO dto, ClientTypeEnum clientType) {
        ...
        String msg = StrUtil.format(clientType.getName()+"用户【{}】单号为【{}】的【{}】单据撤销流程操作 ", UserContext.getDefaultLoginUser().getUserName(), entity.getCode(), "样品借用单");
                ...
                if (CollUtil.isEmpty(skuAvailableQtyDTOS)) {
                    ...
                    if (Objects.isNull(skuAvailableQtyDTO)) {
                        ...
                        if (borrowedQty.compareTo(ledgerQty) > 0) {
                            errorMsg = errorMsg + indexTemp + "、" + CharSequenceUtil.format(ApiError.SAMPLE_AVAILABLE_QTY_EXCEEDS_LEDGER.getMsg(), importDTO.getSkuNo(), "借用") + "；";
                        } else {
    ...
    */
    private void handleData(SampleBorrowInfoEntity sampleBorrowInfoEntity) {
        ...
        if(sampleBorrowInfoEntity.getBorrowDate().isAfter(sampleBorrowInfoEntity.getEstimatedReturnDate())){
            throw new ServiceException(ApiError.SAMPLE_BORROW_DATE_INVALID);
        }

        String borrowUserId = sampleBorrowInfoEntity.getBorrowUserId();
        String lendUserId = sampleBorrowInfoEntity.getLendUserId();
        if(Objects.equals(borrowUserId,lendUserId)){
            throw new ServiceException(ApiError.SAMPLE_BORROW_USER_SAME_FORBIDDEN);
        }
    ...
    public List<SampleBorrowInfoDTO.SampleReturnView> generateSampleReturnView(List<String> detailIdList) {
        ...
        if(CollUtil.isEmpty(list)){
            throw new ServiceException(ApiError.SAMPLE_GENERATE_RETURN_QTY_ZERO);
        }
        // 查找第一个不符合审批通过状态的记录
        ...
        if(firstNotApproved.isPresent()){
            throw new ServiceException(ApiError.SAMPLE_GENERATE_RETURN_VIEW_ALLOWED_APPROVED_ONLY);
        }
        //筛选出可归还数量大于0的数据
        List<SampleBorrowInfoDTO.SampleReturnView> result = list.stream().filter(e -> Objects.nonNull(e.getCanReturnQty()) && e.getCanReturnQty() > 0).collect(Collectors.toList());
