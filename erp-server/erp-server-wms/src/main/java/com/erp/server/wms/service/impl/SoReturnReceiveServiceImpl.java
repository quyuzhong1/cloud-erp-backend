...
/*        List<QcInfoEntity> qcInfoEntities = qcInfoService.listQCBySourceIds(ids);
    ...
    @Override
    public String pdaAdd(SoReturnReceiveDTO.Add dto) {
        /*List<SoReturnReceiveDetailDTO.Add> warehouseReceiveDetailList = dto.getDetailList();
        List<String> poReceiveDetailIds = warehouseReceiveDetailList.stream().map(SoReturnReceiveDetailDTO.Add::getSourceDetailId).distinct().collect(Collectors.toList());
        List<SoReturnDetailEntity> returnDetailEntityList = soReturnFeign.listDetailByIds(poReceiveDetailIds);
