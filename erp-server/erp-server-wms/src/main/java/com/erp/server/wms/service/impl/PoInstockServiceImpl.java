...
public class PoInstockServiceImpl extends SuperServiceImpl<PoInstockMapper, PoInstockEntity> implements PoInstockService {
    ...
    private void autoGeneratePoInstock(List<PurchaseOrderDTO.SubcontractOrderChildDTO> childList, List<PoInstockDetailEntity> poInstockDetailList,
        ...
        //采购订单明细
        List<PurchaseOrderDetailEntity> purchaseOrderDetailList = scmTaskFeign.listByPurchaseOrderIds(childPoIds);
    ...
    /**
     * @param poIds
     * @return List<PoInstockEntity>
     * @description: 根据采购订单Ids查询已审核订单
     * @author Will
     * @date: 2023/6/15 19:19
     */
    private List<PoInstockEntity> listByPoIds(List<String> poIds) {
        return lambdaQuery().in(PoInstockEntity::getPurchaseOrderId, poIds)
                .eq(PoInstockEntity::getInvalidStatus, Boolean.FALSE)
                .list();
    }
    ...
    private void checkQcInfo(List<PoInstockEntity> list,List<PoInstockDetailEntity> detailList) {
        ...
        /**
         * 判断入库单是收货单下推还是非收货单下推
         * 优先收货单明细处理，若非收货单下推数据则根据采购订单明细处理
         */
        ...
        //非收货单下推对应的收货明细
        if (CollectionUtils.isNotEmpty(poIdList)) {
            //收货信息
            List<WarehouseReceiveDetailEntity> receiveDetailList = warehouseReceiveDetailService.listWarehouseReceiveByPodIds(poIdList);
            resultReceiveDetailList.addAll(receiveDetailList);
        }

        if (CollectionUtils.isEmpty(resultReceiveDetailList)) {
            return;
        }
