...
@Service
@Slf4j
public class PlatformAwdShipmentConsumerService<T extends DmpSyncTaskIdDTO> extends AbstractPlatformConsumerHandler<T> {
    ...
    private String getTableName(String platform){
        return CharSequenceUtil.format("{}_{}_{}", PlatformCategoryEnum.THIRD_SYSTEM.getCode(),
                platform, BusinessTypeEnum.AWD_SHIPMENT.getCode());
    }
    ...
    public ApiResult<?> handle(Object ext) {
        PlatformAwdShipmentDTO dto = JSONUtil.toBean(ext.toString(), PlatformAwdShipmentDTO.class);
        log.info("[AWD货件] 消费: dto={}", JSONUtil.toJsonStr(dto));
        // 组合信息
        FbaShipmentEntity entity = FbaShipmentConsumerConverter.INSTANCE.awdShipmentToEntity(dto);
        FbaShipmentExtendEntity extendEntity = FbaShipmentConsumerConverter.INSTANCE.awdshipmentExtendToEntity(dto);

        // 卖家SKU列表
        List<String> mskuList = dto.getDetailList().stream().map(PlatformAwdShipmentReceiveDTO::getMsku).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        // SKU绑定的信息
        ...
        DictCountryEntity toCountryEntity = sysUserFeign.getCountryById(dto.getDeliveryToCountryId());
        extendEntity.setDeliveryToCountryName(Objects.nonNull(toCountryEntity) ? toCountryEntity.getNameCn() : "");
        // 新增或更新
        FbaShipmentEntity oldEntity = fbaShipmentService.getByFbaShipmentId(entity.getFbaShipmentId());
        if (null == oldEntity) {
            // 新增
            fbaShipmentService.awdCheckAndSaveAll(entity, extendEntity,listingInfoWithSkuMappingDTOMap, hasChildrenSkuIds, dto.checkAndGetDetailList());
        } else {
            // 修改
            fbaShipmentService.awdCheckAndUpdateAll(oldEntity, entity, extendEntity,listingInfoWithSkuMappingDTOMap, hasChildrenSkuIds, dto.checkAndGetDetailList());
        }
     ...
     */
    private void checkAndSetCountryWithShop(FbaShipmentEntity entity, PlatformAwdShipmentDTO dto, ShopInfoEntity currentShopEntity) {
        // 补充来源名称
