...
public class PoReturnServiceImpl extends SuperServiceImpl<PoReturnMapper, PoReturnEntity> implements PoReturnService {
    ...
    public PoReturnEntity add(PurchaseReturnOrderDTO.AddDTO dto) {
        ...
        // 整箱退货统一不挂采购订单：无论前端是否回传 purchaseOrderId，都强制清空，保持与 PDA 端语义一致。
...
//        }
        ...
        if (CharSequenceUtil.isBlank(dto.getPurchaseOrderId())){
            ...
            if (CollectionUtils.isNotEmpty(collect)){
                throw new ServiceException(ApiError.PO_RETURN_UNIT_PRICE_REQUIRED);
            }
        }
        //校验补货数量不能大于退货数量
...
//        PurchaseOrderSupplierEntity orderSupplierByOrderId = scmTaskFeign.getOrderSupplierByOrderId(purchaseOrderEntity.getId());
        ...
        if (SourceTypeEnum.AUTO_ADD.getCode().equals(dto.getReturnType())){
            msg = CharSequenceUtil.format("从采购退货单【{}】下推生成了委外退料单【{}】采购退货单【{}】", dto.getParentReturnCode(), dto.getChildSubcontractCode(),code);
        }else {
            if(CharSequenceUtil.isNotBlank(sourceType) && SourceTypeEnum.PURCHASE_ORDER.getCode().equals(sourceType)){
                msg = CharSequenceUtil.format("从采购订单【{}】下推生成了采购退货单【{}】", poReturnEntity.getPurchaseOrderCode(), code);
            }else if (CharSequenceUtil.isNotBlank(sourceType) && SourceTypeEnum.QC_INFO.getCode().equals(sourceType)){
                QcInfoEntity qcInfoEntity = qcInfoService.getById(poReturnEntity.getSourceId());
                if (Objects.nonNull(qcInfoEntity)){
                    msg = CharSequenceUtil.format("从质检单【{}】下推生成了采购退货单【{}】", qcInfoEntity.getCode(), code);
                }
            }else if (CharSequenceUtil.isNotBlank(sourceType) && SourceTypeEnum.PO_INSTOCK.getCode().equals(sourceType)){
                PoInstockEntity poInstockEntity = poInstockService.getById(poReturnEntity.getSourceId());
                if (Objects.nonNull(poInstockEntity)){
                    msg = CharSequenceUtil.format("从采购入库单【{}】下推生成了采购退货单【{}】", poInstockEntity.getCode(), code);
                }
            }
        }
    ...
    public Boolean update(PurchaseReturnOrderDTO.UpdateDTO dto) {
        ...
        // 整箱退货统一不挂采购订单：无论前端是否回传 purchaseOrderId，都强制清空，保持与 PDA 端语义一致。
        if (packReturnDetail
                && CollectionUtils.isNotEmpty(dto.getPurchasePriceDetailList())) {
            dto.setPurchaseOrderId(null);
        }
...
//        }
        ...
        //获取采购单供应商信息
//        PurchaseOrderSupplierEntity orderSupplierByOrderId = scmTaskFeign.getOrderSupplierByOrderId(purchaseOrderEntity.getId());
        if (CharSequenceUtil.isNotBlank(dto.getPurchaseUserId())) {
            FindUserDTO purchaseUser = sysUserFeign.getUserByUserId(dto.getPurchaseUserId());
            poReturnEntity.setPurchaseUserName(purchaseUser.getUserName());
        }
        ...
        poReturnEntity.setPurchaseUserId(dto.getPurchaseUserId());
        poReturnEntity.setSupplierId(dto.getSupplierId());
        if (CharSequenceUtil.isNotBlank(dto.getSupplierId())) {
            SupplierEntity supplierEntity = scmTaskFeign.getSupplierById(dto.getSupplierId());
        ...
        // 后端按“新明细覆盖旧明细”同步售后装箱；前端未传回的旧明细自动按 actualQty=0 释放。
    ...
    public PurchaseReturnOrderDTO.ViewDTO view(String id) {
        ...
        if (CharSequenceUtil.isNotBlank(viewDTO.getReturnMode())) {
        ...
        List<String> mainSupplierIdList = detail.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getMainSupplierId())).map(PoReturnDetailEntity::getMainSupplierId).distinct().collect(Collectors.toList());
        ...
        for (PoReturnEntity purchaseReturnOrderEntity : purchaseReturnOrderEntities) {
            ...
            for (PoReturnDetailEntity poReturnDetailEntity : poReturnDetailEntityList) {
                ...
                if(poReturnDetailEntity.getReturnQty() > receiveQty-instockQty - otherDetailQty){
                    errorCodes.add(CharSequenceUtil.format("采购退货单【{}】sku【{}】退货数量不能大于已收货数量-已入库数量-已提交的质检退货数量【{}】",purchaseReturnOrderEntity.getCode(),poReturnDetailEntity.getSkuNo(),receiveQty-instockQty-otherDetailQty));
                }else{
                ...
                if (CharSequenceUtil.isBlank(purchaseReturnOrderEntity.getPurchaseOrderId())){
                    if (Objects.isNull(poReturnDetailEntity.getReturnPrice())){
                        throw new ServiceException(ApiError.PO_RETURN_SKU_UNIT_PRICE_REQUIRED, purchaseReturnOrderEntity.getCode(), poReturnDetailEntity.getSkuNo());
                    }
                }
            }
        }
    ...
    public BatchResultDTO approve(PoReturnEntity entity, String type, String comment, Boolean isNeedProcess,List<PoReturnDetailEntity> poReturnDetailList) {
        ...
        if (ApproveTypeEnum.PASS.getStatus().equals(type)) {
            ...
            //需要更新采购明细执行状态的父级退货单
            List<String> podIds = poReturnDetailList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getPurchaseOrderDetailId()) && poReturnIdList.contains(obj.getMainId()))
    ...
    public List<PoReturnEntity> autoAddSubcontractReturn(PoReturnEntity entity, List<PoReturnDetailEntity> poReturnDetailList, String confirmStatus,List<String> purchaseDetailIdList) {
        if (Objects.isNull(entity) || CollectionUtils.isEmpty(poReturnDetailList) || CharSequenceUtil.isBlank(entity.getPurchaseOrderId())){
            return null;
        }
        ...
        List<String> parentSubDetailIds = parentPurchaseOrderDetailList.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getSourceDetailId())).map(PurchaseOrderDetailEntity::getSourceDetailId).distinct().collect(Collectors.toList());
        //成品委外订单明细
        ...
        //子件委外订单
        List<String> childSubcontractIds = childSubcontractDetailList.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getMainId())).map(SubcontractOrderDetailEntity::getMainId).distinct().collect(Collectors.toList());
        ...
        for (PurchaseOrderEntity orderEntity : childPurchaseOrderList){
            ...
            if (Objects.isNull(subcontractOrderEntity)){
                throw new ServiceException(CharSequenceUtil.format("自动生成委外退料-采购订单【{}】关联委外订单记录为空",orderEntity.getCode()));
            }
            ...
            if (CollectionUtils.isEmpty(subcontractOrderDetailEntityList1)){
                throw new ServiceException(CharSequenceUtil.format("自动生成委外退料-采购订单【{}】关联委外订单明细为空",orderEntity.getCode()));
            }
            //采购订单明细
            ...
            if (CollectionUtils.isEmpty(detailEntityList)){
                throw new ServiceException(CharSequenceUtil.format("自动生成委外退料-采购订单【{}】关联采购订单明细为空",orderEntity.getCode()));
            }
            ...
            if (Objects.isNull(purchaseOrderSupplierEntity)){
                throw new ServiceException(CharSequenceUtil.format("自动生成委外退料-采购订单【{}】关联供应商记录为空",orderEntity.getCode()));
            }
            //构建委外退料单
        ...
        if (CollectionUtils.isNotEmpty(returnAddDTOList)){
            for (PurchaseReturnOrderDTO.AddDTO addDTO : returnAddDTOList){
                ...
                lambdaUpdate().set(PoReturnEntity::getApproveStatus, ApproveStatusEnum.APPROVE.getStatus())
                        ...
                        .update();
                poReturnEntity.setDeliveryWarehouseId(addDTO.getDeliveryWarehouseId());
    ...
    private PurchaseReturnOrderDTO.AddDTO buildPoReturnAddDTO(PoReturnEntity entity, List<PoReturnDetailEntity> poReturnDetailList, PurchaseOrderEntity orderEntity, SubcontractOrderEntity subcontractOrderEntity, PurchaseOrderSupplierEntity purchaseOrderSupplierEntity, List<PurchaseOrderDetailEntity> detailEntityList, List<SubcontractOrderDetailEntity> subcontractOrderDetailEntityList1, List<BomChildrenSkuDTO> bomList, List<SubcontractOrderDetailEntity> parentSubcontractOrderDetailEntityList) {
        ...
        returnAddDTO.setDeliveryWarehouseId(orderEntity.getDeliveryWarehouseId());
        ...
        for (PurchaseOrderDetailEntity detail : detailEntityList){
            ...
            SubcontractOrderDetailEntity subcontractOrderDetailEntity = subcontractOrderDetailEntityList1.stream().filter(e -> Objects.nonNull(e)
                    && CharSequenceUtil.isNotBlank(e.getSkuId()) && e.getSkuId().equals(detail.getSkuId())
                    && CharSequenceUtil.isNotBlank(detail.getSourceDetailId()) && detail.getSourceDetailId().equals(e.getId())
            ).findFirst().orElse(null);
            if (Objects.isNull(subcontractOrderDetailEntity)){
                throw new ServiceException(CharSequenceUtil.format("采购订单【{}】中SKU【{}】在委外订单【{}】未找到", orderEntity.getCode(), detail.getSkuNo(),subcontractOrderEntity.getCode()));
            }
            //委外子单关联的父级子单id
            String parentId = subcontractOrderDetailEntity.getParentId();
            if (CharSequenceUtil.isBlank(parentId)){
                throw new ServiceException(CharSequenceUtil.format("采购订单【{}】中SKU【{}】在委外订单【{}】关联成品子单明细", orderEntity.getCode(), detail.getSkuNo(),subcontractOrderEntity.getCode()));
            }
            //查询父级skuId
            ...
            if (Objects.isNull(poReturnDetailEntity)){
                throw new ServiceException(CharSequenceUtil.format("采购订单【{}】中SKU【{}】在退货订单【{}】未找到成品明细", orderEntity.getCode(), detail.getSkuNo(),entity.getCode()));
            }
            ...
            dto.setPurchaseWarehouseLocation(detail.getWarehouseLocation());
    ...
    public void syncApprovePoReturnToWdt(PoReturnEntity entity, SyncOperateEnum syncOperateEnum) {
        ...
        if (CollUtil.isNotEmpty(collect)){
            log.warn("采购退货单【{}】同步旺店通时，仓库【{}】存在库存同步配置，跳过同步旺店通",entity.getCode(), entity.getReturnWarehouseId());
    ...
    public BatchResultDTO disApprove(PoReturnEntity entity,List<PoReturnDetailEntity> detailEntityList) {
        ...
        if (CollectionUtils.isNotEmpty(poList)) {
            ...
            throw new ServiceException(ApiError.PO_ORDER_REVERSE_FORBIDDEN,String.join(",",poCodeList));
        }
        //判断是否已生成委外退料单
        ...
        if (CollectionUtils.isNotEmpty(subCodeList) && CollectionUtils.isNotEmpty(codeList)){
            throw new ServiceException(ApiError.PO_SUBCONTRACT_AND_PO_RETURN_REVERSE_FORBIDDEN,String.join(",",subCodeList), String.join(",",codeList));
        }else if (CollectionUtils.isNotEmpty(subCodeList)){
            throw new ServiceException(ApiError.PO_SUBCONTRACT_RETURN_ORDER_REVERSE_FORBIDDEN,String.join(",",subCodeList));
        }else if (CollectionUtils.isNotEmpty(codeList)){
            throw new ServiceException(ApiError.PO_RETURN_ORDER_REVERSE_FORBIDDEN,String.join(",",codeList));
        }
        ...
        List<PoReturnEntity> poReturnEntityList = Collections.singletonList(entity);
        //TODO 待加审核流程
    ...
    /**
     * 修改采购退货单后，后端兜底按最新明细覆盖旧箱唛明细：
     * 本次未传回的旧箱唛明细默认按 actualQty=0 处理，确保移除场景也会释放占用。
     */
    private void syncAfterSalePackForUpdate(PurchaseReturnOrderDTO.UpdateDTO dto, PoReturnEntity poReturnEntity) {
        List<AfterSalePackDTO.DetailDTO> oldBoxDetailList = getAfterSalePackDetailMap(poReturnEntity.getId()).values().stream()
                .flatMap(List::stream)
                .filter(Objects::nonNull)
                .collect(Collectors.toList());
        List<AfterSalePackDTO.DetailDTO> newBoxDetailList = getAfterSalePackDetailsForUpdate(dto);
        if (CollectionUtils.isEmpty(oldBoxDetailList) && CollectionUtils.isEmpty(newBoxDetailList)) {
            return;
        }
        List<AfterSalePackDTO.DetailDTO> syncBoxDetailList = new ArrayList<>();
        Map<String, Integer> detailIndexMap = new LinkedHashMap<>();
        for (AfterSalePackDTO.DetailDTO oldDetail : oldBoxDetailList) {
            AfterSalePackDTO.DetailDTO releaseDetail = new AfterSalePackDTO.DetailDTO();
            releaseDetail.setId(oldDetail.getId());
            releaseDetail.setMainId(oldDetail.getMainId());
            releaseDetail.setCode(oldDetail.getCode());
            releaseDetail.setSkuId(oldDetail.getSkuId());
            releaseDetail.setSkuNo(oldDetail.getSkuNo());
            releaseDetail.setPackQty(oldDetail.getPackQty());
            releaseDetail.setActualQty(MathUtil.ZERO);
            syncBoxDetailList.add(releaseDetail);
            if (CharSequenceUtil.isNotBlank(releaseDetail.getId())) {
                detailIndexMap.put(releaseDetail.getId(), syncBoxDetailList.size() - 1);
            }
        }
        for (AfterSalePackDTO.DetailDTO newDetail : newBoxDetailList) {
            if (newDetail == null) {
                continue;
            }
            Integer detailIndex = CharSequenceUtil.isNotBlank(newDetail.getId()) ? detailIndexMap.get(newDetail.getId()) : null;
            if (detailIndex == null) {
                syncBoxDetailList.add(newDetail);
                if (CharSequenceUtil.isNotBlank(newDetail.getId())) {
                    detailIndexMap.put(newDetail.getId(), syncBoxDetailList.size() - 1);
                }
            } else {
                syncBoxDetailList.set(detailIndex, newDetail);
            }
        }
        syncAfterSalePack(syncBoxDetailList, poReturnEntity);
    }
    ...
    private AfterSalePackEntity buildAfterSalePackUpdate(AfterSalePackEntity oldPack,
        ...
        if (CollectionUtils.isEmpty(detailList)) {
            ...
            Map<String, Integer> actualQtyMap = detailList.stream()
                    .collect(Collectors.groupingBy(AfterSalePackDTO.DetailDTO::getId,
                            Collectors.summingInt(this::getAfterSalePackActualQty)));
            boolean hasActualQty = actualQtyMap.values().stream().anyMatch(qty -> Optional.ofNullable(qty).orElse(MathUtil.ZERO) > MathUtil.ZERO);
            if (!hasActualQty) {
                updateEntity.setIsUse(Boolean.FALSE);
                updateEntity.setIsDifference(Boolean.FALSE);
                updateEntity.setSourceId("");
        ...
        for (Map.Entry<String, List<PoInstockDTO.GeneratePurchaseReturnOrderDTO>> entry : map.entrySet()) {
            ...
            for (PoInstockDTO.GeneratePurchaseReturnOrderDTO detail : value) {
                ...
                if (CharSequenceUtil.isNotBlank(skuNos)) {
                    throw new ServiceException(ApiError.PO_SKU_PUSH_DOWN_NOT_ALLOWED,purchaseOrderEntity.getCode(),skuNos);
                }
    ...
    public String checkSkuInventory(PurchaseReturnOrderDTO.AddDTO dto, List<PurchaseReturnOrderDetailDTO.AddDTO> detailList) {
        ...
        multiInventoryMap.forEach((key, detailGroupList)->{
            ...
            if(isScarce && !ignoreInventorySkuIds.contains(skuId)) {
                ...
                String msg = CharSequenceUtil.format("仓库【{}】仓位【{}】SKU【{}】【缺货：{}个】", warehouseName, warehouseLocationEntity.getName(), skuNo, (sumQty - curInventoryQty));
    ...
    public Boolean autoGeneratePurchaseOrder(BaseIdsDTO.IdsDTO dto) {
        ...
        if (CharSequenceUtil.isNotBlank(codes)) {
            throw new ServiceException(ApiError.PO_RETURN_REF_PO_NOT_APPROVED,codes);
        }
        //下推采购订单
    ...
    public void updateInventoryTransCore (List<PoReturnEntity> list) {
        ...
        //赋值采购订单的仓位
        List<String> purchaseDetailIds = detailEntityList.stream().map(PoReturnDetailEntity::getPurchaseOrderDetailId).filter(StringUtils::isNotBlank).collect(Collectors.toList());
        if(CollectionUtils.isNotEmpty(purchaseDetailIds)){
            List<PurchaseOrderDetailEntity> purchaseOrderDetailEntityList = purchaseOrderDetailService.listByIds(purchaseDetailIds);
            detailEntityList.forEach(v->{
                PurchaseOrderDetailEntity purchaseOrderDetailEntity = purchaseOrderDetailEntityList.stream().filter(t->t.getId().equals(v.getPurchaseOrderDetailId())).findFirst().orElse(null);
                if(Objects.nonNull(purchaseOrderDetailEntity)){
                    v.setPurchaseWarehouseLocation(purchaseOrderDetailEntity.getWarehouseLocation());
                }
            });
        }
    ...
    public void replenishmentInventory(Map<String, PoReturnEntity> replenishmentInventoryMap,
        ...
        if(CollUtil.isNotEmpty(replenishmentInventoryDetailList)) { //采购退货(库存退货，退货补货)明细
            ...
            replenishmentInventoryDetailList.forEach(detail->{
                ...
                // 使用补货数量 增加的在途
                InOutStockDTO inStockDTO = InOutStockDTO.initByReturnOrder(poReturnEntity, detail, InventorySourceTypeEnum.PURCHASE_RETURN_ORDER, detail.getReplenishQty(), InventoryStatusEnum.IN_TRANSIT, true);
                inList.add(inStockDTO);
                // 使用实退数量 减少可用
                InOutStockDTO inOutStockDTO = InOutStockDTO.initByReturnOrder(poReturnEntity, detail, InventorySourceTypeEnum.PURCHASE_RETURN_ORDER, detail.getReturnQty(), InventoryStatusEnum.USABLE, false);
    ...
    public void replenishmentQcInventory(Map<String, PoReturnEntity> replenishmentQcMap,
        ...
        if(CollUtil.isNotEmpty(replenishmentQcDetailList)) {
            replenishmentQcDetailList.forEach(detail->{
                ...
                InOutStockDTO inStockDTO = InOutStockDTO.initByReturnOrder(poReturnEntity, detail, InventorySourceTypeEnum.PURCHASE_RETURN_ORDER, detail.getReplenishQty(), InventoryStatusEnum.IN_TRANSIT, false);
                InventoryInOutStockRuleDTO inTransitRule = new InventoryInOutStockRuleDTO();
                ...
                inventoryTransCoreService.approveByRule(inTransitRule);
                // 使用实退数量 减少待检
                InOutStockDTO outStockDTO = InOutStockDTO.initByReturnOrder(poReturnEntity, detail, InventorySourceTypeEnum.PURCHASE_RETURN_ORDER, detail.getReturnQty(), InventoryStatusEnum.WAIT_QC, false);
    ...
    public void autoAddPurchaseOrder (List<PoReturnEntity> poReturnEntityList, Boolean isAuto) {
        ...
        if (CollectionUtils.isNotEmpty(poList)) {
            ...
            if (CharSequenceUtil.isNotBlank(codes)) {
                throw new ServiceException(ApiError.PO_RETURN_REF_PO_EXISTS,codes);
            }
        }
        List<PoReturnEntity> returnList = poReturnEntityList
                .stream()
                .filter(obj -> CharSequenceUtil.isBlank(obj.getPurchaseOrderId()) && ReturnModeEnum.REPLENISHMENT.getCode().equals(obj.getReturnMode())
        ).collect(Collectors.toList());
        ...
        //部门信息
        List<String> purchaseUserIds = returnList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getPurchaseUserId())).map(PoReturnEntity::getPurchaseOrderId).collect(Collectors.toList());
        List<SysDepartmentUserNumberDTO> departList = sysUserFeign.listDeptUserByUserIdList(purchaseUserIds);

        //供应商信息
        List<String> supplierIds = returnList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getSupplierId())).map(PoReturnEntity::getSupplierId).collect(Collectors.toList());
        ...
        //供应商联系人信息
        List<String> supplierContactIds = returnList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getSupplierContactId())).map(PoReturnEntity::getSupplierContactId).collect(Collectors.toList());
        ...
        for (PoReturnEntity entity : returnList) {
            ...
            //操作日志
            operateLogService.addModuleOperateLog(CharSequenceUtil.format("下推生成采购订单【{}】", code), ModuleTypeEnum.PURCHASE_RETURN_ORDER.getCode(), entity.getId(), "下推采购订单");
        }
    }
    ...
    public String pdaAdd(PurchaseReturnOrderDTO.AddDTO dto) {
        // 整箱退货：所有 SKU 明细行必须填写仓位，任意一行为空则拦截。
        checkPackReturnWarehouseLocationForAdd(dto);
        // 整箱退货由 add(...) 内部统一清空 purchaseOrderId，这里仅用作下方 PDA 拆单逻辑的门禁。
        boolean packReturnDetail = CollectionUtils.isNotEmpty(dto.getPurchasePriceDetailList())
                && CharSequenceUtil.equals(resolveReturnDetailType(dto.getReturnDetailType(), hasAfterSalePackDetailsForAdd(dto)), ReturnDetailTypeEnum.PACK.getCode());
        if (!packReturnDetail && CharSequenceUtil.isNotBlank(dto.getPurchaseOrderId())) {
    ...
    public Boolean pdaUpdate(PurchaseReturnOrderDTO.UpdateDTO dto) {
        // 整箱退货：所有 SKU 明细行必须填写仓位，任意一行为空则拦截。
        checkPackReturnWarehouseLocationForUpdate(dto);
        // 整箱退货由 update(...) 内部统一清空 purchaseOrderId，这里仅用作下方 PDA 拆单逻辑的门禁。
        boolean packReturnDetail = CollectionUtils.isNotEmpty(dto.getPurchasePriceDetailList())
                && CharSequenceUtil.equals(resolveReturnDetailType(dto.getReturnDetailType(), hasAfterSalePackDetailsForUpdate(dto)), ReturnDetailTypeEnum.PACK.getCode());
        if (!packReturnDetail && CharSequenceUtil.isNotBlank(dto.getPurchaseOrderId())) {
     ...
     */
    private void checkInventoryQty (PoReturnEntity poReturnEntity) {
        ...
        if(StringUtils.isNotBlank(poReturnEntity.getPurchaseOrderId())){
            purchaseOrderEntity = purchaseOrderService.getByIdOpt(poReturnEntity.getPurchaseOrderId()).orElse(null);
        }
        List<PurchaseOrderDetailEntity> purchaseOrderDetailEntities = new ArrayList<>();
        if (purchaseOrderEntity != null) {
            purchaseOrderDetailEntities = purchaseOrderDetailService.listByPurchaseOrderId(purchaseOrderEntity.getId());
            poReturnEntity.setDeliveryWarehouseId(purchaseOrderEntity.getDeliveryWarehouseId());
        }
        //仓库Id
        ...
        for (PoReturnDetailEntity detailEntity :  detailList) {
            ...
            if(Objects.equals(returnMode, ReturnModeEnum.REPLENISHMENT.getCode())
                ...
                throw new ServiceException(ApiError.WH_SKU_STOCK_INSUFFICIENT, detailEntity.getSkuNo());
    ...
    public BatchResultDTO returnConfirm(String id) {
        ...
        if (!PoReturnConfirmStatusEnum.WAIT_CONFIRM.getCode().equals(entity.getConfirmStatus())) {
            throw new ServiceException(ApiError.BILL_RETURN_WAIT_CONFIRM_STATUS_REQUIRED);
        }
        ...
        //更新对账明细的状态
        PoReconciliationDetailDTO.UpdateBusinessStatusDTO statusDTO = new PoReconciliationDetailDTO.UpdateBusinessStatusDTO(Collections.singletonList(id), ConfirmStatusEnum.CONFIRM.getCode());
    ...
    public Boolean unusualFeedback(PurchaseReturnOrderDTO.UnusualFeedbackParamDTO dto) {
        ...
        if (!CollectionUtils.isEmpty(dto.getAttachList())) {
            ...
            if (dto.getAttachList().size() > 5) {
                throw new ServiceException(ApiError.COMMON_ATTACH_QTY_MAX_FIVE);
            }
            //保存附件
        ...
        if (ObjectUtils.isEmpty(cfgSettingEntity)) {
            throw new ServiceException(ApiError.PO_RETURN_CFG_SETTING_NOT_EXISTS);
        }
        CfgSettingValueDTO.PoReturnSettingDTO poReturnSettingDTO = BeanUtil.toBean(cfgSettingEntity.getDataJson(), CfgSettingValueDTO.PoReturnSettingDTO.class);
        ...
        if (PoReturnUnusualTypeEnum.QUANTITY_ISSUE.getCode().equals(dto.getUnusualType())) {
            ...
            throw new ServiceException(ApiError.COMMON_UNUSUAL_TYPE_NOT_EXISTS);
        }
        //查询岗位名称
        ...
        return lambdaUpdate()
                ...
                .set(CharSequenceUtil.isNotBlank(sysPostEntity.getPostName()), PoReturnEntity::getUnusualHandleUserName, sysPostEntity.getPostName())
    ...
    public void supplierExportList(PurchaseReturnOrderDTO.SupplierPagingParamDTO dto, HttpServletResponse response) {
        ...
        //分页查询所有数据
        List<PurchaseReturnOrderDTO.SupplierPagingViewDTO> allRecords = new ArrayList<>();
        int pageSize = 1000;
        int currPage = 1;
        IPage<PurchaseReturnOrderDTO.SupplierPagingViewDTO> pageData;
        do {
            pageData = this.baseMapper.supplierPaging(new Page<>(currPage, pageSize), dto);
            if (CollUtil.isNotEmpty(pageData.getRecords())) {
                allRecords.addAll(pageData.getRecords());
            }
            currPage++;
        } while (pageData.getRecords().size() == pageSize && currPage <= pageData.getPages());
        ...
        List<ProductDetailEntity> detailEntityList = plmTaskFeign.getByIdList(skuIdList);
        for (PurchaseReturnOrderDTO.SupplierPagingViewDTO record : allRecords) {
            //退货来源名称
            ReturnOrderSourceEnum returnOrderSourceEnum = Objects.equals(record.getSourceType(), SourceTypeEnum.QC_INFO.getCode()) ?
                    ReturnOrderSourceEnum.QC : ReturnOrderSourceEnum.OTHER;
            record.setReturnOrderSource(returnOrderSourceEnum.getCode());
    ...
    public PurchaseReturnOrderDTO.PushDownSubcontractOrderViewDTO pushDownSubcontractOrderView(PurchaseReturnOrderDTO.detailIdsDTO detailIdsDTO) {
        ...
        // 获取所有采购退货订单ID
        List<String> mainIds = poReturnDetailList.stream().map(item -> item.getMainId()).distinct().collect(Collectors.toList());
        
        // 检查所有采购退货订单是否属于同一供应商
        List<PoReturnEntity> poReturnEntities = this.listByIds(mainIds);
        if (CollectionUtils.isEmpty(poReturnEntities)) {
            throw new ServiceException(ApiError.PO_RETURN_NOT_EXISTS);
        }
        
        // 检查所有采购退货订单是否属于同一供应商
        String supplierId = poReturnEntities.get(0).getSupplierId();
        boolean sameSupplier = poReturnEntities.stream().allMatch(e -> e.getSupplierId().equals(supplierId));
        if (!sameSupplier) {
            throw new ServiceException(ApiError.PO_RETURN_ONLY_SAME_SUPPLIER);
        }
        
        // 检查所有采购退货订单状态是否为已审核且已确认
        boolean allApprovedAndConfirmed = poReturnEntities.stream().allMatch(e -> 
            e.getApproveStatus().equals(ApproveStatusEnum.APPROVE.getStatus()) && 
            e.getConfirmStatus().equals(PoReturnConfirmStatusEnum.CONFIRM.getStatus())
        );
        if (!allApprovedAndConfirmed) {
            throw new ServiceException(ApiError.PO_RETURN_ONLY_APPROVED_CONFIRMED);
        }
        
        // 取第一张采购退货单的信息
        PoReturnEntity firstPoReturnEntity = poReturnEntities.get(0);
        ...
        //采购订单信息
        if (StringUtils.isNotBlank(firstPoReturnEntity.getPurchaseOrderId())) {
            PurchaseOrderEntity purchaseOrderEntity = scmTaskFeign.getPurchaseOrderById(firstPoReturnEntity.getPurchaseOrderId());
        ...
        // 按SKU和退款单价分组
        Map<String, Map<BigDecimal, List<PoReturnDetailEntity>>> skuPriceMap = poReturnDetailList.stream()
                .collect(Collectors.groupingBy(
                        PoReturnDetailEntity::getSkuNo,
                        Collectors.groupingBy(PoReturnDetailEntity::getReturnPrice)
                ));

        // 处理每个分组
        for (Map.Entry<String, Map<BigDecimal, List<PoReturnDetailEntity>>> skuEntry : skuPriceMap.entrySet()) {
            String skuNo = skuEntry.getKey();
            Map<BigDecimal, List<PoReturnDetailEntity>> priceMap = skuEntry.getValue();

            for (Map.Entry<BigDecimal, List<PoReturnDetailEntity>> priceEntry : priceMap.entrySet()) {
                BigDecimal returnPrice = priceEntry.getKey();
                List<PoReturnDetailEntity> detailList = priceEntry.getValue();

                // 计算总返修数量
                int totalReturnQty = 0;
                int totalPushQty = 0;
                for (PoReturnDetailEntity detail : detailList) {
                    totalReturnQty += detail.getReturnQty();
                    if (Objects.nonNull(sourceIdToQtyMap)) {
                        Integer pushQty = sourceIdToQtyMap.get(detail.getId());
                        if (Objects.nonNull(pushQty)) {
                            totalPushQty += pushQty;
                        }
                    }
                }
                int repairQty = totalReturnQty - totalPushQty;

                if (repairQty <= 0) {
                    throw new ServiceException(ApiError.PO_RETURN_REPAIR_QTY_NOT_ALLOW_BIGGER_THAN_RETURN_QTY, skuNo);
                }

                // 获取第一个明细的信息
                PoReturnDetailEntity firstDetail = detailList.get(0);
                ...
                List<PurchaseReturnOrderDTO.PushDownSubcontractOrderDetailViewDTO> childList = new ArrayList<>();

                //父行信息
                //sku信息
                String productName = skuList.stream()
                        .filter(item -> Objects.equals(item.getId(), firstDetail.getSkuId()))
                        .findFirst()
                        .map(ProductDetailEntity::getName)
                        .orElse("");
                pushDownSubcontractOrderDetailViewDTO.setSkuId(firstDetail.getSkuId());
                pushDownSubcontractOrderDetailViewDTO.setSkuNo(firstDetail.getSkuNo());
                pushDownSubcontractOrderDetailViewDTO.setProductName(productName);

                //供应商信息
                SupplierEntity supplierEntity = scmTaskFeign.getSupplierById(firstPoReturnEntity.getSupplierId());
                if (Objects.nonNull(supplierEntity)) {
                    pushDownSubcontractOrderDetailViewDTO.setSupplierId(firstPoReturnEntity.getSupplierId());
                    pushDownSubcontractOrderDetailViewDTO.setSupplierName(firstPoReturnEntity.getSupplierName());
                ...
                pushDownSubcontractOrderDetailViewDTO.setRepairPrice(returnPrice);
                pushDownSubcontractOrderDetailViewDTO.setRepairAmount(MathUtil.multiplyWithTwo(returnPrice, repairQty));

                //采购退货单信息
                pushDownSubcontractOrderDetailViewDTO.setCurrency(firstDetail.getCurrency());
                pushDownSubcontractOrderDetailViewDTO.setCurrencySymbol(firstDetail.getCurrencySymbol());
                // 多个来源明细ID，用逗号分隔
                String sourceDetailIds = detailList.stream().map(PoReturnDetailEntity::getId).collect(Collectors.joining(","));
                pushDownSubcontractOrderDetailViewDTO.setSourceDetailId(sourceDetailIds);
                ...
                childPushDownSubcontractOrderDetailViewDTO.setSkuId(firstDetail.getSkuId());
                childPushDownSubcontractOrderDetailViewDTO.setSkuNo(firstDetail.getSkuNo());
                childPushDownSubcontractOrderDetailViewDTO.setProductName(productName);
                if (Objects.nonNull(supplierEntity)) {
                    childPushDownSubcontractOrderDetailViewDTO.setSupplierId(firstPoReturnEntity.getSupplierId());
                    childPushDownSubcontractOrderDetailViewDTO.setSupplierName(firstPoReturnEntity.getSupplierName());
                ...
                childPushDownSubcontractOrderDetailViewDTO.setCurrency(firstDetail.getCurrency());
                childPushDownSubcontractOrderDetailViewDTO.setCurrencySymbol(firstDetail.getCurrencySymbol());
                childPushDownSubcontractOrderDetailViewDTO.setQty(totalReturnQty);
                childPushDownSubcontractOrderDetailViewDTO.setDeliveryQty(totalReturnQty);
                childPushDownSubcontractOrderDetailViewDTO.setPrice(returnPrice);
                childPushDownSubcontractOrderDetailViewDTO.setAmount(MathUtil.multiplyWithTwo(returnPrice, totalReturnQty));
                childPushDownSubcontractOrderDetailViewDTO.setSourceDetailId(sourceDetailIds);

                //子行
                childList.add(childPushDownSubcontractOrderDetailViewDTO);
                pushDownSubcontractOrderDetailViewDTO.setChildList(childList);
                //父行
    ...
    public List<PurchaseReturnOrderDTO.SubcontractOrderDTO> listSubcontractOrder(String poId) {
        ...
        List<String> skuIds = list.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getSkuId())).map(PurchaseReturnOrderDTO.SubcontractOrderDTO::getSkuId).distinct().collect(Collectors.toList());
        List<SkuVO> skuList = plmTaskFeign.listSkuProductByIds(skuIds);
    ...
    public List<PurchasePriceDTO.PriceDTO> batchGetPurchasePrice(List<PurchasePriceDTO.PriceDTO> list) {
        ...
        //采购组织Id
        List<String> purchaseOrgIdList = list.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getPurchaseOrgId())).map(PurchasePriceDTO.PriceDTO::getPurchaseOrgId).distinct().collect(Collectors.toList());
        List<BaseIdDTO.CodeDTO> companyList = sysUserFeign.getAccountingCompanyList(purchaseOrgIdList);
        if (CollectionUtils.isEmpty(companyList)){
            return Collections.emptyList();
        }
        //skuId
        List<String> skuIdList = list.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getSkuId())).map(PurchasePriceDTO.PriceDTO::getSkuId).distinct().collect(Collectors.toList());
        List<SkuVO> skuVOList = plmTaskFeign.listSkuProductByIds(skuIdList);
        if (CollectionUtils.isEmpty(skuVOList)) {
            return Collections.emptyList();
        }
        //供应商Id
        List<String> supplierIdList = list.stream().filter(e -> Objects.nonNull(e) && CharSequenceUtil.isNotBlank(e.getSupplierId())).map(PurchasePriceDTO.PriceDTO::getSupplierId).distinct().collect(Collectors.toList());
        ...
        for (PurchasePriceDTO.PriceDTO priceDTO : list) {
            ...
            if (CollectionUtils.isNotEmpty(viewList)) {
                viewDTO = viewList.stream().filter(obj -> Objects.nonNull(obj)
                                && CharSequenceUtil.isNotBlank(obj.getSkuId()) && CharSequenceUtil.isNotBlank(priceDTO.getSkuId()) && obj.getSkuId().equals(priceDTO.getSkuId())
                                && CharSequenceUtil.isNotBlank(obj.getSupplierId()) && CharSequenceUtil.isNotBlank(priceDTO.getSupplierId()) && obj.getSupplierId().equals(priceDTO.getSupplierId())
                                && CharSequenceUtil.isNotBlank(obj.getPurchaseOrgId()) && CharSequenceUtil.isNotBlank(priceDTO.getPurchaseOrgId()) && CharSequenceUtil.equals(obj.getPurchaseOrgId(),priceDTO.getPurchaseOrgId()))
                        .findFirst().orElse(null);
    ...
    @Override
    public List<PurchasePriceDTO.PushDownPurchaseView> pushDownPurchaseView(List<String> detailIds) {
        List<PurchasePriceDTO.PushDownPurchaseView> pushDownPurchaseViews = baseMapper.pushDownPurchaseView(detailIds);
        ...
        if(CollectionUtils.isNotEmpty(purchaseOrderEntityList)){
            ...
            purchaseOrderDetailEntityList = scmTaskFeign.listByPurchaseOrderIds(poIds);
        }
    ...
    public Boolean pushDownPurchase(List<PurchasePriceDTO.PushDownPurchaseView> pushDownPurchaseViews) {
        ...
        purchaseUserIds.add(UserContext.getDefaultLoginUser().getUid());
        List<SysDepartmentUserNumberDTO> departList = sysUserFeign.listDeptUserByUserIdList(purchaseUserIds);
        Map<String,List<PurchasePriceDTO.PushDownPurchaseView>> map = pushDownPurchaseViews.stream().collect(Collectors.groupingBy(PurchasePriceDTO.PushDownPurchaseView::getId));
        map.forEach((key,val)->{
            PurchasePriceDTO.PushDownPurchaseView mainView = val.get(0);
            //新增采购订单
            PurchaseOrderDTO.AddDTO addDTO = new PurchaseOrderDTO.AddDTO();
            addDTO.setSourceId(mainView.getId());
            addDTO.setSourceCode(mainView.getCode());
            addDTO.setSourceType(SourceTypeEnum.PO_RETURN.getCode());
            addDTO.setType(PurchaseOrderTypeEnum.ENUM_RETURN.getCode());
            addDTO.setPurchaseDate(Objects.isNull(mainView.getPurchaseDate())?LocalDate.now():mainView.getPurchaseDate());
            addDTO.setPurchaseUserId(StringUtils.isBlank(mainView.getPurchaseUserId())?UserContext.getDefaultLoginUser().getUid():mainView.getPurchaseUserId());
            //采购部门
            if (CollectionUtils.isNotEmpty(departList)) {
                String deptId = departList.stream().filter(obj -> obj.getUserId().equals(addDTO.getPurchaseUserId())).findFirst().flatMap(obj -> Optional.ofNullable(obj.getDepartmentId())).orElse("");
                addDTO.setPurchaseDeptId(deptId);
            }
            //采购组织
            addDTO.setPurchaseOrgId(mainView.getPurchaseOrgId());
            addDTO.setDeliveryWarehouseId(mainView.getDeliveryWarehouseId());
            //采购供应商信息
            PurchaseOrderSupplierDTO.AddDTO supplierDTO = new PurchaseOrderSupplierDTO.AddDTO();
            supplierDTO.setSupplierId(mainView.getSupplierId());
            supplierDTO.setSupplierContactId(mainView.getSupplierContactId());
            //供应商信息
            if (CollectionUtils.isNotEmpty(supplierList)) {
                SupplierEntity supplierEntity = supplierList.stream().filter(obj -> obj.getId().equals(mainView.getSupplierId())).findFirst().orElse(null);
                if (ObjectUtils.isEmpty(supplierEntity)) {
                    throw new ServiceException(ApiError.SUPPLIER_NOT_FOUND);
                }
                supplierDTO.setPaymentCondition(supplierEntity.getPaymentCondition());
            ...
            //供应商联系人信息
            if (CollectionUtils.isNotEmpty(supplierContactList)) {
                SupplierContactEntity supplierContactEntity = supplierContactList.stream().filter(obj -> obj.getId().equals(mainView.getSupplierContactId())).findFirst().orElse(null);
                if (ObjectUtils.isNotEmpty(supplierContactEntity)) {
                    //联系人电话
                    supplierDTO.setContactTelNumber(supplierContactEntity.getTelNumber());
                }
            ...
            for (PurchasePriceDTO.PushDownPurchaseView detailView:val) {
                PurchaseOrderDetailDTO.AddDTO addDetailDTO = new PurchaseOrderDetailDTO.AddDTO();
                addDetailDTO.setSourceDetailId(detailView.getDetailId());
                addDetailDTO.setSkuId(detailView.getSkuId());
            ...
            String code = scmTaskFeign.addPurchaseOrder(addDTO);
            //操作日志
            operateLogService.addModuleOperateLog(CharSequenceUtil.format("下推生成采购订单【{}】", code), ModuleTypeEnum.PURCHASE_RETURN_ORDER.getCode(), key, "下推采购订单");
        });
        return true;
    }
    ...
    private void applyReturnReasonType(PoReturnEntity poReturnEntity, String returnReasonType, String returnRemark) {
        if (CharSequenceUtil.isBlank(returnReasonType)) {
            returnReasonType = PoReturnReasonTypeEnum.DEFECT.getCode();
                ...
                && !PoReturnReasonTypeEnum.OTHER.getCode().equals(returnReasonType)) {
            throw new ServiceException(ApiError.WH_PO_RETURN_REASON_TYPE_INVALID);
        }
        poReturnEntity.setReturnReasonType(returnReasonType);
        if (PoReturnReasonTypeEnum.OTHER.getCode().equals(returnReasonType)) {
            if (CharSequenceUtil.isBlank(returnRemark)) {
                throw new ServiceException(ApiError.WH_PO_RETURN_REMARK_REQUIRED);
            }
