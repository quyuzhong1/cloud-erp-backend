...
public class FirstMileDeliveryServiceImpl extends SuperServiceImpl<FirstMileDeliveryMapper, FirstMileDeliveryEntity> implements FirstMileDeliveryService {
    ...
    public BaseResultDTO.AddDTO add(FirstMileDeliveryDTO.AddDTO addDTO) {
        ...
        if(!save) {
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_SAVE_FAILED);
        }
    ...
    public Boolean consumeDeclareAutoGenerateTask(AutoGenerateBillDTO dto) {
        ...
        try {
            ...
            if (CollectionUtils.isEmpty(mergeDeclareBillDTOS)) {
                throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_DECLARE_SOURCE_NOT_FOUND, entity.getCode());
            }
            ...
            if (!Boolean.TRUE.equals(autoGenerateResult)) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_DETAIL_MID_AUTO_GENERATE_FAILED, "头程发货单", entity.getCode(), "返回失败");
            }
...
//            }
    ...
    public List<FirstMileDeliveryDTO.TabListDTO> tabList(PermissionsDTO param) {
        ...
        // 不存在的状态赋值为0
        Set<String> existStatusSet = list.stream().map(FirstMileDeliveryDTO.TabListDTO::getTabFlag).collect(Collectors.toSet());
        ...
        if (Objects.isNull(awdOutstockEntity)) {
            if (Objects.isNull(requisitionApplication)
                ...
                if (CollectionUtils.isEmpty(taskEntityList)) {
                    throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_PACKING_TASK_NOT_GENERATED);
                }
                ...
                if(Objects.nonNull(cfgOverweightDetailDTO) && cfgOverweightDetailDTO.isCheckStatusWhenApprove()){
                    if(!(taskEntity.getPackingStatus().equals(PackingTaskStatusEnum.PACKED.getCode()) && taskEntity.getWeightingStatus().equals(PackingWeightStatusEnum.WEIGHTED.getCode()))){
                        throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_PACKING_WEIGHT_REQUIRED_FOR_APPROVE);
                    }
                }
        ...
        for (FirstMileDeliveryDetailEntity detailEntity : detailEntityList) {
            ...
            if ((Objects.nonNull(requisitionApplication) &&  ThirdDeliveryTypeEnum.THIRD_TO_THIRD.getCode().equals(requisitionApplication.getDeliveryType()))
                ...
                if (CollUtil.isEmpty(availableLocation)){
                    throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_AVAILABLE_INVENTORY_INSUFFICIENT, deliveryWarehouse.getName(), detailEntity.getSkuNo(), detailEntity.getDeliveryQty(), 0);
                }
                int sum = availableLocation.stream().mapToInt(InventoryEntity::getQty).sum();
                if (sum < detailEntity.getDeliveryQty()){
                    throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_AVAILABLE_INVENTORY_INSUFFICIENT, deliveryWarehouse.getName(), detailEntity.getSkuNo(), detailEntity.getDeliveryQty(), sum);
                }
    ...
    */
    ...
    public BatchResultDTO disApprove(String id) {
        FirstMileDeliveryEntity entity = super.getByIdOpt(id).orElseThrow(() -> new ServiceException(ApiError.FIRST_MILE_SHIPMENT_NOT_FOUND));
        // 反审核条件判断
    ...
    public BatchResultDTO delete(FirstMileDeliveryEntity entity, PackingTaskEntity packingTask) {
        ...
        if(CollectionUtils.isNotEmpty(machineInfoEntityList)){
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_MACHINE_EXISTS_DELETE_FORBIDDEN,
                    machineInfoEntityList.stream().map(MachineInfoEntity::getCode).collect(Collectors.joining(",")));
        }
        //存在下游单据（海外入库单），不能删除
        ...
        if (CollUtil.isNotEmpty(inboundEntityList)){
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_INBOUND_EXISTS_DELETE_FORBIDDEN,
                    inboundEntityList.stream().map(OverseasWarehouseInboundEntity::getCode).filter(CharSequenceUtil::isNotBlank).collect(Collectors.joining(",")));
        }
    ...
    */
                ...
                if (isOverseasInboundDemandType(entity.getDemandType())) {
                    ...
                    if (CollectionUtils.isNotEmpty(overseasProviderWarehouseEntities) && Objects.nonNull(providerEntity)
                        ...
                        if (200 != resultInfo.getCode()) {
                            ...
                            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_THIRD_WAREHOUSE_APPROVE_PUSH_FAILED, resultInfo.getMsg());
                        }
                ...
                if (CollectionUtils.isNotEmpty(taskEntityList)) {
                    ...
                    if(packed){
                        ...
                        try {
                            ...
                            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_LOGISTICS_AUTO_GENERATE_FAILED, entity.getCode(), e.getMessage());
                        }
    ...
    public FirstMileDeliveryDTO.ViewDTO view(String id) {
        //发货单主信息
        FirstMileDeliveryEntity firstMileDeliveryEntity = super.getByIdOpt(id).orElseThrow(()->new ServiceException(ApiError.FIRST_MILE_SHIPMENT_NOT_FOUND));
    ...
    **/
    ...
    public void startProcess(FirstMileDeliveryEntity entity) {
        ...
        if (!result.isSuccess()) {
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_WORKFLOW_START_FAILED, result.getMsg());
        }
    }
    ...
    */
    ...
    public OverseasWarehouseInboundDTO.ViewDTO getGenerateOverseasWarehouseInboundView(String id) {
        ...
        if (CharSequenceUtil.isBlank(destWarehouseId)) {
            throw new ServiceException(ApiError.FIRST_MILE_SHIPMENT_DEST_WAREHOUSE_REQUIRED);
        }
        // 所属平台:未绑定海外仓为空
    ...
    public WmsCartonSpecDTO.ListPackingDTO listPacking(String id) {
        FirstMileDeliveryEntity firstMileDeliveryEntity = Optional.ofNullable(this.getById(id)).orElseThrow(()->new ServiceException(ApiError.FIRST_MILE_SHIPMENT_NOT_FOUND));
                ...
                if (CollUtil.isNotEmpty(midEntityList)) {
                    ...
                    if (CollUtil.isNotEmpty(generatedSourceIds)) {
                        ...
                        if (CollUtil.isNotEmpty(generatedCodes)) {
                            throw new ServiceException(ApiError.LOGISTICS_DECLARE_SOURCE_GENERATED, String.join("、", generatedCodes));
                        }
                    }
