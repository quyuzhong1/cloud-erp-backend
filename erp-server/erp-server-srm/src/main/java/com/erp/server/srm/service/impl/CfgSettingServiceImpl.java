...
public class CfgSettingServiceImpl extends SuperServiceImpl<CfgSettingMapper, CfgSettingEntity> implements CfgSettingService {
    ...
    public void add(CfgSettingDTO.AddDTO addDTO) {
        ...
        for (CfgSettingEntity cfgSettingEntity : cfgSettingEntities) {
            ...
            // 操作日志
            String msg = CharSequenceUtil.format("用户【{}】新增【{}】单据id为【{}】", UserContext.getDefaultLoginUser().getUserName(), ApiError.COMMON_SYSTEM_CONFIG_MANAGEMENT.getMsg(), cfgSettingEntity.getId());
    ...
    public Boolean update(CfgSettingDTO.UpdateDTO updateDTO) {
        ...
        if (CollectionUtils.isNotEmpty(cfgSettingEntities)) {
            for (CfgSettingEntity cfgSettingEntity : cfgSettingEntities) {
                ...
                if(null == old){
                    throw  new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, ApiError.COMMON_SYSTEM_CONFIG_MANAGEMENT.getMsg());
                }
                ...
                String msg =  CharSequenceUtil.format("用户【{}】编辑id为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), cfgSettingEntity.getId(), ApiError.COMMON_SYSTEM_CONFIG_MANAGEMENT.getMsg());
    ...
    private void handleData(CfgSettingEntity cfgSettingEntity) {
        ...
        int count = lambdaQuery()
                ...
                .count();
        if (count > 0) {
            throw new ServiceException(ApiError.SUPPLIER_CONFIG_ALREADY_EXISTS);
        }
