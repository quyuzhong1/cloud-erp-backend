...
//                    .reduce(MathUtil.ZERO, Integer::sum);
    ...
    public DeliveryOrderDTO.ViewDTO viewByCode(String code) {
        ...
        Map<String,SkuVO> skuVOMap = plmTaskFeign.listSkuBaseByIds(detailList.stream().map(DeliveryOrderDetailDTO.ViewDTO::getSkuId).distinct().collect(Collectors.toList())).stream().collect(Collectors.toMap(SkuVO::getSkuId, Function.identity(),(v1, v2)->v1));
