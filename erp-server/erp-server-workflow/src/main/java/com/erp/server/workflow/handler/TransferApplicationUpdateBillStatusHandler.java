...
@Component
@Slf4j
public class TransferApplicationUpdateBillStatusHandler implements UpdateBillStatusHandler {

    @Resource
    ThirdProcessManagementService thirdProcessManagementService;

    @Resource
    TransferApplicationFeign transferApplicationFeign;
    ...
    public void updateBillStatus(JSONObject jsonObject, String billId) throws Exception {
        List<TransferApplicationEntity> transferApplicationEntityList = transferApplicationFeign.listByCodes(Arrays.asList(billId));
        if (CollUtil.isEmpty(transferApplicationEntityList)) {
            throw new ServiceException("未找到调用申请单，请检查三方生成查询的单据编号是否正确，{}", billId);
        }
        TransferApplicationEntity entity = transferApplicationEntityList.get(0);
        //更新审核状态
        ...
        TransferApplicationDTO.UpdateApprovalStatusDTO updateApprovalStatusDTO = new TransferApplicationDTO.UpdateApprovalStatusDTO(entity, entity.getApproveStatus());
        transferApplicationFeign.updateApproveStatus(updateApprovalStatusDTO);
    }
