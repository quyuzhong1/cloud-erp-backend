 ...
 * 直接调拨单 测试
 */
public class KingdeeCustomerInfoTest {
    public static void main(String[] args) {
        KingdeeApiUtils customerApiService = new KingdeeApiUtils("BD_Customer");
        // 列表字段
        String fieldKeys = "FCUSTID,FDocumentStatus,FForbidStatus,FName,FNumber";
        LinkedList<String> queryFilters = new LinkedList<>();
        queryFilters.add(StrUtil.format("FNumber in ({})", "'108-SHEIN'"));
...
//        queryFilters.add(StrUtil.format("FCustId = '{}'", "126840"));
        ...
        maps.parallelStream().forEach(map -> {
            // 先检查禁用状态
            if ("B".equalsIgnoreCase(String.valueOf(map.get("FForbidStatus")))) {
