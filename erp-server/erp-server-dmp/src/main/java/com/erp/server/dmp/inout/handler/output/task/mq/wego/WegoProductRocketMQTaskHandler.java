...
/**
 * WEGO 仓库商品 Output：dmp_product_info + dmp_sku_info → RocketMQ → PlatformListingConsumer。
 * {@code dmp_sku_info.status} 已在 Input 阶段归一为 Active/Inactive，直接写入 platformStatus。
 */
@Service
@Scope("prototype")
public class WegoProductRocketMQTaskHandler extends DmpOutputRocketMQTaskHandler {
     ...
     *   <li>{@code status} → {@code platformStatus}（Input 已归一 Active/Inactive）</li>
    ...
    public PlatformProductDTO convert(DmpProductInfoEntity productInfo, DmpSkuInfoEntity skuInfo, String cfgOutputId) {
        ...
        String sourcePlatform = StringUtils.defaultIfBlank(productInfo.getSourcePlatform(), OmsPlatformEnum.WE_GO.getCode());
