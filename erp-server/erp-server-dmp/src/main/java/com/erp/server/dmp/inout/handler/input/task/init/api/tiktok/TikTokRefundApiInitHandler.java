...
@Slf4j
@Scope("prototype")
public class TikTokRefundApiInitHandler implements DmpInputApiInitHandler {
    @Resource
    ...
    public List<DmpInputTaskInitDTO> getApiData(DmpInputApiInitRequest dmpInputApiInitRequest) {
        ...
        while (true) {
            ...
            if (!Objects.equals(apiResult.getCode(), 200)) {
                log.error("调用url={},入参params={}, TikTok查询退款订单数据失败，返回值 responseMap={}", sb.toString(), params.toString(), JSONUtil.toJsonStr(apiResult));
                throw new RuntimeException(StrUtil.format("调用url={},入参params={}, TikTok查询退款订单数据失败，返回值 responseMap={}",
            ...
            try {
                ...
                log.error("调用url={},入参params={}, TikTok查询退款订单数据解析失败，返回值 responseMap={}", url + path, params.toString(), JSONUtil.toJsonStr(apiResult));
                throw new RuntimeException(StrUtil.format("调用url={},入参params={}, TikTok查询退款订单数据解析失败，返回值 responseMap={}",
