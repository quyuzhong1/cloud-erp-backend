...
import com.erp.server.dmp.pull.mapper.BiReturnOrderItemMapper;
...
public class BiReturnOrderItemServiceImpl extends ServiceImpl<BiReturnOrderItemMapper, BiReturnOrderItemEntity>
     ...
     **/
    public Boolean add(BiReturnOrderItemEntity biReturnOrderItemEntity, String platformSign) {
        //拆单sku
        List<BiReturnOrderItemEntity> dmpReturnOrderItemEntities = splitOrderItem(Arrays.asList(biReturnOrderItemEntity), platformSign);
     ...
     **/
    @Override
    public Boolean batchAdd(List<BiReturnOrderItemEntity> dmpOrderInfoEntityList, String platformSign) {
        //拆单sku
        List<BiReturnOrderItemEntity> dmpReturnOrderItemEntities = splitOrderItem(dmpOrderInfoEntityList, platformSign);
     ...
     **/
    @Override
    public Boolean batchUpdate(List<BiReturnOrderItemEntity> dmpOrderInfoEntityList, String platformSign) {
        //拆单sku
        List<BiReturnOrderItemEntity> dmpReturnOrderItemEntities = splitOrderItem(dmpOrderInfoEntityList, platformSign);
     ...
     **/
    public BiReturnOrderItemEntity getOrderByReturnOrderId(String returnOrderId) {
        LambdaQueryWrapper<BiReturnOrderItemEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(BiReturnOrderItemEntity::getReturnOrderId, returnOrderId);
    ...
    @Override
    public void checkOrderItem(List<BiReturnOrderItemEntity> itemList, String platformSign) {
        List<BiReturnOrderItemEntity> insertList = new ArrayList<>();
        List<BiReturnOrderItemEntity> updateList = new ArrayList<>();
        for (BiReturnOrderItemEntity orderItemBean : itemList) {
            Optional<BiReturnOrderItemEntity> dmpReturnOrderItemEntity = lambdaQuery()
                    .eq(BiReturnOrderItemEntity::getErpOrderItemId, orderItemBean.getErpOrderItemId())
                    .last("limit 1")
                    .oneOpt();
     ...
     **/
    @Override
    public List<BiReturnOrderItemEntity> splitOrderItem(List<BiReturnOrderItemEntity> itemEntityList, String platformSign) {
        List<BiReturnOrderItemEntity> itemListAll = new ArrayList<>();
        ...
        //遍历订单详情
        for (BiReturnOrderItemEntity itemEntity : itemEntityList) {
            //设置通用参数
            ...
            for (int i = 0;i < splitSkuDTOS.size(); i++) {
                ...
                BiReturnOrderItemEntity entity = new BiReturnOrderItemEntity();
    ...
    @Override
    public List<BiReturnOrderItemEntity> getItemByMainId(String mainId) {
        return this.lambdaQuery().eq(BiReturnOrderItemEntity::getReturnOrderId, mainId)
                .eq(BiReturnOrderItemEntity::getIsDeleted,false)
                .list();
    }
}
