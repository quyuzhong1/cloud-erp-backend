...
public class DmpOrderItemSplitServiceImpl extends ServiceImpl<DmpOrderItemSplitMapper, DmpOrderItemSplitEntity>
     ...
     **/
    @Override
    public Boolean add(DmpOrderItemSplitEntity dmpOrderInfoEntity, String platformSign) {
        //拆单
        List<DmpOrderItemSplitEntity> itemEntityList = splitOrderItem(Arrays.asList(dmpOrderInfoEntity), platformSign);
     ...
     **/
    @Override
    public Boolean batchUpdate(List<DmpOrderItemSplitEntity> dmpOrderInfoEntityList, String platformSign) {
        //拆单
        List<DmpOrderItemSplitEntity> itemEntityList = splitOrderItem(dmpOrderInfoEntityList, platformSign);
     ...
     **/
    @Override
    public DmpOrderItemSplitEntity getByErpOrderItemId(String erpOrderItemId) {
        LambdaQueryWrapper<DmpOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(DmpOrderItemSplitEntity::getErpOrderItemId, erpOrderItemId);
     ...
     **/
    @Override
    public List<DmpOrderItemSplitEntity> getByOrderId(String orderId) {
        LambdaQueryWrapper<DmpOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(DmpOrderItemSplitEntity::getOrderId, orderId);
     ...
     **/
    @Override
    public Boolean updateOrderItemByErpOrderItemId(DmpOrderItemSplitEntity dmpOrderItemSplitEntity) {
        LambdaQueryWrapper<DmpOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
    ...
    public void updateNewSign(Map<String, List<NewProductDTO>> dto) {
        ...
        listingNotNullList.forEach(req -> {
            ...
            partition.forEach(obj -> {
                LambdaUpdateWrapper<DmpOrderItemSplitEntity> updateWrapper = new LambdaUpdateWrapper<>();
                updateWrapper.set(DmpOrderItemSplitEntity::getNewSign, 1);
                updateWrapper.in(DmpOrderItemSplitEntity::getId, obj);
...
*/
     ...
     **/
    @Override
    public List<DmpOrderItemSplitEntity> splitOrderItem(List<DmpOrderItemSplitEntity> itemEntityList, String platformSign) {
        List<DmpOrderItemSplitEntity> itemListAll = new ArrayList<>();
        ...
        //遍历订单详情
        for (DmpOrderItemSplitEntity dmpOrderItemSplitEntity : itemEntityList) {
            DmpOrderItemEntity originalItem = BeanMapperUtils.map(DmpOrderItemEntity.class, dmpOrderItemSplitEntity);
            //设置通用参数
            SplitSkuDTO splitSkuDTO = new SplitSkuDTO();
            splitSkuDTO.setPlatformSign(platformSign);
            splitSkuDTO.setId(dmpOrderItemSplitEntity.getItemId());
            splitSkuDTO.setCleanCostPrice(dmpOrderItemSplitEntity.getCleanCostPrice());
            splitSkuDTO.setIsSplitSku(dmpOrderItemSplitEntity.getIsSplitSku());
            splitSkuDTO.setOriginalSkuNo(dmpOrderItemSplitEntity.getOriginalSkuNo());
            splitSkuDTO.setSkuNo(dmpOrderItemSplitEntity.getSkuNo());
            splitSkuDTO.setQuantity(dmpOrderItemSplitEntity.getQuantity());
            if (dmpOrderItemSplitEntity.getIsGift() != null) {
                splitSkuDTO.setIsGift(dmpOrderItemSplitEntity.getIsGift());
            } else {
                splitSkuDTO.setIsGift(2);
            }
            splitSkuDTO.setAmountAfter(dmpOrderItemSplitEntity.getAmountAfter());
            //拆单
