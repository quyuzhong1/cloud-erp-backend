...
import com.erp.sdk.oms.amz.spapi.model.productpricing.Price;
import com.erp.sdk.oms.amz.spapi.model.productpricing.PriceList;
...
@Service
@Scope("prototype")
public class DmpInputAmzProductPricingInitHandler extends DmpInputAmzCommonInitHandler {
    ...
    public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
        ...
        // 过滤不支持查询明细的sku
        List<Map<String, Object>> canQueryList = findMongoData.stream().filter(this::missingAsin).collect(Collectors.toList());
        ...
        List<String> allSellerSku = waitQueryList.stream()
                .map(e -> e.getOrDefault("sellerSku", "").toString())
                .filter(StringUtils::isNotBlank)
                .distinct()
                .collect(Collectors.toList());
        // 批量查询数量仅支持20
        List<List<String>> partition = Lists.partition(allSellerSku, 20);

        for (List<String> skusList : partition) {
            try {
                // 获取动态速率
                ...
                // 请求亚马逊接口获取价格
                String itemType = "Sku";
                ApiResponse<GetPricingResponse> apiResponse = api.getPricingWithHttpInfo(marketPlaceEnum.getMarketplaceId(), itemType, null, skusList, null);

                List<String> limitArray = apiResponse.getHeaders().get(ApiClient.X_AMAZON_RATE_LIMIT);
                rateLimitStr = limitArray.get(0);
                PriceList priceList = apiResponse.getData().getPayload();
                ...
                if (429 == e.getCode()) {
                    ...
                    log.warn("【亚马逊listing价格】 platformShopCode={},首次429等待恢复:放弃当前请求任务", shopInfoDTO.getPlatformShopCode());
                    // 触发限流不执行当前
                    DmpInputInitResponse initDmpResponse = (DmpInputInitResponse) dmpResponse;
                    initDmpResponse.setDoNextStatus(false);
                    return Collections.emptyList();
     ...
     * 转换JSONObject
     */
    private JSONObject convertJsonObject(Price item, String marketplaceId, String shopId, String platformShopCode) {
        JSONObject jsonObject = (JSONObject) JSON.toJSON(item);
        jsonObject.put("marketplaceId", marketplaceId);
        jsonObject.put("shopId", shopId);
        jsonObject.put("platformShopCode", platformShopCode);
        return jsonObject;
    }
    ...
    private void checkAndGetRedisList(List<Map<String, Object>> canQueryList, List<JSONObject> resultList, List<Map<String, Object>> waitQueryList) {
        for (Map<String, Object> findMongoDataItem : canQueryList) {
            ...
            if (StringUtils.isBlank(productId)) {
                ServiceException.runError("亚马逊listing价格异常:无productId：{}", findMongoDataItem);
            }
            // 缓存获取结果
            String shipmentIdResultKey = StrUtil.format(RedisCacheConstants.AMZ_SP_API_RESULT_PREFIX, AmazonRequestTypeRateLimiterEnum.PRODUCT_PRICING.getBusinessTypeName(), productId);
     ...
     * 当前sku是否需要查询ASIN
     */
    public boolean missingAsin(Map<String, Object> mongoItem) {
        String productIdType = mongoItem.getOrDefault("productIdType", "").toString();
        // ProductIdType=1, 已包含ASIN;
        return !"1".equalsIgnoreCase(productIdType);
    }
}
