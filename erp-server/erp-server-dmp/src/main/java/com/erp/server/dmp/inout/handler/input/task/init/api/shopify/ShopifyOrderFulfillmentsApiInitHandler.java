...
@Slf4j
@Scope("prototype")
public class ShopifyOrderFulfillmentsApiInitHandler extends DmpInputInitHandler {
    ...
    public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
        ...
        if (null == shopInfoDTO) {
            log.error("[Shopify配送信息下载]从缓存中获取shopify token 失败: shopId={}", findMongoData.get(0).get("nextLevelId").toString());
            throw new ServiceException();
        }

        for (String orderId : orderIds) {
            JSONObject fulfillmentsObj = shopifyRestClientService.getShopifyRestClient(shopInfoDTO.getShopDomain(), shopInfoDTO.getAccessToken()).getFulfillments(orderId);
            if (null == fulfillmentsObj) {
                continue;
            }
            JSONArray fulfillmentsList = JSON.parseArray(JSON.toJSONString(fulfillmentsObj.get("fulfillments")));
            if (CollectionUtil.isEmpty(fulfillmentsList)) {
                continue;
            }

            DmpInputTaskInitDTO dmpInputTaskInitDTO = new DmpInputTaskInitDTO();
            dmpInputTaskInitDTO.setMsg(JSONArray.toJSONString(fulfillmentsList));
