 ...
 */
@Slf4j
@Component("amzReportFbaMyiAllInventoryHandler")
public class AmzReportFbaMyiAllInventoryHandler extends DmpMongoHandler {
    ...
    public Integer findAndFillDataOrHandle(DmpMongoHandleTaskEntity mongoHandleTaskEntity, Boolean queryIsAddOrUpdate) {
        ...
        // 指定的mongo表
        String mongoTableName = MongoTableNameContant.DATA_REPORT_AMZ_FBA_MYI_ALL_INVENTORY;
        List<ReportFbaMyiAllInventoryMongoDTO> allList = dmpMongoHandleTaskService.findMongoData(mongoHandleTaskEntity.getLastId(), handleCount, mongoTableName, ReportFbaMyiAllInventoryMongoDTO.class, queryIsAddOrUpdate);
        if (CollectionUtils.isEmpty(allList)) {
            log.warn("亚马逊库存管理报告已归档处理服务处理结束：处理数据为空:handleType={}", mongoHandleTaskEntity.getHandleType());
            return 0;
        }
        // 记录最大ID和下次执行时间
