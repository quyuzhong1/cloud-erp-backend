...
@Slf4j
@Component
public class ThirdVirtualWarehouseStrategy implements ThirdMappingStrategy {
    @Resource
...
//        warehouse = listDTOS.get(0);
    ...
    public ThirdMappingDTO.MappingViewDTO view(ThirdMappingDTO.ViewParamDTO viewParamDTO) {
        ...
        //获取系统仓库
        List<VirtualWarehouseEntity> listDTOS =
                Optional.ofNullable(wmsVirtualWarehouseFeign.listByIds(Collections.singletonList(sysId))).orElse(new ArrayList<>());
        if (CollectionUtils.isEmpty(listDTOS)) {
    ...
    private void makeWarehouseViewDto(ThirdMappingDTO.ViewParamDTO viewParamDTO, String sysId, ThirdMappingDTO.MappingViewDTO mappingViewDTO, List<VirtualWarehouseEntity> listDTOS) {
        //获取第三方数据信息
    ...
    static void getViewDto(ThirdMappingDTO.MappingViewDTO mappingViewDTO, List<VirtualWarehouseEntity> listDTOS, ThirdWarehouseService thirdWarehouseService, OverseasProviderFeign overseasProviderFeign, List<ThirdMappingDTO.ViewDTO> viewDTOList, List<ThirdMappingEntity> thirdMappingEntityList) {
