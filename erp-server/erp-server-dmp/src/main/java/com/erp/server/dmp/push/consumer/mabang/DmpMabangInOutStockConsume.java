...
public class DmpMabangInOutStockConsume implements RocketMQListener<Object>  {
    ...
    public void onMessage(Object ext) {
        try {
            ...
            DmpPushTaskEntity dmpPushTaskEntity = dmpPushTaskService.getById(syncTaskId);
            if(Objects.isNull(dmpPushTaskEntity)) {
            ...
            String syncStatusName = SyncStatusEnum.getNameByCode(dmpPushTaskEntity.getStatus());
            log.warn("ERP【{}】原单据id：【{}】，同步马帮状态【{}】", sourceTypeName, dmpPushTaskEntity.getSourceId(), syncStatusName);

            // 同步成功的不处理
            if(Objects.equals(dmpPushTaskEntity.getStatus(), SyncStatusEnum.SUCCESS_SYNC.getCode())) {
            ...
            mabangInOutStockService.sendToMabangInOutStock(dmpPushTaskEntity, mabangInOutStockDTO, inventoryInOutEnum);
        }catch (Exception e) {
