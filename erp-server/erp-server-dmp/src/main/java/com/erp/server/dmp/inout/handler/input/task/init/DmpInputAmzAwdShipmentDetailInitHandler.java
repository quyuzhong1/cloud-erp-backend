...
@Service
@Scope("prototype")
public class DmpInputAmzAwdShipmentDetailInitHandler extends DmpInputAmzCommonInitHandler {
    @Resource
    ...
    public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
        ...
        for (Map<String, Object> findMongo : findMongoData) {
            ...
            // 缓存获取结果
            String shipmentIdResultKey = StrUtil.format(RedisCacheConstants.AMZ_SP_API_RESULT_PREFIX, AmazonRequestTypeRateLimiterEnum.AWD_SHIPMENT_DETAIL.getBusinessTypeName(), shipmentId);
            ...
            try {
                ...
                if (Objects.isNull(inboundShipment)){
                    log.warn("AWD货件详情为空:shipmentId={}",shipmentId);
                    continue;
                }
                JSONObject json = (JSONObject) JSON.toJSON(inboundShipment);
                json.put("shopId",shopId);
                json.put("shopName",shopInfoDTO.getName());
                json.put("platformShopCode",shopInfoDTO.getPlatformShopCode());
                List<JSONObject> curJsonList = Collections.singletonList(json);
                // 缓存倒redis
                ...
                if (429 == e.getCode()) {
                    ...
                    log.warn("【AWD货件明细拉取】 platformShopCode={},首次429等待恢复:放弃当前请求任务", shopInfoDTO.getPlatformShopCode());
                    // 触发限流不执行当前
                    DmpInputInitResponse initDmpResponse = (DmpInputInitResponse) dmpResponse;
                    initDmpResponse.setDoNextStatus(false);
                    return Collections.emptyList();
                }
                throw new ServiceException("[Amazon SP-APi] 查询AWD货件item失败" + e);
            } catch (LWAException e) {
                throw new RuntimeException("[Amazon SP-APi] 查询AWD货件明细失败" + e);
            }
            log.warn("查询亚马逊AWD货件详情成功, shipmentId={}, platformShopCode={}", shipmentId, shopInfoDTO.getPlatformShopCode());
        }
