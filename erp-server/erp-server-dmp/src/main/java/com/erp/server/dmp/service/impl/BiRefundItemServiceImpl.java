...
import com.erp.server.dmp.pull.mapper.BiRefundItemMapper;
 ...
 */
@Service
public class BiRefundItemServiceImpl extends ServiceImpl<BiRefundItemMapper, BiRefundItemEntity>
    implements BiRefundItemService {
    @Resource
     ...
     **/
    @Override
    public Boolean add(BiRefundItemEntity biRefundItemEntity, String platformSign) {
        List<BiRefundItemEntity> dmpRefundItemEntities = splitOrderItem(Arrays.asList(biRefundItemEntity), platformSign);
     ...
     **/
    @Override
    public Boolean batchAdd(List<BiRefundItemEntity> biRefundItemEntityList, String platformSign) {
        List<BiRefundItemEntity> dmpRefundItemEntities = splitOrderItem(biRefundItemEntityList, platformSign);
     ...
     **/
    @Override
    public Boolean batchUpdate(List<BiRefundItemEntity> biRefundItemEntityList, String platformSign) {
        List<BiRefundItemEntity> dmpRefundItemEntities = splitOrderItem(biRefundItemEntityList, platformSign);
    ...
    public Boolean deleteRefundItemByRefundId(String refundId) {
        LambdaQueryWrapper<BiRefundItemEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(BiRefundItemEntity::getRefundId, refundId);
        return this.remove(lambdaQueryWrapper);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void checkOrderItem(List<BiRefundItemEntity> itemList, String platformSign) {
        List<BiRefundItemEntity> insertList = new ArrayList<>();
        List<BiRefundItemEntity> updateList = new ArrayList<>();
        for (BiRefundItemEntity orderItemBean : itemList) {
            Optional<BiRefundItemEntity> dmpRefundItemEntity = lambdaQuery()
                    .eq(BiRefundItemEntity::getErpOrderItemId, orderItemBean.getErpOrderItemId())
                    .oneOpt();
...
//                    updateById(orderItemBean);
     ...
     **/
    @Override
    public List<BiRefundItemEntity> splitOrderItem(List<BiRefundItemEntity> itemEntityList, String platformSign) {
        List<BiRefundItemEntity> itemListAll = new ArrayList<>();
        ...
        //遍历订单详情
        for (BiRefundItemEntity itemEntity : itemEntityList) {
            //设置通用参数
            ...
            for (int i = 0;i < splitSkuDTOS.size(); i++) {
                ...
                BiRefundItemEntity entity = new BiRefundItemEntity();
