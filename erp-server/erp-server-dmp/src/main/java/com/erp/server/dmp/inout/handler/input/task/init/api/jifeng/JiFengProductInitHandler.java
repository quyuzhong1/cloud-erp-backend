...
@Service
@Scope("prototype")
public class JiFengProductInitHandler extends DmpInputInitHandler {
    ...
    public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
        ...
        if(null == overseasProviderEntity) {
            throw new ServiceException(DmpBasicSystemCodeEnum.JIFENG.getCode() + "对应授权ID信息不存在,nextId:"+dmpInputTaskEntity.getNextLevelId());
        }

        JiFengBaseResp<List<JiFengProductResp.RowsDTO>> resp = jiFengService.getProductList(overseasProviderEntity.getAuthJson());
        if(resp == null) {
        ...
        if(resp.getCode() != 0) {
            if(resp.getMessage().contains("Invalid ACCESS TOKEN")){
                ...
                resp = jiFengService.getProductList(overseasProviderEntity.getAuthJson());
                if(resp == null) {
                    return Collections.emptyList();
                }
                if(resp.getCode() != 0) {
                    throw new ServiceException("极风获取产品列表失败,code:"+resp.getCode()+",msg:"+resp.getMessage());
                }
            }else{
                throw new ServiceException("极风获取产品列表失败,code:"+resp.getCode()+",msg:"+resp.getMessage());
            }
