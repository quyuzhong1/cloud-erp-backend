...
@Service
@Scope("prototype")
public class JiFengReturnInstockRocketMQTaskHandler extends DmpOutputRocketMQTaskHandler {
    ...
    public PlatformReturnInstockDTO convert(DmpThirdReturnInboundEntity dmpMainEntity, List<DmpThirdReturnInboundDetailEntity> dmpDetailList, String cfgOutputId) {
        ...
        // T状态转换
        String erpStatus = JiFengEnums.ReturnInstockTypeEnum.getErpStatus(returnStatus);
        ...
        if (CollectionUtils.isEmpty(filterDmpDetaiList)){
            log.warn("极风退货找不到上架数量大于0的明细: {}", JSONUtil.toJsonStr(dmpMainEntity));
            return null;
        }
        // 明细
