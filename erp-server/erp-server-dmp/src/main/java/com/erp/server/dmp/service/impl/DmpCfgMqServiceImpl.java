...
public class DmpCfgMqServiceImpl extends SuperServiceImpl<DmpCfgMqMapper, DmpCfgMqEntity> implements DmpCfgMqService {
    ...
    public Boolean update(DmpCfgMqDTO.UpdateDTO updateDTO) {
        ...
        old = Optional.ofNullable(old).orElseThrow(() -> new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "输入输出mq信息"));
    ...
    @Override
    public DmpCfgMqDTO.ViewDTO view(String id) {
        DmpCfgMqEntity dmpCfgMqEntity = super.getByIdOpt(id).orElseThrow(() -> new ServiceException("未找到输入输出mq信息数据"));
        DmpCfgMqDTO.ViewDTO data = BeanMapperUtils.map(DmpCfgMqDTO.ViewDTO.class, dmpCfgMqEntity);
        // 数据填充处理
        fillOne(data);
        // TODO 查询明细数据（如果有的话）
        return data;
    ...
    private void fillOne(DmpCfgMqDTO.ViewDTO data) {
