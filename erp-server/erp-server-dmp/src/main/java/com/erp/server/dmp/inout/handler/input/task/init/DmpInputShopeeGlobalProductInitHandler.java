...
@Service
@Scope("prototype")
public class DmpInputShopeeGlobalProductInitHandler extends DmpInputInitHandler{
	@Resource
	...
	public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
		...
		while(hasNextPage) {
	    	...
	    	while(data == null) {
	    		...
	    		if(data == null) {
	    			if(count == 10) {
	    				throw new ServiceException("调用shopee全球产品信息接口重试" + count + "失败");
	    			}
	    			try {
	...
	private ShopeeResponse execute(GlobalProductRequest productRequest){
		ShopeeResponse response = null;
		try {
			response = shopeeGlobalProductService.getGlobalProductList(productRequest);
		} catch (Exception e) {
			Throwable cause = e.getCause();
			if(cause instanceof SSLHandshakeException || cause instanceof SocketTimeoutException) {
				return null;
			}
			throw new ServiceException("调用shopee全球产品信息接口报错，错误原因：" + ExceptionUtil.stacktraceToOneLineString(e));
		}
		
		if(response != null) {
			String error = response.getError();
			if(StringUtils.isNotBlank(error)) {
				throw new ServiceException("调用shopee全球产品信息接口报错，错误原因：" + response.getMessage());
			}
		}
