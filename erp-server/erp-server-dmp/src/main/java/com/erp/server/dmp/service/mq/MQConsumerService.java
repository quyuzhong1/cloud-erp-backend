...
//            throw new RuntimeException("tag2 失败测试");
    ...
    /**
     * consumeMode = ConsumeMode.CONCURRENTLY  重试3次
     */
    @Service
    ...
    public class ConsumerSend4 implements RocketMQListener<DmpOrderInfoEntity> {
        @Override
        public void onMessage(DmpOrderInfoEntity ext) {
            log.info("监听到消息：msg={}", ext);
            log.info("监听到消息：entity={}", JSONUtil.toJsonStr(ext));
        }
    }
