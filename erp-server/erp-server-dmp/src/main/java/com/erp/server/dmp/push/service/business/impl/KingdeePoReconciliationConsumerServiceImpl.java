...
@Service
@Slf4j
public class KingdeePoReconciliationConsumerServiceImpl implements KingdeePoReconciliationConsumerService {
    ...
    public Boolean saveOrUpdate (KingdeeApiUtils apiUtils,PlatformEntity platformEntity,Map<String, Object> map,JSONObject json,KingdeeParamDTO.SaveParamDTO param) {
        //无需自动审核
        param.setIsAutoAudit(Boolean.FALSE);
        Boolean isAdd = kingdeeCommonService.saveOrUpdate(platformEntity,map,apiUtils,json,param,ApiModuleTypeEnum.PAYABLE_INFO.getCode());
        if (isAdd) {
            //给明细id赋值
    ...
    public JSONArray setDetailIdForJSONObject (KingdeeApiUtils apiUtils,Map<String, Object> map) {
        ...
        //查询子单据id
        String fieldKeys = "FEntityDetail_FEntryID";
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (list.size() >= queryList.size()) {
                //金蝶明细id赋值
                newJson.set("kingdeeDetailId",queryList.get(i).get("FEntityDetail_FEntryID"));
            }
