                ...
                try {
                    ...
                    if (e instanceof ApiException) {
                        ...
                        if (429 == apiError.getCode()) {
                            if (10 == count) {
                                throw new ServiceException("调用亚马逊入库计划接口重试" + count + "失败");
                            }
                            try {
                            ...
                            if (message.contains("GetInboundPlan is not supported for the cross border Seller Export and Delivery program")){
                                // TODO 临时跳过
                                log.error("调用亚马逊入库计划接口信息异常:{}, error={}", shopInfoDTO.getPlatformShopCode(), ExceptionUtil.stacktraceToString(e));
                            } else {
