...
public class BiOrderItemSplitServiceImpl extends ServiceImpl<BiOrderItemSplitMapper, BiOrderItemSplitEntity>
     ...
     **/
    @Override
    public Boolean add(BiOrderItemSplitEntity dmpOrderInfoEntity, String platformSign) {
        //拆单
        List<BiOrderItemSplitEntity> itemEntityList = splitOrderItem(Arrays.asList(dmpOrderInfoEntity), platformSign);
     ...
     **/
    @Override
    public Boolean batchAdd(List<BiOrderItemSplitEntity> dmpOrderInfoEntityList, String platformSign) {
        //拆单
        List<BiOrderItemSplitEntity> itemEntityList = splitOrderItem(dmpOrderInfoEntityList, platformSign);
        return this.saveBatch(itemEntityList);
    }
     ...
     **/
    @Override
    public Boolean batchUpdate(List<BiOrderItemSplitEntity> dmpOrderInfoEntityList, String platformSign) {
        //拆单
        List<BiOrderItemSplitEntity> itemEntityList = splitOrderItem(dmpOrderInfoEntityList, platformSign);
     ...
     **/
    @Override
    public BiOrderItemSplitEntity getByErpOrderItemId(String erpOrderItemId) {
        LambdaQueryWrapper<BiOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(BiOrderItemSplitEntity::getErpOrderItemId, erpOrderItemId);
     ...
     **/
    @Override
    public List<BiOrderItemSplitEntity> getByOrderId(String orderId) {
        LambdaQueryWrapper<BiOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(BiOrderItemSplitEntity::getOrderId, orderId);
     ...
     * 根据erp平台商品id修改订单商品信息
     *
     * @param biOrderItemSplitEntity 订单商品信息
     * @return java.lang.Boolean
     * @Author Luo_WG
     * @Date 2022/11/14 21:39
     **/
    @Override
    public Boolean updateOrderItemByErpOrderItemId(BiOrderItemSplitEntity biOrderItemSplitEntity) {
        LambdaQueryWrapper<BiOrderItemSplitEntity> lambdaQueryWrapper = new LambdaQueryWrapper();
        lambdaQueryWrapper.eq(BiOrderItemSplitEntity::getErpOrderItemId, biOrderItemSplitEntity.getErpOrderItemId());
        return this.update(biOrderItemSplitEntity, lambdaQueryWrapper);
    }
    ...
    public void checkOrderItem(List<BiOrderItemSplitEntity> orderItem, LocalDate platformCreateTime, String platformSign) {
        List<BiOrderItemSplitEntity> insertList = new ArrayList<>();
        for (BiOrderItemSplitEntity orderItemBean : orderItem) {
...
//                    updateById(orderItemBean);
        ...
        biOrderItemService.deleteByOrderIds(orderIds);

        //新增新数据
        if (CollectionUtil.isNotEmpty(insertList)) {
            //保存未拆分数据
            List<BiOrderItemEntity> itemEntityList = BeanMapper.copyList(insertList, BiOrderItemEntity.class);
            biOrderItemService.batchAdd(itemEntityList, platformSign);
    ...
    public void updateNewSign(Map<String, List<NewProductDTO>> dto) {
        ...
        listingNotNullList.forEach(req -> {
            ...
            partition.forEach(obj -> {
                LambdaUpdateWrapper<BiOrderItemSplitEntity> updateWrapper = new LambdaUpdateWrapper<>();
                updateWrapper.set(BiOrderItemSplitEntity::getNewSign, 1);
                updateWrapper.in(BiOrderItemSplitEntity::getId, obj);
...
*/
     ...
     **/
    @Override
    public List<BiOrderItemSplitEntity> splitOrderItem(List<BiOrderItemSplitEntity> itemEntityList, String platformSign) {
        List<BiOrderItemSplitEntity> itemListAll = new ArrayList<>();
        ...
        //遍历订单详情
        for (BiOrderItemSplitEntity biOrderItemSplitEntity : itemEntityList) {
            BiOrderItemEntity originalItem = BeanMapperUtils.map(BiOrderItemEntity.class, biOrderItemSplitEntity);
            //设置通用参数
            SplitSkuDTO splitSkuDTO = new SplitSkuDTO();
            splitSkuDTO.setPlatformSign(platformSign);
            splitSkuDTO.setId(biOrderItemSplitEntity.getItemId());
            splitSkuDTO.setCleanCostPrice(biOrderItemSplitEntity.getCleanCostPrice());
            splitSkuDTO.setIsSplitSku(biOrderItemSplitEntity.getIsSplitSku());
            splitSkuDTO.setOriginalSkuNo(biOrderItemSplitEntity.getOriginalSkuNo());
            splitSkuDTO.setSkuNo(biOrderItemSplitEntity.getSkuNo());
            splitSkuDTO.setQuantity(biOrderItemSplitEntity.getQuantity());
            if (biOrderItemSplitEntity.getIsGift() != null) {
                splitSkuDTO.setIsGift(biOrderItemSplitEntity.getIsGift());
            } else {
                splitSkuDTO.setIsGift(2);
            }
            splitSkuDTO.setAmountAfter(biOrderItemSplitEntity.getAmountAfter());
            //拆单
            ...
            biOrderItemService.saveOrUpdate(originalItem);
            for (SplitSkuDTO skuDTO : splitSkuDTOS) {
                BiOrderItemSplitEntity entity = new BiOrderItemSplitEntity();
                BeanMapper.copy(biOrderItemSplitEntity, entity);
    ...
    public List<SplitSkuDTO> splitSku(SplitSkuDTO splitSkuDTO, List<DmpBomEntity> machining, List<BomChildrenSkuDTO> allBomList) {
        ...
        if (PlatformEnum.MABANG.getDesc().equals(splitSkuDTO.getPlatformSign())) {
            ...
            if (StringUtils.isBlank(dmpBomEntity.getId())) {
                ...
                errorLogEntity.setMsg(String.format(ApiError.BOM_FOR_MABANG_EXIST.getMsg(), skuNo));
            ...
            if (StringUtils.isBlank(dmpBomEntity.getFinancialCode())) {
                ...
                errorLogEntity.setSkuNo(skuNo);
                errorLogEntity.setMsg(String.format(ApiError.PRODUCT_SKU_FIN_CODE_NOT_FOUND.getMsg(), skuNo));
                dmpSplitErrorLogService.save(errorLogEntity);
            }
    ...
    public Boolean deleteByOrderIds(List<String> orderIds) {
        ...
        return lambdaUpdate().in(BiOrderItemSplitEntity::getOrderId, orderIds).remove();
    }
    ...
    private SplitSkuDTO handleNewSplitSku (SplitSkuDTO splitSkuDTO, String skuNo,List<DmpSplitErrorLogEntity> errorList,BomChildrenSkuDTO bomChildrenSkuDTO) {
        ...
        if (ObjectUtil.isEmpty(dmpSkuCostEntity)) {
            ...
            errorLogEntity.setFinancialCode("");
            errorLogEntity.setSkuNo(skuNo);
            errorLogEntity.setMsg(String.format(ApiError.PRODUCT_SKU_NOT_COST.getMsg(),skuNo));
