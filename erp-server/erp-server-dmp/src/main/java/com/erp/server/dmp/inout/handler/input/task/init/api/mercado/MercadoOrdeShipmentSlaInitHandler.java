...
@Service
@Scope("prototype")
public class MercadoOrdeShipmentSlaInitHandler extends DmpInputInitHandler {
	@Resource
	...
	public List<DmpInputTaskInitDTO> getInitData(DmpInputInitRequest dmpRequest, DmpInputTaskResponse dmpResponse) {
		...
		for (Map<String, Object> findMongoDatum : findMongoData) {
			...
			if (!Objects.equals(shipmentResult.getCode(), 200) && !Objects.equals(shipmentResult.getCode(), 201)) {
				log.error("调用url={},入参params={}, 美客多sla数据失败，返回值 responseMap={}", url + path, orderParams.toString(), JSONUtil.toJsonStr(shipmentResult));
				throw new RuntimeException(StrUtil.format("调用url={},入参params={}, sla请求失败，返回值 responseMap={}",
			...
			try {
				...
				e.printStackTrace();
				log.error("美客多sla接口数据解析错误，数据={}", shipmentResult.getData());
				throw new RuntimeException(StrUtil.format("调用url={},入参params={}, sla数据解析失败，返回值 responseMap={}",
						url + path, orderParams.toString(), JSONUtil.toJsonStr(shipmentResult)));
			}
