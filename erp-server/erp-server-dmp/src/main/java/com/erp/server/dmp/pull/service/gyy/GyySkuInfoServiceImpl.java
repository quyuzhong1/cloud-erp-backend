...
//        }
    ...
    public void analysisOrder(GyySkuInfoEntity gyySkuInfoEntity){
        List<BiSkuInfoEntity> biSkuInfoEntitylist = new ArrayList<>();
        ...
        if (combineItems.size() > 0) {
            for (CombineItemsBean combineItem : combineItems) {
                ...
                //商品编码
                biSkuInfoEntity.setItemCode(combineItem.getItemCode());

                //sku编号
                biSkuInfoEntity.setSkuNo(combineItem.getItemCode());

                //中文名
                biSkuInfoEntity.setNameCn(combineItem.getItemName());

                //英文名
                biSkuInfoEntity.setNameEn("");

                //统一成本价
                biSkuInfoEntity.setDefaultCost(gyySkuInfoEntity.getCostPrice());

                //商品状态:1.自动创建;2.待开发;3.正常;4.清仓;5.停止销售
                biSkuInfoEntity.setStatus(0);

                //商品创建时间
                if (StringUtils.isNotBlank(gyySkuInfoEntity.getCreateDate())) {
                    biSkuInfoEntity.setSkuCreateTime(LocalDateTime.parse(gyySkuInfoEntity.getCreateDate(),sdf));
                }

                //商品修改时间
                if (StringUtils.isNotBlank(gyySkuInfoEntity.getModifyDate())) {
                    biSkuInfoEntity.setSkuUpdateTime(LocalDateTime.parse(gyySkuInfoEntity.getModifyDate(), sdf));
                }

                //品牌
                biSkuInfoEntity.setBrandName(gyySkuInfoEntity.getItemBrandName());
                ...
                //售价
                biSkuInfoEntity.setSalePrice(gyySkuInfoEntity.getSalesPrice());

                //申报价格
                biSkuInfoEntity.setDeclarePrice(new BigDecimal(BigInteger.ZERO));
                ...
                //平台标识
                biSkuInfoEntity.setPlatformSign(PlatformEnum.GYY.getDesc());

                biSkuInfoEntity.setCreateTime(LocalDateTime.now());
                biSkuInfoEntitylist.add(biSkuInfoEntity);
            }

        } else {
            BiSkuInfoEntity biSkuInfoEntity = new BiSkuInfoEntity();
            //sku编号
            biSkuInfoEntity.setSkuNo(gyySkuInfoEntity.getCode());

            //中文名
            biSkuInfoEntity.setNameCn(gyySkuInfoEntity.getName());
        ...
        for (BiSkuInfoEntity biSkuInfoEntity : biSkuInfoEntitylist) {
            biSkuInfoService.checkOrder(biSkuInfoEntity);
        }
    }
