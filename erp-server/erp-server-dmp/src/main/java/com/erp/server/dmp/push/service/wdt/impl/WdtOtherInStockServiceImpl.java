...
public class WdtOtherInStockServiceImpl implements WdtOtherInStockService {
    ...
    public void executeConsumer(CreateOtherStockinRequest stockinRequest) {
        ...
        try {
            ...
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单异常: %s", message));
        }
        if (response.getStatus() != 0) {
            log.error("旺店通其他出库单推送失败，request：{}， response：{}", stockinRequest, response);
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单失败: %s, %s, %s", stockinRequest.getOuterNo(), response.getStatus(), response.getMessage()));
        }
        if (null != response.getData() && null != response.getData().getStatus() && 0 != response.getData().getStatus()) {
            ...
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单审核失败: %s, %s, %s", stockinRequest.getOuterNo(), response.getStatus(), response.getMessage()));
        }
    }
    ...
    public void executeSelfConsumer(CreateOtherStockinRequest request) {
        ...
        try {
            ...
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单失败: %s", e.getMessage()));
        }
        if (response.getStatus() != 0) {
            log.error("推送旺店通其他出库单失败:{}", response.getMessage());
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单失败: %s", response.getMessage()));
        }
        Map<String, Object> data = response.getData();
        if(ObjectUtils.isNotEmpty(data) && !data.get("status").equals("0")) {
            log.error("推送旺店通其他出库单异常:{}", data.get("message"));
            throw new ServiceException(ApiError.ERROR_WDT_API.getCode(), String.format("推送旺店通其他入库单异常: %s", data.get("message")));
        }
    }
