...
@Service
@Scope("prototype")
public class WeiShiReturnInstockRocketMQTaskHandler extends DmpOutputRocketMQTaskHandler {
    ...
    public PlatformReturnInstockDTO convert(DmpThirdReturnInboundEntity dmpMainEntity, List<DmpThirdReturnInboundDetailEntity> dmpDetailList, String cfgOutputId) {
        ...
        String returnType = dmpMainEntity.getReturnType();
        // T状态转换
        String erpReturnType = WeiShiEnums.ReturnInstockTypeEnum.getErpStatus(returnType);
        if (StringUtils.isBlank(erpReturnType)) {
            return null;
        }
        String status = dmpMainEntity.getStatus();
        ...
        dto.setReturnType(erpReturnType);
        ...
        if (CollectionUtils.isEmpty(filterDmpDetaiList)){
            log.warn("纬狮退货找不到上架数量大于0的明细: {}", JSONUtil.toJsonStr(dmpMainEntity));
            return null;
        }
        // 明细
