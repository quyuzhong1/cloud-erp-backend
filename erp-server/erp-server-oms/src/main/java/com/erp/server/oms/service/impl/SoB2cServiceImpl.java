...
public class SoB2cServiceImpl extends SuperServiceImpl<SoB2cMapper, SoB2cEntity> implements SoB2cService {
    ...
    private static final int EARLY_PERSIST_RETRY_COUNT = 2;
    ...
    // 新增方法：处理 fullyManagedPaging 的逻辑
    private IPage<SoB2cDTO.ListDTO> handleFullyManagedPaging(Page query, PagingParamDTO params) {
    ...
    public PagingVO<SoB2cDTO.ListDTO> paging(PagingDTO<SoB2cDTO.PagingParamDTO> pagingParamDTO) {
        ...
        if (Objects.nonNull(isOutStock)) {
            ...
            Page<SoB2cDTO.ListDTO> query = new Page<>(pagingParamDTO.getCurrPage(), pagingParamDTO.getPageSize());
            IPage<SoB2cDTO.ListDTO> pageData = handlePaging(query, params, dynamicDataSource, secondQuery);
            fillList(pageData.getRecords(), false);
    ...
    private PagingVO filterIsVirtualOutStockList(PagingDTO<SoB2cDTO.PagingParamDTO> pagingParamDTO, Boolean isVirtualOutStock) {
        ...
        IPage<SoB2cDTO.ListDTO> pageData = this.baseMapper.paging(query, pagingParamDTO.getParams(), Boolean.TRUE);
        if (CollUtil.isEmpty(pageData.getRecords())) {
        ...
        if (isVirtualOutStock) {
            ...
            list = list.stream().filter(v -> CollUtil.isNotEmpty(v.getDetailList())).collect(Collectors.toList());
        } else {
            for (SoB2cDTO.ListDTO listDTO : list) {
                listDTO.setDetailList(listDTO.getDetailList().stream().filter(v -> v.getDetailLabelDTO().getIsVirtualOutStock() == null || !v.getDetailLabelDTO().getIsVirtualOutStock()).collect(Collectors.toList()));
            }
        ...
        list = CollectionUtils.paginateList(list, pagingParamDTO.getPageSize(), pagingParamDTO.getCurrPage());
    ...
    public List<SoB2cDTO.TabListDTO> tabList(PermissionsDTO param) {
        ...
        String tabSql = soB2cQueryHandler.getTabSql(SoB2cTabEnum.ENUM_ALL.getCode());
        HashMap<String, String> map = new HashMap<>();
        map.put("default", tabSql);
        searchParamDTO.setSqlMap(map);
        List<ListCountDto> listCount = this.baseMapper.listCount(searchParamDTO);

        for (SoB2cTabEnum item : values) {
            SoB2cDTO.TabListDTO resultDTO = new SoB2cDTO.TabListDTO();
            Integer count = filterAndCountList(listCount, item);
    ...
    private Integer filterAndCountList(List<ListCountDto> listCount, SoB2cTabEnum item) {
        return listCount.stream().filter(l -> {
            switch (item) {
                case ENUM_ALL:
                    return true;
                case ENUM_PAYMENT:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAYMENT.getCode()) && !l.getInvalidStatus();
                case ENUM_PENDING:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            && (l.getApproveStatus().equals(ApproveStatusEnum.WAIT_SUBMIT.getStatus())
                            || l.getApproveStatus().equals(ApproveStatusEnum.REJECT.getStatus()))
                            && !l.getInvalidStatus() && !l.getIsFrozen() && !l.getBillStatus().equals("frozen");
                case ENUM_APPROVE_ING:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            && l.getApproveStatus().equals(ApproveStatusEnum.APPROVE_ING.getStatus())
                            && !l.getInvalidStatus() && !l.getIsFrozen() && !l.getBillStatus().equals("frozen");
                case ENUM_IN_DISTRIBUTION:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            && l.getApproveStatus().equals(ApproveStatusEnum.APPROVE.getStatus())
                            && (l.getBillStatus().equals(SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode())
                            || l.getBillStatus().equals(SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode()))
                            && !l.getInvalidStatus();
                case ENUM_WAIT_SHIPPED:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            && l.getApproveStatus().equals(ApproveStatusEnum.APPROVE.getStatus())
                ...
                case ENUM_FROZEN:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            ...
                            && !l.getInvalidStatus();
                case ENUM_INVALID:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode()) && l.getInvalidStatus();
                case ENUM_ORDER_ERROR:
                    return l.getPayStatus().equals(SoB2cPayStatusEnum.ENUM_PAID.getCode())
                            && org.apache.commons.lang3.StringUtils.isNotBlank(l.getSignOrderError())
                            && !l.getInvalidStatus();
                default:
                    return false;
            }
    ...
    public String processOrderCreation(SoB2cDTO.AddDTO dto) {
        ...
        SoB2cServiceImpl bean = ApplicationContextUtils.getBean(SoB2cServiceImpl.class);

        // 通过代理调用，确保 add() 方法上的独立事务生效
        SoB2cEntity add = bean.add(dto, null);
        String id = add.getId();
        //检查是否备案并修改状态
        ...
        SoB2cDTO.RuleResultDTO orderRuleResult = bean.orderRule(id);
        //匹配成功
        ...
        if (ruleMatch && isPass) {
            ...
            SoB2cDTO.RuleResultDTO warehouseRuleResult = bean.warehouseRule(orderRuleResult.getId(), orderRuleResult.getSoB2cDetailList(), orderRuleResult.getMap());
            Boolean warehouseRuleMatch = warehouseRuleResult.getIsRuleMatch();
            if (warehouseRuleMatch) {
                // 通过代理调用，确保 logisticsRule() 方法上的独立事务生效
                SoB2cDTO.RuleResultDTO logisticsRuleResult = bean.logisticsRule(id, new HashMap<>(), false);
                ...
                if (isRuleMatch) {
                    ...
                    bean.checkProductRegistrationAndUpdate(id, "");
                    //申报信息规则
                    bean.declareRule(id, new HashMap<>(), Boolean.FALSE, false);
                }
                soB2cRuleService.handleAutoLogisticsAction(id, logisticsRuleResult);
            }
        }
        //自动计算预估运费到订单的预估运费字段（异步）
        bean.autoCalcEstimatedShippingCost(Collections.singletonList(id));
        return add.getCode();
    }
    ...
    public SoB2cEntity add(SoB2cDTO.AddDTO addDTO, String code) {
        ...
        //新增订单分类
        if (CollUtil.isNotEmpty(addDTO.getCategoryIdList())) {
    ...
    private void calculateSizeByAdd(SoB2cLogisticsDTO.AddDTO logisticsDTO, List<SoB2cDetailDTO.AddDTO> detailList) {
        if (Objects.isNull(logisticsDTO) || CollUtil.isEmpty(detailList)) {
            return;
        }
    ...
    public List<SplitSkuDTO> getTransferDeclareProductBySoInfo(String soId) {
        ...
        if (CollUtil.isEmpty(soB2cDetailEntities)) {
            throw new ServiceException(ApiError.SO_B2C_DETAIL_NOT_FOUND);
        }
    ...
    private List<SplitSkuDTO> splitByBatchSoDetail(List<SoB2cDetailEntity> soB2cDetailEntities, List<SoB2cEntity> soB2cEntityList, List<SoB2cDeclareProductDTO.ViewDTO> declareProductList, boolean judgeCombinationFlag) {
        ...
        soB2cDetailEntities.forEach(detailEntity -> {
            SoB2cEntity entity = soB2cEntityList.stream().filter(e -> e.getId().equals(detailEntity.getMainId())).findFirst().orElse(null);
            queryDetailDTOList.add(InventorySkuCostDTO.QueryDetailDTO.builder()
                    ...
                    .build());
            List<BomChildrenSkuDTO> bomChildrenSkuDTOS1 = skuChildMap.get(detailEntity.getSkuId());
            if (CollUtil.isNotEmpty(bomChildrenSkuDTOS1)) {
                bomChildrenSkuDTOS1.forEach(bomChildrenSkuDTO -> {
                    queryDetailDTOList.add(InventorySkuCostDTO.QueryDetailDTO.builder()
                            .skuId(bomChildrenSkuDTO.getSkuId()).warehouseId(detailEntity.getWarehouseId()).shopId(entity.getShopId()).billDate(entity.getBillDate())
                            .build());
                });
            }
        });
        List<InventorySkuCostDTO.SkuCostDTO> skuCostDTOS = logisticsFeign.listSkuCostByDetailList(queryDetailDTOList);
        List<BomChildrenSkuDTO> finalBomChildrenSkuDTOS = bomChildrenSkuDTOS;
        soB2cDetailEntities.forEach(soB2cDetailEntity -> {
            BomChildrenSkuDTO skuDTO = finalBomChildrenSkuDTOS.stream().filter(e -> StrUtil.isNotEmpty(e.getParentSkuId())
            ...
            String warehouseId = soB2cDetailEntity.getWarehouseId();
            if (Objects.nonNull(skuDTO) && BomTypeEnum.COMBINATION.getType().equals(skuDTO.getType()) && (isCombination || !judgeCombinationFlag)) {
                if (StrUtil.isNotEmpty(soB2cDetailEntity.getSkuId()) && CollUtil.isNotEmpty(skuChildMap.get(soB2cDetailEntity.getSkuId()))) {
                    ...
                    BigDecimal totalPrice = bomChildrenSkuDTOS1.stream().map(e -> {
                        InventorySkuCostDTO.SkuCostDTO skuCostDTO = skuCostDTOS.stream().filter(f -> CharSequenceUtil.isNotBlank(warehouseId) && CharSequenceUtil.isNotBlank(e.getSkuId()) && f.getSkuId().equals(e.getSkuId()) && Objects.equals(f.getWarehouseId(), warehouseId)).findFirst().orElse(null);
                    ...
                    bomChildrenSkuDTOS1.forEach(bomChildrenSkuDTO -> {
                        ...
                        if (Objects.nonNull(viewDTO) && viewDTO.getPrice().compareTo(BigDecimal.ZERO) > 0) {
                            ...
                            InventorySkuCostDTO.SkuCostDTO skuCostDTO = skuCostDTOS.stream().filter(f -> CharSequenceUtil.isNotBlank(warehouseId) && CharSequenceUtil.isNotBlank(bomChildrenSkuDTO.getSkuId()) && f.getSkuId().equals(bomChildrenSkuDTO.getSkuId()) && Objects.equals(f.getWarehouseId(), warehouseId)).findFirst().orElse(null);
                            BigDecimal actualTaxCost = BigDecimal.ZERO;
                            if (Objects.nonNull(skuCostDTO)) {
                                BigDecimal productCost = Objects.nonNull(skuCostDTO.getProductCost()) ? skuCostDTO.getProductCost() : BigDecimal.ZERO;
                                BigDecimal firstMileShippingCost = Objects.nonNull(skuCostDTO.getFirstMileShippingCost()) ? skuCostDTO.getFirstMileShippingCost() : BigDecimal.ZERO;
                                BigDecimal clearanceCustomsTax = Objects.nonNull(skuCostDTO.getClearanceCustomsTax()) ? skuCostDTO.getClearanceCustomsTax() : BigDecimal.ZERO;
                                actualTaxCost = MathUtil.add(productCost, firstMileShippingCost).add(clearanceCustomsTax);
                            } else {
                                actualTaxCost = Objects.nonNull(bomProduct.getActualTaxCost()) ? bomProduct.getActualTaxCost() : BigDecimal.ZERO;
                            }
    ...
    public List<SplitSkuDTO> splitBySoDetail(List<SoB2cDetailEntity> soB2cDetailEntities, List<String> skuIds, String soCode, boolean judgeCombinationFlag) {
        ...
        soB2cDetailEntities.forEach(soB2cDetailEntity -> {
            ...
            if (Objects.nonNull(skuDTO) && BomTypeEnum.COMBINATION.getType().equals(skuDTO.getType()) && (isCombination || !judgeCombinationFlag)) {
                if (Objects.nonNull(skuChildMap) && StrUtil.isNotEmpty(soB2cDetailEntity.getSkuId()) && CollUtil.isNotEmpty(skuChildMap.get(soB2cDetailEntity.getSkuId()))) {
                    List<BomChildrenSkuDTO> bomChildrenSkuDTOS1 = skuChildMap.get(soB2cDetailEntity.getSkuId());
                    bomChildrenSkuDTOS1.forEach(bomChildrenSkuDTO -> {
                        LogisticsProductDTO.ProductDTO bomProduct = skuMap.get(bomChildrenSkuDTO.getSkuId());
                        splitSkuDTOS.add(SplitSkuDTO.builder()
...
//                        .currency(Objects.nonNull(productDTO) ? productDTO.getDestCurrency() : "")
    ...
    public BatchResultDTO approve(ApproveOneDTO dto, Boolean isMatch, String ruleName) {
        ...
        //校验是否被冻结
        if (entity.getIsFrozen()) {
            throw new ServiceException(ApiError.SO_INTERCEPTED_STATUS_NOT_UPDATE, entity.getCode());
        }
        //更换sku走审核流
...
//        syncSoB2cService.syncDataToSdy(entity, soB2cDetailEntityList, SyncOperateEnum.OPERATE_DELETE.getCode());
    ...
    public BatchResultDTO unInvalid(String id, SoB2cInvalidTypeEnum soB2cInvalidTypeEnum) {
        ...
        if (InvalidStatusEnum.NOT_VOIDED.getStatus().equals(entity.getInvalidStatus())) {
            throw new ServiceException(ApiError.SO_B2C_REVERSE_VOID_FORBIDDEN, entity.getCode());
        }
        if (!soB2cInvalidTypeEnum.getCode().equals(entity.getInvalidType())) {
            throw new ServiceException(ApiError.SO_B2C_CANCEL_NOT_SUPPORTED, entity.getCode(), soB2cInvalidTypeEnum.getName());
        }
...
//        }
    ...
    public BatchResultDTO updateRemark(String id, String remark) {
        ...
        if (InvalidStatusEnum.VOIDED.getStatus().equals(entity.getInvalidStatus())) {
            throw new ServiceException(ApiError.SO_B2C_UPDATE_REMARK_FORBIDDEN);
        }
        //未付款数据不能操作
        soB2cCoreService.checkPayMent(entity);
    ...
    public BatchResultDTO updateCategory(String id, SoB2cCategoryTypeEnum typeEnum, List<String> categoryIdList) {
        ...
        if (SoB2cBillStatusEnum.ENUM_FROZEN.getCode().equals(entity.getBillStatus()) || InvalidStatusEnum.VOIDED.getStatus().equals(entity.getInvalidStatus())) {
            throw new ServiceException(ApiError.SO_B2C_UPDATE_CATEGORY_FORBIDDEN);
        }
        //未付款数据不能操作
        soB2cCoreService.checkPayMent(entity);
        //原分类
    ...
    public List<SoB2cDTO.ViewSoB2cDistributionDTO> viewSoB2cDistribution(BaseIdsDTO.IdsDTO dto) {
        ...
        if (CollUtil.isEmpty(soB2cLogisticsList)) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_NOT_FOUND);
        }
    ...
    public BatchResultDTO saveSoB2cDistribution(String id, SoB2cDTO.SaveSoB2cDistributionDTO dto) {
        ...
        if (!SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode().equals(entity.getBillStatus())
            ...
            throw new ServiceException(ApiError.SO_B2C_DISTRIBUTION_STATUS_REQUIRED, entity.getCode());
        }
        //只有已审核数据支持配货
        ...
        if (CollUtil.isEmpty(detailList)) {
            throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "销售订单明细");
        }
        ...
        if (channelIds.size() > 1) {
            throw new ServiceException(ApiError.SO_B2C_MULTI_CHANNEL_FORBIDDEN);
        }
        //物流信息
        ...
        Boolean isRegistration = CollUtil.isEmpty(notRegistrationSkuNoList);
        //表示未备案
        if (!isRegistration) {
    ...
    private ServiceException createBlacklistException(LogisticsChannelEntity channel,
        ...
        return new ServiceException(ApiError.LOGISTICS_CHANNEL_BLACKLIST,
    ...
    private ServiceException createBlacklistCountryException(LogisticsChannelEntity channel,
                                                             String country) {
        return new ServiceException(ApiError.LOGISTICS_CHANNEL_COUNTRY_BLACKLIST,
    ...
    public BatchResultDTO getLogisticsCodeInner(String id, Boolean isDelivery) {
        ...
        if (!SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode().equals(entity.getBillStatus())) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_CODE_ONLY_DISTRIBUTION, entity.getCode());
        }
        //只有已审核数据支持配货
        if (!ApproveStatusEnum.APPROVE.equals(entity.getApproveStatus())) {
            throw new ServiceException(ApiError.SO_B2C_NOT_APPROVED_DISTRIBUTION_FORBIDDEN, entity.getCode());
        ...
        if (StringUtils.isNotBlank(soB2cLogisticsEntity.getCode())) {
            ...
            if (!cancelResult.isSuccess() && cancelResult.getCode() != -1) {
                ...
                log.error("{}原因是：{}", ApiError.LOGISTICS_CANCEL_FAILED.getMsg(), cancelResult.getMsg());
                throw new ServiceException(ApiError.LOGISTICS_CANCEL_FAILED, entity.getCode());
            }
            //如果取消物流单 则需要清空物流单信息
        ...
        if (CollUtil.isEmpty(declareList) && !isFullyManagedOrder(entity.getDictPlatform())) {
    ...
    private LogisticsBillDTO.GenerateBillDTO makeGenerateBillDTO(SoB2cEntity entity, SoB2cLogisticsEntity soB2cLogisticsEntity) {
        ...
        if (Objects.isNull(logisticsChannel)) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_METHOD_NOT_FOUND);
        }
        ...
        if (!isFullyManagedOrder(entity.getDictPlatform())) {
            ...
            //获取申报信息
            List<SoB2cDeclareProductEntity> declareList = soB2cDeclareProductService.listBySoId(id);
            if (CollUtil.isEmpty(declareList)) {
                //申报信息不能为空
                throw new ServiceException(ApiError.SO_B2C_DECLARE_INFO_NOT_FOUND, entity.getCode());
            }
            ...
            if (CollUtil.isEmpty(productVOS2)) {
                throw new ServiceException(ApiError.SO_DETAIL_SKU_ALL_EMPTY_FORBIDDEN, entity.getCode());
            }
...
//        checkDeclareInfo(productVOS,entity);
    ...
    private void checkDeclareInfo(List<LogisticsProductVO> productVOS, SoB2cEntity entity) {
        ...
        productVOS.forEach(logisticsProductVO -> {
            if (StrUtil.isEmpty(logisticsProductVO.getSkuNo())) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_SKU_NOT_FOUND, entity.getCode());
            }
            if (StrUtil.isEmpty(logisticsProductVO.getDeclareChineseName())) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_CN_NAME_REQUIRED, logisticsProductVO.getSkuNo());
            }
            if (StrUtil.isEmpty(logisticsProductVO.getDeclareEnglishName())) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_EN_NAME_REQUIRED, logisticsProductVO.getSkuNo());
            }
            if (Objects.isNull(logisticsProductVO.getDestDeclarePrice()) || logisticsProductVO.getDestDeclarePrice().compareTo(BigDecimal.ZERO) < 1) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_PRICE_REQUIRED, logisticsProductVO.getSkuNo());
            }
            if (StrUtil.isEmpty(logisticsProductVO.getDestCurrency())) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_CURRENCY_REQUIRED, logisticsProductVO.getSkuNo());
            }
            if (StrUtil.isEmpty(logisticsProductVO.getDestCurrencySymbol())) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_CURRENCY_SYMBOL_REQUIRED, logisticsProductVO.getSkuNo());
            }
            if (Objects.isNull(logisticsProductVO.getWeight()) || logisticsProductVO.getWeight() <= 0) {
                throw new ServiceException(ApiError.LOGISTICS_DECLARE_WEIGHT_REQUIRED, logisticsProductVO.getSkuNo());
            }
        });
    }

    @Override
    @DistributeLocker(businessType = DistributeKeyConstant.SO_B2C_ORDER_KEY, keyName = "id", waiteTime = 60)
    ...
    public BatchResultDTO submitDelivery(String id, String channelId) {
        ...
        if (ObjectUtils.isEmpty(entity)) {
            ...
            return BatchResultDTO.fail(id, entity.getCode(), ApiError.SO_B2C_NOT_FOUND.getMsg());
        }
        //已作废订单不能提交发货
        if (Boolean.TRUE.equals(entity.getInvalidStatus())) {
            soB2cErrorService.removeErrorOrder(id, SoB2cErrorTypeEnum.SUBMIT_DELIVERY.getCode());
            return BatchResultDTO.success(id, entity.getCode(), CharSequenceUtil.format(ApiError.SO_B2C_ORDER_VOIDED.getMsg(), entity.getCode()));
        }
        //全托管并且是平台订单就进行校验状态
            ...
            if (CollUtil.isNotEmpty(stockupOrdersDTOS)) {
                ...
                if (CharSequenceUtil.isNotBlank(status)) {
                    ...
                    if (FullyManagedPlatformStatusEnum.TikTokStatusEnum.INVAILD.getCode().equals(status)) {
                        throw new ServiceException(ApiError.SO_B2C_PLATFORM_ORDER_VOIDED, entity.getCode());
                    }
                }
            }
        }
        ...
        if (!ApproveStatusEnum.APPROVE.equals(approveStatusEnum)) {
            throw new ServiceException(ApiError.SO_B2C_APPROVED_REQUIRED_FOR_DELIVERY, entity.getCode());
        }
        ...
        if (CollUtil.isNotEmpty(warehouseIdList)) {
            throw new ServiceException(ApiError.SO_DELIVERY_WAREHOUSE_NOT_FOUND);
        }
        //发货仓库id 集合
        ...
        if (deliveryWarehouseIdList.size() > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_DELIVERY_WAREHOUSE_CONFLICT, soCode);
        }
        ...
        if (CollUtil.isEmpty(deliveryWarehouseList)) {
            throw new ServiceException(ApiError.SO_DELIVERY_WAREHOUSE_NOT_FOUND);
        }
        //仓库经营类型
        ...
        Boolean isApi = CollUtil.isNotEmpty(overseasWarehouseList);
        //必须要有物流渠道，没有物流单号可以提交发货
        if (isApi) {
            if (StringUtils.isBlank(logisticsChannelId)) {
                throw new ServiceException(ApiError.SO_B2C_LOGISTICS_CHANNEL_REQUIRED, soCode);
            }
        } else {
    ...
    private void checkInventory(SoB2cEntity entity, List<SoB2cDetailEntity> list, List<String> deliveryWarehouseIdList, String warehouseManageType) {
        ...
        Map<String, Integer> inventoryMap = inventoryList.stream()
                .collect(Collectors.toMap(
                        obj -> obj.getSkuId() + "-" + obj.getWarehouseId(),
                        InventoryQtyDTO.SkuInventoryTotalDTO::getInventoryTotal,
                        Integer::sum
                ));
        for (SoB2cDeliveryDTO.DeliverySkuDTO deliverySkuDTO : deliverySkuList) {
            ...
            WarehouseDTO.UpdateDTO warehouseInfo = CollUtil.isNotEmpty(warehouseInfoList) ? warehouseInfoList.get(0) : null;
            ...
            //验证是否存在可用库存
            Integer usableQty = inventoryMap.getOrDefault(deliverySkuDTO.getSkuId() + "-" + soB2cDetailEntity.getWarehouseId(), MathUtil.ZERO);
            ...
            if (MathUtil.compareTo(deliveryQty, usableQty) > MathUtil.ZERO) {
                throw new ServiceException(ApiError.SO_B2C_SKU_INVENTORY_NOT_FOUND, entity.getCode(), soB2cDetailEntity.getSkuNo(), soB2cDetailEntity.getWarehouseName());
            }
        }
    }
    ...
    public List<SoB2cDeliveryDTO.DeliverySkuDTO> listDeliverySku(String shopId, List<SoB2cDetailEntity> soDetailList, String platform, String warehouseManageType, Boolean distinctFlag) {
        ...
        ListingInfoParamDTO param = buildListingInfoParam(shopId, platform, skuIds, platformNos);
        Boolean isSelfBuild = WarehouseManageTypeEnum.SELF_BUILD.getCode().equals(warehouseManageType);
    ...
    private ListingInfoParamDTO buildListingInfoParam(String shopId, String platform, List<String> skuIds, List<String> platformNos) {
        ...
        param.setShopIdList(Collections.singletonList(shopId));
    ...
    private void processSkuMapping(List<SoB2cDetailEntity> soDetailList, List<ListingInfoWithSkuMappingDTO> skuMappingList, String warehouseManageType, Boolean isSelfBuild, List<String> wantSplitSkuIdList, List<String> wantSplitSkuIdAndPlatformList) {
        ...
        for (SoB2cDetailEntity soB2cDetailEntity : soDetailList) {
            ...
            ListingInfoWithSkuMappingDTO skuMappingDTO = skuMappingList.stream()
                    .filter(s -> skuId.equals(s.getProductSkuId()) && soB2cDetailEntity.getPlatformSkuNo().equals(s.getPlatformSkuNo())
    ...
    private void populateResultList(List<SoB2cDetailEntity> soDetailList, List<String> wantSplitSkuIdAndPlatformList, List<BomChildrenSkuDTO> bomChildrenSkuList, List<SoB2cDeliveryDTO.DeliverySkuDTO> resultList) {
        ...
        for (SoB2cDetailEntity detailEntity : soDetailList) {
            ...
            if (wantSplitSkuIdAndPlatformList.contains(skuId + platformSkuNo)) {
                List<BomChildrenSkuDTO> bomSkuList = bomChildrenSkuList.stream()
                        .filter(req -> req.getParentSkuId().equals(skuId) && combinationType.equals(req.getType()))
                        .collect(Collectors.toList());

                if (CollUtil.isNotEmpty(bomSkuList)) {
                    addBomSkuDetails(bomSkuList, detailEntity, resultList);
                } else {
                    addDefaultSkuDetail(detailEntity, resultList);
                }
            } else {
                addDefaultSkuDetail(detailEntity, resultList);
            }
    ...
    private void addBomSkuDetails(List<BomChildrenSkuDTO> bomSkuList, SoB2cDetailEntity detailEntity, List<SoB2cDeliveryDTO.DeliverySkuDTO> resultList) {
        BigDecimal price = Objects.nonNull(detailEntity.getPrice()) ? detailEntity.getPrice() : BigDecimal.ZERO;
        int total = bomSkuList.stream().mapToInt(BomChildrenSkuDTO::getQuantity).sum();
        for (BomChildrenSkuDTO bomSku : bomSkuList) {
            ...
            deliverySku.setQty(bomSku.getQuantity());
            deliverySku.setPrice(price.multiply(BigDecimal.valueOf(bomSku.getQuantity())).divide(BigDecimal.valueOf(total), 2, RoundingMode.HALF_UP));
            setCommonSkuDetails(deliverySku, detailEntity);
            resultList.add(deliverySku);
        }
    }

    private void addDefaultSkuDetail(SoB2cDetailEntity detailEntity, List<SoB2cDeliveryDTO.DeliverySkuDTO> resultList) {
        SoB2cDeliveryDTO.DeliverySkuDTO deliverySku = new SoB2cDeliveryDTO.DeliverySkuDTO();
        deliverySku.setSkuId(detailEntity.getSkuId());
        deliverySku.setSkuNo(detailEntity.getSkuNo());
        deliverySku.setPlatformSkuNo(detailEntity.getPlatformSkuNo());
        deliverySku.setQty(1);
        deliverySku.setPrice(detailEntity.getPrice());
        setCommonSkuDetails(deliverySku, detailEntity);
        resultList.add(deliverySku);
    }

    private void setCommonSkuDetails(SoB2cDeliveryDTO.DeliverySkuDTO deliverySku, SoB2cDetailEntity detailEntity) {
    ...
    private void fillMissingSkuNos(List<SoB2cDeliveryDTO.DeliverySkuDTO> resultList) {
        List<SoB2cDeliveryDTO.DeliverySkuDTO> noSkuNOSkuList = resultList.stream()
                .filter(s -> StringUtils.isBlank(s.getSkuNo()))
                .collect(Collectors.toList());

        List<String> noSkuNOSkuIdList = noSkuNOSkuList.stream()
                .map(SoB2cDeliveryDTO.DeliverySkuDTO::getSkuId)
                .collect(Collectors.toList());
        ...
        for (SoB2cDeliveryDTO.DeliverySkuDTO item : resultList) {
            if (StringUtils.isBlank(item.getSkuNo())) {
                String wantSkuNo = detailList.stream()
                        .filter(s -> s.getId().equals(item.getSkuId()))
                        .map(ProductDetailEntity::getSkuNo)
                        .findFirst()
                        .orElse("");
     ...
     * @param newChannelId
     */
    @DistributeLocker(businessType = DistributeKeyConstant.SO_B2C_THIRD_DELIVERY_ORDER_KEY, keyName = "entity.id", waiteTime = 60)
    public void thirdWarehouseCreateOutStock(SoB2cEntity entity, String warehouseId, String warehouseManageType, SoB2cLogisticsEntity logisticsEntity, OverseasProviderWarehouseDTO.ViewDTO overseasProviderWarehouse, List<SoB2cDetailEntity> detailList, String newChannelId) {
        ...
        if (Objects.nonNull(channelEntity.getIsPushLabel()) && channelEntity.getIsPushLabel()) {
            ...
            if (CollUtil.isEmpty(soB2cLabelEntityList)) {
        ...
        //国家三字码赋值
        if(CharSequenceUtil.isNotBlank(receiver.getCountry())){
        ...
        for (SoB2cDeliveryDTO.DeliverySkuDTO deliverySkuDTO : wantSkuList) {
            ...
            outboundReqItem.setSkuId(deliverySkuDTO.getSkuId());
            outboundReqItem.setSkuNo(deliverySkuDTO.getSkuNo());
            outboundReqItem.setPrice(deliverySkuDTO.getPrice());
            outboundReqItem.setSourceSkuId(deliverySkuDTO.getSourceSkuId());
            outboundReqItem.setSourceSkuNo(deliverySkuDTO.getSourceSkuNo());
            outboundReqItem.setPlatformDetailId(deliverySkuDTO.getPlatformDetailId());
            outboundReqItem.setDetailId(deliverySkuDTO.getDetailId());
        ...
        createOutboundReq.setPlanPackageNo(logisticsEntity.getPlanPackageNo());
        createOutboundReq.setPlanSupplierId(logisticsEntity.getPlanSupplierId());
        //通过订单处理规则处理参数
    ...
    public ApiResult<ThirdWarehouseQueryOutboundResponse> createThirdWarehouseOutbound(SoB2cEntity entity, String warehouseId, ThirdWarehouseCreateOutboundReq createOutboundReq, final int retryCount, ThirdWarehouseDeliveryEntity thirdWarehouseDeliveryEntity) {
        ...
        String queryReferenceNo = CharSequenceUtil.isNotBlank(createOutboundReq.getReferenceNo()) ? createOutboundReq.getReferenceNo() : Objects.nonNull(thirdWarehouseDeliveryEntity) ? thirdWarehouseDeliveryEntity.getCode() : null;
        ApiResult<ThirdWarehouseQueryOutboundResponse> queryOutboundResult = null;
        //如果已存在发货单，不处理
        if (CharSequenceUtil.isNotBlank(queryReferenceNo)) {
            if (CharSequenceUtil.isBlank(entity.getShippingOrderNo())) {
                ...
                queryOutboundReq.setErpOrderCode(queryReferenceNo);
                ...
                try {
                    queryOutboundResult = thirdWarehouseFeign.queryOutboundOrder(queryOutboundReq);
                    log.warn("查询三方仓发货单结果:{}", JSONUtil.toJsonStr(queryOutboundResult));
                    if (Objects.isNull(queryOutboundResult)) {
                    ...
                    if (shouldRetryCreateOutbound(queryOutboundResult)) {
                        throw new ServiceException("查询三方仓出库单异常，{}", queryOutboundResult.getMsg());
                    }
                } catch (Exception e) {
                    if (isRetryableThirdWarehouseTimeoutMessage(e.getMessage())) {
                        return retryCreateThirdWarehouseOutbound(entity, warehouseId, createOutboundReq, retryCount, thirdWarehouseDeliveryEntity, queryOutboundResult, type, e);
                    }
                    throw new ServiceException("查询三方仓出库单异常，{}", e.getMessage());
                }
            } else {
        ...
        try {
            if (Objects.nonNull(thirdWarehouseDeliveryEntity)) {
                ...
                createOutboundReq.setReferenceNo(thirdWarehouseDeliveryEntity.getCode());
            } else if (CharSequenceUtil.isBlank(createOutboundReq.getReferenceNo())) {
            ...
            addModuleOperateLogRequiresNew(CharSequenceUtil.format("B2C销售订单【{}】创建三方仓发货单失败，原因：{}", entity.getCode(), e.getMessage()), entity.getId(), "创建三方仓发货单失败");
        ...
        try {
            ...
            if (Objects.isNull(apiResult)) {
                throw new ServiceException("调用三方仓出库单返回为空");
            }
            if (!apiResult.isSuccess()) {
                if (shouldRetryCreateOutbound(apiResult)) {
            ...
            return retryCreateThirdWarehouseOutbound(entity, warehouseId, createOutboundReq, retryCount, thirdWarehouseDeliveryEntity, apiResult, type, e);
        }
    }

    /**
     * 建单成功后提前落库 shippingOrderNo。
     * 由于建单前存在较长耗时的远程调用，订单在此期间可能已被其他动作更新（version 已变化），
     * 若仍用调用前读到的旧 version 做乐观锁条件，更新必然失败。
     * 因此每次尝试前都重新查询最新记录以获取最新 version，缩小乐观锁冲突窗口；
     * 重试仍失败时仅 warn，由外层 thirdWarehouseCreateOutStock 的无条件更新兜底。
     */
    private void persistEarlyShippingOrderNoWithRetry(SoB2cEntity entity, String earlyShippingOrderNo) {
        for (int attempt = 1; attempt <= EARLY_PERSIST_RETRY_COUNT; attempt++) {
            SoB2cEntity latest = soB2cService.getById(entity.getId());
            if (Objects.isNull(latest)) {
                log.warn("三方仓建单成功，提前落库 shippingOrderNo 时订单不存在，soCode={}", entity.getCode());
                return;
            }
            if (CharSequenceUtil.isNotBlank(latest.getShippingOrderNo())) {
                entity.setShippingOrderNo(latest.getShippingOrderNo());
            ...
            boolean updated = this.lambdaUpdate()
                    ...
                    .eq(SoB2cEntity::getVersion, latest.getVersion())
                    .update();
            if (updated) {
                entity.setShippingOrderNo(earlyShippingOrderNo);
                entity.setVersion(latest.getVersion() + 1);
                log.info("三方仓建单成功，提前落库 shippingOrderNo={}, soCode={}，第{}次尝试成功", earlyShippingOrderNo, entity.getCode(), attempt);
                return;
            }
            log.warn("三方仓建单成功，提前落库 shippingOrderNo 因版本冲突失败（第{}次尝试），soCode={}", attempt, entity.getCode());
        }
        log.warn("三方仓建单成功，提前落库 shippingOrderNo 重试{}次后仍失败，交由外层最终更新兜底，soCode={}", EARLY_PERSIST_RETRY_COUNT, entity.getCode());
    }

    private ApiResult<ThirdWarehouseQueryOutboundResponse> retryCreateThirdWarehouseOutbound(SoB2cEntity entity, String warehouseId, ThirdWarehouseCreateOutboundReq createOutboundReq, final int retryCount, ThirdWarehouseDeliveryEntity thirdWarehouseDeliveryEntity, ApiResult<ThirdWarehouseQueryOutboundResponse> apiResult, String type, Exception e) {
        log.warn("订单{}三方仓发货单号{}第{}次执行失败: {}", entity.getCode(), createOutboundReq.getReferenceNo(), retryCount + 1, e.getMessage());
        if (retryCount + 1 < MAX_RETRY_COUNT) {
            try {
                ...
                addModuleOperateLogRequiresNew(CharSequenceUtil.format("B2C销售订单【{}】三方仓发货失败，三方仓发货单【{}】，原因：{}，进行第{}次重试", entity.getCode(), createOutboundReq.getReferenceNo(), e.getMessage(), retryCount + 2), entity.getId(), "三方仓发货重试");
    ...
    private void addModuleOperateLogRequiresNew(String content, String businessId, String operation) {
        try {
            operateLogService.addModuleOperateLogRequiresNew(content, ModuleTypeEnum.SO_B2C.getCode(), businessId, operation);
        } catch (Exception logEx) {
            log.warn("B2C销售订单操作日志写入失败，businessId:{}, operation:{}, content:{}", businessId, operation, content, logEx);
        }
    }

    private boolean shouldRetryCreateOutbound(ApiResult<ThirdWarehouseQueryOutboundResponse> apiResult) {
        if (Objects.isNull(apiResult)) {
            return false;
        }
        ...
        return Objects.equals(ApiError.WH_OVERSEAS_INTERFACE_EXCEPTION.getCode(), apiResult.getCode())
                && CharSequenceUtil.isNotBlank(apiResult.getMsg())
                && isRetryableThirdWarehouseTimeoutMessage(apiResult.getMsg());
    }

    private boolean isRetryableThirdWarehouseTimeoutMessage(String message) {
    ...
    private String getApiResultCode(ApiResult<?> apiResult) {
        ...
        return apiResult.getCode().toString();
    }
    ...
    public BatchResultDTO deliveryIntercept(String id, String remark) {
        ...
        //已打标拦截不支持重复提交
        if (entity.getIsIntercept()) {
            throw new ServiceException(ApiError.SO_DELIVERY_ALREADY_INTERCEPTED);
        }
        ...
        if (entity.hasPlatformWarehouseOrder()) {
            throw new ServiceException(ApiError.LOGISTICS_PLATFORM_WAREHOUSE_NOT_INTERCEPT);
        }
        ...
        if (!SoB2cBillStatusEnum.ENUM_WAIT_SHIPPED.getCode().equals(entity.getBillStatus())) {
            throw new ServiceException(ApiError.SO_DELIVERY_STATUS_REQUIRED_FOR_INTERCEPT);
        }
    ...
    public BatchResultDTO addIntercept(String remark, SoB2cEntity entity, SoB2cLogisticsEntity logisticsEntity) {
        ...
        if (StringUtils.isBlank(logisticsEntity.getLogisticsChannelId())) {
            return BatchResultDTO.fail(entity.getId(), entity.getCode(), ApiError.LOGISTICS_CHANNEL_NOT_FOUND.getMsg() + logisticsEntity.getLogisticsChannelId());
        }
        //检测是否是API 对接的仓库
    ...
    public BatchResultDTO cancelDeliveryIntercept(String id) {
        ...
        //未打标拦截的订单不支持取消拦截
        if (!entity.getIsIntercept()) {
            throw new ServiceException(ApiError.LOGISTICS_NOT_INTERCEPTED_CANNOT_CANCEL);
        }
        ...
        if (CollUtil.isEmpty(interceptList)) {
            throw new ServiceException(ApiError.LOGISTICS_INTERCEPT_STATUS_INVALID);
        }
        ...
        if (CollUtil.isNotEmpty(interceptStatusList)) {
    ...
    public Integer mergePagingCount(SoB2cDTO.MergePagingParamDTO pagingParamDTO) {
        ...
        return CollUtil.isEmpty(list) ? MathUtil.ZERO : list.stream().reduce(MathUtil.ZERO, Integer::sum);
    }
    ...
    public String mergeSave(List<String> ids) {
        ...
        for (SoB2cEntity entity : list) {
            ...
            if (entity.getIsNotMerge()) {
                throw new ServiceException(ApiError.SO_B2C_NOT_NEED_MERGE, entity.getCode());
            }
            ...
            if (count > 0) {
                throw new ServiceException(ApiError.SO_B2C_ALREADY_MERGED_OR_SPLIT);
            }
            //未付款数据不能操作
            soB2cCoreService.checkPayMent(entity);
            ...
            if ("AFN".equals(labelJsonDTO.getFulfillmentChannel())) {
                throw new ServiceException(ApiError.SO_B2C_MERGE_FBA_FORBIDDEN, entity.getCode());
            }
            //菜鸟官方仓订单不支持合并
            ...
            if (cainiaoCount > 0) {
                throw new ServiceException(ApiError.SO_B2C_MERGE_CAINIAO_FORBIDDEN, entity.getCode());
            }
            //速卖通非已税订单不支持合并
            ...
            if (taxCount > 0) {
                throw new ServiceException(ApiError.SO_B2C_MERGE_TAX_ORDER_FORBIDDEN, entity.getCode());
            }
            //shopee订单不支持合并
            if (PlatformDictEnum.SHOPEE.getCode().equals(entity.getDictPlatform())) {
                throw new ServiceException(ApiError.SO_B2C_SHOPEE_MERGE_FORBIDDEN, entity.getCode());
            }
            //美客多订单不支持合并
            if (PlatformDictEnum.MERCADOLIBRE.getCode().equals(entity.getDictPlatform())) {
                throw new ServiceException(ApiError.SO_B2C_MERCADO_MERGE_FORBIDDEN, entity.getCode());
            }
            //美客多订单不支持合并
            ...
            if (PlatformDictEnum.TIK_TOK.getCode().equals(entity.getDictPlatform())) {
                throw new ServiceException(ApiError.SO_B2C_TIKTOK_MERGE_FORBIDDEN, entity.getCode());
            }
        }
        ...
        if (CollUtil.isEmpty(soB2cReceiverList)) {
        ...
        if (ObjectUtils.isEmpty(rate) || MathUtil.compareTo(BigDecimal.ZERO, rate) == MathUtil.ZERO) {
            throw new ServiceException(ApiError.COMMON_EXCHANGE_RATE_NOT_EXIST, billDate, list.get(0).getCurrency());
        }
...
//        logisticsAddDTO.setWeight(weight);
        ...
        if (StringUtils.isNotBlank(platformCodeListStr) && platformCodeListStr.length() > 1024) {
            throw new ServiceException(ApiError.SO_PLATFORM_ORDER_MERGE_TOO_LONG);
        }
...
//        }
    ...
    public SoB2cDTO.ViewDTO view(String id) {
        ...
        if (StringUtils.isBlank(logisticsChannelName)) {
            logisticsChannelName = CollUtil.isNotEmpty(list) ? list.get(0).getCnName() : "";
        }
        ...
        if (CollUtil.isNotEmpty(soOutstockEntityList) && Objects.nonNull(soOutstockEntityList.get(0).getBillDate())) {
...
//        logisticsDTO.setActualShippingCost(logisticsBillCostFeign.getActualLogisticCost(soB2cEntity.getId()));
        ...
        List<SoB2cDeliveryEntity> soB2cDeliveryEntities = soB2cDeliveryFeign.listBySourceId(Collections.singletonList(id))
                ...
                .collect(Collectors.toList());
        ;
        if (CollUtil.isNotEmpty(soB2cDeliveryEntities)) {
        ...
        List<BomChildrenSkuDTO> finalBomChildrenList = bomChildrenList;
        detailList.forEach(v -> {
            //子级BOM
            List<BomChildrenSkuDTO> childList = finalBomChildrenList.stream()
            ...
            ListingInfoEntity listingInfoEntity = listingInfoEntityList.stream().filter(obj -> v.getSourcePlatform().equals(SoB2cSourcePlatformEnum.ENUM_THIRD_PLATFORM.getCode())
                    && obj.getPlatformSkuNo().equals(v.getPlatformSkuNo()) && obj.getPlatform().equals(data.getDictPlatform())
                    && obj.getPlatformSpuNo().equals(v.getPlatformSpuNo())).findFirst().orElse(null);
    ...
    private void fillOne(SoB2cDTO.ViewDTO data) {
        ...
        if (StringUtils.isNotBlank(data.getLogisticsDTO().getLogisticsChannelId())) {
            ...
            if (CollUtil.isNotEmpty(authChannelViewDTOList)) {
                authChannelViewDTO = authChannelViewDTOList.get(0);
            }
        }
        if (Objects.nonNull(authChannelViewDTO) && CollUtil.isNotEmpty(data.getDetailList())) {
            //仓库id
...
//            }
    ...
    private void fillList(List<SoB2cDTO.ListDTO> list, Boolean isFullyManaged) {
        ...
        List<String> ids = list.stream().map(SoB2cDTO.ListDTO::getId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        //发货单信息
        List<SoB2cDeliveryEntity> deliveryList = soB2cDeliveryFeign.listBySourceId(ids);
        Map<String, SoB2cDeliveryEntity> deliveryEntityMap = deliveryList.stream()
                .filter(e -> !SoB2cDeliveryStatusEnum.CANCEL_DELIVERY.getCode().equals(e.getStatus()))
                .collect(Collectors.toMap(SoB2cDeliveryEntity::getSourceId,Function.identity(),(e1, e2) -> e1.getCreateTime().isAfter(e2.getCreateTime()) ? e1 : e2));
        //买家信息
        List<SoB2cReceiverEntity> receiverList = soB2cReceiverService.listByMainIds(ids);
        Map<String, SoB2cReceiverEntity> receiverEntityMap = receiverList.stream().collect(Collectors.toMap(SoB2cReceiverEntity::getMainId, Function.identity()));
        //军区信息
        List<String> partitionIds = receiverList.stream().map(SoB2cReceiverEntity::getPartitionId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        List<DictPartitionEntity> partitionEntityList = CollUtil.isNotEmpty(partitionIds) ? sysPartitionFeign.listByPartitionIds(partitionIds) : Collections.emptyList();
        Map<String, String> partitionMap = partitionEntityList.stream().collect(Collectors.toMap(BaseEntity::getId, DictPartitionEntity::getName));
        //店铺
        List<String> shopIdList = list.stream().map(SoB2cDTO.ListDTO::getShopId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        List<ShopInfoEntity> shopInfoList = shopInfoService.lambdaQuery().select(ShopInfoEntity::getId, ShopInfoEntity::getId, ShopInfoEntity::getName).in(ShopInfoEntity::getId, shopIdList).list();
        Map<String, String> shopMap = CollUtil.isEmpty(shopInfoList) ? Collections.emptyMap() : shopInfoList.stream().collect(Collectors.toMap(ShopInfoEntity::getId, ShopInfoEntity::getName));
        // 国家信息
        List<String> countryList = receiverList.stream().map(SoB2cReceiverEntity::getCountry).filter(StringUtils::isNotBlank).distinct().collect(Collectors.toList());
        List<DictCountryEntity> countryEntityList = CollUtil.isNotEmpty(countryList) ? sysDictFeign.listCountryByIds(countryList) : Collections.emptyList();
        ...
        Map<String, LocalDateTime> requiredDeliveryTimeMap = CollUtil.isEmpty(extendEntityList) ? Collections.emptyMap() : extendEntityList.stream()
        ...
        Map<String, SoB2cDeclareProductDTO.ViewDTO> declareProductMap = CollUtil.isNotEmpty(declareProductList) ? declareProductList.stream()
        ...
        List<SkuVO> skuList = CollUtil.isNotEmpty(skuIdList) ? plmTaskFeign.listSkuLogisticsByIds(skuIdList) : Collections.emptyList();
        ...
        List<String> ignoreInventorySkuIds = CollUtil.isNotEmpty(ignoreInventorySkuList) ? ignoreInventorySkuList.stream().map(SkuVO::getSkuId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList()) : Collections.emptyList();

        //仓库id
        List<String> warehouseIdList = allDetailList.stream().map(SoB2cDetailEntity::getWarehouseId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        //获取第三方仓海外信息
        List<OverseasProviderWarehouseDTO.ViewDTO> overseasProviderWarehouseList = CollUtil.isNotEmpty(warehouseIdList) ? wmsOverseasWarehouseFeign.listByWarehouseIdList(warehouseIdList) : Collections.emptyList();
        Map<String, String> providerCodeMap = CollUtil.isNotEmpty(overseasProviderWarehouseList) ? overseasProviderWarehouseList.stream().collect(Collectors.toMap(OverseasProviderWarehouseDTO.ViewDTO::getWarehouseId, OverseasProviderWarehouseDTO.ViewDTO::getProviderCode)) : Collections.emptyMap();
        //查询库存信息
        ...
        skuInventoryDTO.setSkuIdList(skuIdList);
        List<InventoryQtyDTO.SkuInventoryStatusTotalDTO> inventoryList = CollUtil.isNotEmpty(warehouseIdList) ? inventoryFeign.listSkuInventoryStatusByParam(skuInventoryDTO) : Collections.emptyList();
        // 将 inventoryList 转换为 Map，键为 skuId + warehouseId + inventoryStatus，值为库存总数
        Map<String, Integer> inventoryMap = inventoryList.stream()
                .collect(Collectors.toMap(
        ...
        //虚拟仓库存
        List<String> virtualWarehouseIdList = allDetailList.stream().map(SoB2cDetailEntity::getVirtualWarehouseId).filter(CharSequenceUtil::isNotBlank).distinct().collect(Collectors.toList());
        ...
        List<VirtualInventoryDTO.VirtualInventoryQtyDTO> virtualInventoryList = CollUtil.isNotEmpty(warehouseIdList) && CollUtil.isNotEmpty(virtualWarehouseIdList) ? virtualInventoryFeign.listInventoryQty(paramDTO) : Collections.emptyList();
        ...
        //虚拟仓库信息
        List<VirtualWarehouseEntity> virtualWarehouseList = CollUtil.isNotEmpty(virtualWarehouseIdList) ? FeignQuery.getByIds(VirtualWarehouseEntity.class, virtualWarehouseIdList) : Collections.emptyList();
        Map<String, String> virtualWarehouseMap = virtualWarehouseList.stream().collect(Collectors.toMap(VirtualWarehouseEntity::getId, VirtualWarehouseEntity::getName));
        //B2C销售订单合并拆分关联表
        List<SoB2cRefEntity> soB2cRefList = soB2cRefService.listBySourceIdOrTargetId(ids);
        Map<String, List<SoB2cRefEntity>> refListMap = soB2cRefList.stream().collect(Collectors.groupingBy(SoB2cRefEntity::getTargetId));
        //物流信息
        List<SoB2cLogisticsEntity> logisticsEntityList = soB2cLogisticsService.listByMainIds(ids);
        Map<String, SoB2cLogisticsEntity> logisticsEntityMap = CollUtil.isNotEmpty(logisticsEntityList) ? logisticsEntityList.stream().collect(Collectors.toMap(SoB2cLogisticsEntity::getMainId, Function.identity())) : Collections.emptyMap();
        //渠道信息
        ...
        if (CollUtil.isNotEmpty(billVos)) {
        ...
        Map<String, SoB2cFinanceEntity> financeEntityMap = CollUtil.isNotEmpty(soB2cFinanceEntityList) ? soB2cFinanceEntityList.parallelStream().collect(Collectors.toMap(SoB2cFinanceEntity::getMainId, Function.identity())) : Collections.emptyMap();
        //SKU对照表信息
        List<SkuMappingDTO.ListSkuParamDTO> listParamList = allDetailList.stream().filter(obj -> CharSequenceUtil.isNotBlank(obj.getSkuNo())).map(obj -> new SkuMappingDTO.ListSkuParamDTO(obj.getSkuNo(), obj.getWarehouseId(), list.stream().filter(e -> e.getId().equals(obj.getMainId())).findFirst().flatMap(e -> Optional.ofNullable(e.getDictPlatform())).orElse(""))).collect(Collectors.toList());
        ...
        Map<String, TransferLogisticsChannelDTO.ListSelectDTO> transferMap = transferInfoList.stream().collect(Collectors.toMap(TransferLogisticsChannelDTO.ListSelectDTO::getId, Function.identity()));
        ...
        List<SoMultiChannelEntity> soMultiChannelEntityList = CollUtil.isNotEmpty(soIds) ? soMultiChannelService.getLastBySoId(soIds, CreateStatusEnum.FAILED.getCode()) : new ArrayList<>();
        Map<String, SoMultiChannelEntity> soMultiChannelEntityMap = soMultiChannelEntityList.stream()
                .collect(Collectors.toMap(SoMultiChannelEntity::getSoId, Function.identity(), (existing, replacement) ->
        ...
        Map<String, String> deliveryCodeMap = CollUtil.isNotEmpty(ids) ? soB2cDeliveryFeign.getDeliveryCodeBySourceId(ids) : new HashMap<>();
        ...
        for (SoB2cDTO.ListDTO data : list) {
            SoB2cDeliveryEntity soB2cDeliveryEntity = deliveryEntityMap.get(data.getId());
            if (Objects.nonNull(soB2cDeliveryEntity)) {
                data.setCreateDeliveryTime(soB2cDeliveryEntity.getDeliveryTime());
                data.setFinishPrintTime(soB2cDeliveryEntity.getFinishPrintTime());
            }
            SoB2cReceiverEntity soB2cReceiverEntity = receiverEntityMap.get(data.getId());
            if (Objects.nonNull(soB2cReceiverEntity)) {
                data.setBuyerId(soB2cReceiverEntity.getCustomerId());
                data.setBuyerName(soB2cReceiverEntity.getName());
                data.setCountry(soB2cReceiverEntity.getCountry());
                data.setCountryName(soB2cReceiverEntity.getCountryName());
                data.setEmail(soB2cReceiverEntity.getEmail());
                data.setTelNumber(soB2cReceiverEntity.getTelNumber());
                data.setPostCode(soB2cReceiverEntity.getPostCode());
                data.setProvinceName(soB2cReceiverEntity.getProvinceName());
                data.setCityName(soB2cReceiverEntity.getCityName());
                data.setPartitionId(soB2cReceiverEntity.getPartitionId());
            }
            data.setPartitionName(partitionMap.get(data.getPartitionId()));
            SoB2cLogisticsEntity soB2cLogisticsEntity = logisticsEntityMap.get(data.getId());
            if (Objects.nonNull(soB2cLogisticsEntity)) {
                data.setIossTaxNo(soB2cLogisticsEntity.getIossTaxNo());
                data.setShippingCost(soB2cLogisticsEntity.getActualShippingCost().compareTo(BigDecimal.ZERO) <= 0 ? soB2cLogisticsEntity.getEstimatedShippingCost() : soB2cLogisticsEntity.getActualShippingCost());
                data.setShippingCostCurrency(soB2cLogisticsEntity.getActualShippingCost().compareTo(BigDecimal.ZERO) <= 0 ? soB2cLogisticsEntity.getEstimatedShippingCurrency() : soB2cLogisticsEntity.getActualShippingCurrency());
                data.setLogisticsChannelId(soB2cLogisticsEntity.getLogisticsChannelId());
                data.setLogisticsChannelName(soB2cLogisticsEntity.getLogisticsChannelName());
                data.setTransferLogisticsChannelId(soB2cLogisticsEntity.getTransferLogisticsChannelId());
                data.setTransferLogisticsSupplierId(soB2cLogisticsEntity.getTransferLogisticsSupplierId());
                data.setWeight(soB2cLogisticsEntity.getWeight());
                data.setWeightUnit(soB2cLogisticsEntity.getWeightUnit());
                data.setLength(soB2cLogisticsEntity.getLength());
                data.setWidth(soB2cLogisticsEntity.getWidth());
                data.setHeight(soB2cLogisticsEntity.getHeight());
                data.setSizeUnit(soB2cLogisticsEntity.getSizeUnit());
                data.setSellerLogisticName(soB2cLogisticsEntity.getName());
                data.setLogisticsCode(soB2cLogisticsEntity.getCode());
                data.setTrackCode(soB2cLogisticsEntity.getTrackNo());
            }
            ...
            if (SoB2cMultiChannelTypeEnum.AMAZON_FAILED.getCode().equals(data.getMultiChannelType())) {
                SoMultiChannelEntity soMultiChannelEntity = soMultiChannelEntityMap.get(data.getId());
            ...
            //店铺
            data.setShopName(shopMap.get(data.getShopId()));
            // 国家=买家信息国家
            data.setCountryName(countryNameMap.getOrDefault(data.getCountry(), ""));
            //最晚发货时间
            data.setRequiredDeliveryTime(requiredDeliveryTimeMap.get(data.getId()));
            //重量单位,默认g
            data.setWeightUnit(UnitEnum.WeightUnitEnum.G.getCode());

            //中转信息
            TransferLogisticsChannelDTO.ListSelectDTO transferInfo = transferMap.get(data.getTransferLogisticsChannelId());
            ...
            //合并拆分标签
            List<SoB2cRefEntity> refList = refListMap.getOrDefault(data.getId(), Collections.emptyList());
            if (CollUtil.isNotEmpty(refList)) {
                Map<String, Long> refTypeCounts = refList.stream()
                        .filter(obj -> InvalidStatusEnum.NOT_VOIDED.getStatus().equals(data.getInvalidStatus()))
                        .collect(Collectors.groupingBy(SoB2cRefEntity::getType, Collectors.counting()));

                long mergeCount = refTypeCounts.getOrDefault(SoB2cOptionTypeEnum.ENUM_MERGE.getCode(), 0L);
                if (mergeCount > 0) {
                    labelDTO.setRefType(SoB2cOptionTypeEnum.ENUM_MERGE.getCode());
                    labelDTO.setMergeCount((int) mergeCount);
                }

                long splitCount = refTypeCounts.getOrDefault(SoB2cOptionTypeEnum.ENUM_SPLIT.getCode(), 0L);
                if (splitCount > 0) {
            ...
            //明细信息
            List<String> warehouseIds = warehouseMap.get(data.getId());
            if (CollUtil.isNotEmpty(warehouseIds)) {
                Boolean isOverseasProviderWarehouse = warehouseIds.stream().anyMatch(providerCodeMap::containsKey);
            ...
            for (SoB2cDetailDTO.ListDTO detailDTO : soB2cDetailList) {
                ...
                //是否是组合SKU
                if (CollUtil.isNotEmpty(bomChildrenList)) {
                ...
                String key = detailDTO.getPlatformSkuNo() + "-" + detailDTO.getPlatformSpuNo() + "-" + data.getDictPlatform();
                ListingInfoEntity listingInfoEntity = detailDTO.getSourcePlatform().equals(SoB2cSourcePlatformEnum.ENUM_THIRD_PLATFORM.getCode()) ? listingInfoMap.get(key) : null;
                if (ObjectUtils.isNotEmpty(listingInfoEntity)) {
                ...
                detailDTO.setUseableQty(inventoryMap.getOrDefault(detailDTO.getSkuId() + "-" + detailDTO.getWarehouseId() + "-" + InventoryStatusEnum.USABLE.getCode(), MathUtil.ZERO));
                detailDTO.setFreezeQty(inventoryMap.getOrDefault(detailDTO.getSkuId() + "-" + detailDTO.getWarehouseId() + "-" + InventoryStatusEnum.FROZEN.getCode(), MathUtil.ZERO));
                //存在仓库则需要判断是否缺货
                if (CharSequenceUtil.isNotBlank(detailDTO.getWarehouseId())) {
                    ...
                    if ((SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode().equals(data.getBillStatus())
                        ...
                        //实体仓缺货
                        Boolean isOutStock = isOutStock(bomChildrenList, inventoryMap, detailDTO, ignoreInventorySkuIds);
                        detailLabelDTO.setIsOutStock(isOutStock);
                    }
                }
                //存在虚拟仓库则判断是否缺货
                if (CharSequenceUtil.isNotBlank(detailDTO.getVirtualWarehouseId())) {
                    detailDTO.setVirtualWarehouseName(virtualWarehouseMap.getOrDefault(detailDTO.getVirtualWarehouseId(), ""));
                    //虚拟仓缺货处理
                    isVirtualOutStock(bomChildrenList, virtualInventoryMap, detailLabelDTO, detailDTO);
                    //缺货订单
                    if ((SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode().equals(data.getBillStatus())
                            || SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode().equals(data.getBillStatus()))) {
                ...
                //申报信息
                SoB2cDeclareProductDTO.ViewDTO viewDTO = declareProductMap.get(detailDTO.getSkuId() + "-" + detailDTO.getId());
                if (Objects.nonNull(viewDTO)) {
            ...
            dto.setShippingFee(data.getShippingFee());
            dto.setShopId(data.getShopId());
            dto.setExchangeRate(data.getExchangeRate());
            dto.setCurrency(data.getCurrency());

            //物流信息
            dto.setSoB2cFinanceEntity(financeEntityMap.get(data.getId()));
            //物流信息
            SoB2cLogisticsEntity logisticsEntity = logisticsEntityMap.getOrDefault(data.getId(), new SoB2cLogisticsEntity());
            dto.setSoB2cLogisticsEntity(logisticsEntity);
            dto.setSoB2cDetailList(detailListMap.getOrDefault(data.getId(), Collections.emptyList()));
            ...
            //提交发货是否选择渠道
            String logisticsPlatform = logisticsPlatformMap.get(data.getLogisticsChannelId());
            String fromWarehouseId = data.getFromWarehouseId();
            if (CharSequenceUtil.isNotBlank(logisticsPlatform) && CharSequenceUtil.isNotBlank(fromWarehouseId)) {
                String providerCode = providerCodeMap.get(fromWarehouseId);
                data.setIsSelectChannel(StringUtils.isNotBlank(providerCode) && !providerCode.equals(logisticsPlatform));
            }
        }
    }
     ...
     */
    @Override
    public Boolean isOutStock(List<BomChildrenSkuDTO> bomChildrenList, Map<String, Integer> inventoryMap
        ...
        for (BomChildrenSkuDTO childrenSkuDTO : childList) {
            //可用库存
            Integer childUseableQty = inventoryMap.getOrDefault(childrenSkuDTO.getSkuId() + "-" + detailDTO.getWarehouseId() + "-" + InventoryStatusEnum.USABLE.getCode(), MathUtil.ZERO);
     ...
     */
    @Override
    public void isVirtualOutStock(List<BomChildrenSkuDTO> bomChildrenList, Map<String, Integer> virtualInventoryMap, SoB2cDetailDTO.DetailLabelDTO detailLabelDTO, SoB2cDetailDTO.ListDTO detailDTO) {
        //返回信息
        ...
        if (!isCombination) {
            //虚拟仓是否缺货
            String key = detailDTO.getSkuId() + "-" + detailDTO.getVirtualWarehouseId() + "-" + detailDTO.getWarehouseId();
            Integer virtualUsableQty = virtualInventoryMap.getOrDefault(key, MathUtil.ZERO);
        ...
        for (BomChildrenSkuDTO childrenSkuDTO : childList) {
            ...
            //虚拟仓是否缺货
            String key = childrenSkuDTO.getSkuId() + "-" + detailDTO.getVirtualWarehouseId() + "-" + detailDTO.getWarehouseId();
            Integer childVirtualUsableQty = virtualInventoryMap.getOrDefault(key, MathUtil.ZERO);
        ...
        detailDTO.setChildScarceList(childScarceList);
        if (CollUtil.isNotEmpty(childScarceList)) {
            //bom最小可用数
    ...
    private void validateSubmit(SoB2cEntity entity) {
        ...
        if (InvalidStatusEnum.VOIDED.getStatus().equals(entity.getInvalidStatus()) || SoB2cBillStatusEnum.ENUM_FROZEN.getCode().equals(entity.getBillStatus())) {
            throw new ServiceException(ApiError.SO_B2C_SUBMIT_FORBIDDEN_WHEN_FROZEN_OR_VOIDED, entity.getCode());
        }
        //未付款数据不支持提交
        soB2cCoreService.checkPayMent(entity);
        ...
        if (isFullyManagedOrder(entity.getDictPlatform())) {
            ...
            for (SoB2cDetailEntity detailEntity : soB2cDetailEntityList) {
                if (Objects.equals(detailEntity.getIsGift(), Boolean.FALSE) && (Objects.isNull(detailEntity.getPrice()) || detailEntity.getPrice().compareTo(BigDecimal.ZERO) <= 0)) {
                    throw new ServiceException(ApiError.COMMON_BUSINESS_NOT_EXIST, entity.getCode(), detailEntity.getSkuNo());
                }
            }
        }
    ...
    public void handleData(SoB2cEntity soB2cEntity, Boolean exchangeRateThrow, Boolean checkPayTime) {
        ...
        if (SourceTypeEnum.SELF_ADD.getCode().equals(soB2cEntity.getSourceType()) && StringUtils.isBlank(soB2cEntity.getTransactionSubType())) {
            throw new ServiceException(ApiError.BILL_MANUAL_SUB_TYPE_REQUIRED, soB2cEntity.getCode());
        }
        //店铺
    ...
    private void fillMergeData(List<SoB2cDTO.MergeListDTO> records, String permissionSql) {
        ...
        if (CollUtil.isEmpty(mergeMainList)) {
            return;
        }
        //产品详细
        ...
        for (SoB2cDTO.MergeListDTO mergeListDTO : records) {
            ...
            //主表数据
            if (CollUtil.isNotEmpty(mergeMainList)) {
                ...
                if (CollUtil.isEmpty(mainList) || mainList.size() == 1) {
                    continue;
                }
    ...
    private void checkMergeData(List<SoB2cEntity> list, List<SoB2cLogisticsEntity> soB2cLogisticsList,
        ...
        if (platformCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_PLATFORM_CONFLICT);
        }
        //店铺
        long shopCount = list.stream().map(SoB2cEntity::getShopId).distinct().count();
        if (shopCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_SHOP_CONFLICT);
        }
        //币别
        long currencyCount = list.stream().map(SoB2cEntity::getCurrency).distinct().count();
        if (currencyCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_CURRENCY_CONFLICT);
        }
        //物流方式
        ...
        if (logisticsMethodCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_LOGISTICS_METHOD_CONFLICT);
        }

        //买家
        long nameCount = soB2cReceiverList.stream().map(SoB2cReceiverEntity::getName).distinct().count();
        if (nameCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_BUYER_CONFLICT);
        }
        //收货人
        ...
        if (receiverNameCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_RECEIVER_CONFLICT);
        }
        //收货地址
        ...
        if (addressCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_ADDRESS_CONFLICT);
        }
        ...
        if (warehouseCount > MathUtil.ONE) {
            throw new ServiceException(ApiError.SO_B2C_MERGE_WAREHOUSE_CONFLICT);
        }
    }
    ...
    public Map<String, Boolean> approveRule(String id, List<SoB2cDetailEntity> detailList, Map<String, Object> map) {
        ...
        if (Objects.nonNull(approveSuccess) && approveSuccess) {
            ...
            if (!submit.getSuccess()) {
                throw new ServiceException(ApiError.BILL_SUBMIT_FAILED, SourceTypeEnum.SO_B2C.getName());
            }
    ...
    public Map<String, Object> handleMatchJson(String id, List<SoB2cDetailEntity> detailList, Map<String, Object> map) {
        ...
        //汇总skuIds
        if (CollUtil.isNotEmpty(childSkuIds) && CollUtil.isNotEmpty(skuIdList)) {
        ...
        Boolean isThirdWarehouse = CollUtil.isNotEmpty(overseasWarehouseList);
        ...
        map.put("vatInvoiceStatus", CharSequenceUtil.isNotBlank(soB2cEntity.getVatInvoiceStatus()) ? soB2cEntity.getVatInvoiceStatus() : "");
...
//        }
    ...
    public BatchResultDTO falseDelivery(String id) {
        ...
        if (!(SoB2cBillStatusEnum.ENUM_WAIT_SHIPPED.getCode().equals(entity.getBillStatus())
            ...
            if (SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode().equals(entity.getBillStatus()) || SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode().equals(entity.getBillStatus())) {
                throw new ServiceException(ApiError.SO_DISTRIBUTION_MANUAL_SHIP_ALLOWED);
            } else {
                throw new ServiceException(ApiError.SO_MANUAL_SHIP_ALLOWED_APPROVED_PENDING_ONLY);
            }
        }
        ...
        if (StringUtils.isBlank(code)) {
            throw new ServiceException(ApiError.LOGISTICS_NO_TRACKING_NUMBER_CANNOT_MANUAL_SHIP);
        }
    ...
    private SoB2cDTO.RuleResultDTO getRuleResultDTO(String id, Map<String, Object> map, Boolean isCheckProductRegistration) {
        ...
        Boolean autoTrackNoOnly = Boolean.FALSE;
        Boolean autoTrackNoInRange = Boolean.FALSE;
        ...
        if (result) {
            ...
            autoTrackNoOnly = matchResult.getAutoTrackNoOnly();
            autoTrackNoInRange = matchResult.getAutoTrackNoInRange();
        ...
        resultDTO.setAutoTrackNoOnly(autoTrackNoOnly);
        resultDTO.setAutoTrackNoInRange(autoTrackNoInRange);
    ...
    public List<WalmartShipDTO> getWalmartShipOrderParam(String soId) {
        ...
        if (CollUtil.isNotEmpty(soB2cRefEntities)) {
        ...
        if (CollUtil.isEmpty(soB2cEntities)) {
            throw new ServiceException(ApiError.SO_B2C_NOT_FOUND);
        }
    ...
    public List<BatchResultDTO> transferDeclare(List<String> ids) {
        ...
        if (CollUtil.isNotEmpty(notChannelInfoList)) {
        ...
        for (SoB2cDTO.MergeTransferDTO mergeTransferDTO : mergeTransferDTOList) {
            ...
            if (CollUtil.isNotEmpty(alreadyTransferCode)) {
            ...
            if (CollUtil.isNotEmpty(alreadyDeclareCode)) {
            ...
            if (CollUtil.isNotEmpty(wantPackageCode)) {
    ...
    public BatchResultDTO disApprove(String id) {
        ...
        if (!billStatusList.contains(billStatus)) {
            throw new ServiceException(ApiError.SO_B2C_REVERSE_APPROVE_STATUS_LIMIT);
        }
        //预报成功不支持反审核
        if (TransferStatusEnum.SUCCESS.getCode().equals(entity.getTransferStatus())) {
...
//        syncOrderToDmp(entity.getId(), SyncOperateEnum.OPERATE_DISAPPROVE.getCode());
    ...
    public SoB2cDTO.FinancialInfoDTO getFinancialInfo(SoB2cDTO.FinancialParamDTO dto, Boolean isAdd) {
        ...
        if (Objects.isNull(soB2cFinanceEntity.getShippingCost())) {
            soB2cFinanceEntity.setShippingCost(dto.getShippingFee());
        }

        //店铺信息
        ShopCostEntity shopCostEntity = shopCostService.getByShopId(dto.getShopId());
        //平台费
        ...
        if (ObjectUtils.isNotEmpty(dto.getIsCny()) && dto.getIsCny()) {
            ...
            financialInfoDTO.setAmount(MathUtil.multiplyWithTwo(totalAmount, dto.getExchangeRate()));
            financialInfoDTO.setItemCost(itemCost);
            //运费收入
            financialInfoDTO.setShippingCost(MathUtil.multiplyWithTwo(financialInfoDTO.getShippingCost(), dto.getExchangeRate()));
            //物流成本
            financialInfoDTO.setLogisticsCost(MathUtil.multiplyWithTwo(financialInfoDTO.getLogisticsCost(), dto.getExchangeRate()));
            //平台费
            financialInfoDTO.setPlatformCost(MathUtil.multiplyWithTwo(financialInfoDTO.getPlatformCost(), dto.getExchangeRate()));
        } else {
            financialInfoDTO.setCurrency(dto.getCurrency());
            financialInfoDTO.setAmount(totalAmount);
            financialInfoDTO.setItemCost(MathUtil.divide(itemCost, dto.getExchangeRate()));
        }
...
//        syncSoB2cService.syncDataToSdy(entity, Arrays.asList(handleDetailEntity), SyncOperateEnum.OPERATE_APPROVE.getCode());
    ...
    public SoOutstockDTO.GenerateB2cDTO getSoOutstockInfoByCode(String soCode) {
        ...
        if (Objects.isNull(entity)) {
            throw new ServiceException(ApiError.BILL_NOT_EXIST_WITH_TYPE, "B2C销售订单");
        }
        String id = entity.getId();
...
//        String sourceType = SourceTypeEnum.SO_B2C.getCode();
        ...
        if (CollUtil.isEmpty(accountingCompanyList)) {
            throw new ServiceException(ApiError.WH_NOT_EXIST_ORG, warehouseList.get(0).getName());
        }
        ...
        for (SoB2cDetailEntity detailItem : detailList) {
            ...
            if (!entity.hasPlatformWarehouseOrder() && CollUtil.isNotEmpty(deliveryList)) {
                // 自发货单有捆绑商品拆分
    ...
    public Boolean updateSoB2cStatusAndDeliveryTime(SoB2cDTO.UpdateDeliveryTimeDTO deliveryTimeDTO) {
        ...
        Boolean updateResult = lambdaUpdate().in(SoB2cEntity::getId, deliveryTimeDTO.getSoB2cIds())
                ...
                .update();
        if (CollUtil.isEmpty(deliveryTimeDTO.getSoB2cLogisticsList())) {
            //修改发货时间
...
//    @GlobalTransactional(rollbackFor = Exception.class, timeoutMills = 120000)
    ...
    public PagingVO<PackageDTO.PagingViewDTO> packagePing(PagingDTO<PackageDTO.PagingParamDTO> dto) {
        ...
        List<LogisticsChannelDTO.BaseDTO> baseList = CollUtil.isNotEmpty(logisticsChannelIdList) ? logisticsFeign.listChannelInfoById(logisticsChannelIdList) : Collections.emptyList();
    ...
    public Boolean getPackageSupplierSetting(String logisticsSupplierId, CfgSettingEntity entity) {
        ...
        if (ObjectUtil.isNotEmpty(entity) && ObjectUtil.isNotEmpty(entity.getDataJson())) {
            ...
            if (CollUtil.isNotEmpty(dto.getSupplierIds()) && dto.getSupplierIds().contains(logisticsSupplierId)) {
                return Boolean.TRUE;
            }
        }
        return Boolean.FALSE;
    ...
    public SettingForecastDTO.CheckRegistrationResultDTO getCheckRegistrationResult(String id, String logisticsChannelId) {
        ...
        List<String> skuIdList = productList.stream().map(SplitSkuDTO::getSkuId).distinct().collect(Collectors.toList());
        List<LogisticsProductDTO.ProductDTO> skuInfoList = logisticsProductFeign.listLogisticsProduct(skuIdList);
        skuNoList = skuNoList.stream().filter(v -> {
            LogisticsProductDTO.ProductDTO productDTO = skuInfoList.stream().filter(s -> s.getSkuNo().equals(v)).findFirst().orElse(null);
    ...
    public Boolean updateShippingOrderNo(List<TransferDeclareDTO.ShippingOrderDTO> list) {
        ...
        //更新或添加记录
        if (CollUtil.isNotEmpty(addOrUpdateErrors)) {
            soB2cErrorService.saveOrUpdateBatch(addOrUpdateErrors);
        }
        //删除失败记录
        if (CollUtil.isNotEmpty(deleteErrorIds)) {
    ...
    public void batchRemoveSignError(List<String> mainIds, String type) {
        if (CollUtil.isEmpty(mainIds) || StringUtil.isEmpty(type)) {
            return;
        }
        this.lambdaUpdate().set(SoB2cEntity::getSignOrderError, "").
    ...
    public PagingVO<SoB2cDTO.ExcelExportDTO> exportSoB2C(PagingDTO<SoB2cDTO.ExportParamDTO> dto) {
        ...
        try {
            if (Objects.nonNull(isOutStock)) {
                //必须选仓库而且只能选一个
                List<String> warehouseIdList = CollectionUtils.convertStrClzToList(advanceQueryDTOList.stream()
                        .filter(Objects::nonNull)
    ...
    public List<SoB2cDTO.ChangeDeliverySkuViewDTO> changeDeliverySkuView(List<String> ids) {
        ...
        if (CollUtil.isNotEmpty(notChangeList)) {
        ...
        if (CollUtil.isNotEmpty(deliveryFailedCodes)) {
    ...
    public SoB2cDTO.SoB2cDataDTO listSoB2cData(SoB2cDTO.SoB2cDataParamDTO paramDTO) {
        //校验必填
        if (CollUtil.isEmpty(paramDTO.getB2cSoIdList()) && CollUtil.isEmpty(paramDTO.getB2cSoCodeList())) {
        ...
        //只有编号，没有id
        if (CollUtil.isEmpty(b2cSoIdList) && CollUtil.isNotEmpty(paramDTO.getB2cSoCodeList())) {
    ...
    public BatchResultDTO skuMappingBatch(String soId) {
        ...
        if (!SourceTypeEnum.SO_B2C.getCode().equalsIgnoreCase(entity.getSourceType())) {
            ...
            throw new ServiceException(ApiError.SO_B2C_SOURCE_ONLY_MAPPING_UPDATE_ALLOWED);
        }
...
//        }
    ...
    public List<BatchResultDTO> orderForecast(SoB2cDTO.TransferDeclareDTO dto) {
        ...
        for (SoB2cEntity soB2cEntity : soB2cEntityList) {
            ...
            if (CollUtil.isEmpty(transferDeclareProductDTOList)) {
    ...
    public void orderForecastUpdateSoAndError(List<SoB2cEntity> updateB2cList, List<String> deleteErrorIds, List<SoB2cErrorEntity> addOrUpdateErrors, List<SoB2cLogisticsEntity> updateLogisticList) {
        ...
        if (CollUtil.isNotEmpty(updateB2cList)) {
        ...
        if (CollUtil.isNotEmpty(updateLogisticList)) {
    ...
    private List<SoB2cDTO.ExcelExportDTO> handleExport(List<SoB2cDTO.ExcelExportDTO> records, String exportType, Boolean isOutStock) {
        ...
        //虚拟仓库信息
        List<VirtualWarehouseEntity> virtualWarehouseList = CollUtil.isEmpty(virtualWarehouseIdList) ? new ArrayList<>() : FeignQuery.getByIds(VirtualWarehouseEntity.class, virtualWarehouseIdList);
        ...
        for (SoB2cDTO.ExcelExportDTO exportDTO : records) {
            ...
            //是否缺货
            if (isOutStock) {
                exportDTO.setUseableQty(inventoryMap.getOrDefault(exportDTO.getSkuId() + "-" + exportDTO.getWarehouseId() + "-" + InventoryStatusEnum.USABLE.getCode(), MathUtil.ZERO));
                //存在仓库则需要判断是否缺货
                if (CharSequenceUtil.isNotBlank(exportDTO.getWarehouseId())) {
                    ...
                    if ((SoB2cBillStatusEnum.ENUM_WAIT_DISTRIBUTION.getCode().equals(exportDTO.getBillStatus())
                        ...
                        Boolean outStock = isOutStock(bomChildrenList, inventoryMap, detailDTO, ignoreInventorySkuIds);
            ...
            if (CharSequenceUtil.isNotBlank(exportDTO.getVirtualWarehouseId())) {
                ...
                Integer virtualUsableQty = virtualInventoryMap.getOrDefault(exportDTO.getSkuId() + "-" + exportDTO.getVirtualWarehouseId() + "-" + exportDTO.getWarehouseId(), MathUtil.ZERO);
            ...
            exportDTO.setLabelDetailList(getLabelDetailList(exportDTO, skuVOMap, bomChildrenList, inventoryMap, ignoreInventorySkuIds, detailDTO, virtualInventoryMap, virtualWarehouseList));
            //销售套装bom
            if (CollUtil.isNotEmpty(childList)) {
                ...
                for (BomChildrenSkuDTO bomChildrenSkuDTO : childList) {
                    ...
                    if (CharSequenceUtil.isNotBlank(exportDTO.getVirtualWarehouseId())) {
                        //虚拟仓是否缺货
                        Integer childVirtualUsableQty = virtualInventoryMap.getOrDefault(bomChildrenSkuDTO.getSkuId() + "-" + exportDTO.getVirtualWarehouseId() + "-" + exportDTO.getWarehouseId(), MathUtil.ZERO);
                ...
                if (SoB2cExportTypeEnum.PARENT_EXPORT.getCode().equals(exportType)) {
                    if (CollUtil.isNotEmpty(qtyList)) {
     ...
     * @return
     */
    private String getLabelDetailList(SoB2cDTO.ExcelExportDTO exportDTO, Map<String, SkuVO> skuVOMap, List<BomChildrenSkuDTO> bomChildrenList, Map<String, Integer> inventoryMap, List<String> ignoreInventorySkuIds, SoB2cDetailDTO.ListDTO detailDTO, Map<String, Integer> virtualInventoryMap, List<VirtualWarehouseEntity> virtualWarehouseList) {
        ...
        List<SoB2cDetailDTO.PropertyDTO> propertyDTOList = soB2cDetailService.handlePropertyDTOList(skuPropertyDTO);
        if (CollUtil.isNotEmpty(propertyDTOList)) {
        ...
        if (StringUtils.isNotBlank(detailLabel)) {
            ...
            labelDetailStr.append(CollUtil.isNotEmpty(labelJsonDTO.getTagList()) && labelJsonDTO.getTagList().contains("AE_PLUS_RU") ? "AE_PLUS," : "");
            labelDetailStr.append(CollUtil.isNotEmpty(labelJsonDTO.getTagList()) && labelJsonDTO.getTagList().contains("HBA_UP_EXPRESS") ? "AE_合单," : "");
            labelDetailStr.append(CollUtil.isNotEmpty(labelJsonDTO.getTagList()) && labelJsonDTO.getTagList().contains("leadTimeTag#10") ? "十日达," : "");
            labelDetailStr.append(CollUtil.isNotEmpty(labelJsonDTO.getTagList()) && labelJsonDTO.getTagList().contains("leadTimeTag#12") ? "12日达," : "");
            labelDetailStr.append(CollUtil.isNotEmpty(labelJsonDTO.getTagList()) && labelJsonDTO.getTagList().contains("leadTimeTag#15") ? "15日达," : "");
    ...
    private String getLabelOrderList(SoB2cDTO.ExcelExportDTO data, List<SoB2cRefEntity> soB2cRefList, List<BomChildrenSkuDTO> childList, SoB2cDeliveryEntity soB2cDeliveryEntity) {
        ...
        labelOrderStr.append(CollUtil.isNotEmpty(childList) ? "组合产品," : "");
        ...
        if (CollUtil.isNotEmpty(soB2cRefList)) {
            //合并
...
//            List<SoB2cDeliveryEntity> list = soB2cDeliveryFeign.listBySourceId(Collections.singletonList(entity.getId()));
            ...
            List<SoB2cDeliveryEntity> list = FeignQuery.create(SoB2cDeliveryEntity.class)
                    ...
                    .list();
            if (!CollUtil.isEmpty(list)) {
                throw new ServiceException(ApiError.SO_DELIVERY_EXISTS_MAPPING_UPDATE_FORBIDDEN);
            }
        }
    }
    ...
    public BatchResultDTO declareRule(String id, HashMap<String, Object> map, Boolean isUpdate, Boolean isUpdatePackingWeight) {
        ...
        //已存在申报信息 则不进行规则匹配
        List<SoB2cDeclareProductEntity> declareProductList = soB2cDeclareProductService.listBySoId(id);
        if (Objects.nonNull(isUpdate) && !isUpdate && CollUtil.isNotEmpty(declareProductList)) {
            throw new ServiceException(ApiError.SO_B2C_DECLARE_ALREADY_EXISTS);
        }
        SoB2cLogisticsEntity logisticsEntity = soB2cLogisticsService.getByMainId(id);
        if (ObjectUtil.isEmpty(logisticsEntity)) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_NOT_FOUND);
        ...
        if (StrUtil.isBlank(logisticsEntity.getLogisticsChannelId())) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_CHANNEL_REQUIRED, entity.getCode());
        }
        if (isFullyManagedOrder(entity.getDictPlatform())) {
        ...
        if (StrUtil.isBlank(logisticsEntity.getLogisticsChannelId()) || StrUtil.isBlank(logisticsPlatform)) {
            throw new ServiceException(ApiError.SO_B2C_LOGISTICS_PLATFORM_REQUIRED, entity.getCode());
        }
        if (!SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode().equals(entity.getBillStatus())
                || !ApproveStatusEnum.APPROVE.getStatus().equals(entity.getApproveStatus().getStatus())
        ) {
            throw new ServiceException(ApiError.SO_B2C_DISTRIBUTION_DECLARE_STATUS_INVALID, entity.getCode());
        }
        if (map.isEmpty()) {
            List<SoB2cDetailEntity> detailList = soB2cDetailService.listByMainId(id);
        ...
        if (isUpdatePackingWeight) {
            ...
            if (CollUtil.isNotEmpty(splitSkuDTOS)) {
                List<String> keyList = new ArrayList<>();
                keyList.add(CalculateSizeEnum.GROSS_WEIGHT.getCode());
                List<DictBasicEntity> byKeyList = dictBasicService.getByKeyList(keyList);
                Map<String, String> collect = byKeyList.stream().collect(Collectors.toMap(DictBasicEntity::getType, DictBasicEntity::getValue));
...
//        List<BomChildrenSkuDTO> skuDTOS = plmTaskFeign.listBomBySkuIds(skuIds);
    ...
    public void initCostPrice(SoB2cDTO.CostPriceDTO dto) {
        ...
        if (CollUtil.isEmpty(dto.getIds()) && Objects.isNull(dto.getStartTime())) {
            return;
        }
    ...
    public BatchResultDTO addGift(SoB2cEntity entity, List<SoB2cDTO.GiftDTO> dtoList, SoB2cLogisticsEntity LogisticsEntity, List<SoB2cDetailEntity> detailEntityList) {
        ...
        //待提交和审核不通过的订单允许添加赠品
        if (!(ApproveStatusEnum.WAIT_SUBMIT.equals(entity.getApproveStatus()) || ApproveStatusEnum.REJECT.equals(entity.getApproveStatus()))) {
            throw new ServiceException(ApiError.SO_B2C_ADD_GIFT_STATUS_FORBIDDEN);
        }
        //订单明细数据整理
        ...
        if (CollUtil.isNotEmpty(detailList) && CollUtil.isNotEmpty(productDTOS)) {
            detailList.forEach(soB2cDetailEntity -> {
    ...
    private void calculateSize(SoB2cLogisticsEntity logisticsEntity, List<SoB2cDetailEntity> detailList) {
        if (Objects.isNull(logisticsEntity) || CollUtil.isEmpty(detailList)) {
            return;
        }
        List<String> skuIds = detailList.stream().map(SoB2cDetailEntity::getSkuId).collect(Collectors.toList());
        //根据明细进行sku拆分
    ...
    public List<SoB2cReceiverDTO.ViewDTO> getReceiverInfo(List<String> ids) {
        ...
        if (CollUtil.isEmpty(soB2cReceiverEntities)) {
            return Collections.emptyList();
        }
        //step3：信息整合并返回
        //国家列表
    ...
    public BatchResultDTO updateReceiverInfo(SoB2cReceiverDTO.UpdateBaseDTO dto) {
        ...
        if (Objects.isNull(entity)) {
            return BatchResultDTO.fail(dto.getMainId(), dto.getMainId(), ApiError.SO_NOT_FOUND.getMsg());
        }
        ...
        if (Objects.isNull(old)) {
            return BatchResultDTO.fail(entity.getId(), entity.getCode(), String.format(ApiError.BILL_NOT_EXIST_WITH_TYPE.getMsg(), "B2C销售订单买家信息表"));
        }
        //step3 更新买家物流信息
    ...
    public List<BatchResultDTO> fetchOrder(List<String> ids) {
        ...
        if (CollUtil.isNotEmpty(selfAddList)) {
        ...
        if (CollUtil.isEmpty(soB2cErrors)) {
        ...
        if (CollUtil.isNotEmpty(noErrorList)) {
...
//        String prefix = "data:application/pdf;base64,";
    ...
    public void clearOutDateBySoIds(List<String> clearOutDateSoIds) {
        if (CollUtil.isEmpty(clearOutDateSoIds)) {
            return;
        }
        lambdaUpdate()
...
//                : ""), ModuleTypeEnum.SO_B2C.getCode(), entity.getId(),"亚马逊多渠道发货");
    ...
    public BatchResultDTO deliveryWithNotOutbound(SoB2cDTO.DeliveryWithNotOutboundDTO dto, SoB2cEntity soB2cEntity, SoB2cLogisticsEntity soB2cLogisticsEntity, List<SoB2cDetailEntity> detailEntityList, SoB2cReceiverEntity soB2cReceiverEntity, LogisticsChannelDTO.BaseDTO baseDTO, List<String> noInventorySkuIdList, OverseasProviderWarehouseDTO.ViewDTO overseasWarehouse, SoB2cEntity oldSoB2cEntity, SoB2cLogisticsEntity oldLogisticsEntity, List<SoB2cDetailEntity> oldDetailEntityList) {
        ...
        try {
            UserContext.setIsUserSystem(true);
            redisUtil.set(CharSequenceUtil.format(RedisCacheConstants.SO_B2C_NOT_OUTBOUND_KEY + ":{}", soB2cEntity.getId()), Boolean.TRUE);
            ...
            UserContext.clearIsUserSystem();
            redisUtil.del(CharSequenceUtil.format(RedisCacheConstants.SO_B2C_NOT_OUTBOUND_KEY + ":{}", soB2cEntity.getId()));
        }
        if (dto.getPlatformShipFlag() && this.checkPlatformShipOrder(soB2cEntity.getId()) && !soB2cEntity.hasPlatformWarehouseOrder()) {
            ...
            try {
                ...
                throw new ServiceException(CharSequenceUtil.format("平台标发失败:{}", e.getMessage()));
            }
        }
        ...
        if (!dto.getPlatformShipFlag() && soB2cEntity.getThirdSystem().equals(PlatformDictEnum.LING_XING.getCode())) {
            try {
                ...
                throw new ServiceException(CharSequenceUtil.format("领星标记订单不发货失败:{}", e.getMessage()));
            }
        }
        // 不出库发货虚拟商品同步数帝云
        ...
        if (CollUtil.isNotEmpty(noInventorySkuDetailList)) {
    ...
    public void rollbackBeforeDelivery(String soB2cId, SoB2cEntity oldSoB2cEntity, SoB2cLogisticsEntity oldLogisticsEntity, List<SoB2cDetailEntity> oldDetailEntityList) {
        ...
        if (CollUtil.isNotEmpty(oldDetailEntityList)) {
    ...
    private void verifyDetailWarehouseRollback(String soB2cId, List<SoB2cDetailEntity> oldDetailEntityList) {
        ...
        List<String> rollbackFailedDetailIdList = oldDetailEntityList.stream()
                ...
                .collect(Collectors.toList());
        if (CollUtil.isNotEmpty(rollbackFailedDetailIdList)) {
    ...
    @Transactional(rollbackFor = Exception.class)
    @GlobalTransactional(rollbackFor = Exception.class, timeoutMills = 120000)
    public BatchResultDTO getLogisticsLabel(SoB2cEntity entity, SoB2cLogisticsEntity soB2cLogisticsEntity, Boolean checkBillStatus) {
        if (!SoB2cBillStatusEnum.ENUM_IN_DISTRIBUTION.getCode().equals(entity.getBillStatus()) && checkBillStatus) {
    ...
    private void handleImportSuccessList(List<B2CSoImportExcelDTO> successList, List<B2CSoImportExcelDTO> errorList) {
        ...
        for (Map.Entry<String, List<B2CSoImportExcelDTO>> entry : groupedByNo.entrySet()) {
            ...
            if (Objects.isNull(currencyEntity)) {
                ...
                if (currency.equals(CurrencyEnum.CNY.getCurrencyCode())) {
                    ...
                    if (MathUtil.compareTo(exchangeRate, MathUtil.ZERO) == MathUtil.ZERO) {
                        errorMsgList.add(MessageUtils.getMessage(ApiError.COMMON_EXCHANGE_RATE_NOT_EXIST, soB2cEntity.getCreateTime().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")), soB2cEntity.getCurrency()));
                    } else {
                        soB2cEntity.setExchangeRate(null == exchangeRate ? BigDecimal.ZERO : exchangeRate);
                    }
                }
            ...
            for (B2CSoImportExcelDTO detail : list) {
                ...
                if (CollUtil.isNotEmpty(msgList) || CollUtil.isNotEmpty(errorMsgList)) {
    ...
    private List<String> resolveRetryPlatformOutboundRecordIds(SoB2cEntity currentEntity, SoB2cErrorEntity b2cError) {
        ...
        if (StringUtils.isNotBlank(sourceCode) && StringUtils.isNotBlank(outputClass)) {
            ...
            log.warn("平台出库异常重试查询DMP记录, soB2cId={}, sourceCode={}, outputClass={}, recordCount={}",
                    ...
                    sourceCode,
                    outputClass,
                    CollUtil.isEmpty(latestRecords) ? 0 : latestRecords.size());
