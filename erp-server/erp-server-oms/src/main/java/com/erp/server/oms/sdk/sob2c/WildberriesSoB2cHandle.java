...
@Slf4j
@Component
@PlatformSoB2cAnnotate(method = PlatformDictEnum.WILDBERRIES)
public class WildberriesSoB2cHandle extends AbstractSoB2cHandle  {
    ...
    public Boolean handleRule(SoB2cEntity mainEntity) {
        ...
        try {
            ...
            log.error("[Wildberries订单规则处理失败]:order={},msg={}", mainEntity.getPlatformCode(), e.getMessage());
        }
        return true;
    }
    ...
    public Boolean handleSoOutStock(PlatformOrderDTO dto, SoB2cDTO.PullOrderResultDTO resultDTO, SoB2cEntity mainEntity) {
        ...
        if (isShipped && hasPlatformWarehouse) {
            try {
                ...
                log.error("[Wildberries生成销售出库单异常]:order={},msg={}", mainEntity.getCode(), e.getMessage());
