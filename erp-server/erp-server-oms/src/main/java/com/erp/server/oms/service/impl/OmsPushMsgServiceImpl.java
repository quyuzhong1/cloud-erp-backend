...
public class OmsPushMsgServiceImpl extends SuperServiceImpl<OmsPushMsgMapper, OmsPushMsgEntity> implements OmsPushMsgService {
    ...
    public BaseResultDTO.AddDTO add(OmsPushMsgDTO.AddDTO addDTO) {
        ...
        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", UserContext.getDefaultLoginUser().getUserName(), "本地推送消息单" , omsPushMsgEntity.getId());
    ...
    */
    ...
    public Boolean update(OmsPushMsgDTO.UpdateDTO updateDTO) {
        ...
        // 记录主单操作日志
            ...
            String msg = StrUtil.format("用户【{}】编辑id为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), omsPushMsgEntity.getId(), "本地推送消息单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
