    ...
    protected String handleSqlLogic(String field, Object value, String compareCodeSplicingValueSql) {
        ...
        if ("deliveryCode".equals(field)) {
            if (DynamicDataSourceTypeEnum.isDoris(DynamicDataSourceThreadLocal.get())) { // 切换到doris
                return "(sb2c.id in ( SELECT so_id FROM erp_wms.third_warehouse_delivery twd where twd.is_deleted = false  AND twd.status != 'cancelDelivery' and twd.code " + compareCodeSplicingValueSql + " ) OR (sb2c.id IN ( SELECT source_id FROM erp_wms.so_b2c_delivery sbd WHERE sbd.is_deleted = FALSE AND sbd.status != 'cancelDelivery' AND sbd.code " + compareCodeSplicingValueSql + " )))";
            } else {
                return "(sb2c.id in ( SELECT so_id FROM foreign_third_warehouse_delivery twd where twd.is_deleted = false  AND twd.status != 'cancelDelivery' and twd.code " + compareCodeSplicingValueSql + " ) OR (sb2c.id IN ( SELECT source_id FROM so_b2c_delivery sbd WHERE sbd.is_deleted = FALSE AND sbd.status != 'cancelDelivery' AND sbd.code " + compareCodeSplicingValueSql + " )))";
            }
