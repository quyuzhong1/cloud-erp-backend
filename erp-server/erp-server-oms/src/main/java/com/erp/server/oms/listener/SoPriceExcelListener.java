...
public class SoPriceExcelListener extends AnalysisEventListener<ImportSoPriceExcelDTO> {
     ...
     * 错误信息
     */
    private List<ImportSoPriceExcelDTO> errorList = new ArrayList<>();

    /**
     * 可以添加的数据
     */
    private List<SoPriceDTO.ImportAddDTO> handleList = new ArrayList<>();
    ...
    @Override
    public void invoke(ImportSoPriceExcelDTO excelDTO, AnalysisContext analysisContext) {
        ...
        if(StrUtils.isNotEmpty(addDTO.getCustomerId())) {
            if(Objects.nonNull(detailDTO.getMinQty()) && Objects.nonNull(detailDTO.getMaxQty())) {
                ...
                if(existPriceMap.containsKey(detailDTO.getSkuId())) {
                    ...
                    List<SoPriceDetailEntity> existPriceList = existPriceMap.get(detailDTO.getSkuId());
                    List<String> detailIds = Lists.newArrayList();
                    for(SoPriceDetailEntity price : existPriceList) {
        ...
        List<SoPriceDetailDTO.ImportSaveDTO> detailList = addDTO.getDetailList();
