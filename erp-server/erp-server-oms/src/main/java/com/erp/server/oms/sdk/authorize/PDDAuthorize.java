...
import com.sdk.oms.pdd.dto.PDDShopInfoDTO;
import com.sdk.oms.pdd.service.PddService;
...
@Slf4j
@Component
@PlatformAnnotate(method = PlatformDictEnum.PDD)
public class PDDAuthorize implements IShopAuthorizeService<T> {
    ...
    public String getShopAuthorizeUrl(ShopAuthorizeUrlDTO dto) {
        ...
        AppClientEnum appClient = AppClientEnum.PDD_AUTHORIZE;
    ...
    public Boolean shopAuthorize(ShopAuthorizeDTO dto, HttpServletResponse response) {
        // 校验是否是本系统发起
        String stateKey =  CharSequenceUtil.format(RedisCacheConstants.AUTH_PDD_STATE, dto.getState());
        ...
        AppClientEnum appClient = AppClientEnum.PDD_ACCESS_TOKEN;
        CfgAppClientDTO.FindDTO findDTO = new CfgAppClientDTO.FindDTO();
        findDTO.setBusinessType(appClient.getBusinessType());
        findDTO.setDictPlatform(appClient.getPlatform());
        findDTO.setPlatformType(appClient.getPlatformType());
        ...
        AuthTokenCreateResponse.PopAuthTokenCreateResponse tokenDTO = pddService.createToken(paramMap);
        ...
        // token 过期时间为7天
        LocalDateTime localDateTime = LocalDateTime.ofInstant(Instant.ofEpochSecond(tokenDTO.getExpiresAt()), ZoneId.systemDefault());
        ...
        PDDShopInfoDTO shopInfoDTO = new PDDShopInfoDTO();
    ...
    public Boolean refreshToken(RefreshShopTokenDTO dto) {
        //先获取授权店铺 然后根据授权店铺进行
        CfgAppClientDTO.FindDTO findDTO = new CfgAppClientDTO.FindDTO();
        AppClientEnum appClientEnum = AppClientEnum.PDD_ACCESS_TOKEN;
        ...
        //请求SDK刷新token
        AuthTokenRefreshTokenResponse.PopAuthTokenRefreshResponse tokenDTO = null;

        try {
            tokenDTO = pddService.refreshToken(refreshTokenDTO);
        } catch (Exception e) {
            log.info("::::: PDD刷新token失败 ::::: 错误信息：" + e.getMessage());
        ...
        String tokenKey =  CharSequenceUtil.format(RedisCacheConstants.REDIS_PLATFORM_TOKEN, PlatformDictEnum.PDD.getCode(), dto.getShopId());
        redisUtil.del(tokenKey);

        //更新店铺token
        shopAuthService.refreshToken(shopAuthEntity.getId(), accessToken, refreshToken, 3600*24, tokenExpireTime);
    ...
    private void refreshErrorWarn(ShopAuthEntity shopAuthEntity, Exception e) {
        //记录错误次数
        String refreshTokenKey =  CharSequenceUtil.format(RedisCacheConstants.REDIS_REFRESH_PLATFORM_TOKEN, PlatformDictEnum.PDD.getCode(), shopAuthEntity.getShopId());
        ...
        if (ObjectUtil.isNotEmpty(refreshTokenNumObj)) {
            ...
            if (refreshTokenNum >= 3) {
                ...
                warnMsgInfo.setBizName(PlatformDictEnum.PDD.getName());
                warnMsgInfo.setErpServerModuleEnum(ErpServerModuleEnum.ERP_SERVER_OMS);
                warnMsgInfo.setTitle( CharSequenceUtil.format("平台【{}】店铺id{}刷新token失败",PlatformDictEnum.PDD.getName(),shopAuthEntity.getShopId()));
