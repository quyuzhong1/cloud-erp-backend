...
public class TeMuSoB2cHandle extends AbstractSoB2cHandle  {
    ...
    public Boolean handleRule(SoB2cEntity mainEntity) {
        ...
        try {
            ...
            log.error("[TeMu订单规则处理失败]:order={},msg={}", mainEntity.getPlatformCode(), e.getMessage());
        }
        return true;
    }
    ...
    public Boolean handleSoOutStock(PlatformOrderDTO dto, SoB2cDTO.PullOrderResultDTO resultDTO, SoB2cEntity mainEntity) {
        ...
        if (isShipped && hasPlatformWarehouse) {
            ...
            if(CollectionUtils.isEmpty(soB2cDetailEntityList) || soB2cDetailEntityList.stream().allMatch(v->StringUtils.isBlank(v.getWarehouseId()))){
                return true;
            }
            try {
