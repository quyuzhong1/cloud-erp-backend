...
//            code = sysUserFeign.getBusinessNo(new SysCodeDTO(BusinessNoConstant.XSD, BusinessNoTypeEnum.CODE_XSD.getCode()));
    ...
    public Boolean approveEnd(BaseApproveParamDTO dto, List<SoInfoEntity> list) {
        ...
        if (dto.getType().equals(ApproveType.PASS)) {
            ...
            //审核通过发送金蝶
            list.forEach(obj -> {
                DmpPushTaskEntity pushTaskEntity = syncKingdeeSoService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_APPROVE.getCode());
    ...
    public Boolean disApprove(BaseIdsDTO.IdsDTO dto) {
        ...
        if (result) {
            ...
            // TODO 收款字段需补
            list.forEach(obj -> {
                DmpPushTaskEntity pushTaskEntity = syncKingdeeSoService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_DISAPPROVE.getCode());
                pushTaskList.add(pushTaskEntity);
            });
            //推送同步中台dmp任务
        ...
        if (result) {
            ...
            if (CollectionUtils.isNotEmpty(syncList)) {
                syncList.forEach(obj -> {
                    DmpPushTaskEntity pushTaskEntity = syncKingdeeSoService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_DELETE.getCode());
                    pushTaskList.add(pushTaskEntity);
                });
                //推送同步中台dmp任务
    ...
    public Boolean invalid(List<String> ids, String remark) {
        ...
        List<DmpPushTaskEntity> pushTaskList = new ArrayList<>();
        list.forEach(obj -> {
            DmpPushTaskEntity pushTaskEntity = syncKingdeeSoService.syncDataToKingdee(obj, SyncOperateEnum.OPERATE_INVALID.getCode());
            pushTaskList.add(pushTaskEntity);
        });
        //推送金蝶
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronizationAdapter() {
...
//            mailAddress = customerInfo.getMailAddress();
    ...
    public SkuCostProfitDTO.SkuCostProfitResult getSkuCostProfit(SkuCostProfitDTO.SkuCostProfitParam costParam) {
        ...
        if (CollUtil.isNotEmpty(purchasePriceList)) {
            supplierSkuPrice = purchasePriceList.stream().filter(r -> Objects.equals(r.getSkuId(), costParam.getSkuId()) && Objects.equals(r.getSupplierId(), supplierId)).findFirst().orElse(null);
        ...
        if (Objects.nonNull(supplierSkuPrice)) {
            ...
            if (Objects.nonNull(purchaseDate)) {
                ...
                rate = dmpTaskFeign.getRate(purchaseDateStr, supplierSkuPrice.getCurrency());
            }
    ...
    public void brushCostData(LocalDate startDate, LocalDate endDate) {
        ...
        for (SoInfoEntity soInfoEntity : soList) {
            ...
            for (SoDetailEntity item : detailList) {
                // 计算毛利成本
                soDetailService.calCost(purchasePriceList, skuList, soInfoEntity.getBillDate(), item, Boolean.TRUE);
