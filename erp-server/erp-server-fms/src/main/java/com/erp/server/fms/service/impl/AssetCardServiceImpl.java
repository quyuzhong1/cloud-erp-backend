...
public class AssetCardServiceImpl extends SuperServiceImpl<AssetCardMapper, AssetCardEntity> implements AssetCardService {
    ...
    public BaseResultDTO.AddDTO add(AssetCardDTO.AddDTO addDTO) {
        ...
        operateLogService.addModuleOperateLog(msg, null, assetCardEntity.getId(), "新增操作");
        // TODO 新增明细（如果有明细的话）
    ...
    */
    ...
    public Boolean update(AssetCardDTO.UpdateDTO addOrUpdateDTO) {
        ...
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLogByObj(old, assetCardEntity, null, assetCardEntity.getId(), msg);
        return Boolean.TRUE;
    }
    ...
    public List<AssetCardDTO.TabListDTO> tabList(PermissionsDTO param) {
        ...
        statusList.parallelStream().forEach(status -> {
            ...
            list.add(new AssetCardDTO.TabListDTO(status, 0));
        }
        });
        list.add(new AssetCardDTO.TabListDTO("all", list.stream().mapToInt(AssetCardDTO.TabListDTO::getCount).sum()));
        // 计算合计数量
        return list;
    }
