...
public class AssetDisposalDetailServiceImpl extends SuperServiceImpl<AssetDisposalDetailMapper, AssetDisposalDetailEntity> implements AssetDisposalDetailService {
    ...
    @Override
    public BaseResultDTO.AddDTO add(AssetDisposalDetailDTO.AddDTO addDTO) {
        AssetDisposalDetailEntity assetDisposalDetailEntity = new AssetDisposalDetailEntity();
        BeanMapperUtils.copy(addDTO, assetDisposalDetailEntity);

        // 数据处理
        handleData(assetDisposalDetailEntity);

        log.info("开始新增资产处置单资产明细单");
        boolean save = super.save(assetDisposalDetailEntity);
        if(!save) {
            throw new ServiceException("资产处置单资产明细单保存失败");
        }

        // 操作日志
        String msg = StrUtil.format("用户【{}】新增【{}】单据id为【{}】", UserContext.getDefaultLoginUser().getUserName(), "资产处置单资产明细单" , assetDisposalDetailEntity.getId());
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLog(msg, null, assetDisposalDetailEntity.getId(), "新增操作");
        // TODO 新增明细（如果有明细的话）

        return new BaseResultDTO.AddDTO(assetDisposalDetailEntity.getId(), assetDisposalDetailEntity.getId());
    }
    ...
    */
    ...
    public Boolean update(AssetDisposalDetailDTO.UpdateDTO addOrUpdateDTO) {
        ...
        // 数据处理
        handleData(assetDisposalDetailEntity);
        log.info("编辑 开始修改资产处置单资产明细单数据，id：【{}】", old.getId());
        boolean save = super.updateById(assetDisposalDetailEntity);
        if(!save) {
            throw new ServiceException("资产处置单资产明细单保存失败");
        }
        ...
        // 记录主单操作日志
            log.info("编辑 开始记录资产处置单资产明细单日志数据，id：【{}】", assetDisposalDetailEntity.getId());
            String msg = StrUtil.format("用户【{}】编辑id为【{}】的【{}】单据 ", UserContext.getDefaultLoginUser().getUserName(), assetDisposalDetailEntity.getId(), "资产处置单资产明细单");
        // TODO 此处的null需修改为日志模块类型，moduleType查看ModuleTypeEnum枚举类
        operateLogService.addModuleOperateLogByObj(old, assetDisposalDetailEntity, null, assetDisposalDetailEntity.getId(), msg);
        return Boolean.TRUE;
    }
