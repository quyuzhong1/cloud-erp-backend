...
public class ConfigBuilder {
    ...
    public List<DTOInfo> getDtoInfoList() {
        return dtoInfoList;
    }

    public ConfigBuilder setDtoInfoList(List<DTOInfo> dtoInfoList) {
    ...
    public ConfigBuilder setVoInfoList(List<VOInfo> voInfoList) {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        // 自定义包信息
        Map<String, String> configPackageInfo = config.getPackageInfo();
        if(null != configPackageInfo) {
            packageInfo = configPackageInfo;
        } else {
        ...
        if (null != configPathInfo) {
            ...
            setPathInfo(pathInfo, template.getDto(getGlobalConfig().isKotlin()), outputDir, ConstVal.DTO_PATH, ConstVal.DTO);
            setPathInfo(pathInfo, template.getVo(getGlobalConfig().isKotlin()), outputDir, ConstVal.VO_PATH, ConstVal.VO);
        }
    }
    ...
    private void handlerStrategy(StrategyConfig config) {
        ...
        for(TableInfo tableInfo : tableInfoList) {
            dtoInfo = new DTOInfo();
            BeanUtils.copyProperties(tableInfo,dtoInfo);
            dtoInfos.add(dtoInfo);
        }

        dtoInfoList = dtoInfos;

        List<VOInfo> voInfos = new ArrayList<>();
        VOInfo voInfo = null;
        for(TableInfo tableInfo : tableInfoList) {
            voInfo = new VOInfo();
            BeanUtils.copyProperties(tableInfo,voInfo);
            voInfos.add(voInfo);
        }
            ...
            try (PreparedStatement preparedStatement = connection.prepareStatement(tablesSql);
                ...
                while (results.next()) {
                    ...
                    if (StringUtils.isNotEmpty(tableName)) {
                        ...
                        tableInfo.setUpperName(tableName.toUpperCase());
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, StrategyConfig config) {
        ...
        try {
            //开启日志
            boolean isEnableLog = config.isEnableLog();

            String tableFieldColumnsSql = dbQuery.tableFieldColumnsSql();

            String tableFieldsSql = dbQuery.tableFieldsSql();
            Set<String> h2PkColumns = new HashSet<>();
            if (DbType.POSTGRE_SQL == dbType) {
                tableFieldsSql = String.format(tableFieldsSql, dataSourceConfig.getSchemaName(), tableName);
                tableFieldColumnsSql = String.format(tableFieldColumnsSql, tableName);
            } else if (DbType.DB2 == dbType) {
                tableFieldsSql = String.format(tableFieldsSql, dataSourceConfig.getSchemaName(), tableName);
                tableFieldColumnsSql = String.format(tableFieldColumnsSql, dataSourceConfig.getSchemaName(), tableName);
            } else if (DbType.ORACLE == dbType) {
                tableName = tableName.toUpperCase();
                tableFieldsSql = String.format(tableFieldsSql.replace("#schema", dataSourceConfig.getSchemaName()), tableName);
                tableFieldColumnsSql = String.format(tableFieldColumnsSql.replace("#schema", dataSourceConfig.getSchemaName()), tableName);
            ...
            boolean isDefaultEnable = config.isDefaultValue();
            boolean isValidate = config.isDtoValidate();
            Map<String, Map> fieldDefaultMap = new HashMap<>(10);
            if(isDefaultEnable || isValidate) {
                PreparedStatement preparedStatement1 = connection.prepareStatement(tableFieldColumnsSql);
                ResultSet resultSet1 = preparedStatement1.executeQuery();
                while (resultSet1.next()) {
                    Map<String, Object> dataMap = new HashMap<>(10);
                    dataMap.put("isNotNullField", "YES".equalsIgnoreCase(resultSet1.getString("NULL")) ? false : true);
                    dataMap.put("defaultValue", resultSet1.getObject(dbQuery.fieldDefault()));
                    //dataMap.put("isShowDefaultValue", null != resultSet1.getObject(dbQuery.fieldDefault()) ? true : false);
                    dataMap.put("isShowDefaultValue", false);
                    ...
                    if(columnType.contains("(") && columnType.contains(")")) {
                        if(!columnType.contains("decimal") && !columnType.contains("numeric")) {
                            int maxLength = Integer.parseInt(columnType.substring(columnType.indexOf("(" ) + 1, columnType.indexOf(")" )));
                            dataMap.put("columnLength", maxLength);
                        } else {
                            String decimalLength = columnType.substring(columnType.indexOf("(" ) + 1, columnType.indexOf(")" ));
                            String[] decimalArray = decimalLength.split(",");
                            dataMap.put("maxIntegerLength", Integer.parseInt(decimalArray[0]) - Integer.parseInt(decimalArray[1]));
                            dataMap.put("maxPreciseLength", Integer.parseInt(decimalArray[1]));
                            dataMap.put("columnLength", Integer.parseInt(decimalArray[0]));
                        }
                    }
                    fieldDefaultMap.put(resultSet1.getString(dbQuery.fieldName()), dataMap);
                }
            }
            ...
            try (
                ...
                while (results.next()) {
                    ...
                    if(isDefaultEnable || isValidate) {
                        field.setDefaultValue(StringUtils.null2EmptyWithTrim(fieldDefaultMap.get(columnName).get("defaultValue")));
                        field.setNotNullField((Boolean) fieldDefaultMap.get(columnName).get("isNotNullField"));
                        field.setShowDefaultValue((Boolean) fieldDefaultMap.get(columnName).get("isShowDefaultValue"));
                        field.setMaxLength((Integer)fieldDefaultMap.get(columnName).get("columnLength"));
                        if(Objects.nonNull(fieldDefaultMap.get(columnName).get("maxIntegerLength"))) {
                            field.setMaxIntegerLength((Integer)fieldDefaultMap.get(columnName).get("maxIntegerLength"));
                        }
                        if(Objects.nonNull(fieldDefaultMap.get(columnName).get("maxPreciseLength"))) {
                            field.setMaxPreciseLength((Integer)fieldDefaultMap.get(columnName).get("maxPreciseLength"));
                        }
                    }
                    ...
                    if (isId && !haveId) {
                        ...
                        if(isEnableLog) {
                            if(columnName.equalsIgnoreCase("code")) {
                                //暂写死，否则要修改枚举类
                                field.setLogType("CODE");
                            } else {
                                field.setLogType("NONE");
                            }
                        }
                    ...
                    if (commentSupported) {
                        ...
                        field.setValidComment(comment);
                        //
                        if(comment != null && comment.contains("-")) {
                            field.setValidComment(comment.split("-")[0]);
                        }
                    }
