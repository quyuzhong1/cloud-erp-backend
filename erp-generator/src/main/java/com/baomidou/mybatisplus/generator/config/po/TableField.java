...
public class TableField {
    ...
    private boolean notNullField;//是否非空字段,YES表示可以为空，NO表示不能为空
    private boolean showDefaultValue;//是否显示默认值
    ...
    private Integer maxIntegerLength;//整数位最大位数
    private Integer maxPreciseLength;//小数位最大长度
    ...
    protected TableField setConvert(StrategyConfig strategyConfig) {
        if (strategyConfig.getEntityTableFieldAnnotationEnable()) {
            this.convert = true;
            return this;
        }
