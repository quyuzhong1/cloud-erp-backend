...
public class AutoGenerator {
    ...
    protected ConfigBuilder pretreatmentConfigBuilder(ConfigBuilder config) {
        ...
        for (TableInfo tableInfo : tableList) {
            /* ---------- 添加导入包 ---------- */
            // 如果继承了父类就不用引用包了
            if (config.getGlobalConfig().isActiveRecord() && StringUtils.isEmpty(config.getStrategyConfig().getSuperEntityClass())) {
                // 开启 ActiveRecord 模式
                tableInfo.setImportPackages(Model.class.getCanonicalName());
            }
            if (config.getStrategyConfig().getEntityTableFieldAnnotationEnable()) {
                // 表注解
                tableInfo.setImportPackages(TableName.class.getCanonicalName());
            }
            if (config.getStrategyConfig().getLogicDeleteFieldName() != null && tableInfo.isLogicDelete(config.getStrategyConfig().getLogicDeleteFieldName())
                && StringUtils.isEmpty(config.getStrategyConfig().getSuperEntityClass()) ) {
                // 逻辑删除注解
